<?php
// -- /opret/komplet.php -------------------------------------- 2022-02-28 --
// Copyright (c) 2003-2022 Saldi.dk ApS
// --------------------------------------------------------------------------
// 20211220 CA  Remark of using Betalingsservice to avoid invoice fees
// 20220221 CA  Ordering the basic solution Komplet
// 20220228 CA  Corrected three small errors
...
if (isset($_POST['navn']) && isset($_POST['email'])) {
	...
	$eventuelt = trim($_POST['eventuelt']);
	$betingelser = trim($_POST['betingelser']);
...
#	}
	...
	$qtxt = "select id from kundedata where regnskab='".db_escape_string($regnskab)."' and aktiv = '0'";
	$q2=db_select($qtxt,__FILE__ . " linje " . __LINE__);
	if ($r1 = db_fetch_array($q1)) {
		alert("Der er allerede oprettet et regnskab med navnet $regnskab");
	}	elseif ($r2 = db_fetch_array($q2)) {
		alert("Der eksisterer et inaktivt regnskab med navnet $regnskab");
...
#		if ($ip=='95.166.170.121') echo "INSERT INTO kundedata (firmanavn,  addr1, addr2, postnr, bynavn, kontakt, tlf , email , cvrnr, regnskab, brugernavn, kodeord, kontrol_id, aktiv, oprettet,slettet,referer,affiliate)values('$firma', '$adresse', '$adresse2', '$postnr', '$bynavn', '$navn', '$telefon' , '$email' , '$cvr', '$regnskab', '$brugernavn', '$kodeord', '$kontrol_id', '0','$dd','','$referer','$affiliate')";
		...
		$qtxt.= "kontrol_id,aktiv,oprettet,slettet,referer,affiliate) ";
		$qtxt.= " values ";
		$qtxt.= "('".db_escape_string($firma)."','".db_escape_string($adresse)."','".db_escape_string($adresse2)."',";
		$qtxt.= "'".db_escape_string($postnr)."','".db_escape_string($bynavn)."','".db_escape_string($navn)."',";
		$qtxt.= "'".db_escape_string($telefon)."','".db_escape_string($email)."','".db_escape_string($cvr)."',";
		$qtxt.= "'".db_escape_string($regnskab)."','".db_escape_string($brugernavn)."','".db_escape_string($kodeord)."',";
		$qtxt.= "'".db_escape_string($kontrol_id)."',0,'$dd','','$referer','$affiliate')";
...
print	"<form action=\"komplet.php\"  onsubmit=\"return Form1_Validator(this)\" method=\"POST\" name=\"Form1\">\n";
...
print "<tr><td colspan=2><b><big>$tekst</big></b></td>
	...
	print "<td>Løsning:</td>\n";
	$spantekst="Inklusiv:";
	if ( $posteringer_inkl < 0 ) $spantekst.="<br />- ubegrænset antal posteringer";
	...
	print "<td><span onmouseover=\"return overlib('$spantekst', WIDTH=800);\" onmouseout=\"return nd();\">".$solution_prices['solution_name']." regnskabssystem for ".$prisprregnskab." kr. pr. måned</span></td>\n";
	print "</tr><tr><td></td></tr>\n";
	if ( $posteringer_inkl > -1 ) {
	...
	if ( $brugere_inkl > -1 ) {
		...
		print "<td><span onmouseover=\"return overlib('$spantekst', WIDTH=800);\" onmouseout=\"return nd();\"><input type=\"text\" class=\"inputbox inputbox-medium\" style=\"text-align:right\" name=\"brugerantal\" value=\"$brugerantal\" ONBLUR=\"beregn()\"></span> ".$prisprbruger." kr. pr. ekstra samtidig bruger pr. m&aring;ned";
	...
	if ( $faktureringsgebyr > -1 ) {
		...
		if ( $betalingsservice ) $spantekst.="<br>Undgå gebyret ved tilmelding til Betalingsservice.";
		print "<span onmouseover=\"return overlib('$spantekst', WIDTH=800);\"> heraf faktureringsgebyr p&aring; ".$faktureringsgebyr." kr.</span>";
	}
	print "</td></tr>\n";
...
<?php	
	...
	if ((isset($regnskab))&&(isset($brugernavn))&&(isset($kodeord))) {
		$query=db_select("select id from regnskab where regnskab='".db_escape_string($regnskab)."'",__FILE__ . " linje " . __LINE__);
