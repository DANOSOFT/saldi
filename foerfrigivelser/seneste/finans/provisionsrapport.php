...
// --------------------------------------------finans/provisionsrapport.php--------------lap 1.9.4----------
// LICENS
//
...
if ((substr($rettigheder,4,1)!='1') && (!$ansat_id[1])) {
	...
	$q = db_select("SELECT ansatte.id as id FROM ansatte, provision WHERE ansatte.id=provision.ansat_id and ansatte.lukket!='on' and provision.provision>0 ORDER BY ansatte.navn");
...
#		$default_slutdato=$box3."-".$slutmaaned."-".$slutaar;
		...
		print "<td width=\"10%\" $top_bund><font face=\"Helvetica, Arial, sans-serif\" color=\"#000066\"><small><a href=../includes/luk.php accesskey=L>Luk</a></small></td>";
        	print "<td width=\"80%\" $top_bund><font face=\"Helvetica, Arial, sans-serif\" color=\"#000066\"><small>Provisionsrapport</small></td>";
		...
		print "</select>&nbsp;";
		print "</form>\n";
		print "<tr><td colspan='3'>$font<b>$ref</b></td></tr>";
# Kan ikke forstaa, hvorfor der ikke skal vaere en overskrift, hvis der blot er flere... 
	} else print "<tr><td colspan='3'>$font<b>$ref</b></td></tr>";
	...
	else print "<tr><td colspan=7>$font Fakturerede ordrer i perioden.</td><tr>";
	print "<tr><td colspan=7><br></td><tr>";
	if ($grundlag=='bet') print "<tr><td width=15%>$font Fakturadato</td><td>$font Betalingsdato</td><td align=right width=10%>$font Fakturanr</td><td align=right width=15%>$font Kostpris</td><td align=right width=15%>$font Salgspris</td><td align=right width=15%>$font D&aelig;knings-<br />bidrag</td><td align=right width=15%>$font Provision</td><tr>";
	else print "<tr><td width=15%>$font Fakturadato</td><td>$font Forfaldsdato</td><td align=right width=10%>$font Fakturanr</td><td align=right width=15%>$font Kostpris</td><td align=right width=15%>$font Salgspris</td><td align=right width=15%>$font D&aelig;knings-<br />bidrag</td><td align=right width=15%>$font Provision</td><tr>";
...
while ($r1 = db_fetch_array($q1)) {
	...
	if ($grundlag=='bet') {
		...
		$pro_sum=0;
		$faktliste=array();
		$q1 = db_select("SELECT ordrer.firmanavn as firmanavn, ordrer.fakturadate as faktdate, openpost.faktnr as faktnr, ordrer.id as ordre_id, ordrer.betalingsbet as betalingsbet, ordrer.betalingsdage, grupper.box6 as box6, grupper.id as gruppe_id from adresser, openpost, ordrer, grupper where  (ordrer.art='DO' or ordrer.art='DK') and adresser.gruppe=grupper.kodenr and grupper.art='DG' $personkilde and adresser.id=openpost.konto_id and ordrer.fakturanr=openpost.faktnr and ordrer.konto_id=openpost.konto_id and openpost.udlignet = '0' and openpost.faktnr>0 order by openpost.transdate, openpost.faktnr");
		while ($r1 = db_fetch_array($q1)) {
			if (!in_array($r1['faktnr'], $faktliste)) {
				$x++;
				$pris[$x]=0;
