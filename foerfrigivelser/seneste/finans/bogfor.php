<?php
// --------------------------------finans/bogfor.php--------lap 1.9.2b------09.04.08----
// LICENS
//
...
#echo "$debet[$y] har d_momsart $d_momsart[$y]<br>";	
	}
	if (strlen($kredit[$y])>0){
		list ($kredit[$y], $k_momsart[$y])=gruppeopslag($k_type[$y], $kredit[$y]);
#echo "$kredit[$y] har k_momsart $k_momsart[$y]<br>";	
	}
}
#echo "P1 $posteringer<br>"; 
...
#echo "D momsberegning($debet[$y], $d_amount[$y], $d_momsart[$y], $k_momsart[$y])<br>";		
	list ($d_amount[$y], $d_moms[$y], $d_momskto[$y], $d_modkto[$y])=momsberegning($debet[$y], $d_amount[$y], $d_momsart[$y], $k_momsart[$y]);
#echo "D svar: $d_amount[$y], $d_moms[$y], $d_momskto[$y], $d_modkto[$y]<br>";	
	}
	if ((!$momsfri[$y])&&($kredit[$y]>0)&&($k_amount[$y]>0)){
#echo "K momsberegning($kredit[$y], $k_amount[$y], $k_momsart[$y], $d_momsart[$y])<br>";		
		list ($k_amount[$y], $k_moms[$y], $k_momskto[$y], $k_modkto[$y])=momsberegning($kredit[$y], $k_amount[$y], $k_momsart[$y], $d_momsart[$y]);
#echo "K svar:	$k_amount[$y], $k_moms[$y], $k_momskto[$y], $k_modkto[$y]<br>";
	}
}
#echo "1 D $d_modkto[$y] |K $k_modkto[$y]<br>";	
/*
...
#echo "2 D $d_modkto[$y] - $d_momskto[$y] - $d_moms[$y]|K $k_modkto[$y] - $k_momskto[$y] - $k_moms[$y]<br>";	
}
$kontoantal=0;
$kontoliste=array()	;
for ($y=1; $y<=$posteringer; $y++) {
...
#echo "KL $kontoliste[$y] | $kontodebet[$y] | $kontokredit[$y]<br>";	
	}
}
...
#			$row =db_fetch_array($query);
		...
		if ($row['valuta']) {
			$r2 = db_fetch_array(db_select("select * from valuta where gruppe = '$row[valuta]' and valdate <= '$row[transdate]' order by valdate desc"));
			if ($r2['kurs']) {
				$amount[$y]=$row['amount']*$r2['kurs']/100;
				db_modify("update kassekladde set valutakurs = '$r2[kurs]' where id = '$postid[$y]'");
			}
			else print "<BODY onLoad=\"javascript:alert('Ups - ingen valutakurs for denne dato')\">";	
			} else $amount[$y]=$row['amount'];
...
# Konti fra kontoplanen bliver forbigået i funktionen
		if (((strstr($d_type[$y],'D'))||(strstr($d_type[$y],'K')))&&($debet[$y]>0)) {
			openpost($d_type[$y], $debet[$y], $bilag[$y], $faktura[$y], $amount[$y], $beskrivelse[$y], $transdate[$y], $bilag[$y], $valuta[$y], $valutakurs[$y]);
			list ($debet[$y], $d_momsart[$y]) =gruppeopslag($d_type[$y], $debet[$y]);
#echo "$debet[$y] har d_momsart $d_momsart[$y]<br>";	
		...
		if ((($k_type[$y]=='D')||($k_type[$y]=='K'))&&($kredit[$y]>0)) {
			openpost($k_type[$y], $kredit[$y], $bilag[$y], $faktura[$y], $amount[$y]*-1, $beskrivelse[$y], $transdate[$y], $bilag[$y], $valuta[$y], $valutakurs[$y]);
			list ($kredit[$y], $k_momsart[$y])=gruppeopslag($k_type[$y], $kredit[$y]);
#echo "$kredit[$y] har k_momsart $k_momsart[$y]<br>";	
...
#echo "KMOMS $k_moms[$y]<br>";		
			if ($d_momskto[$y]>0) $transantal++;
			if ($k_momskto[$y]>0) $transantal++; 
			if ($eufaktnr[$y]!=$faktura[$y]&&$d_momskto[$y]>0&&$k_momskto[$y]>0) $transantal--; # indsat 280807 grundet fejl ved konti (i kontoplan) m. eumoms 
			if ($debet[$y]>0) {
#echo "A insert into transaktioner (kontonr, bilag, transdate, logdate, logtime, beskrivelse, debet, faktura, kladde_id,afd, ansat, projekt, valuta, valutakurs, ordre_id)values($debet[$y], $bilag[$y], '$transdate[$y]', '$logdate', '$logtime', '$beskrivelse[$y]', '$d_amount[$y]', '$faktura[$y]', $kladde_id, '$afd[$y]', '$ansat[$y]', '$projekt[$y]', '$valuta[$y]', '$valutakurs[$y]', '$ordre_id[$y]')<br>";
...
#					$transmonth[$transtjek]=$month;
			...
			if ($d_momskto[$y]>0) {
#	echo "C insert into transaktioner (kontonr, bilag, transdate, logdate, logtime, beskrivelse, debet, faktura, kladde_id,afd, ansat, projekt, valuta, valutakurs, ordre_id)values($d_momskto[$y], $bilag[$y], '$transdate[$y]', '$logdate', '$logtime', '$beskrivelse[$y]', '$d_moms[$y]', '$faktura[$y]', $kladde_id, '$afd[$y]', '$ansat[$y]', '$projekt[$y]', '$valuta[$y]', '$valutakurs[$y]', '$ordre_id[$y]')<br>";
...
#					 $transmonth[$transtjek]=$month;
			...
			if ($k_momskto[$y]>0) {
#	echo "Y $y<br>";				
#echo "D insert into transaktioner (kontonr, bilag, transdate, logdate, logtime, beskrivelse, kredit, faktura, kladde_id,afd, ansat, projekt, valuta, valutakurs, ordre_id)values($k_momskto[$y], $bilag[$y], '$transdate[$y]', '$logdate', '$logtime', '$beskrivelse[$y]', '$k_moms[$y]', '$faktura[$y]', $kladde_id, '$afd[$y]', '$ansat[$y]', '$projekt[$y]', '$valuta[$y]', '$valutakurs[$y]', '$ordre_id[$y]')<br>";
...
######################################################################################################################################
function openpost($art, $debet, $bilag, $faktura, $amount, $beskrivelse, $transdate, $bilag_id, $valuta, $valutakurs){
	global $connection;
	global $regnaar;
	global $kladde_id;
