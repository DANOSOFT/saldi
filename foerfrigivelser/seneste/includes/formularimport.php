...
// ----------/systemdata/formularimport.php-----lap 1.1.4.------11.03.2008---------------------------------
// LICENS
//
...
function formularimport($formular) 
{
	$fp=fopen($formular,"r");
	if ($fp) {
		...
		while (!feof($fp)) {
			$linje=fgets($fp);
			$linje=str_replace("\n","",$linje);
			list($formular, $art, $beskrivelse, $placering, $xa, $ya, $xb, $yb, $str, $color, $font, $fed, $kursiv, $side, $sprog) = split(chr(9),$linje);
			...
			if ((substr($placering,0,1))=="'"&&(substr($placering,-1))=="'") $placering=(substr($placering,1,strlen($placering)-2));
			...
			if ($xa>0) {
				...
				db_modify("insert into formularer (formular,art,beskrivelse,xa,ya,xb,yb,placering,str,color,font,fed,kursiv,side,sprog)values('$formular','$art','$beskrivelse','$xa','$ya','$xb','$yb','$placering','$str','$color','$font','$fed','$kursiv','$side','$sprog')"); 
			}
		}
		fclose($fp);
