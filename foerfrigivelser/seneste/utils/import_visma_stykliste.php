...
if ($x > $slutlinje) {
	...
	$q = db_select("SELECT DISTINCT indgaar_i from styklister order by indgaar_i");
	while ($r=db_fetch_array($q)) {
		db_modify("update varer set samlevare = 'on' where id=$r[indgaar_i]");
	 	db_modify("update ordrelinjer set samlevare = 'on' where vare_id = $r[indgaar_i]");
	}
	echo "Stykliste import afsluttet med succes<br>";
}
?>