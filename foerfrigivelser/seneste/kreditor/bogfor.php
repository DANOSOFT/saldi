...
@session_start();
$s_id=session_id();
// -------------------kreditor/bogfor.php-------lap 1.9.2a------07.04.2008-----------------------
/// LICENS
//
...
if ($valuta && $valuta!='DKK') {
	if ($r= db_fetch_array(db_select("select valuta.kurs as kurs, grupper.box3 as difkto from valuta, grupper where grupper.art='VK' and grupper.box1='$valuta' and valuta.gruppe=grupper.kodenr and valuta.valdate <= '$levdate' order by valuta.valdate desc"))) {
		$valutakurs=$r['kurs']*1;
		$difkto=$r['difkto']*1;
	...
	if ($fejl==0){
		...
		while ($row = db_fetch_array($query)){
			if (($row[posnr]>0)&&(strlen(trim(($row[varenr])))>0)){
				...
				if ($valutakurs) $dkpris[$x]=round($pris[$x]*$valutakurs/100,2); # Omregning til DKR.		
		...
		for ($x=1; $x<=$linjeantal; $x++) {
			if (($vare_id[$x])&&($antal[$x]!=0)) {
				...
				if ($box8!='on'){
					db_modify("update ordrelinjer set bogf_konto='$box3' where id='$linje_id[$x]'");
						if ($r=db_fetch_array(db_select("select id from batch_kob where linje_id=$linje_id[$x]"))) {
							db_modify("update batch_kob set pris = '$dkpris[$x]', fakturadate='$levdate' where id='$r[id]'");
					}
				} else {
					...
					if ($antal[$x]>0) {
						...
						if ($row = db_fetch_array($query)) {
							...
							db_modify("update batch_kob set pris = '$dkpris[$x]', fakturadate='$levdate' where id=$batch_id");
							$q2 = db_select("select linje_id from batch_salg where batch_kob_id=$batch_id");
							while ($r2 = db_fetch_array($q2)) {
								db_modify("update ordrelinjer set kostpris = '$dkpris[$x]' where id=$r2[linje_id]");
							}
						}
					} else {
					 $query = db_select("select * from batch_kob where linje_id=$kred_linje_id[$x]");
						...
						while ($row = db_fetch_array($query)) {
							db_modify("update batch_kob set pris = '$dkpris[$x]', fakturadate='$levdate' where id=$row[id]");
							$q2 = db_select("select linje_id from batch_salg where batch_kob_id=$batch_id");
							while ($r2 = db_fetch_array($q2)) {
								db_modify("update ordrelinjer set kostpris = '$dkpris[$x]' where id=$r2[linje_id]");
							}
		...
		db_modify("update ordrer set status=3, fakturadate='$levdate', modtagelse = '$modtagelse', valuta = '$valuta', valutakurs = '$valutakurs' where id=$id");
...
#		$refnr;
		...
		$r = db_fetch_array(db_select("select box2 from grupper where art = 'KG' and kodenr='$r[gruppe]'"));
