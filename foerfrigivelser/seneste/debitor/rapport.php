...
// ------------------------debitor/rapport.php-------lap 1.9.3b---16.04.2008
// LICENS
//
...
#		print "<tr><td colspan=8><hr></td></tr>";
		...
		$primo=0;
		$primoprint=0;
		$q2 = db_select("select * from openpost where konto_id=$kto_id[$x] and transdate<='$regnslut' order by transdate, faktnr, refnr");
...
#				else {$stil="<span style='color: rgb(0, 0, 0);'>";}
				...
				if ($amount > 0) {
				if (($r2[udlignet]!='1')&&($forfaldsdag<$currentdate)) print "<td><span style='color: rgb(255, 0, 0);'>$font <small>".dkdato($forfaldsdag)."</small></td><td align=right><span style='color: rgb(255, 0, 0);'>$font <small><a href=\"../includes/udlign_openpost.php?post_id=$r2[id]&regnaar=$regnaar&maaned_fra=$maaned_fra&maaned_til=$maaned_til&konto_fra=$konto_fra&konto_til=$konto_til&retur=../debitor/rapport.php\">$tmp</a></small></td><td></td>";
