...
@session_start();
$s_id=session_id();
// ------------debitor/bogfor.php------- patch 1.9.2a ---07.04.2008------
// LICENS
//
...
echo "A update batch_salg set pris=$pris, fakturadate='$fakturadate' where id=$batch_id[$x]<br>"; 
...
echo "C insert into batch_kob (linje_id, vare_id, ordre_id, pris, lager) values ($res_linje_id, $vare_id[$x], $kob_ordre_id, $pris, $lager)<br>"; #Antal indsaettes ikke - dette styres i "reservation"
...
echo "D update reservation set batch_kob_id=$batch_kob_id[$x] where linje_id = $res_linje_id<br>";
				db_modify("update reservation set batch_kob_id=$batch_kob_id[$x] where linje_id = $res_linje_id");
echo "E update batch_salg set batch_kob_id=$batch_kob_id[$x] where id=$batch_id[$x]<br>";		
...
echo "F insert into ordrelinjer (posnr, antal, pris, rabat, ordre_id, bogf_konto) values ('-1',$antal[$x], '$pris', 0, $id, $box2)<br>";
				db_modify("insert into ordrelinjer (posnr, antal, pris, rabat, ordre_id, bogf_konto) values ('-1',$antal[$x], '$pris', 0, $id, $box2)");
				$pris=$pris*-1;
echo "G insert into ordrelinjer (posnr, antal, pris, rabat, ordre_id, bogf_konto) values ('-1',$antal[$x], '$pris', 0, $id, $box3)<br>";
...
echo "H update ordrelinjer set kostpris = $kostpris where id=$ordre_linje_id<br>";
			db_modify("update ordrelinjer set kostpris = $kostpris where id=$ordre_linje_id");
			if ($box1&&$box2) {
echo "insert into ordrelinjer (posnr, antal, pris, rabat, ordre_id, bogf_konto) values ('-1',$antal[$x], '$kostpris', 0, $id, $box2)<br>";
				db_modify("insert into ordrelinjer (posnr, antal, pris, rabat, ordre_id, bogf_konto) values ('-1',$antal[$x], '$kostpris', 0, $id, $box2)");
				$kostpris=$kostpris*-1;
echo "insert into ordrelinjer (posnr, antal, pris, rabat, ordre_id, bogf_konto) values ('-1',$antal[$x], '$kostpris', 0, $id, $box3)<br>";
...
echo "A update batch_kob set pris=$pris, fakturadate='$fakturadate' where id=$batch_id<br>";
...
echo "B update ordrelinjer set bogf_konto=$box4 where id=$ordre_linje_id<br>";
		...
		if ($box9=='on'){
			...
			if (!$pris){$pris=0;}
			if ($valutakurs) $pris=$pris*100/$valutakurs;
echo "C insert into ordrelinjer (posnr, antal, pris, rabat, ordre_id, bogf_konto) values ('-1', $antal, $pris, 0, $id, $box3)<br>";
			db_modify("insert into ordrelinjer (posnr, antal, pris, rabat, ordre_id, bogf_konto) values ('-1', $antal, $pris, 0, $id, $box3)");
			$pris=$pris*-1;
echo "D insert into ordrelinjer (posnr, antal, pris, rabat, ordre_id, bogf_konto) values ('-1', $antal, $pris, 0, $id, $box2)<br>";
		...
		for ($t=1;$t<=2;$t++)	{			
			...
			for ($y=1;$y<=$ordrelinjer;$y++) {
				if ($bogf_konto[$y]) {
					...
					$d_kontrol=$d_kontrol+$debet; $k_kontrol=$k_kontrol+$kredit;
# echo "$t - $valutakurs - insert into transaktioner (bilag, transdate, beskrivelse, kontonr, faktura, debet, kredit, kladde_id, afd, logdate, logtime, projekt, ansat, ordre_id) values ('$bilag', '$transdate', '$beskrivelse', '$bogf_konto[$y]', '$fakturanr', '$debet', '$kredit', '0','$afd', '$logdate', '$logtime', '$projekt', '$ansat', '$id')<br>";
	...
	if ($diff=round($d_kontrol-$k_kontrol,2)) {
		if ($valuta!='DKK' && abs($diff)<=$maxdif) { #Der maa max vaere en afvigelse paa 1 oere pr ordrelinje m fremmed valuta;
			...
			db_modify("insert into transaktioner (bilag, transdate, beskrivelse, kontonr, faktura, debet, kredit, kladde_id, afd, logdate, logtime, projekt, ansat, ordre_id) values ('$bilag', '$transdate', '$beskrivelse', '$difkto', '$fakturanr', '$debet', '$kredit', '0', '$afd', '$logdate', '$logtime', '$projekt', '$ansat', '$id')");
		} else {
