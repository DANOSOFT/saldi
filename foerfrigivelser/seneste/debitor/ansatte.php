...
// -------- debitor/ansatte.php (modul nr. 6)----------lap 1.9.3a ----- 15.04.2008 ----------
// LICENS
//
...
if ($_POST){
 	...
 	if ($submit=="Slet") {
 		...
 		print "<meta http-equiv=\"refresh\" content=\"0;URL=kreditorkort.php?returside=$returside&ordre_id=$ordre_id&id=$konto_id&fokus=$fokus\">";
 	 	...
 	 	elseif ($id > 0){
 	 	 	db_modify("update ansatte set navn = '$navn', konto_id = '$konto_id', addr1 = '$addr1', addr2 = '$addr2', postnr = '$postnr', bynavn = '$bynavn', email = '$email', tlf = '$tlf', fax = '$fax', mobil = '$mobil', cprnr = '$cprnr', notes = '$notes', lukket = '' where id = '$id'");
 	 	}
 	}
}
...
print "<td width=\"10%\" $top_bund>$font<small><a href=debitorkort.php?returside=$returside&id=$konto_id&fokus=$fokus accesskey=L>Luk</a></small></td>";
print "<td width=\"80%\" $top_bund>$font<small>$row[firmanavn] - Ansatte</small></td>";
print "<td width=\"10%\" $top_bund>$font<small><a href=ansatte.php?returside=$returside&ordre_id=$ordre_id&fokus=$fokus&konto_id=$konto_id accesskey=N>Ny</a><br></small></td>";
print "</tbody></table>";
print "</td></tr>";
