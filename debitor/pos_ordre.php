...
//
// --- debitor/pos_ordre.php -----patch 4.1.0 ----2024-07-29--------------
// LICENSE
//
...
if ($kasse && $kassebeholdning && !isset($_POST['zRapport'])) {
	$calc = findtekst('3089|Beregn',$sprog_id);
...
if ($afslut || $status >= 3 || $fokus == 'modtaget') {
	...
	if ($afd)
		db_modify("update grupper set box12= '' where art='POSBUT' and box12 is NULL", __FILE__ . " linje " . __LINE__); #fjernes 20170104
...
function kasseoptalling(
	...
	#cho "$baseCurrency TG $tilgang Om $omsatning<br>";
...
#		} else {
	...
	if (!count($valuta)) displayLine('Kontant',$tilgang,$baseCurrency);
	if ($kontosum) {
