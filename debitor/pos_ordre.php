...
//
// --- debitor/pos_ordre.php -----patch 4.0.8 ----2024-04-15--------------
// LICENSE
//
...
if($bordvalg=if_isset($_POST['bordvalg'])) {
  ...
  if (count($bord) == 0){
    ...
    else print "<meta http-equiv=\"refresh\" content=\"0;URL=../bordplaner/table_plan.php?id=$id\">\n";
  } else {
    if ($bordnr || $bordnr == '0') print "<meta http-equiv=\"refresh\" content=\"0;URL=pos_ordre.php?bordnr=$bordnr\">\n";
...
if (isset($_POST['kasse']) && $_POST['kasse']) {
	...
	if (isset($pfs) && $pfs) setcookie('saldi_pfs',$pfs,time()-60,'/');
...
if ($godkendt=='OK') { # 20131205
	...
	if (!$betaling && isset($_GET['cardscheme'])) $betaling = if_isset($_GET['cardscheme']);

	if (!$betaling && isset($_GET['amount'])) $betaling = if_isset($_GET['amount']);
	$betaling2=if_isset($_GET['betaling2']);

	$modtaget=if_isset($_GET['modtaget']);
	if (!$modtaget && isset($_GET['amount'])) $modtaget = if_isset($_GET['amount']);
	$modtaget2=if_isset($_GET['modtaget2']);

	$indbetaling=if_isset($_GET['indbetaling']);

	$kortnavn=if_isset($_GET['kortnavn']); 	
	if (!$kortnavn && isset($_GET['cardscheme'])) $kortnavn = if_isset($_GET['cardscheme']);
...
if ($konto_id || $kontonr) {
	...
	if ($betalingsbet!='Kontant') $modtaget=(float)$r['betalt'];
	$sum=(float)$r['sum'];
	$betaling='ukendt';
...
#	$modtaget2=if_isset($_POST['modtaget2']);
	...
	if (isset($_POST['antal_ny'])) $antal_ny=strtolower(trim($_POST['antal_ny']));
...
function opret_posordre($konto_id,$kasse){
	global $bordnr,$bruger_id;$brugernavn;
	global $db;
	global $firmanavn;
	global $kontonr;
	global $momssats;
...
#		print "<td><input class='inputbox' type='text' style='text-align:right;font-size:$ifs;width:25px' name='pfs' value='$pfs'></td></tr>";
		...
		for($x=0;$x<count($afd);$x++) {
			$kasse=$x+1;
			if (!count($afd_nr) && $kasse==$nuv_kasse) print "<option value=\"$kasse\">$kasse</option>\n";
...
function posbogfor ($kasse,$regnstart,$reportNumber) {
	...
	if ($udtages) $udtages=(float)usdecimal($udtages,2);
	...
	if ($diffkonto) {
		...
		if ($r = db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__))) {
			if ($tmp=trim($r['moms'])) { # f.eks S3
				...
				if ($r2['box1']) $diffVatAccount=(int)$r2['box1'];
				if ($r2['box2']) $diffVatRate=(int)$r2['box2'];
			} 
		} 
	}
	$x=0;
		...
		if ($ValutaDifKonti[$x]) {
			...
			if ($r = db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__))) {
				if ($tmp=trim($r['moms'])) { # f.eks S3
					...
					$r2 = db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__));
					if ($r2['box1']) $ValutaDiffVatAccount[$x]=(int)$r2['box1'];
					if ($r2['box2']) $ValutaDiffVatRate[$x]=(int)$r2['box2'];
				}
			}
		} 
	}
...
function kasseoptalling (
  ...
  $kr_andet = (float)$kr_andet;
	...
	if ($r = db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__))) $change_cardvalue=$r['var_value'];
	$qtxt = "select var_value from settings where var_name = 'useCommission' and var_grp = 'items'";
	if ($r = db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__))) $useCommission = $r['var_value'];
	$qtxt = "select var_value from settings where var_name = 'comissionSetteTime' and var_grp = 'items'";
	if ($r = db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__))) $comissionSetteTime=$r['var_value'];
	...
	$qtxt="select id,var_value from settings where var_name = 'commissionAccountNew' and var_grp = 'items'";
	if ($r = db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__))) $commissionAccountNew=$r['var_value'];
	$qtxt="select id,var_value from settings where var_name = 'customerCommissionAccountNew' and var_grp = 'items'";
	if ($r = db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__))) $customerCommissionAccountNew=$r['var_value'];
	$qtxt="select id,var_value from settings where var_name = 'customerCommissionAccountUsed' and var_grp = 'items'";
	if ($r = db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__))) $customerCommissionAccountUsed=$r['var_value'];
...
#	print "<input type=\"hidden\" name=\"kontosalg\" value=\"$kontosalg\">\n";
	...
	if ($valuta[0]) {
		for ($x=0;$x<count($valuta);$x++) {
			...
			if ($log) file_put_contents($logfil,"$valuta[$x] $optval[$x]\n",FILE_APPEND);
		}
	}
	$pfnavn="../temp/".$db."/kasseopg".str_replace("-","",$kasse).".txt";
...
#cho "$valuta[$x] TG $tilgang Om $omsatning<br>"; 	
			...
			if ($optalt || $optalt=='0'){
				...
				if ($log) file_put_contents($logfil,"Udtages $ValutaUdtages[$x] $valuta[$x]\n",FILE_APPEND);
			}
		}
	}
...
#		}
	...
	if (!count($valuta) && $tilgang != '0,00') displayLine('Kontant',$tilgang,'DKK');
	if ($kontosum) {
		print "<tr><td colspan='2'><b>".findtekst(592, $sprog_id)."</b></td><td align='right'><b>".dkdecimal($kontosum,2)."</b> DKK</td></tr>\n";
		if ($log) file_put_contents($logfil,"Konto $kontosum\n",FILE_APPEND);
	}
...
#cho "Incl $incl_moms<br>\n";
	...
	if ($printserver) {
		...
		$href="http://$printserver/kundedisplay.php?tekst=".urlencode($beskrivelse)."&pris=".dkdecimal($pris,2)."&ryd=$ryd";
...
function udskriv_kasseopg($id,$kasse,$pfnavn) {
	...
	$bon='';
#cho "udskriv_kasseopg(ID $id, Kasse $kasse, PF $pfnavn)<br>";
	$fp=fopen("$pfnavn","r");
