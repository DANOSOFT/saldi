<?php
// --------- /debitor/serienummer.php-----------patch 3.2.7----2012.02.02----------
// LICENS
//
		...
		if ($antal>0) {
			for ($x=1; $x<=$sn_antal; $x++) {
				if (trim($valg[$x])=="on" && $y<$antal) {
					$y++;
					db_modify("update serienr set salgslinje_id='$linje_id' where id=$sn_id[$x]",__FILE__ . " linje " . __LINE__);
				} elseif ($y>$antal) db_modify("update serienr set salgslinje_id=0 where id=$sn_id[$x]",__FILE__ . " linje " . __LINE__);
				elseif ($sn_id[$x]) db_modify("update serienr set salgslinje_id=0 where id=$sn_id[$x]",__FILE__ . " linje " . __LINE__);
			}
		} elseif ($art=='DK') {
			for ($x=1; $x<=$sn_antal; $x++) {
				if (trim($valg[$x])=="on") {
...
if ($antal>0) {	
	...
	$tmp=$kred_linje_id*-1;
	if ($art=='KO') {
		$query = db_select("select * from serienr where salgslinje_id!='$kred_linje_id' and (salgslinje_id='$kred_linje_id' or salgslinje_id='$tmp') order by serienr",__FILE__ . " linje " . __LINE__);
		while ($row = db_fetch_array($query)) {
		...
		$q = db_select("select serienr.salgslinje_id,serienr.id,serienr.serienr from serienr,ordrer,ordrelinjer where ordrer.art='DO' and ordrer.status>='3' and ordrer.konto_id='$konto_id' and 
			ordrelinjer.ordre_id=ordrer.id and ordrelinjer.vare_id=$vare_id and abs(serienr.salgslinje_id)=ordrelinjer.id",__FILE__ . " linje " . __LINE__);
		while ($r = db_fetch_array($q)) {
			$sn_antal++;
			if ($r['salgslinje_id']<0) $tmp= "checked";
