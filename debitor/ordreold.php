...
#						if ($varenr[$x]==$rvnr) $posnr_ny[$x]+=1000;
				...
				if ($linje_id[$x]) {
					...
					if ($saet[$x] || ($varenr[$x] && $varenr[$x]==$rvnr)) {
						// Update sæt header line - handle lager changes
						if (isset($lager[$x]) && $lager[$x] && $samlevare[$x] == 'on') {
							// When lager is changed for sæt header, update all items in the sæt
							$qtxt="select saet from ordrelinjer where id='$linje_id[$x]'";
							$r=db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__));
							if ($r['saet']) {
								$saet_num = $r['saet'];
								// Update the sæt header lager
								db_modify("update ordrelinjer set leveres='$leveres[$x]',lager='$lager[$x]' where id='$linje_id[$x]'",__FILE__ . " linje " . __LINE__);
								// Update all items in the sæt to have the same lager
								db_modify("update ordrelinjer set lager='$lager[$x]' where ordre_id='$id' and saet='$saet_num'",__FILE__ . " linje " . __LINE__);
							} else {
...
#  }
    ...
    #  }
	if ($saet && $samlevare) {
		// Render sæt header with editable lager field
		($x)?$y=NULL:$y='_';
		...
		print "<td valign='top'><input class='inputbox' type='text' style='text-align:right;width:50px;' name='posn$x' value='$ny_pos' readonly='readonly' disabled></td>\n";
		...
		print "size=\"12\" name=\"vare$y$x\" value=\"$varenr\" disabled></td>\n";
		print "<td valign='top'><input class='inputbox' type='text' style='background: none repeat scroll 0 0 #e4e4ee' readonly=\"readonly\" size=\"4px\" value=\"\" disabled></td>\n";
		print "<td valign='top'><input class='inputbox' type='text' style='background: none repeat scroll 0 0 #e4e4ee' readonly=\"readonly\" size=\"4px\" value=\"$enhed\" disabled></td>\n";
		
		// Editable lager field for sæt header (only for sæt, not individual items)
		$lagerId = $lager;
		if ($lagerantal > 1) {
			$lagerDisplay = $lager;
			for ($l=0;$l<count($lagernr);$l++) {
			...
			print "<td valign='top'><input type='hidden' name='lager[$x]' value='$lagerId'>";
			print "<input class='inputbox' type='text' style='text-align:right;width:35px' name=\"lagr$x\" value=\"$lagerDisplay\" onchange=\"javascript:docChange = true;\"></td>\n";
		}
		
		$title=var2str($beskrivelse,$id,$posnr,$varenr,$dkantal,$enhed,$dkpris,$dkprocent,$serienr,$varemomssats,$dkrabat);
		print "<td valign='top' title=\"$title\"><textarea class=\"autosize inputbox ordreText comment\" readonly=\"readonly\" rows=\"1\" cols=\"58\" disabled>$beskrivelse</textarea></td>\n";
		
		// Price column (hidden for sæt header)
		print "<td><input type=\"hidden\" name=\"pris$x\" value=\"$dkpris\"></td>\n";
		print "<td><input type=\"hidden\" name=\"raba$x\" value=\"$dkrabat\"></td>\n";
		if ($procentfakt) print "<td><input type=\"hidden\" name=\"proc$x\" value=\"$dkprocent\"></td>\n";
		
		// Amount column
		if ($varenr) {
			if ($incl_moms && !$momsfri) {
				$tmp=dkdecimal($antal*($pris+$pris*$varemomssats/100),2);
			} else {
				$tmp=dkdecimal($pris,2);
			}
			print "<td valign='top' align=\"right\"><input class='inputbox' type='text' readonly=\"readonly\" style=\"background: none repeat scroll 0 0 #e4e4ee; text-align:right\" size=\"10\" value=\"$tmp\" disabled></td>\n";
		} else {
			print "<td></td>\n";
		}
		...
		print "<input type=\"hidden\" name=\"linje_id[$x]\" value=\"$linje_id\">\n";
		print "<input type=\"hidden\" name=\"samlevare[$x]\" value=\"on\">\n";
		print "<input type=\"hidden\" name=\"saet[$x]\" value=\"$saet\">\n";
		print "</tr>\n";
		// Set flag to skip normal rendering below
		$saet_header_rendered = true;
  }  else {
		$saet_header_rendered = false;
		$txtColor = 'black';
		$qtyTitle = '';
...
print "<td valign='top'><input class='inputbox' type='text' style='text-align:right;width:50px;' name='posn$x' value='$ny_pos' $disabled></td>\n";
		...
		if ($lagerantal > 1) {
			...
			// For individual sæt items (not the header), disable the lager field
			$lager_disabled = $disabled;
			if ($saet && !$samlevare) {
				$lager_disabled = 'disabled';
			}
      		print "<td valign = 'top'><input class = 'inputbox' type = 'text' style=\"text-align:right;width:35px\" name=\"lagr$x\" value=\"$lager\" onchange=\"javascript:docChange = true;\" $lager_disabled></td>\n";
		}
		$title=var2str($beskrivelse,$id,$posnr,$varenr,$dkantal,$enhed,$dkpris,$dkprocent,$serienr,$varemomssats,$dkrabat);
	...
	// Skip rendering if sæt header was already rendered
	if (isset($saet_header_rendered) && $saet_header_rendered) {
		return "$sum\t$dbsum\t$blandet_moms\t$moms\t$antal_ialt\t$leveres_ialt\t$tidl_lev_ialt\t$tidl_lev\t$levdiff";
	}
	if ($saet) {
