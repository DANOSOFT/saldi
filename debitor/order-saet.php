...
if (!strstr($fokus,'lev_') && isset($_GET['konto_id']) && is_numeric($_GET['konto_id'])) { # <- 2008.05.11
	...
	if ($kontoansvarlig){
		$q = db_select("select navn,afd from ansatte where id='$kontoansvarlig'",__FILE__ . " linje " . __LINE__);
		$r = db_fetch_array($q);
		$ansat_navn=$r['navn'];
		if (!$afd) $afd=$r['afd'];
	} else {
...
if (!$id && $konto_id && $kontonr) {
	if (!$afd) $afd = (int)get_settings_value('afd', 'brugerAfd', 1, $bruger_id);
...
#cho "B Status $status<br>";
	...
	$oldRef = trim(if_isset($_POST['oldRef']));
...
#	}
	...
	if ($db == 'pos_147') {
		if ($afd == 1) $formularsprog = 'Holbaek';
		elseif ($afd == 2) $formularsprog = 'Hilleroed';
	}
...
#						if ($varenr[$x]==$rvnr) $posnr_ny[$x]+=1000;
		...
		if (($posnr_ny[0])&&(!strstr($b_submit,'Opslag'))) {
			if ($varenr[0]) {
				$samlevare[0]='';
				if ($brugsamletpris) {
					$low = strtolower($varenr[0]);
					$up  = strtoupper($varenr[0]);
					$qtxt = "SELECT id,samlevare,salgspris FROM varer WHERE varenr = '$varenr[0]' or varenr_alias = '$varenr[0]' or stregkode = '$varenr[0]'";
					$qtxt.= "or lower(varenr) = '$low' or upper(varenr) = '$up'";
					$r=db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__));
...
#	print "<!--Function ordreside start-->";
	...
	global $oio,$oioubl,$oldRef,$omkunde,$ordresum;
...
#cho __line__. "$fokus<br>";	
	...
	if (db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__))) {
		...
		$debitorkort.= "<input type=\"button\" id='debitorkort' style=\"width:125px\" value=\"".lcfirst(findtekst(356,$sprog_id))."\" ";
...
#				if ($valutakurs)$kostsum*=$valutakurs/100; #20140116
			...
			if ($lagerantal>1) {
				...
				for ($l=0;$l<count($lagernr);$l++) {
					if ($lagernr[$l]==$lager[$x] && strlen($lagernavn[$l])<=2) {
						$lagerDisplay=$lagernavn[$l];
						if($bruger_id == -1){
							echo "<script>console.log($lager)</script>";
						}
						break;
					}
				}
...
#		print "<td align=\"center\">".dkdecimal($procenttillag,2)."% tillæg ".dkdecimal($tillag,2)." </td>\n";
		...
		if (($art!='DK')&&(!$krediteret)) {
			...
			print "<td align=\"center\" title=\"$title\"><input type=\"submit\" class=\"button gray medium\" value=\"".findtekst(2014, $sprog_id)."\" name=\"b_submit\"></td>\n";
		}
		print "</tbody></table></td></tr>\n"; #<- Tabel 2.5.2
		print "</tbody></table></td></tr>\n"; #<- Tabel 2.5
		print "</tbody></table></td></tr>\n"; #<- Tabel 2
...
#			$card_enabled="'on'.chr(9).'on'.chr(9).'on'.chr(9).'on'.chr(9).'on'.chr(9).'on'.chr(9).'on'.chr(9).'on'.chr(9).'on'.chr(9).'on'.chr(9).'on'.chr(9).'on'.chr(9).'on'.chr(9).'on'.chr(9).'on'";
		...
		if(isset($r['id'])){
			...
			if (!in_array($ref,$ansat)) {
				$qtxt = "select navn from ansatte,brugere where brugernavn='$ref' and ansatte.id=".nr_cast('brugere.ansat_id')."";
				$r=db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__));
...
/*
			if ($ref != $oldRef) {
				$qtxt = "select brugere.id from ansatte,brugere where ansatte.navn = '$ref' ";
				$qtxt.= "and brugere.ansat_id = ansatte.id";
				$r=db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__));
				$afd = get_settings_value('afd', 'brugerAfd', 1, $r['id']);
			}
*/
			if (!$afd) $afd = get_settings_value('afd', 'brugerAfd', 1, $bruger_id);
			print "</td><td></td>\n";
		...
		if ($vis_lev_addr || !$kontonr) {
			...
			if ($vis_saet) {
				...
				$modtaget = if_isset(dkdecimal($_GET['modtaget'],2));

				if (if_isset($_GET['godkendt'])=='OK' && usdecimal($_GET['modtaget'],2)==usdecimal($dkfelt_2,2)) {
					$betalt=usdecimal($modtaget,2);
					if ($_GET['cardscheme']) $felt_1=$_GET['cardscheme'];
					if ($_GET['kortnavn']) $felt_1=$_GET['kortnavn'];
					db_modify("update ordrer set betalt='$betalt',felt_1='$felt_1', felt_2='$felt_2' where id = '$id'",__FILE__ . " linje " . __LINE__);
				}
				if ($betalt) {
					$disabled='disabled';
...
#if ($bruger_id == -1) echo  "$useLane3000<br>";
						if ($useLane3000) $href = "payments/lane3000.php?amount=$dkfelt_2&id=$id&return_url=ordre.php&";
						else $href="http://".$terminal_ip[$felt_5-1]."/pointd/kvittering.php?url=$url&id=$id&kasse=$afd&kommando=kortbetaling&";
...
#				$antal[0]=NULL;
			...
			if ($lagerantal>1) {
				$stockId = $lager[0];
 				for ($l=0;$l<count($lagernr);$l++) {
					if ($lagernr[$l]==$lager[0] && strlen($lagernavn[$l])<=2) $lager[0]=$lagernavn[$l];
				}
			}
			...
			if (($status==2&&$bogfor!=0)||($status>0&&$hurtigfakt=='on')) {
				...
				if ($art!='DK' && !$dan_kn) {
					...
					if ($vis_saet) {
						...
						$qtxt.= "AND var_grp = 'deb_ordre' AND var_value = 'on' AND pos_id = $afd";
						$q = db_select($qtxt,__FILE__ . " linje " . __LINE__);
						($r = db_fetch_array($q))?$vis_betalingslink = 1:$vis_betalingslink = 0;
						if (!$betalt && $vis_betalingslink) $disabled='disabled';
					} 
					if ($r=db_fetch_array(db_select("select beskrivelse,box1 from grupper where art = 'AFD' and kodenr = '$afd'",__FILE__ . " linje " . __LINE__))) {
						$afd_navn=$r['beskrivelse'];
					...
					if($felt_1 == "Konto" || $felt_1 == "Kontant" || $felt_1 == "Tilgodebevis" || $felt_1 == "Gavekort" || $felt_1 == "Mobilepay"){
						$disabled='';
					}
					$disabled='';
...
#	}
	...
	#cho "x $beskrivelse $pris<br>";
		...
		if ($lagerantal > 1) {
			for ($l=0;$l<count($lagernr);$l++) {
				if ($lagernr[$l]==$lager && strlen($lagernavn[$l])<=2) {
					$lager=$lagernavn[$l];
				}
			}
