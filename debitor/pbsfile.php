...
@session_start();
$s_id=session_id();
// ------------debitor/pbsfile.php------- patch 3.2.5---2011-12-02------
// LICENS
//
...
if (!$afslut) for ($x=1;$x<=$lnr;$x++)	print "<tr><td>".str_replace(" ","&nbsp;",$linje[$x])."</td></tr>";
if ($afslut || $afsendt) print "<tr><td title=\"Kilk for at &aring;bne PBS filen. H&oslash;jreklik for at gemme.\" align=center> H&oslash;jreklik <b><a href=\"$filnavn\" target=\"blank\">her</a></b> for at gemme PBS filen</td></tr>";
...
#		}	
		...
		$lnr++;
		$linje[$lnr]="BS052".$pbs_nr[0]."0241".$recordnr."00001".$kontonr.$pbs_nr[$x]." ".$beskrivelse."\n";
		if ($afslut) {
			if ($charset=="UTF-8") $linje[$lnr]=utf8_encode($linje[$lnr]);
			db_modify("insert into pbs_linjer (liste_id,linje) values ('$id','$linje[$lnr]')",__FILE__ . " linje " . __LINE__);
		}
