...
// 20260908 CDX/LH Complete optional performed-by selection, escaping and translation (SD-558).

require_once __DIR__ . '/orderIncludes/performedBy.php';
...
if (!$id && $konto_id && $kontonr && !strstr($b_submit, 'Opslag')) {
	...
	$qtxt .= "'$cvrnr','$ean','$institution','$email','$mail_fakt','$phone','$notes','DO','$ordredate','$momssats','$tidspkt','$ref','" . db_escape_string((string)$hvem) . "',";
...
if ($b_submit) {
	...
	$hvem         = isset($_POST['hvem']) && is_string($_POST['hvem']) ? $_POST['hvem'] : null;
...
if (($status < 3 || strstr($b_submit, "Kopi") || strstr($b_submit, "Kred")) && $b_submit) {
	...
	if (!$id && !$gl_id && $konto_id && $firmanavn) {  # Opretter ny ordre fra konto id
		...
		$qtxt .= "'$lev_bynavn', '$lev_kontakt', '$lev_land', '$lev_email', '$betalingsdage', '$betalingsbet', ";
		$qtxt .= "'$cvrnr', '$ean', '$institution', '$email', '$mail_fakt', '$phone', '$notes', '$art', '$ordredate', ";
		$qtxt .= "'$momssats', $status, '$ref','" . db_escape_string((string)$hvem) . "', '$lev_adr', '$valuta', '$masterprojekt', '$formularsprog', '$pbs', '$afd', ";
		...
		if ($id) {
			...
			if ($tidspkt && $who) {
				...
				if ($hvem === null) {
					$orderId = intval($id);
					$r = db_fetch_array(db_select("select hvem from ordrer where id = '$orderId'", __FILE__ . " linje " . __LINE__));
					$hvem = $r['hvem'] ?? '';
				}
...
if ((strstr($b_submit, 'Kopi')) || (strstr($b_submit, 'Kred'))) {
	...
	if ((!$id) && ($konto_id)) {
		...
		$qtxt .= "'$cvrnr','$ean','$institution','$email','$mail_fakt','$phone','$notes','$art','$ordredate','$momssats','$status','$ref','" . db_escape_string((string)$hvem) . "','$lev_adr',";
...
if ($b_submit == 'del_ordre') {
	...
	$qtxt .= "'$r[ordredate]','$r[momssats]','$r[tidspkt]','" . db_escape_string($r['ref']) . "','" . db_escape_string((string)$r['hvem']) . "','$r[status]',";
...
function ordreside($id, $regnskab)
{
	...
	if ($status >= 3) {
		...
		print "<input type=\"hidden\" name=\"hvem\" value=\"" . htmlspecialchars((string)$hvem, ENT_QUOTES, 'UTF-8') . "\">";
		...
		*/
		...
		print "<tr class='tableTexting2'><td><b>" . findtekst('5149|Udført af', $sprog_id) . "</b></td><td>" . htmlspecialchars((string)$hvem, ENT_QUOTES, 'UTF-8') . "</td></tr>\n";
		...
		*/
		...
		print "<tr><td>" . findtekst('5149|Udført af', $sprog_id) . "</td>\n";
		print "<td><select style=\"width:130px;\" class='inputbox' name=\"hvem\" onchange=\"docChange = true;\" $disabled>\n";
		print performedByOptions($hvem, $ansat ?? array(), $brugernavn);
		print "</select>\n";
