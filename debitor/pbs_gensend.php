...
if (isset($_POST['sidste_id'])) {
	if (substr($rettigheder, $modulnr, 1) < '1') {
		$_SESSION['pbs_gensend_besked'] = findtekst('5186|Du har ikke rettigheder til at gensende til PBS', $sprog_id);
	} else {
...
print "<td width=\"80%\" $top_bund>" . findtekst('5179|PBS-historik', $sprog_id) . "</td>";
...
if (count($s['forsoeg'])) {
	...
	print "<tr><td>#</td><td>" . findtekst('5181|Leverance', $sprog_id) . "</td><td>" . findtekst('438|Dato', $sprog_id) . "</td>";
	print "<td>" . findtekst('65|Oprettet', $sprog_id) . "</td><td>" . findtekst('990|Bruger', $sprog_id) . "</td>";
	print "<td>" . findtekst('494|Status', $sprog_id) . "</td><td>" . findtekst('5185|Gensendt fra forsøg', $sprog_id) . "</td></tr>";
	foreach ($s['forsoeg'] as $f) {
		if (!$f['afsendt']) {
			$status = findtekst('5188|I kø', $sprog_id);
		} elseif ($f['resultat'] == 'afvist') {
			$status = findtekst('3011|Afvist', $sprog_id) . " " . dkdato($f['resultat_dato']) . ": " . htmlspecialchars($f['resultat_ref']);
		} else {
			$status = findtekst('5182|Afsendt', $sprog_id) . " - " . findtekst('5183|Afventer svar', $sprog_id);
		}
...
if ($s['kan_gensendes']) {
	...
	print findtekst('5177|Afvisning fra Nets (advis/fejlkode og dato)', $sprog_id) . "<br>";
	print "<textarea name=\"ref\" rows=\"3\" cols=\"60\"" . ($s['kraever_ref'] ? " required" : "") . "></textarea><br>";
	print "<input type=\"submit\" class=\"button gray medium\" name=\"gensend\" value=\"" . findtekst('5170|Gensend til PBS', $sprog_id) . "\">";
	print "</form></td></tr>";
}
print "</tbody></table>";
print "</body></html>";
