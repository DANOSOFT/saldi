...
// ------------/admin/opret.php-----patch 3.2.4------ 2012-02-20 --------
// LICENS
//
...
#	db_modify("CREATE SEQUENCE id START 1 INCREMENT 1 MAXVALUE 9223372036854775807 MINVALUE 1 CACHE 1",__FILE__ . " linje " . __LINE__);
	...
	db_modify("CREATE TABLE regulering (id serial NOT NULL,vare_id integer,lager integer,beholdning numeric(15,3),optalt numeric(15,3),tidspkt text,bogfort bool,transdate date,logtime time,bogfort_af text, PRIMARY KEY (id))",__FILE__ . " linje " . __LINE__);
...
#	 include("../includes/kontoplan.php"); 
		...
		if ($fp) {
			...
			while (!feof($fp)) {
				$x++;	
				list($kontonr[$x],$beskrivelse[$x],$kontotype[$x],$moms[$x],$fra_kto[$x]) = explode(chr(9),fgets($fp));
				if (!$kontonr[$x]) {$x--;}
			}
			$kontoantal=$x;
			for ($x=1; $x<=$kontoantal; $x++){
				$beskrivelse[$x]=addslashes(trim(str_replace('"','',$beskrivelse[$x])));
