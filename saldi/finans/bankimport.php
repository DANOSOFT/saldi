	...
	if ($fp) {
		...
		while (!feof($fp)) {
			...
			if ($skriv_linje==1){
				...
				if ($amount>=0) db_modify("insert into kassekladde (bilag, transdate, beskrivelse, d_type, debet, amount, momsfri, kladde_id) values ('$bilag', '$transdate', '$beskrivelse', 'F', '$kontonr', '$amount', 'on', '$kladde_id')");
				else {
					$amount=$amount*-1;
					db_modify("insert into kassekladde (bilag, transdate, beskrivelse, k_type, kredit, amount, momsfri, kladde_id) values ('$bilag', '$transdate', '$beskrivelse', 'F', '$kontonr', '$amount', 'on', '$kladde_id')");
				}
				$bilag++;
			}
		}
