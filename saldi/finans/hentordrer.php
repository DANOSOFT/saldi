<?php
// -------------------------------------------finans/hentordrer.php------------lap 1.0.02------------------
// LICENS
//
	...
	for ($x=1;$x<=$ordreantal;$x++){
		...
		if (substr($art[$x],0,1)=='K'){
			if ($sum[$x] < 0) {
				...
				print "<tr><td>$fakturadato[$x]</td><td>$firmanavn[$x]</td><td><br></td><td><br></td><td>K</td><td>$kontonr[$x]</td><td>$fakturanr[$x]</td><td align=right>$dksum</td>";
			}
...
#							$pris[$y]=$pris[$y]*-1;
						...
						print "<tr><td>$fakturadato[$x]</td><td>$firmanavn[$x]</td><td><br></td><td><br></td><td>F</td><td>$bogf_konto[$y]</td><td>$fakturanr[$x]</td><td align=right>$tmp</td></tr>";
					}
					elseif ($pris[$y]>0) {
						 $tmp=dkdecimal($pris[$y]);
						 print "<tr><td>$fakturadato[$x]</td><td>$firmanavn[$x]</td><td>F</td><td>$bogf_konto[$y]</td><td><br></td><td><br></td><td>$fakturanr[$x]</td><td align=right>$tmp</td></tr>";
					}
				}
			}
			...
			if (substr(trim($row[box1]),0,1)=='E') {
				...
				if ($moms[$x]>0) {
					print "<tr><td>$fakturadato[$x]</td><td>$firmanavn[$x]</td><td><br></td><td><br></td><td>F</td><td>$row[box1]</td><td>$fakturanr[$x]</td><td align=right>$tmp</td></tr>";
				} else {
					print "<tr><td>$fakturadato[$x]</td><td>$firmanavn[$x]</td><td>F</td><td>$row[box1]</td><td><br></td><td><br></td><td>$fakturanr[$x]</td><td align=right>$tmp</td></tr>";
				}
			}
			else {
...
function flytordre($kladde_id, $ordre_id) {
	...
	if ($row = db_fetch_array($query)) {
		...
		if (($ym>=$aarstart)&&($ym<=$aarslut)) {
			...
			$afd=$afd*1; #sikkerhed for at 'afd' har en v�di 
...
#			else {$firmanavn=$firmanavn." - ".$fakturanr;}
			...
			if (substr($art,0,1)=='K') {
				...
				db_modify("insert into kassekladde (bilag, transdate, beskrivelse, k_type, kredit, faktura, amount, momsfri, kladde_id, afd) values ('$bilag', '$transdate', '$beskrivelse', 'K', '$kontonr', '$fakturanr', '$sum', 'on', '$kladde_id', $afd)");
				...
				for ($y=1;$y<=$ordrelinjer;$y++) {
					if ($bogf_konto[$y]) {
						...
						if ($pris[$y]<0) {		 
							$pris[$y]=$pris[$y]*-1;
							db_modify("insert into kassekladde (bilag, transdate, beskrivelse, k_type, kredit, faktura, amount, momsfri, kladde_id, afd) values ('$bilag', '$transdate', '$beskrivelse', 'F', '$bogf_konto[$y]', '$fakturanr', '$pris[$y]', 'on', '$kladde_id', $afd)");
						}
						elseif ($pris[$y]>0) {db_modify("insert into kassekladde (bilag, transdate, beskrivelse, d_type, debet, faktura, amount, momsfri, kladde_id, afd) values ('$bilag', '$transdate', '$beskrivelse', 'F', '$bogf_konto[$y]', '$fakturanr', '$pris[$y]', 'on', '$kladde_id', $afd)");}
					}
				}
				...
				if (substr(trim($row[box1]),0,1)=='E') {
					...
					db_modify("insert into kassekladde (bilag, transdate, beskrivelse, k_type, kredit, faktura, amount, momsfri, kladde_id, afd) values ('$bilag', '$transdate', '$beskrivelse', 'F', '$row[box1]', '$fakturanr', '$moms', 'on', '$kladde_id', $afd)");
				}
				else {
					$query = db_select("select box1 from grupper where art='KM' and kodenr='$box1'");
					$row = db_fetch_array($query);
				...
				db_modify("insert into kassekladde (bilag, transdate, beskrivelse, d_type, debet, faktura, amount, momsfri, kladde_id, afd) values ('$bilag', '$transdate', '$beskrivelse', 'F', '$box1', '$fakturanr', '$moms', 'on', '$kladde_id', $afd)");
			}
			else	{
				if (substr($art,1,1)=='K') $beskrivelse ="Kreditnota - ".$fakturanr;
				else $beskrivelse ="Faktura - ".$fakturanr;
				if ($sum[$x]<0) {db_modify("insert into kassekladde (bilag, transdate, beskrivelse, k_type, kredit, faktura, amount, momsfri, kladde_id, afd) values ('$bilag', '$transdate', '$beskrivelse', 'D', '$kontonr', '$fakturanr', '$sum', 'on', '$kladde_id', $afd)");}
				else {db_modify("insert into kassekladde (bilag, transdate, beskrivelse, d_type, debet, faktura, amount, momsfri, kladde_id, afd) values ('$bilag', '$transdate', '$beskrivelse', 'D', '$kontonr', '$fakturanr', '$sum', 'on', '$kladde_id', $afd)");}
				$query = db_select("select * from ordrelinjer where ordre_id=$id;");
				$y=0;
				$bogf_konto = array();
				while ($row = db_fetch_array($query)) {
				...
				for ($y=1;$y<=$ordrelinjer;$y++) {
					if ($bogf_konto[$y]) {
						...
						{		 
							$pris[$y]=$pris[$y]*-1;
							db_modify("insert into kassekladde (bilag, transdate, beskrivelse, d_type, debet, faktura, amount, momsfri, kladde_id, afd) values ('$bilag', '$transdate', '$beskrivelse', 'F', '$bogf_konto[$y]', '$fakturanr', '$pris[$y]', 'on', '$kladde_id', $afd)");
						}
						elseif ($pris[$y]>0) {db_modify("insert into kassekladde (bilag, transdate, beskrivelse, k_type, kredit, faktura, amount, momsfri, kladde_id, afd) values ('$bilag', '$transdate', '$beskrivelse', 'F', '$bogf_konto[$y]', '$fakturanr', '$pris[$y]', 'on', '$kladde_id', $afd)");}
					}
				}
				$query = db_select("select gruppe from adresser where id='$konto_id';");
				$row = db_fetch_array($query);
				...
				$row = db_fetch_array($query);
				$box1=trim($row[box1]);
				db_modify("insert into kassekladde (bilag, transdate, beskrivelse, k_type, kredit, faktura, amount, momsfri, kladde_id, afd) values ('$bilag', '$transdate', '$beskrivelse', 'F', '$box1', '$fakturanr', '$moms', 'on', '$kladde_id', $afd)");
			}
