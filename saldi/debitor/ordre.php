...
if ($_GET['konto_id']){
	...
	if ($row = db_fetch_array($query)) {
		...
		$notes=$row['notes'];
		$gruppe=$row['gruppe'];
		$kontoansvarlig=$row['kontoansvarlig'];
	 }
	if ($kontoansvarlig){
	...
	if ($gruppe){
		$r = db_fetch_array(db_select("select box1, box5 from grupper where art='DG' and kodenr='$gruppe'"));
	...
	if ((!$id)&&($firmanavn)) {
		...
		db_modify("insert into ordrer (ordrenr, konto_id, kontonr, firmanavn, addr1, addr2, postnr, bynavn, land, betalingsdage, betalingsbet, cvrnr, ean, institution, notes, art, ordredate, momssats, hvem, tidspkt, ref) values ($ordrenr, '$konto_id', '$kontonr', '$firmanavn', '$addr1', '$addr2', '$postnr', '$bynavn', '$land', '$betalingsdage', '$betalingsbet', '$cvrnr', '$ean', '$institution', '$notes', 'DO', '$ordredate', '$momssats', '$brugernavn', '$tidspkt', '$ref')");
	...
	if ($_GET['vare_id']) {
		$query = db_select("select grupper.box5 as box5, ordrer.status as status from ordrer, adresser, grupper where ordrer.id='$id' and adresser.id=ordrer.konto_id and grupper.art='DG' and grupper.kodenr=adresser.gruppe");
		$row = db_fetch_array($query);
		if ($row[status]>2) {
			...
			if ($r2 = db_fetch_array(db_select("select box6, box7 from grupper where art = 'VG' and kodenr = '$row[gruppe]'"))) {
				$momsfri = $r2['box7'];
				if (($r2[box6]!=NULL)&&($rabat>$r2['box6'])) $rabat=$r2['box6'];
			}
			if(!$antal) $antal=1;
			$rabat=$rabat*1;
			if ($linjenr==0) db_modify("insert into ordrelinjer (vare_id, ordre_id, posnr, varenr, beskrivelse, enhed, pris, rabat, serienr, momsfri) values ($vare_id, '$id', '$posnr', '$varenr', '$beskrivelse', '$enhed', '$pris', '$rabat', '$serienr', '$momsfri')");
		}
	}
	...
	if ($_POST['submit']) {
		...
		$kundeordnr =	trim($_POST['kundeordnr']);
		...
		$r = db_fetch_array(db_select("select grupper.box5 as box5 from adresser, grupper where adresser.kontonr='$kontonr' and adresser.art='D' and grupper.art='DG' and grupper.kodenr=adresser.gruppe"));
		$rabatsats=$r['box5']*1;
...
#			if (strlen($beskrivelse[$x])>60){$beskrivelse[$x]=substr($beskrivelse[$x],0,60);}
		...
		if (($kontonr)&&(!$firmanavn)) {
			...
			if ($gruppe) {
				$query = db_select("select box2, box5 from grupper where art='KM' and kodenr='$gruppe'");
				$row = db_fetch_array($query);
				$momssats=$row['box2'];
				$rabatsats=$row['box5']*1;
			}
		}
		if ((!$id)&&($firmanavn)){
			$query = db_select("select ordrenr from ordrer where art='DO' or art='DK' order by ordrenr desc");
...
#			if (!$konto_id) $konto_id=find_konto_id($kontonr);
#			if (!$betalingsdage) $betalingsdage=find_betalingsdage($konto_id);
					...
					if ($row = db_fetch_array($query)) {
						...
						if ($r2 = db_fetch_array(db_select("select box6, box7 from grupper where art = 'VG' and kodenr = '$row[gruppe]'"))) {
							...
							if (($r2['box6']!=NULL)&&($rabat[0] > $r2['box6'])) {
								$rabat[0]=$r2['box6'];
								print "<BODY onLoad=\"javascript:alert('Max rabat på varenr $varenr[0] er $rabat[0] %')\">";
							}
						}
						$rabat[0]=$rabat[0]*1;
						db_modify("insert into ordrelinjer (vare_id, ordre_id, posnr, varenr, beskrivelse, enhed, antal, pris, rabat, serienr, momsfri) values ($vare_id[0], '$id', '$posnr_ny[0]', '$varenr[0]', '$beskrivelse[0]', '$enhed[0]', '$antal[0]', '$pris[0]', '$rabat[0]', '$serienr[0]', '$momsfri[0]')");
					}
				}
		...
		if ((strstr($submit,'Kopi'))||(strstr($submit,'Kred')))	{
			...
			for($x=1; $x<=$linjeantal; $x++) {
				if (($varenr[$x])&&(!$vare_id[$x])) {
					...
					else print "<BODY onLoad=\"Varenummer $varenr[$x] kan ikke findes.\">";
				}
				if ($vare_id[$x]){
...
#	 print "</tr>\n";
		...
		$tmp=$momssum/100*$momssats+0.000001; #puuuh den er grim - men ellers runder den ned ved v. 0,5 øre ??
		$moms=round($tmp,2);
		$ialt=$sum+$moms;
#		$sum=dkdecimal($sum);
#		$moms=dkdecimal($moms);
...
#		print "<tr><td>$font<small>Att.:</td><td colspan=2>$font<small><input type=text size=25 name=kontakt value=\"$kontakt\"></td></tr>\n";
		...
		if ($status>=2) {
			...
			print "</td><td colspan=2>$font<small><input type=text size=10 name=fakturadato value=\"$fakturadato\"></td></tr>\n";
		}
...
#		}		
		...
		print "</SELECT>";
		if ($status==0) print "<tr><td colspan=2 witdh=200>$font<small>Godkend<input type=checkbox name=godkend></td></tr>\n";
		if ($status==1) {
			print "<tr><td colspan=2 witdh=200>$font<small>"; 
			if ($art=='DO') print "Dan faktura"; 
			else print "Dan kreditnota";
			print "<input type=checkbox name=omdan_t_fakt></td></tr>\n";
		}
		print "</tbody></table></td>";
		print "<td width=33%><table cellpadding=0 cellspacing=0 border=0 width=250>";
...
#					if (substr($dkantal[$x]-1,1)==","){$dkantal[$x]=substr($dkantal[$x],0,(strlen($dkantal[$x])-1));}
			...
			if ($status>=1)	{
				if ($vare_id[$x]){
					...
					print "<input type=\"hidden\" name=tidl_lev[$x] value=\"$tidl_lev[$x]\">";
...
#				if ($art=='DO') 
				print "<td align=center width=$width><input type=submit accesskey=\"f\" value=\"Afslut\" name=\"submit\"></td>";
#				else print "<td align=center width=$width><input type=submit accesskey=\"f\" value=\"Kredit&eacute;r\" name=\"submit\"></td>";
			}
			elseif (($linjeantal>0)&&($art=='DO')) {
				print "<td align=center width=$width><input type=submit value=\"&nbsp;Udskriv&nbsp;\" name=\"submit\"></td>";
			}
			if (($sum==0)&&($linjeantal==0)) {print "<td align=center><input type=submit value=\"&nbsp;&nbsp;Slet&nbsp;&nbsp;\" name=\"submit\"></td>";}
