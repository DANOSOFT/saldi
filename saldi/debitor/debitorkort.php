<?php
// ----------------------------------------------------debitor/debitorkort.php-----(modul nr 6)------lap 1.0.7----------
// LICENS
//
...
############################
 	...
 	if ($submit=="Slet") {db_modify("delete from adresser where id = $id");}
 	 	...
 	 	if ($id==0) {
 	 	 	...
 	 	 	elseif($ny_kontonr) {
 	 	 	 	db_modify("insert into adresser (kontonr, firmanavn, addr1, addr2, postnr, bynavn, land, kontakt, tlf, fax, email, web, betalingsdage, kreditmax, betalingsbet, cvrnr, ean, institution, notes, art, gruppe, kontoansvarlig) values ('$ny_kontonr', '$firmanavn', '$addr1', '$addr2', '$postnr', '$bynavn', '$land', '$kontakt', '$tlf', '$fax', '$email', '$web', '$betalingsdage', '$kreditmax', '$betalingsbet', '$cvrnr', '$ean', '$institution', '$notes', 'D', '$gruppe', '$kontoansvarlig')");
 	 	 	 	$q = db_select("select id from adresser where kontonr = '$ny_kontonr' and art = 'D'");
 	 	 	 	$r = db_fetch_array($q);
 	 	 	 	$id = $r[id];
 	 	 	}
