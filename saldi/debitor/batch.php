...
/*			
				...
				if ($valg[$x] < $max_antal[$x])	{print "<BODY onLoad=\"javascript:alert('Ordrenr: $ordrenr[$x] - Der kan ikke v&aelig;lges f&aelig;rre end $max_antal[$x]!')\">";}
				$valgt_antal=$valgt_antal-$valg[$x];
				$rest_antal=$rest_antal-$rest[$x];
			}
*/		
...
*/		
		print "<tr>";
		print "<td>$font<small><span title= 'kostpris: $pris[$x]'>$ordrenr[$x]</small></td>";
		print "<td align = right><small>$font $rest[$x]</small></td>";
	...
	$query = db_select("select * from reservation where batch_salg_id=0 and vare_id = $vare_id and antal > 0"); 
	if ($row = db_fetch_array($query)) {
	...
	while ($row = db_fetch_array($query)) {
		...
		$q2 = db_select("select art, ordrenr, status from ordrer where id=$ordre_id[$x]"); 
		$r2 = db_fetch_array($q2);
		if ($r2[status]<3) {
			$ordrenr[$x]=$r2[ordrenr];
			$art[$x]=$r2[art];
			 ...
			 db_modify("delete from reservation where linje_id=$res_linje_id[$x]");
			break;
		}
		...
		if ($rest[$x]>0) {
			print "<tr>";
			if ($art[$x]=='DK') print "<td>$font $ordrenr[$x] (KN)</td><td align = right>$font $rest[$x]</td>";
			else print "<td>$font $ordrenr[$x]</td><td align = right>$font $rest[$x]</td>";
			print "<td align = right><input type=text style=\"text-align:right\" size=3 name=valg_$x value=$valg[$x]></td>";
			print "</tr>\n";
		}
...
else	#er der tale om en negativ leverance
{	
	...
	print "<tr><td colspan=5><hr></td></tr>\n";
		...
		for ($x=1; $x<=$id_antal; $x++) {
			...
			while ($row = db_fetch_array($query)) {
				$y++;
				print "<tr><td onClick=\"javascript:window.open('ordre.php?id=$kred_ordre_id[$x]','$kred_ordrenr[$x]','width=400,height=400,scrollbars=1,resizable=1,menubar=no,location=no');\";><a href>$font $kred_ordrenr[$x]</a></td><td></td><td><td align = right>$font $row[antal]</td>";
...
if ($batch_antal==0) {
	...
	elseif (($antal<0)||($leveres!=0)) {print "<tr><td collspan=3>$font Denne vare er aldrig blevet solgt til kunden</td></tr>\n";}
