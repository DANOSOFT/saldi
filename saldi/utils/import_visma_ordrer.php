...
if ($fp) {
	...
	while (!feof($fp)) {
		...
		if (($x>=$startlinje) && ($x<$slutlinje) && (substr($linje,0,1)=='"')) { 
			...
			if (($Nm)&&($CustNo)) {
#echo "INSERT INTO ordrer (konto_id, ordrenr, fakturanr, kontonr, firmanavn, addr1, addr2, postnr, bynavn, land, kontakt, tlf, kreditmax, cvrnr, art, ordredate, fakturadate, sum) values ('$konto_id', $OrdNo', '$InvoNo', '$CustNo', '$Nm', '$Ad1', '$Ad2', '$PNo', '$PArea', '$Ctry', '$Pers', '$Phone', '$Fax', '$MailAd', '$notes', '$CrLmt', '$CM', 'DO', '$OrdDt', '$LstInvDt' '$InvoAm')<br>";				
			if ($LstInvDt) {
#				$aar=substr($LstInvDt,0,4);
#				$md=substr($LstInvDt,4,2);
#				$dato=substr($LstInvDt,6,2);
#				$LstInvDt=$aar."-".$md."-".$dato;
#				$status=4;
			#	db_modify("INSERT INTO ordrer (konto_id, ordrenr, fakturanr, kontonr, firmanavn, addr1, addr2, postnr, bynavn, land, kontakt, notes, cvrnr, art, ordredate, fakturadate, sum, status) values ('$konto_id', '$OrdNo', '$InvoNo', '$CustNo', '$Nm', '$Ad1', '$Ad2', '$PNo', '$PArea', '$Ctry', '$Pers', '$notes', '$CM', 'DO', '$OrdDt', '$LstInvDt', '$InvoAm', '4')");
			}
			else db_modify("INSERT INTO ordrer (konto_id, ordrenr, kontonr, firmanavn, addr1, addr2, postnr, bynavn, land, kontakt, notes, cvrnr, art, ordredate, sum, status) values ('$konto_id', '$OrdNo', '$CustNo', '$Nm', '$Ad1', '$Ad2', '$PNo', '$PArea', '$Ctry', '$Pers', '$notes', '$CM', 'DO', '$OrdDt', '$InvoAm', '1')");
