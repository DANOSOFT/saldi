<?php
// ---------------------------------------------includes/opdater.php-------lap 1.0.8--------------------
// LICENS
//
...
if ($backup=$_GET['backup']) {
	$version=$_GET['version']; 
	$dbver=$_GET['$dbver'];
	list($a, $b, $c)=split(".", $dbver); 
	if ($a==0) {opdat_0('1.0', $dbver);}
	if ($a==1) {opdat_1($b, $c);}
	print "<meta http-equiv=\"refresh\" content=\"0;URL=../includes/logout.php\">";
}
...
function opdat_0($version, $dbver){
	...
	if (!$backup) {
		...
		print "<tr><td align=center><a href=../includes/opdater.php?version=$version&dbver=$dbver&backup=OK accesskey=T>$font Forts&aelig;t</a><br></td></tr>";
...
function opdat_1($under_nr, $lap_nr){
	global $version;
	if ($under_nr<=1){
		if ($lap_nr<2){
			transaktion("begin");
			...
			$x=0;
			$query=db_select("SELECT ko18denr FROM grupper where art = 'LG' order by kodenr"); 
			while ($row = db_fetch_array($query)) {
				$x++;
		...
		if ($lap_nr<=6){
			transaktion("begin");
					if (!db_fetch_array(db_select("SELECT attrelid FROM pg_attribute where attname='kontoansvarlig'"))) db_modify("ALTER TABLE adresser ADD kontoansvarlig integer");
		...
		if ($lap_nr<=8){
			...
			if (!db_fetch_array(db_select("SELECT attrelid FROM pg_attribute where attname='box9'"))) {
