...
// -----------------------------------------------------/kreditor/rapport.php---------------lap 1.0.8-------------------
// LICENS
//
...
#		print "<tr><td colspan=8><hr></td></tr>";
 ...
 #			 <td><small><small>".forfaldsdag($row[transdate], $betalingsbet, $betalingsdage)."</small></small></td>";
				...
				if ($row[amount] < 0) {	
					if (($row[udlignet]!='1')&&($forfaldsdag<$currentdate)) print "<td><span style='color: rgb(255, 0, 0);'><small><small>".dkdato($forfaldsdag)."</small></small></td><td></td><td align=right><span style='color: rgb(255, 0, 0);'><small><small><a href=\"../includes/udlign_openpost.php?post_id=$row[id]&regnaar=$regnaar&maaned_fra=$maaned_fra&maaned_til=$maaned_til&konto_fra=$konto_fra&konto_til=$konto_til&retur=../kreditor/rapport.php\">$tmp</a></small></small></td>";
					else print "<td><span style='color: rgb(0, 0, 0);'><small><small>".dkdato($forfaldsdag)."</small></small></td><td></td><td align=right><span style='color: rgb(0, 0, 0);'><small><small>$tmp</small></small></td>";
					$forfaldsum=$forfaldsum+$row[amount];

#					print "<td></td><td align=right>$stil<small><small>$tmp</small></small><td></td></td>";
#					$forfaldsum=$forfaldsum+$row[amount];
				} else {print "</td><td><td align=right><small><small>$tmp</small></small></td><td></td>";}
				$kontosum=$kontosum+$row[amount];
				$tmp=dkdecimal($kontosum);
