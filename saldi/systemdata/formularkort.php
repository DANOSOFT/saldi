<?php
// --------------------------------------systemdata/formularkort -------lap 1.0.9-------------------------
// LICENS
//
...
#	if ($formular==6) {tjek_rykker();}
	...
	if (($_POST['op']) || ($_POST['hojre'])) {
		...
		$query=db_select("select id, xa, xb, ya, yb from formularer where formular=$form_nr");
	...
	if (($form_nr==6)&&($art_nr==2)&&($_POST['gebyr'])) {
		$gebyr=usdecimal($_POST['gebyr']);
		if ($row=db_fetch_array(db_select("select id from formularer where beskrivelse ='GEBYR' and formular=6 and art=2"))) {db_modify("update formularer set xb=$gebyr where id = $row[id]");}
		else {
		db_modify("insert into formularer (beskrivelse, formular, art, xb) values ('GEBYR', 6, 2, $gebyr)");}
	}
...
print "<option>6:Rykkerbrev</option>";
print "</SELECT></td>";
...
elseif ($art_nr==2) {
	if (substr($formular,0,1)==6) {
		if ($row=db_fetch_array(db_select("select xb from formularer where beskrivelse ='GEBYR' and formular=$form_nr and art=$art_nr"))) {$gebyr=dkdecimal($row[xb]);}
		print "<tr><td colspan=11 align=center>$font<small>Rykkergebyr <input type=text align=right size=15 name=gebyr value=$gebyr> kr.</td></tr>";
	...
	if (substr($formular,0,1)==6) {
		print "<option>kunde_firmanavn</option>";
		print "<option>kunde_addr1</option>";
		print "<option>kunde_addr2</option>";
		print "<option>kunde_postnr</option>";
		print "<option>kunde_bynavn</option>";
		print "<option>kunde_land</option>";
		print "<option>kunde_kontakt</option>";
		print "<option>kunde_cvrnr</option>";
	...
	# echo "select * from formularer where formular = $form_nr and art = $art_nr<br>";
	$query=db_select("select * from formularer where formular = $form_nr and art = $art_nr and beskrivelse != 'GEBYR' order by ya desc, xa");
	while ($row=db_fetch_array($query))
	{
		$x++;
		print "<tr>";
...
elseif ($art_nr==4) {
	...
	print "<tr><td align=center>$font<small>Op</td><td><input type=text align=right size=2 name=op></td><tr>";
	print "<tr><td align=center>$font<small>Højre</td><td><input type=text align=right size=2 name=hojre></td><tr>";
}
...
<tr><td	width="100%" height="2.5%" align = "center" valign = "bottom">		
		<table width="100%" align="center" border="0" cellspacing="0" cellpadding="0"><tbody>
			<td width="45%" align=center bgcolor="<?php echo $bgcolor2 ?>"></td>
			<td width="10%" align=center bgcolor="<?php echo $bgcolor2 ?>" onClick="javascript:window.open('formularimport.php', '','left=10,top=10,width=400,height=200,scrollbars=yes,resizable=yes,menubar=no,location=no')" onMouseOver="this.style.cursor = 'pointer'" ><font face="Helvetica, Arial, sans-serif" color="#000000"><small>
			<u>Formularimport</u></small></td>
			<td width="35%" bgcolor="<?php echo $bgcolor2 ?>"></td>
 			<td width="10%" align=right bgcolor="<?php echo $bgcolor2 ?>" onClick="javascript:window.open('logoslet.php', '','left=10,top=10,width=400,height=200,scrollbars=yes,resizable=yes,menubar=no,location=no')" onMouseOver="this.style.cursor = 'pointer'" ><font face="Helvetica, Arial, sans-serif" color="#000000"><small><u>Slet logo</u></small></td>
		</tbody></table>
</td></tr>
</tbody></table>
</body></html>
