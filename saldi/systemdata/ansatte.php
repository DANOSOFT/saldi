<?php
// -----------------------------------/systemdata/ansatte.php--------lap 1.0.8----------------------------
// LICENS
//
...
if ($_POST) {
	...
	if (($id==0)&&($navn)) {
		db_modify("insert into ansatte (navn, initialer, konto_id, addr1, addr2, postnr, bynavn, tlf, fax, privattlf, mobil, email, cprnr, notes, afd) values ('$navn', '$initialer', '$konto_id', '$addr1', '$addr2', '$postnr', '$bynavn', '$tlf', '$fax', '$privattlf', '$mobil', '$email', '$cprnr', '$notes', '$afd')");
		$query = db_select("select id from ansatte where konto_id = '$konto_id' and navn='$navn' order by id desc");
		$row = db_fetch_array($query);
		$id = $row[id];
	} elseif ($id > 0) {
		db_modify("update ansatte set navn = '$navn', initialer='$initialer', konto_id = '$konto_id', addr1 = '$addr1', addr2 = '$addr2', postnr = '$postnr', bynavn = '$bynavn', email = '$email', tlf = '$tlf', fax = '$fax', privattlf= '$privattlf', mobil = '$mobil', cprnr = '$cprnr', notes = '$notes', afd = '$afd' where id = '$id'");
	}
...
print "<td width=\"25%\" bgcolor=\"$bgcolor2\" align = \"right\"><font face=\"Helvetica, Arial, sans-serif\" color=\"#000066\"><small><a href=ansatte.php?returside=$returside&fokus=$fokus&konto_id=$konto_id accesskey=N>Ny</a><br></small></td>";
print "</tbody></table>";
print "</td></tr>";
