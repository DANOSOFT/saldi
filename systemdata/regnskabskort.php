...
if ($menu=='T') {
		...
		print "<button style = 'width:80px;'>".findtekst('2172|Luk', $sprog_id)."</button></a>";
...
#		$primotal = if_isset($_POST['primotal']);
	...
	if (($startmd<1)||($startmd>12)){
		alert(findtekst('2348|Startmåned', $sprog_id)." ".strtolower(findtekst('2352|Skal være mellem', $sprog_id))." 1 ".strtolower(findtekst('2353|Og', $sprog_id))." 12!");
		$startmd="";
	}
	elseif ($startmd<10) $startmd="0".$startmd;
	if (($slutmd<1)||($slutmd>12)){
		alert(findtekst('2349|Slutmåned', $sprog_id)." ".strtolower(findtekst('2352|Skal være mellem', $sprog_id))." 1 ".strtolower(findtekst('2353|Og', $sprog_id))." 12!");
		$slutmd="";
	}
	elseif ($slutmd<10) $slutmd="0".$slutmd;
	if (($startaar<$bundaar)||($startaar>$topaar)){
		alert(findtekst('2350|Startår', $sprog_id)." ".strtolower(findtekst('2352|Skal være mellem', $sprog_id))." $bundaar ".strtolower(findtekst('2353|Og', $sprog_id))." $topaar!");
		$startaar="";
	}
	if (($slutaar<$bundaar)||($slutaar>$topaar)){
		alert(findtekst('2351|Slutår', $sprog_id)." ".strtolower(findtekst('2352|Skal være mellem', $sprog_id))." $bundaar ".strtolower(findtekst('2353|Og', $sprog_id))." $topaar!");
		$slutaar="";
	}
	$startdato=$startaar.$startmd;
	$slutdato=$slutaar.$slutmd;
	if ($slutdato<=$startdato){
		alert(findtekst('2354|Regnskabsår skal slutte senere end det starter', $sprog_id));
		$aaben="";
	}
