...
#	else {
	...
	if (!$skiftnavn) {
		...
		$tekst1=findtekst('749|Brugernavn til SMTP serveren', $sprog_id);
		...
		$tekst1=findtekst('750|Adgangskode til SMTP serveren', $sprog_id);
		...
		$tekst1=findtekst('751|Krypteringsmetode til SMTP serveren', $sprog_id);
...
function personlige_valg() {
	...
	$nuancefarver[0]=findtekst('418|Rød', $sprog_id); $nuancekoder[0]="+00-22-22";
	$nuancefarver[1]=findtekst('419|Grøn', $sprog_id); $nuancekoder[1]="-22+00-22";
	$nuancefarver[2]=findtekst('420|Blå', $sprog_id); $nuancekoder[2]="-22-22+00";
	$nuancefarver[3]=findtekst('421|Gul', $sprog_id); $nuancekoder[3]="+00+00-33";
	$nuancefarver[4]=findtekst('422|Magenta (lyselilla)', $sprog_id); $nuancekoder[4]="+00-33+00";
	$nuancefarver[5]=findtekst('423|Cyan (lyseblå)', $sprog_id); $nuancekoder[5]="-33+00+00";
...
#	print "<input type=hidden name=id value='$id'>";
	...
	print "<tr><td title='".findtekst('207|Hvis du afmærker dette felt', $sprog_id)."'>".findtekst('208|Anvend popup-vinduer', $sprog_id)."</td><td><input class='inputbox' type='checkbox' name='popup' $popup></td></tr>";
...
#	}	else $gl_menu='checked';
	...
	print "<tr><td title='".findtekst('209|Her skriver du de parametrer', $sprog_id)."'>".findtekst('210|Popup-indstillinger', $sprog_id)."</td><td colspan='4'><input class='inputbox' type='text' style='width:600px' name='jsvars' value='$jsvars'></td></tr>";
	if ($menu=='T') {
...
function div_valg() {
	...
	print "<tr bgcolor='$bgcolor5'>\n<td title='".findtekst('186|Hvis dette felt afmærkes', $sprog_id)."'>".findtekst('162|Tvungen valg af debitorgruppe på debitorkort', $sprog_id)."</td>\n";
	print "<td title='".findtekst('186|Hvis dette felt afmærkes', $sprog_id)."'>\n";
	print "<!-- 162 : Tvungen valg af debitorgruppe på debitorkort -->";
	print "<input name='box1' class='inputbox' type='checkbox' $gruppevalg>\n";
	print "</td></tr>\n";
	print "<tr>\n<td title='".findtekst('187|Hvis dette felt afmærkes', $sprog_id)."'>".findtekst('163|Tvungen valg af kundeansvarlig på debitorkort', $sprog_id)."</td>\n";
	print "<td title='".findtekst('187|Hvis dette felt afmærkes', $sprog_id)."'>\n";
...
#	print "</td></tr>\n"; #20131101
	...
	if ($box8) {
		...
		print "<tr bgcolor='$bgcolor'>\n<td title=''>".findtekst('528|- FTP-server hos ebConnect', $sprog_id)."</td>\n";
		print "<td><input name='oiourl' class='inputbox' style='width:150px;' type='text' value='$oiourl'></td>\n</tr>\n";
		print "<tr>\n<td title=''>".findtekst('529|- FTP-brugernavn hos ebConnect', $sprog_id)."</td>\n";
		print "<td><input name='oiobruger' class='inputbox' style='width:150px;' type='text' value='$oiobruger'></td>\n</tr>\n";
		print "<tr>\n<td title=''>".findtekst('530|- FTP-adgangskode hos ebConnect', $sprog_id)."</td>\n";
  ...
  $guid = $r ? str_split($r[0], 7)[0]."-xxxx-xxxx-xxxx-xxxxxxxxxxxx" : "";
	...
	$mtxt=findtekst('3014|Flatpay ID', $sprog_id);
	$mtitle=findtekst('3013|Dit hemmelige Flatpay ID, klik inputtet for at lave en ny', $sprog_id);
  ...
  $APIKEY = $r ? $r[0] : "";
	...
	$mtxt=findtekst('3016|Vibrant API nøjle', $sprog_id);
	$mtitle=findtekst('3017|API nøjlen til din Vibrant APP', $sprog_id);
  print "<tr>\n<td title='$mtitle'><!-- Tekst 3013 -->$mtxt <!-- Tekst 3014 --></td>\n";
  print "<td title='$mtitle'>
  ...
  $r = db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__));
	...
	$mtxt=findtekst('3021|Lav et vibrant login', $sprog_id);
	$mtitle=findtekst('3022|Kontoen du anvender til at logge ind på din vibrant terminal med', $sprog_id);
  print "<tr>\n<td title='$mtitle'><!-- Tekst 3013 -->$mtxt <!-- Tekst 3014 --></td>\n";

  # If an account is already setup, show the "Show account" button
  if ($r) {
	  $ntxt=findtekst('3024|Vis login', $sprog_id); # Show account
...
#				print "<tr><td colspan='3'><span title='Klik her for at hente nye ordrer fra shop'><a href=$apifil target='_blank'>SHOP import</a</span></td></tr>";
		...
		print "<tr><td title='".findtekst('752|greement_id tildeles ved oprettelse af betalingsaftale hos Quickpay', $sprog_id)."'><!--tekst 752-->".findtekst('753|Agreement_id', $sprog_id)."<!--tekst 753--></td><td colspan='3' title='".findtekst('752|greement_id tildeles ved oprettelse af betalingsaftale hos Quickpay', $sprog_id)."'><!--tekst 752--><input type='text' style='text-align:left;width:300px;' name='box9' value = '$box9'</td></tr>";
...
#	}
        ...
        print "<tr bgcolor='$bgcolor5'><td colspan='10'><b><u>".findtekst('792|Prislister', $sprog_id)."</u></b></td></tr>\n";
	...
	for ($x=1;$x<=$antal;$x++) {
		...
		if ($aktiv[$x]) {
			...
			if ( $lev_findes && $lev_id[$x] ) { # 20160226e start
				print "\n        title='".str_replace('$beskrivelse',$beskrivelse[$x],findtekst('425|Afmærk her for at benytte VVS-prislisten fra $beskrivelse', $sprog_id))."'><!--tekst 425-->&nbsp;\n"; # 20160226e slut
			} else {
				print "disabled='disabled' \n";
				print "\n        title='".findtekst('1325|Opret og angiv leverandør før prislisen kan gøres aktiv.', $sprog_id)."'>\n"; # 20160226b slut
			}
