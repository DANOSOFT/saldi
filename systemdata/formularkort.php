<?php
// -------systemdata/formularkort-------lap 3.2.2-----2011.06.30-------
// LICENS
//
...
echo "$ny_beskrivelse[$x]<br>";
					...
					} elseif ($ny_beskrivelse[$x]=="kopier_alt") {
echo "kopier_alt($form_nr,$art_nr,$formularsprog)<br>";
						kopier_alt($form_nr,$art_nr,$formularsprog);
					}
				}	elseif ($id[$x]) {
...
function drop_down($x,$form_nr,$art_nr,$formularsprog,$id,$beskrivelse,$xa,$xb,$ya,$yb,$str,$color,$justering,$font,$fed,$kursiv,$side){
	...
	if ($form_nr>1 && $form_nr<6) {
		print "<option value = \"kopier_alt\">Kopier alt fra tilbud</option>";
	}
	print "</SELECT></td>";
...
function kopier_alt($form_nr,$art_nr,$formularsprog) {
	if ($form_nr&&$art_nr&&$formularsprog) {
		...
		$q=db_select("select * from formularer where formular = '1' and sprog='$formularsprog'",__FILE__ . " linje " . __LINE__);
		while ($r=db_fetch_array($q)) {
			$xa=$r['xa']*1; $ya=$r['ya']*1; $xb=$r['xb']*1; $yb=$r['yb']*1;$str=$r['str']*1;$color=$r['color']*1;
