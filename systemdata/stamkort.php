<?php
// ----------systemdata/stamdata.php------lap 3.1.1-------2011-01-01-----
// LICENS
//
...
if ($_POST) {
	...
	if ($id==0) {
		$query = db_modify("insert into adresser (kontonr,firmanavn,addr1,addr2,postnr,bynavn,tlf,fax,cvrnr,art,bank_navn,bank_reg,bank_konto,email,mailfakt,pbs_nr,pbs,bank_fi) values ('$kontonr','$firmanavn','$addr1','$addr2','$postnr','$bynavn','$tlf','$fax','$cvrnr','S','$bank_navn','$bank_reg','$bank_konto','$email','$mailfakt','$pbs_nr','$pbs','$fi_nr')",__FILE__ . " linje " . __LINE__);
		$query = db_select("select id from adresser where art = 'S'",__FILE__ . " linje " . __LINE__);
		$row = db_fetch_array($query);
		$id = $row[id];
	}	elseif ($id > 0) {
		$query = db_modify("update adresser set kontonr = '$kontonr', firmanavn = '$firmanavn', addr1 = '$addr1', addr2 = '$addr2', postnr = '$postnr', bynavn = '$bynavn', tlf = '$tlf', fax = '$fax', cvrnr = '$cvrnr', bank_navn='$bank_navn', bank_reg='$bank_reg', bank_konto='$bank_konto',email='$email',mailfakt='$mailfakt', notes = '$notes', pbs_nr='$pbs_nr', pbs='$pbs', bank_fi='$fi_nr' where art = 'S'",__FILE__ . " linje " . __LINE__);
		for ($x=1; $x<=$ans_ant; $x++) {
...
print "<tr><td><table border=\"1\"><tbody>";
...
if(db_fetch_array(db_select("select id from ansatte where konto_id = '$id' and lukket != 'on'",__FILE__ . " linje " . __LINE__))) {
	...
	print "<tr><td title = \"$tekst\">e-mail/kopi til ref</td><td><input class=\"inputbox\" type=\"text\" size=\"22\" name=\"email\" value=\"$email\"><input  title = \"$tekst\" class=\"inputbox\" type=\"checkbox\" name=\"mailfakt\" $mailfakt></td></tr>";
} else {
	print "<tr><td>e-mail</td><td><input class=\"inputbox\" type=\"text\" size=\"25\" name=\"email\" value=\"$email\"></td></tr>";
}
...
if ($pbs_nr) {
	...
	if ($pbs) print "<option value=\"B\">Basis</option><option value=\"\">Total</option>";
	else print "<option value=\"\">Total</option><option value=\"B\">Basis</option>";
	print "</select>";
}
print "</td></tr>";
