	...
	for ($x=1; $x<=$kontoantal; $x++) {
		...
		while ($row = db_fetch_array($query)) {
			if ($row[udlignet]!=1){
				$udlignet="0";
				$amount=$row['amount'];
				$query2 = db_select("select betalingsbet, betalingsdage, valuta from ordrer where fakturanr='$row[faktnr]'");
				$row2 = db_fetch_array($query2);
				$valuta=$row2['valuta'];
				if ($valuta && $valuta!='DKK') {
					if ($r2= db_fetch_array(db_select("select valuta.kurs from valuta, grupper where grupper.art='VK' and grupper.box1='$valuta' and valuta.gruppe=grupper.kodenr and valuta.valdate <= '$row[transdate]' order by valuta.valdate"))) {
						$valutakurs=$r2['kurs'];
					} else {
						$tmp = dkdato($row[transdate]);
...
// Bemaerk at der ikke traekkes moms ved bogfoering af rykkergebyr - hejjer ikke selvom gebyret tilhorer en momsbelagt varegruppe.
	global $fakturadate; 
	global $font;
...
*/		
		...
		$d_kontrol=$d_kontrol+$debet; $k_kontrol=$k_kontrol+$kredit;
			db_modify("insert into transaktioner (bilag, transdate, beskrivelse, kontonr, faktura, debet, kredit, kladde_id, afd, logdate, logtime, projekt, ordre_id) values ('$bilag', '$transdate', '$beskrivelse', '$kontonr', '$fakturanr', '$debet', '$kredit', '0', $afd, '$logdate', '$logtime', '$projekt', '$id')");
		$y=0;
		$bogf_konto = array();
		...
		for ($y=1;$y<=$ordrelinjer;$y++) {
			if ($bogf_konto[$y]) {
				...
				$d_kontrol=$d_kontrol+$debet; $k_kontrol=$k_kontrol+$kredit;
				echo 	"insert into transaktioner (bilag, transdate, beskrivelse, kontonr, faktura, debet, kredit, kladde_id, afd, logdate, logtime, projekt, ordre_id) values ('$bilag', '$transdate', '$beskrivelse', '$bogf_konto[$y]', '$fakturanr', '$debet', '$kredit', '0','$afd', '$logdate', '$logtime', '$projekt', '$id')<br>";			
				db_modify("insert into transaktioner (bilag, transdate, beskrivelse, kontonr, faktura, debet, kredit, kladde_id, afd, logdate, logtime, projekt, ordre_id) values ('$bilag', '$transdate', '$beskrivelse', '$bogf_konto[$y]', '$fakturanr', '$debet', '$kredit', '0','$afd', '$logdate', '$logtime', '$projekt', '$id')");
			}
		}
/*		
