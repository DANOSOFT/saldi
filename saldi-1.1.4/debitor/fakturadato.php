...
// --------------debitor/fakturadato.php--------lap 1.1.3-----03.02.2008-----------------
// LICENS
//
...
if ($_POST['submit']) {
	...
	if ((strlen($day)==6&&!$month&&!$year&&!is_numeric($day))||(!is_numeric($day)||!is_numeric($month)||!is_numeric($year)||!checkdate($month,$day,$year))) {
		...
		elseif (checkdate($month,$day,$year)) {
			...
			db_modify("update ordrer set fakturadate='$tmp', levdate='$tmp' where id='$id'");
 			print "<BODY onLoad=\"javascript:window.location = '$returside?id=$id&hurtigfakt=$hurtigfakt	'\">";
			exit;
...
<?php
*/
print "<form name=ordre action=fakturadato.php?returside=$returside&id=$id&hurtigfakt=$hurtigfakt method=post>";
print "<table><tbody>";
print "<tr><td>Angiv fakturadato</td>";
