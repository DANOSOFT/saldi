...
// -------------------------------------------------debitor/levering.php--------lap 1.1.4--------06.03.08--------------
// LICENS
//
...
#		fakturadato($id);
	...
	if ($fejl==0){
		...
		$query = db_select("select * from ordrelinjer where ordre_id = '$id' and samlevare = 'on'");
		while ($row =db_fetch_array($query)){
...
#			}
			if (!$gruppe[$x]) {
				print "<BODY onLoad=\"fejltekst('Vare tilhrer ikke nogen varegruppe - kontroller vare og indstillinger! (pos nr: $posnr[$x])')\">";
				print "<meta http-equiv=\"refresh\" content=\"0;URL=ordre.php?id=$id\">";
				exit;
			}
...
if ($hurtigfakt=='on') {
	...
	print "<meta http-equiv=\"refresh\" content=\"0;URL=bogfor.php?id=$id\">";
}
...
function batch_salg($id, $levdate, $fakturadate, $beholdning, $vare_id, $antal, $pris, $nettopris, $linje_id, $linje_nr, $posnr, $serienr, $lager){
	...
	if (!db_fetch_array(db_select("select * from reservation where linje_id = $linje_id"))) batch($linje_id);  #Hvis der ikke manuelt er reserveret varer tages automatisk fra den ldste indkbsordre
...
function krediter($id, $levdate, $beholdning, $vare_id, $antal, $pris, $linje_id, $serienr, $kred_linje_id) 
{
	...
	if ($kred_linje_id>0) { #if Indsat 071106 grundet fejl ved negativt vareantal pï¿½ordinï¿½ salgsordre.
		...
		$x=0;
		$q = db_select("select * from batch_salg where linje_id=$kred_linje_id");
		while ($r =db_fetch_array($q)) {
			...
			$q2 = db_select("select rest from batch_kob where id=$batch_kob_id[$x]");
			$r2 =db_fetch_array($q2);
			$kob_rest[$x]=$r2[rest]+$batch_kob_antal[$x];
			db_modify("update batch_kob set rest=$kob_rest[$x] where id=$batch_kob_id[$x]");
			lagerstatus($vare_id, $lager, -$batch_kob_antal[$x]);	
			db_modify("insert into batch_salg(batch_kob_id, vare_id, linje_id, salgsdate, ordre_id, antal, lev_nr) values ($batch_kob_id[$x], $vare_id, $linje_id, '$levdate', $id, -$batch_kob_antal[$x], '$lev_nr')"); # Rettet til $antal fra $batch_kob_antal[$x] -- rettet tilbage 12.11.07 dat det ikke fungerer hvis antal != batch_kob_antal[$x]  .
			$q3 = db_select("select id from batch_salg where batch_kob_id=$batch_kob_id[$x] and vare_id=$vare_id and linje_id=$linje_id and salgsdate='$levdate' and ordre_id=$id and antal=-$batch_kob_antal[$x] and lev_nr='$lev_nr' order by id desc"); #se ovenfor.
			$r3 =db_fetch_array($q3);
			$batch_salg_id[$x]=$r3[id]; 
			if ($serienr) {
				$q4 = db_select("select * from serienr where salgslinje_id=-$kred_linje_id");
				while ($r4 =db_fetch_array($q4)) {
					db_modify("insert into serienr (kobslinje_id, vare_id, batch_kob_id, serienr, batch_salg_id, salgslinje_id) values ($r4[kobslinje_id], $r4[vare_id], $r4[batch_kob_id], '$r4[serienr]', $batch_salg_id[$x], $linje_id)"); 
					db_modify("update serienr set batch_salg_id=-$r4[batch_salg_id] where id=$r4[id]");
				}
			}
		}
	} else {
		db_modify("update ordrelinjer set kred_linje_id = '-1' where id = $linje_id"); #indsat 20071004
...
function samlevare($linje_id, $v_id, $leveres) 
{
	...
	for ($x=1; $x<=$antal; $x++) {
		if ($r=db_fetch_array(db_select("select * from varer where id=$vare_id[$x]"))) {
			$stk_antal[$x]=$stk_antal[$x]*$leveres;
			db_modify("insert into ordrelinjer (ordre_id, varenr, vare_id, beskrivelse, antal, leveres, pris, samlevare, posnr) values ('$id', '$r[varenr]', '$vare_id[$x]', '$r[beskrivelse]', '$stk_antal[$x]', '$stk_antal[$x]', 0, $linje_id, '100' )");
		}
	}
}
