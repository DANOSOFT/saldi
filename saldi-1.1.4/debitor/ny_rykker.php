...
// -----------------------debitor/rykkerprint-----lap 1.1.5---06.03.08-------
// LICENS
//
...
echo "Kurs 	$valutakurs<br>";		
			...
			$q2 = db_select("select * from openpost where konto_id = $konto_id[$i] and udlignet != 1");
			$rykker_id[$i]=$r['id'];
			$id=$konto_id[$i];
			$r= db_fetch_array(db_select("select * from adresser where id ='$konto_id[$i]'"));
			while ($r2 = db_fetch_array($q2)) {
				...
				if ($forfaldsdag<date("Y-m-d")) {
					db_modify("insert into ordrelinjer (enhed, ordre_id, serienr, beskrivelse) values ('$r2[id]', '$rykker_id[$i]', '$r2[transdate]', '$r2[beskrivelse]')");
...
echo 	"gebyr $gebyr<br>";				
			...
			if ($rykkernr<=3) {
				...
				$r= db_fetch_array(db_select("select id from ordrer where ordrenr ='$r[ordrenr]' and art = '$art'"));
				$ny_rykker_id[$i]=$r['id'];
				$q2 = db_select("select * from ordrelinjer where ordre_id = '$rykker_id[$i]'");
				while ($r2=db_fetch_array($q2)) {
					db_modify("insert into ordrelinjer (enhed, ordre_id, serienr, beskrivelse) values ('$r2[enhed]', '$ny_rykker_id[$i]', '$r2[transdate]', '$r2[beskrivelse]')");
				}
				...
				$topniveau=$topniveau.$rykker_id[$i];
			}
		}
	}
} 
