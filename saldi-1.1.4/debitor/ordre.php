<?php
// ----------------------------------debitor/ordrer---------lap 1.1.3b------11.10.07------
// LICENS
//
		...
		if (($posnr_ny[0])&&(!strstr($submit,'Opslag'))) {
			if ($varenr[0]) {
				...
				if ($row = db_fetch_array($query)) {
					...
					$serienr=$row['serienr'];
					if ($valuta && $valuta!='DKK') {
						if ($r2= db_fetch_array(db_select("select valuta.kurs from valuta, grupper where grupper.art='VK' and grupper.box1='$valuta' and valuta.gruppe=grupper.kodenr and valuta.valdate <= '$ordredate' order by valuta.valdate"))) {
					...
					$rabat[0]=$rabat[0]*1;
echo "B insert into ordrelinjer (vare_id, ordre_id, posnr, varenr, beskrivelse, enhed, antal, pris, rabat, serienr, momsfri, samlevare) values ($vare_id[0], '$id','$posnr_ny[0]','$varenr[0]','$beskrivelse[0]','$enhed[0]','$antal[0]','$pris[0]','$rabat[0]','$serienr[0]','$momsfri[0]','$samlevare[0]')<br>";
...
#							if (!$kostpris[$x]) $kostpris[$x]=$kostpris[$x]+$r2[pris]*$row[antal];
						...
						elseif (!$kostpris[$x]) {
							...
							if ($art=='DO') $kostpris[$x]=$kostpris[$x]+$r2['kostpris'];
						}
