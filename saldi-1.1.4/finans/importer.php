<?php
// -------------------------------------------finans/importer.pgp------------patch 1.0.02------------------
// LICENS
//
...
if(($_GET)||($_POST)) {
	...
	print "<td width=\"25%\" bgcolor=\"$bgcolor2\"><font face=\"Helvetica, Arial, sans-serif\" color=\"#000066\"><small><a href=kassekladde.php?kladde_id=$kladde_id accesskey=L>Luk</a></small></td>";
...
#			echo "The file ". basename( $_FILES['uploadedfile']['name']). " has been uploaded";
	...
	elseif($splitter){
		vis_data($kladde_id, $filnavn, $splitter, $feltnavn, $feltantal);
		}
	else {
		upload($kladde_id);
	}
...
print "<tr><td colspan=$feltantal align=center>$font Sepatatortegn &nbsp;<select name=splitter>\n";
...
print "<input type=\"hidden\" name=\"kladde_id\" value=$kladde_id>";
print "&nbsp; <input type=\"submit\" value=\"Vis\" /></td></tr>";
...
for ($y=0; $y<$feltantal; $y++) {
	...
	if ($feltnavn[$y]!='Bel&oslash;b') print "<option>Bel&oslash;b</option>\n";
	print "</select>";
}
print "</form>";
$fp=fopen("../temp/".$filnavn,"r");
if ($fp) {
	...
	while (!feof($fp)) {
		...
		for ($y=0; $y<$feltantal; $y++) {
			...
			print "<td><small>$felt[$y]&nbsp;</small></td>";
		}
		print "</tr>";
	}
}
