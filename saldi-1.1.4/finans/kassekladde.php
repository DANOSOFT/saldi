...
// ------------------finans/kassekladde.php------------lap 1.1.4---12.12.07----
// LICENS
//
...
$fejl=NULL; $kontrolkonto=NULL; $kontrolsaldo=NULL; $simuler=NULL; $kladde_id=NULL; $bogfort=NULL; $kladdenote=NULL; $regnstart=NULL; 
...
#		if ((substr($bilag[$x],0,1)=="-")||(substr($bilag[$x],-1,1)=="-")) $bilag[$x] = "-";
...
#		if (!isset($dato[$x]))$dato[$x]=NULL; 			if (!isset($beskrivelse[$x]))$beskrivelse[$x]=NULL;	if (!isset($d_type[$x]))$d_type[$x]=NULL;
#		if (!isset($debet[$x]))$debet[$x]=NULL;			if (!isset($k_type[$x]))$k_type[$x]=NULL;						if (!isset($kredit[$x]))$kredit[$x]=NULL;
#		if (!isset($faktura[$x]))$faktura[$x]=NULL;	if (!isset($belob[$x]))$belob[$x]=NULL;							if (!isset($momsfri[$x]))$momsfri[$x]=NULL;
...
$x=0;
if ($kladde_id) {
$bilag=array(); $dato=array(); $beskrivelse=array(); $d_type=array(); $debet=array(); $k_type=array(); $kredit=array(); 
$faktura=array(); $belob=array(); $afd=array(); $ansat=array(); $ansat_id=array(); $projekt=array(); $valuta=array(); $momsfri=array();
	...
	else	$query = db_select("select * from kassekladde where kladde_id = $kladde_id order by bilag, transdate, beskrivelse, id");
	while ($row = db_fetch_array($query)) {
		$x++;
		$valuta[$x]='DKK';
		$id[$x]=$row['id'];
		$bilag[$x]=$row['bilag'];
		if ($fejl) $dato[$x]=$row['transdate'];
		else $dato[$x]=dkdato($row['transdate']);
...
#		if ($y==1&&!$dato[$y]&&!$beskrivelse[$y]&&!$debet[$y]&&!$kredit[$y]&&!$faktura[$y]&&!$belob[$y]) {
#			echo "ZZZ";
#		}
...
#	print"<td width=\"10%\" $top_bund align=\"right\">$font<small><a href=../debitor/debitorkort.php?returside=../includes/luk.php target=blank accesskey=N>Ny</a></small></td>";
	print "<td width=\"10%\" $top_bund align=\"right\" onMouseOver=\"this.style.cursor = 'pointer'\"; onClick=\"JavaScript:window.open('../debitor/debitorkort.php?returside=../includes/luk.php', '', 'statusbar=no,menubar=no,titlebar=no,toolbar=no,scrollbars=yes,resizable=yes');\">$font<small><u>Ny</u></small></td>";
	...
	print"<tbody><tr>";
	print"<td><small><b>$font <a href='kassekladde.php?sort=kontonr&funktion=debitoropslag&x=$x&fokus=$fokus&kladde_id=$kladde_id&id=$id&bilag=$bilag&dato=$dato&beskrivelse=$beskrivelse&d_type=$d_type&debet=$debet&k_type=$k_type&kredit=$kredit&faktura=$faktura&belob=$belob&momsfri=$momsfri&afd=$afd&lobenr=$lobenr'>Kundenr</b></small></td>";
	print"<td><small><b>$font <a href='kassekladde.php?sort=firmanavn&funktion=debitoropslag&x=$x&fokus=$fokus&kladde_id=$kladde_id&id=$id&bilag=$bilag&dato=$dato&beskrivelse=$beskrivelse&d_type=$d_type&debet=$debet&k_type=$k_type&kredit=$kredit&faktura=$faktura&belob=$belob&momsfri=$momsfri&afd=$afd&lobenr=$lobenr'>Navn</b></small></td>";
	print"<td><small><b>$font <a href='kassekladde.php?sort=addr1&funktion=debitoropslag&x=$x&fokus=$fokus&kladde_id=$kladde_id&id=$id&bilag=$bilag&dato=$dato&beskrivelse=$beskrivelse&d_type=$d_type&debet=$debet&k_type=$k_type&kredit=$kredit&faktura=$faktura&belob=$belob&momsfri=$momsfri&afd=$afd&lobenr=$lobenr'>Adresse</b></small></td>";
	...
	print"<td><small><b>$font <a href='kassekladde.php?sort=postnr&funktion=debitoropslag&x=$x&fokus=$fokus&kladde_id=$kladde_id&id=$id&bilag=$bilag&dato=$dato&beskrivelse=$beskrivelse&d_type=$d_type&debet=$debet&k_type=$k_type&kredit=$kredit&faktura=$faktura&belob=$belob&momsfri=$momsfri&afd=$afd&lobenr=$lobenr'>Postnr</b></small></td>";
	print"<td><small><b>$font <a href='kassekladde.php?sort=bynavn&funktion=debitoropslag&x=$x&fokus=$fokus&kladde_id=$kladde_id&id=$id&bilag=$bilag&dato=$dato&beskrivelse=$beskrivelse&d_type=$d_type&debet=$debet&k_type=$k_type&kredit=$kredit&faktura=$faktura&belob=$belob&momsfri=$momsfri&afd=$afd&lobenr=$lobenr'>By</b></small></td>";
	print"<td><small><b>$font <a href='kassekladde.php?sort=kontakt&funktion=debitoropslag&x=$x&fokus=$fokus&kladde_id=$kladde_id&id=$id&bilag=$bilag&dato=$dato&beskrivelse=$beskrivelse&d_type=$d_type&debet=$debet&k_type=$k_type&kredit=$kredit&faktura=$faktura&belob=$belob&momsfri=$momsfri&afd=$afd&lobenr=$lobenr'>Kontaktperson</b></small></td>";
...
function kreditoropslag($sort, $fokus, $opslag_id, $id, $kladde_id, $bilag, $dato, $beskrivelse, $d_type, $debet, $k_type, $kredit, $faktura, $belob, $momsfri, $afd, $projekt, $ansat, $valuta, $lobenr) {
	...
	print "<td width=\"10%\" $top_bund align=\"right\" onMouseOver=\"this.style.cursor = 'pointer'\"; onClick=\"JavaScript:window.open('../kreditor/kreditorkort.php?returside=../includes/luk.php', '', 'statusbar=no,menubar=no,titlebar=no,toolbar=no,scrollbars=yes,resizable=yes');\">$font<small><u>Ny</u></small></td>";
	print"</tbody></table>";
	print"</td></tr>\n";
	print"<tr><td valign=\"top\">";
	print"<table cellpadding=\"1\" cellspacing=\"1\" border=\"0	\" width=\"100%\" valign = \"top\">";
	...
	while ($row = db_fetch_array($query)){
		...
		$kontonr=trim($row['kontonr']);
		print "<tr>";
		if ((strstr($fokus,"debe"))||(strstr($fokus,"d_ty"))){$tmp = "<td><small>$font <a href='kassekladde.php?x=$x&fokus=$fokus&kladde_id=$kladde_id&id=$id&bilag=$bilag&dato=$dato&beskrivelse=$beskr&d_type=$d_type&debet=$kontonr&k_type=$k_type&kredit=$kredit&faktura=$faktura&belob=$belob&momsfri=$momsfri&afd=$afd&lobenr=$lobenr'>";}
		else {$tmp="<a href='kassekladde.php?x=$x&fokus=$fokus&kladde_id=$kladde_id&id=$id&bilag=$bilag&dato=$dato&beskrivelse=$beskr&d_type=$d_type&debet=$debet&k_type=$k_type&kredit=$kontonr&faktura=$faktura&belob=$belob&momsfri=$momsfri&afd=$afd&lobenr=$lobenr'>";} 
		print "<td><small>$font $tmp $row[kontonr]</a><br></small></td>";
		print "<td><small>$font $tmp ". stripslashes($row[firmanavn]) ."</a><br></small></td>";
		print "<td><small>$font $tmp $row[addr1]</a><br></small></td>";
...
#	print"<td><small><b>$font <a 'href=kassekladde.php?sort=konto_nr&funktion=openpost&x=$x&fokus=$fokus&kladde_id=$kladde_id&id=$id&bilag=$bilag&dato=$dato&beskrivelse=$beskrivelse&d_type=$d_type&debet=$debet&k_type=$k_type&kredit=$kredit&faktura=$faktura&belob=$belob&momsfri=$momsfri&afd=$afd&lobenr=$lobenr'>Kundenr</b></small></td>";
	...
	print"<td><small><b>$font <a href='kassekladde.php?sort=faktnr&funktion=openpost&x=$x&fokus=$fokus&kladde_id=$kladde_id&id=$id&bilag=$bilag&dato=$dato&beskrivelse=$beskrivelse&d_type=$d_type&debet=$debet&k_type=$k_type&kredit=$kredit&faktura=$faktura&belob=$belob&momsfri=$momsfri&afd=$afd&lobenr=$lobenr'>Fakturanr</b></small></td>";
	print"<td><small><b>$font <a href='kassekladde.php?sort=transdate&funktion=openpost&x=$x&fokus=$fokus&kladde_id=$kladde_id&id=$id&bilag=$bilag&dato=$dato&beskrivelse=$beskrivelse&d_type=$d_type&debet=$debet&k_type=$k_type&kredit=$kredit&faktura=$faktura&belob=$belob&momsfri=$momsfri&afd=$afd&lobenr=$lobenr'>Dato</b></small></td>";
	print"<td><small><b>$font <a href='kassekladde.php?sort=amount&funktion=openpost&x=$x&fokus=$fokus&kladde_id=$kladde_id&id=$id&bilag=$bilag&dato=$dato&beskrivelse=$beskrivelse&d_type=$d_type&debet=$debet&k_type=$k_type&kredit=$kredit&faktura=$faktura&belob=$belob&momsfri=$momsfri&afd=$afd&lobenr=$lobenr'>Bel&oslash;b</b></small></td>";
	print" </tr>\n";
...
#	if ($bilag=="-"){$bilag="";} PHR 02.10.06
	...
	if (($bilag != "-")&&(($bilag)||($beskrivelse)||($kredit)||($debet)||($faktura)||($belob))) {
		...
		if ((!$fejl)&&($d_type=="D")&&($debet>0)&&(!in_array($debet,$debitornr))) {
			$alerttekst='Debitor '.$debet.' eksisterer ikke (Bilag nr '.$bilag.')\nKladden er IKKE gemt!';
			print "<BODY onLoad=\"javascript:alert('$alerttekst')\">";
			$fejl=1;
		}
		if ((!$fejl)&&($k_type=="D")&&($kredit>0)&&(!in_array($kredit,$debitornr))) {
			$alerttekst='Debitor '.$kredit.' eksisterer ikke (Bilag nr '.$bilag.')\nKladden er IKKE gemt!';
			print "<BODY onLoad=\"javascript:alert('$alerttekst')\">";
			$fejl=1;
		}
		if ((!$fejl)&&($d_type=="K")&&($debet>0)&&(!in_array($debet,$kreditornr))) {
			$alerttekst='Kreditor '.$debet.' eksisterer ikke (Bilag nr '.$bilag.')\nKladden er IKKE gemt!';
			print "<BODY onLoad=\"javascript:alert('$alerttekst')\">";
			$fejl=1;
		}
		if ((!$fejl)&&($k_type=="K")&&($kredit>0)&&(!in_array($kredit,$kreditornr))) {
...
function opdater($kladde_id)
{
	...
	while ($r=db_fetch_array($q)) {
		if (($r['bilag']!="-")&&($r['transdate'])) { 
			...
			if ($r['id']) {
				db_modify("update kassekladde set bilag = '$r[bilag]', transdate = '$transdate', beskrivelse = '$beskrivelse', d_type = '$d_type', debet = '$debet', k_type = '$k_type', kredit = '$kredit', faktura = '$r[faktura]', amount = '$amount', momsfri = '$momsfri', afd= '$afd', projekt= '$projekt', ansat= '$ansat_id', valuta= '$valutakode' where id = '$r[id]'");
	...
	if ($bilagsnr) {
		...
		if ($row = db_fetch_array($query)){
			...
			while ($row = db_fetch_array($query)) {
				...
				$valuta=$row['valuta']*1;
				if ($ny_dato) $date=usdate($ny_dato);
...
function regnskabsaar($regnaar)
{
	if ($row = db_fetch_array(db_select("select box1, box2, box3, box4 from grupper where art='RA' and kodenr='$regnaar'"))){
		...
		$slut=usdate("31-".trim($row['box3'])."-".trim($row['box2']))	;
	} else {
