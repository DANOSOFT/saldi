...
// --------------------------------/admin/opret.php-----patch 1.1.4------11.03.2008--------
// LICENS
//
...
function opret ($sqhost, $squser ,$sqpass, $db, $admin, $passwd, $std_kto_plan)
{
	...
	######## Adresser ########
	db_modify("CREATE TABLE adresser (id serial NOT NULL, firmanavn varchar, addr1 varchar, addr2 varchar, postnr varchar, bynavn varchar, land varchar, kontakt varchar, tlf varchar, fax varchar, email varchar, web varchar, bank_navn varchar, bank_reg varchar, bank_konto varchar, notes varchar, rabat numeric, momskonto integer, kreditmax numeric, betalingsbet varchar, betalingsdage integer DEFAULT 0, kontonr varchar, cvrnr varchar, ean varchar, institution varchar, art varchar, gruppe integer, kontoansvarlig integer, oprettet date, kontaktet date, kontaktes date, PRIMARY KEY (id))");
	...
	db_modify("CREATE TABLE transaktioner (id serial NOT NULL, kontonr integer, bilag integer, transdate date, logtime time without time zone, beskrivelse varchar, debet numeric, kredit numeric, faktura varchar, kladde_id integer, projekt numeric, ansat numeric, logdate date, afd integer, ordre_id integer, PRIMARY KEY (id))");
...
#	 include("../includes/kontoplan.php"); 
		...
		if ($fp) {
			...
			for ($x=1; $x<=$kontoantal; $x++){
				...
				db_modify("INSERT INTO kontoplan (kontonr, beskrivelse, kontotype, fra_kto, til_kto, moms, lukket, regnskabsaar) values ('$kontonr[$x]', '$beskrivelse[$x]', '$kontotype[$x]', '$fra_kto[$x]', '$til_kto[$x]', '$moms[$x]', '','1')");
			}
			fclose($fp);
		}
		...
		db_modify("insert into grupper (beskrivelse, kode, kodenr, art, box1, box2, box4) values ('Danske Debitorer', 'D', '1', 'DG', 'S1', '56100', '58000')");
		db_modify("insert into grupper (beskrivelse, kode, kodenr, art, box1, box2, box4) values ('Danske Kreditorer', 'K', '1', 'KG', 'K1', '65100', '58000')");
