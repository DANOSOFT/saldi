...
<html>
	<head>
		...
		<script type=\"text/javascript\" src=\"../javascript/jquery.medarbejdermappe.js\"></script>
...
function arbejdsseddelskemaliste() {
	...
	if ($_POST['ma_skema_limit']) {
		$_SESSION['ma_skema_limit']=$ma_skema_limit;
	} else {
		$ma_skema_limit=$_SESSION['ma_skema_limit'];
	}
	
	if ($unsetsort) {
		unset($_SESSION['ma_kontrolskema_sort'],
					$_SESSION['ma_kontrolskema_datotid'],$sortstyle[0],
					$_SESSION['ma_kontrolskema_skema'],$sortstyle[1],
					$_SESSION['ma_kontrolskema_opgave'],$sortstyle[2],
					$_SESSION['ma_kontrolskema_sagsnr'],$sortstyle[3],
	...
	if ($sort) $_SESSION['ma_kontrolskema_sort']=$sort;
	else $sort=$_SESSION['ma_kontrolskema_sort'];
	...
	*/
	if ($ans_id) {
		...
		if ($ans_id) print "<div style=\"text-align: center;padding-top: 5px;color: green;\"><h3>($ans_ini) $ans_navn</h3></div>";
		print "
		<div class=\"contentsoeg\">
		<form name=\"skemasoeg\" action=\"mm_kontrolskemaer.php?funktion=arbejdsseddelskemaliste\" method=\"get\">
			<table border=\"0\" cellspacing=\"0\" width=\"828\">
				...
				<tbody>
					<tr>
						...
						<td><input class=\"textinput ma_dato\" type=\"text\" value=\"\" id=\"ma_dato\" name=\"ma_dato\" tabindex=\"1\"/></td>
						<td><input class=\"textinput ma_sagsnr\" type=\"text\" value=\"\" id=\"ma_sagsnr\" name=\"ma_sagsnr\" tabindex=\"2\"/></td>
						<td><input class=\"textinput ma_af\" type=\"text\" value=\"\" id=\"ma_af\" name=\"ma_af\" tabindex=\"3\"/></td>
						<td><input class=\"textinput ma_adresse\" type=\"text\" value=\"\" id=\"ma_adresse\" name=\"ma_adresse\" tabindex=\"4\"/></td>
					...
					<div style=\"float:right;$display\">
						...
						<select name=\"ma_skema_limit\" class=\"selectinputloen\" style=\"width:76px;\" onchange=\"this.form.submit()\">\n";
							...
							for ($i=0;$i<count($limitarray);$i++) {
									if ($ma_skema_limit==$limitarray[$i]) print "<option value=\"$limitarray[$i]\">$limitnavn[$i]</option>\n"; 
								}
								for ($i=0;$i<count($limitarray);$i++) {
									if ($ma_skema_limit!=$limitarray[$i]) print "<option value=\"$limitarray[$i]\">$limitnavn[$i]</option>\n"; 
								}
...
function kontrolseddelskemaliste() {
	...
	if ($_POST['mk_skema_limit']) {
		$_SESSION['mk_skema_limit']=$mk_skema_limit;
	} else {
		$mk_skema_limit=$_SESSION['mk_skema_limit'];
	}
	
	if ($unsetsort) {
		unset($_SESSION['mk_kontrolskema_sort'],
					$_SESSION['mk_kontrolskema_datotid'],$sortstyle[0],
					$_SESSION['mk_kontrolskema_skema'],$sortstyle[1],
					$_SESSION['mk_kontrolskema_opgave'],$sortstyle[2],
					$_SESSION['mk_kontrolskema_sagsnr'],$sortstyle[3],
	...
	if ($sort) $_SESSION['mk_kontrolskema_sort']=$sort;
	...
	*/
	if ($ans_id) {
		...
		print "
		<div class=\"contentsoeg\">
		<form name=\"skemasoeg\" action=\"mm_kontrolskemaer.php?funktion=kontrolseddelskemaliste\" method=\"get\"><!-- funktion skal være en variable -->
			<table border=\"0\" cellspacing=\"0\" width=\"828\">
				<thead>
					<tr>
						<th width=\"100\">Dato</th>
				...
				<tbody>
					<tr>
						...
						<td><input class=\"textinput mk_dato\" type=\"text\" value=\"\" id=\"mk_dato\" name=\"mk_dato\" tabindex=\"1\"/></td>
						<td><input class=\"textinput mk_sagsnr\" type=\"text\" value=\"\" id=\"mk_sagsnr\" name=\"mk_sagsnr\" tabindex=\"2\"/></td>
						<td><input class=\"textinput mk_af\" type=\"text\" value=\"\" id=\"mk_af\" name=\"mk_af\" tabindex=\"3\"/></td>
						<td><input class=\"textinput mk_adresse\" type=\"text\" value=\"\" id=\"mk_adresse\" name=\"mk_adresse\" tabindex=\"4\"/></td>
						<td style=\"padding:0px;\"><input type=\"hidden\" class=\"id\" value=\"\" name=\"tjekskema_id\"></td>
						<td style=\"padding:0px;\"><input type=\"hidden\" class=\"fase\" value=\"\" name=\"sag_fase\"></td>
						<td style=\"padding:0px;\"><input type=\"hidden\" class=\"sag_id\" value=\"\" name=\"sag_id\"></td>
						<td style=\"padding:0px;\"><input type=\"hidden\" class=\"tjek_id\" value=\"\" name=\"tjek_id\"></td>
				...
				<div style=\"height:25px;padding:10px 12px 0 12px;#background-color:#f2f2f2;\">
					...
					<div style=\"float:right;$display\">
						...
						<select name=\"mk_skema_limit\" class=\"selectinputloen\" style=\"width:76px;\" onchange=\"this.form.submit()\">\n";
							...
							for ($i=0;$i<count($limitarray);$i++) {
									if ($mk_skema_limit==$limitarray[$i]) print "<option value=\"$limitarray[$i]\">$limitnavn[$i]</option>\n"; 
								}
								for ($i=0;$i<count($limitarray);$i++) {
									if ($mk_skema_limit!=$limitarray[$i]) print "<option value=\"$limitarray[$i]\">$limitnavn[$i]</option>\n"; 
								}
...
function vis_arbejdsseddel() {
	...
	if (!$sag_id) { header("location: mm_kontrolskemaer.php?funktion=arbejdsseddelskemaliste"); exit(); }
...
function vis_kontrolskema() {
	...
	if (!$sag_id) { header("location: mm_kontrolskemaer.php?funktion=kontrolseddelskemaliste"); exit(); }
  ...
  print "<tbody>\n";
	...
	for ($x=1;$x<=count($id);$x++) {
		...
		*/
			...
			print "<tr><td colspan=\"2\" title=\"$assign_id[$x]==$l_id\"><p>".$tjekpunkt[$x]."</p></td>\n";
