		...
		//$filnavn=htmlentities($filnavn,ENT_COMPAT,$charset);
			...
			for($x =0; $x< $count;$x++){
				...
				if (!file_exists("../mappe/".$db."/".$kilde_id)) {
					...
					if (!file_exists("../mappe/".$db."/".$kilde_id)) mkdir("../mappe/".$db."/".$kilde_id); 
				}
				$tmp="../mappe/".$db."/".$kilde_id."/".$ny_id.".".$filtype;
...
if (isset($_POST['ret_bilag']) && $bilag_id) {
		...
		*/
		db_modify("update mappebilag set beskrivelse='$bilag_beskrivelse' where id = '$bilag_id'",__FILE__ . " linje " . __LINE__);
				print "<meta http-equiv=\"refresh\" content=\"0;URL=medarbejdermappe.php?funktion=vis_mappe&amp;mappe_id=$kilde_id\">";
	}
################################################################################################################
...
*/
print "<div class=\"maincontentLargeHolder\">\n";
print "<form enctype=\"multipart/form-data\" action=\"bilag_mappe.php?mappe_id=$kilde_id\" method=\"POST\">\n";
print "<table border=\"0\" cellspacing=\"0\" id=\"dataTable\" class=\"dataTable\">\n";
print "<tbody>\n";
