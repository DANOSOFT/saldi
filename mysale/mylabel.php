...
$medlem = $tmp = '';
$page = 0;
(isset($_POST['labelName'])) ? $labelName = $_POST['labelName'] : $labelName = array();
(isset($_GET['id'])) ? $id = $_GET['id'] : $id = array();
...
if ($id) {
	...
	if (strpos($kto, "|"))
		list($accountId, $account) = explode('|', $kto);
	else
		$account = $kto;
...
if ($mySale) {
	...
	for ($a = 1; $a <= $rows; $a++) {
		for ($b = 1; $b <= $cols; $b++) {
			...
			#if ($bruger_id == -1) echo "$a $b (".$labelId[$a][$b]."|".$labelName[$a][$b] ."|". $price[$a][$b]."<br>";
			...
			if ($labelId[$a][$b]) {
				#if ($bruger_id == -1) echo __line__." ".$labelId[$a][$b]."<br>";
				$barcode[$a][$b] = dechex($price[$a][$b] * 100);
				while (strlen($barcode[$a][$b]) < 6)
					$barcode[$a][$b] = '0' . $barcode[$a][$b];
				$barcode[$a][$b] = $labelId[$a][$b] . $barcode[$a][$b];
				while (strlen($barcode[$a][$b]) < 12)
					$barcode[$a][$b] = '0' . $barcode[$a][$b];
				$qtxt = NULL;
				...
				if ($print) {
					if (isset($selectLabel[$a][$b]) && $selectLabel[$a][$b])
						$printIds .= $labelId[$a][$b] . ',';
					if (isset($labelName[$a][$b]) && $labelName[$a][$b] && $price[$a][$b])
						$pL .= $labelId[$a][$b] . ',';
				}
			} elseif ($labelName[$a][$b] || $price[$a][$b]) {
				#if ($bruger_id == -1) echo __line__." >$medlem< ".$labelId[$a][$b]."<br>";
...
if ($accountId)
	...
	$qtxt .= "kontonr='" . db_escape_string($account) . "' ";
$r = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__));
$accountId = $r['id'];
$account = $r['kontonr'];
...
#f ($bruger_id == '-1') echo "$qtxt<br>";
$r = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__));
$itemId = $r['id'];
$itemNo = $r['varenr'];

if (strlen($itemNo) != 9) {
	echo "fejl i varenr $itemNo";
	exit;
}
...
($medlem) ? $rows = ceil($productLimit / 5) : $rows = 13;
# <-- 20210426
$a = $b = 0;
...
if ($condition == 'new')
	print findtekst(1951, $sprog_id) . "</div><br>";
else
	print findtekst(1952, $sprog_id) . "</div>";
if ($mobile) {
	print "<br>";
} else {
	print "<br>";
...
if ($mobile) {
	...
	for ($a = 1; $a <= $rows; $a++) {
		for ($b = 1; $b <= 5; $b++) {
			if (!isset($barcode[$a][$b]))
				$barcode[$a][$b] = NULL;
			if (!isset($labelId[$a][$b]))
				$labelId[$a][$b] = NULL;
			if (!isset($labelName[$a][$b]))
				$labelName[$a][$b] = NULL;
			if (!isset($lastPrint[$a][$b]))
				$lastPrint[$a][$b] = NULL;
			if (!isset($price[$a][$b]))
				$price[$a][$b] = NULL;
			$count++;
			if ($productLimit) {
				...
				if ($labelId[$a][$b] && $lastPrint[$a][$b])
					$showLabel = 1;
				if ($showLabel && !$labelId[$a][$b])
					$freeLabels--;
			} else
				$showLabel = 1;
			if ($showLabel) {
				...
				if ($mobile)
					print "<td class='tdLabel'; $title>";
				else
					print "<td class='tdLabel'; $title>";
				print "<input type='hidden' name='labelId[$a][$b]' value='" . $labelId[$a][$b] . "'>";
				if ($mobile)
					print "<input type='text' $ro maxlength='$labelsize' class='inputLabel' name='labelName[$a][$b]' ";
				else
					print "<input type='text' $ro maxlength='$labelsize' class='inputLabel' name='labelName[$a][$b]' ";
				...
				if ($labelId[$a][$b])
					print "<input style='transform:scale(3);' type='checkbox' name='selectLabel[$a][$b]' $all>";
				else
	...
	($medlem && $db != 'pos_76' && $db != 'pos_92' && $db != 'pos_111') ? $lines = ceil($productLimit / 5) : $lines = 13;
	for ($a = 1; $a <= $lines; $a++) {
		...
		if (!$productLimit || $count <= $productLimit) {
			...
			for ($b = 1; $b <= 5; $b++) {
				...
				if (!$productLimit || $showLabel) {
					...
					print "value='" . htmlentities($labelName[$a][$b], ENT_QUOTES) . "' placeholder='Beskrivelse'>";
					if ($labelId[$a][$b])
						print "<input type='checkbox' name='selectLabel[$a][$b]' $all>";
					else
						print "<input type='hidden' name='selectLabel[$a][$b]' value=''>";
