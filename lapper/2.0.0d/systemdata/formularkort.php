<?php
// -------systemdata/formularkort -------lap 2.0.0c-------30.04.2008------------------
// LICENS
//
...
if ($_POST) {
	...
	if ($submit=='Opdater' && $form_nr>=6 && $form_nr<=8 && $art_nr==2 && $gebyr) {
		$gebyr=$_POST['gebyr'];
		if ($r1=db_fetch_array(db_select("select id from varer where varenr = '$gebyr'"))) { 
			if ($r2=db_fetch_array(db_select("select id from formularer where beskrivelse ='GEBYR' and formular='$form_nr' and art=2 and sprog='$sprog'"))) {
				db_modify("update formularer set xb='$r1[id]' where id = $r2[id]");
			}
				else {
					db_modify("insert into formularer (beskrivelse, formular, art, xb, sprog) values ('GEBYR', '$form_nr', '2', '$r1[id]', '$sprog')");
				}
