...
@session_start();
$s_id=session_id();
// ------------debitor/bogfor.php------- patch 2.0.0d ---01.05.2008------
// LICENS
//
...
#		else {$moms=round($r['sum']*$r['momssats']/100,2);}
		...
		if ($valutakurs) {$kredit=round($kredit*$valutakurs/100,2);$debet=round($debet*$valutakurs/100,2);} # Omregning til DKR.		
		$d_kontrol=$d_kontrol+$debet; $k_kontrol=$k_kontrol+$kredit;
		if ($moms) db_modify("insert into transaktioner (bilag, transdate, beskrivelse, kontonr, faktura, debet, kredit, kladde_id, afd, logdate, logtime, projekt, ansat, ordre_id) values ('$bilag', '$transdate', '$beskrivelse', '$box1', '$fakturanr', '$debet', '$kredit', '0', '$afd', '$logdate', '$logtime', '$projekt', '$ansat', '7$id')");
