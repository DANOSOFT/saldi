<?php
// -------------kreditor/ordre.php----------lap 2.0.0m------2008-06-04----
// LICENS
//
...
if ($_GET['varenr']) {
	...
	if ($_POST) {
		...
		if ($valuta && $valuta!='DKK') {
			if ($r= db_fetch_array(db_select("select valuta.kurs as kurs from valuta, grupper where grupper.art='VK' and grupper.box1='$valuta' and valuta.gruppe=grupper.kodenr and valuta.valdate <= '$ordredate' order by valuta.valdate desc"))) {
				$valutakurs=$r['kurs'];
			} else {
				$tmp = dkdato($ordredate);
				print "<BODY onLoad=\"javascript:alert('Der er ikke nogen valutakurs for $valuta den $ordredate')\">";
  	...
  	transaktion("begin");
		...
		if (($kontonr)&&(!$firmanavn)) {
			$query = db_select("select * from adresser where kontonr = '$kontonr' and art = 'K'");
			if ($row = db_fetch_array($query)) {
				$konto_id=$row[id];
...
#			if (!$lager_id) {$lager_id='0';}
			...
			db_modify("insert into ordrer (ordrenr, konto_id, kontonr, firmanavn, addr1, addr2, postnr, bynavn, land, kontakt, lev_navn, lev_addr1, lev_addr2, lev_postnr, lev_bynavn, lev_kontakt, betalingsdage, betalingsbet, cvrnr, notes, art, ordredate, momssats, status, ref, sum, lev_adr, hvem, tidspkt, valuta, kred_ord_id) values ($ordrenr, $konto_id, '$kontonr', '$firmanavn', '$addr1', '$addr2', '$postnr', '$bynavn', '$land', '$kontakt', '$lev_navn', '$lev_addr1', '$lev_addr2', '$lev_postnr', '$lev_bynavn', '$lev_kontakt', '$betalingsdage', '$betalingsbet', '$cvrnr', '$notes', '$art', '$ordredate', '$momssats', $status, '$ref', '$sum', '$lev_adr', '$brugernavn', '$tidspkt', '$valuta', '$kred_ord_id')");
			...
			if (($posnr_ny[0]>0)&&(!strstr($submit,'Opslag'))) {
				if ($varenr[0]) {
					...
					if ($row = db_fetch_array($query)) {
						...
						if (!$lev_varenr[0]) {
							if (!$konto_id) {
								$q = db_select("select * from adresser where kontonr = '$kontonr' and art = 'K'");
...
#			if ($kred_ord_id) {
#				db_modify("update ordrer set kred_ord_id='$kred_ord_id' where id='$id'");}
			for($x=1; $x<=$linjeantal; $x++) {
				if (!$vare_id[$x]) {
...
#		elseif ($status==1){print "<tr><td>$font<small>Modtag</td><td><input type=checkbox name=modtag></td></tr>\n";}
	...
	#		print "<tr><td><textarea style=\"font-family: helvetica,arial,sans-serif;\" name=lev_adr rows=5 cols=35>$lev_adr</textarea></td></tr>\n";
		...
		print "<tr>";
		if ($status==1) {
			print "<td align=center>$font<small>pos</td><td align=center>$font<small>varenr</td><td align=center>$font<small>Lev. vnr</td><td align=center>$font<small>antal</td><td align=center>$font<small>enh.</td><td align=center>$font<small>beskrivelse</td><td align=center>$font<small>pris</td><td align=center>$font<small>%</td><td align=center>$font<small>ialt</td>";
			if ($art=='KK') {print "<td colspan=2 align=center>$font<small>returner</td>";}
			else {print "<td align=center>$font<small>modt.</td>";}
		}
		else {print "<td align=center>$font<small>pos</td><td align=center>$font<small>varenr</td><td align=center>$font<small>Lev. vnr</td><td align=center>$font<small>antal</td><td>$font<small>enhed</td><td align=center>$font<small>beskrivelse</td><td align=center>$font<small>pris</td><td align=center>$font<small>%</td><td align=center>$font<small>ialt</td>";}
		print "</tr>\n";
/*		
		if ($valuta && $valuta!='DKK') {
			if ($r= db_fetch_array(db_select("select valuta.kurs from valuta, grupper where grupper.art='VK' and grupper.box1='$valuta' and valuta.gruppe=grupper.kodenr and valuta.valdate <= '$ordredate' order by valuta.valdate desc"))) {
...
#				$ialt=dkdecimal($ialt);
			...
			print "<td>$font<small><input type=\"text\" style=\"text-align:right\" size=2 name=anta$x value='$dkantal[$x]'></td>";
			print "<td>$font<small><input readonly=readonly size=3 value=\"$enhed[$x]\"></td>";
			...
			print "<td align=right>$font<small><input readonly=\"readonly\" style=\"text-align:right\" size=10 value=\"$tmp\"></td>";
			if ($status>=1) {
				if ($vare_id[$x]) {
...
#print "<BODY onClick=\"JavaScript:window.open('batch.php?linje_id=$linje_id', '', 'statusbar=no,menubar=no,titlebar=no,toolbar=no,scrollbars=yes, location=1');\">";
		...
		if ($art!='KK') {
			...
			print "<td>$font<small><input readonly=readonly size=3></td>";
		}
		else {
		...
		print "<td>$font<small><input type=\"text\" style=\"text-align:right\" size=4 name=raba0></td>";
			print "<td>$font<small><input readonly=readonly size=10></td>";
...
function vareopslag($sort, $fokus, $id, $vis, $ref, $find) {
	...
	if ($kontonr) {
		$query = db_select("select id from adresser where kontonr = '$kontonr' and art = 'K'");
