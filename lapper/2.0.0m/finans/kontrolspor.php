<?php
// ----------finans/kontrolspor.php-------------lap 2.0.0f-----05.05.2008-----
// LICENS
//
...
print "<td align=center><span title= 'Angiv max antal linjer som skal vises pr side'><input type=text size=3 style=\"text-align:right\" name=linjeantal value=$linjeantal></td>";
$tmp=$start+$linjeantal;
if ($next>0) {
...
function udskriv($idnumre, $bilagsnumre, $kladdenumre, $fakturanumre, $kontonumre, $transdatoer, $logdatoer, $debetbelob, $kreditbelob, $logtid, $beskrivelse, $sort, $start, $skriv) {
	...
	$x=0;
	$query = db_select("select transaktioner.*, kontoplan.beskrivelse as kontonavn from transaktioner, kontoplan $udvaelg kontoplan.regnskabsaar='$regnaar' and kontoplan.kontonr = transaktioner.kontonr order by $sort");
	while ($row =db_fetch_array($query)) {
		...
		if (($x>=$start)&&($x<$start+$linjeantal) && ($udskriv)){
			...
			if ($skriv) {
				...
				$debetsum=$debetsum+$row['debet'];
				$kreditsum=$kreditsum+$row['kredit'];
				print "<td align=right><small>$font $dato<br></small></td>";
				...
				print "<td align=right><small>$font<span title='$row[kontonavn]'> $row[kontonr]<br></span></small></td>";
	...
	if ($debetsum || $kreditsum) {
		...
		print "<td colspan=8>$font<small>Kontrolsum<br></small></td><td align=right>$font<small>".dkdecimal($debetsum)."<br></small></td><td align=right>$font<small>".dkdecimal($kreditsum)."<br></small></td><td><small><br></small></td></tr>";
	}
#	print "<tr><td colspan=11><hr></td></tr>";
	return ($y);
} #endfunction udskriv()
