<?php
// ----------------------------------debitor/ordrer---------lap 2.0.0i------2008-05-27------
// LICENS
//
				...
				if ($row = db_fetch_array($query)) {$provisionsfri[$x]=$row['provisionsfri'];}
					...
					while ($row = db_fetch_array($query)) {
						...
						if ($kobs_ordre_id[$z]) {
							...
							$r3=db_fetch_array(db_select("select valutakurs from ordrer where id = $kobs_ordre_id[$z]"));
						if ($r3['valutakurs']) $kobs_valutakurs=$r3['valutakurs'];
						else $kobs_valutakurs=100;
#							if ($r2['fakturadate']<2000-01-01) { #udeladt 270508
							$r2=db_fetch_array(db_select("select pris from ordrelinjer where id = $r2[linje_id]"));#}	
...
#		db_modify("update ordrer set valutakurs='$valutakurs' where ordre_id = '$ordre_id'");
		...
		for ($x=1; $x<=$linjeantal; $x++) {
			...
			print "<td>$font<small><input type=\"text\" style=\"text-align:right\" size=3 name=anta$x value=$dkantal[$x]><input readonly=readonly size=3 value=\"$enhed[$x]\"></td>";
...
#	if ($ialt) {
				 ...
				 print "<td align=right>$font<span title= 'db: $dk_db[$x] - dg: $dk_dg[$x]%'><small><input readonly=\"readonly\" style=\"text-align:right\" size=10 value=\"$tmp\"></td>";
#		 	}			
#			else print "<td></td>";
		...
		if ($kontonr) {
			...
			print "<td>$font<small><input type=\"text\" style=\"text-align:right\" size=3 name=anta0><input readonly=\"readonly\" size=3></td>";
