...
/*     $query = db_select("SELECT * FROM online WHERE session_id = '$s_id' ORDER BY logtime DESC LIMIT 1", __FILE__ . " linje " . __LINE__);
    ...
    if(isset($_GET["deleteBooking"])){
        ...
        $query = db_modify("DELETE FROM rentalperiod WHERE id = $id", __FILE__ . " linje " . __LINE__);
    ...
    if(isset($_GET["deleteItem"])){
        ...
        while($res = db_fetch_array($query)){
            $query2 = db_modify("DELETE FROM ordrer WHERE id = $res[order_id]", __FILE__ . " linje " . __LINE__);
            $query2 = db_modify("DELETE FROM ordrelinjer WHERE ordre_id = $res[order_id]", __FILE__ . " linje " . __LINE__);
        }
        $query = db_modify("DELETE FROM rentalperiod WHERE item_id = $id", __FILE__ . " linje " . __LINE__);
        if($query){
...
/*     if(isset($_GET["getBookings"])){
    ...
    if(isset($_GET["createReservation"])){
        $data = json_decode(file_get_contents('php://input'), true);
        $query = db_modify("INSERT INTO rentalreserved (item_id, \"from\", \"to\", comment) VALUES ($data[item_id], $data[from], $data[to], '$data[text]')", __FILE__ . " linje " . __LINE__);
        if($query){
    ...
    if(isset($_GET["getReservations"])){
        ...
        while($res = db_fetch_array($query)){
            ...
            $reservations[$i]["from"] = $res["from"];
            $reservations[$i]["to"] = $res["to"];
    ...
    if(isset($_GET["editReservationDates"])){
        $data = json_decode(file_get_contents('php://input'), true);
        $query = db_modify("UPDATE rentalreserved SET \"from\" = $data[from], \"to\" = $data[to] WHERE id = $data[id]", __FILE__ . " linje " . __LINE__);
        if($query){
            echo json_encode("spærringen er nu opdateret");
        }else{
            echo json_encode("Der skete en fejl");
...
/*     if(isset($_GET["getXCSRFToken"])){
    ...
    function CreateOrder($data){
        ...
        $query = db_select("SELECT id FROM ordrer WHERE ordrenr = $ordrenr", __FILE__ . " linje " . __LINE__);
