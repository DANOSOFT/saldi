(async () => {
  ...
  const singleItem = async (item) => {
    ...
    const closedDays = await getClosedDays()
    const closedDates = []
    const closedDaysWithoutBookings = []
    if (closedDays.success !== false) {
      closedDays.forEach(i => {
        ...
        closedDaysWithoutBookings.push(formattedDate)
      })
    }
    ...
    const datePick = flatpickr(fromCalendar, {
      ...
      onDayCreate: function (dObj, dStr, fp, dayElem) {
        ...
        // Skip if date is closed or no bookings exist
        if (closedDaysWithoutBookings.includes(dateStr) || !bookings || bookings.msg === "Der er ingen bookinger") {
            return;
        }

        // Find latest booking before current date
        const lastBooking = bookings
            .filter(booking => new Date(booking.to * 1000) < date)
            .sort((a, b) => b.to - a.to)[0];

        // Determine start date (either day after last booking or today)
        const startDate = lastBooking ? 
            new Date(lastBooking.to * 1000) : 
            new Date()
        startDate.setDate(startDate.getDate() + 1)
        startDate.setHours(0, 0, 0, 0)

        // Calculate available days (excluding closed days)
        let availableDays = 0
        let currentCheck = new Date(startDate)

        while (currentCheck <= date) {
            const checkStr = formatDate(currentCheck)
            if (!closedDaysWithoutBookings.includes(checkStr)) {
                availableDays++
            }
            currentCheck.setDate(currentCheck.getDate() + 1)
        }

        // Check for existing booking on current date
        const hasBookingOnDay = bookings.some(booking => 
            new Date(booking.from * 1000).toDateString() === date.toDateString()
        )

        // Add green dot if:
        // 1. It's an available day (not closed)
        // 2. Either first available day or weekly interval
        // 3. No existing booking
        if (!hasBookingOnDay && 
            !closedDates.includes(dateStr) && 
            (availableDays === 1 || (availableDays - 1) % 7 === 0)) {
