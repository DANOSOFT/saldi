...
#echo "ialt ".dkdecimal($total)."<br>";
	...
	print  "<tr><td align=center>Dato</td><td align=center>Kassekladde</td><td align=center>bilag</td><td align=center>bel&oslash;b</td></tr>";	
	for($x=1;$x<=$fejlantal;$x++) {
			print "<tr><td>".dkdato($fejldate[$x])."</td><td align=right>$fejlkladde[$x]</td><td align=right>$fejlbilag[$x]</td><td align=right>".dkdecimal($fejlsum[$x])."</td></tr>";
			$sum=$sum+$fejlsum[$x];	
		}
		print "<tr><td colspan=3>Kontrolsum</td><td align=right>".dkdecimal($sum)."</td></tr>";
