<?php
// ------------------index/install.php-------2.0.7----2009-05-21-22:06---
// LICENS
//
...
if (isset($_POST['opret'])){
	...
	db_modify("CREATE TABLE brugere(id serial NOT NULL, brugernavn varchar(50), kode varchar(50), status boolean, regnskabsaar integer, rettigheder varchar(25), PRIMARY KEY (id))",__FILE__ . " linje " . __LINE__);
	...
	db_modify("CREATE TABLE regnskab (id serial NOT NULL,	regnskab varchar(25), dbhost varchar(25), dbuser varchar(25), db varchar(25), version varchar(10), sidst varchar(10), brugerantal numeric, posteringer numeric, posteret numeric, lukket varchar(2),administrator varchar(2), PRIMARY KEY (id))",__FILE__ . " linje " . __LINE__);
	...
	db_modify("CREATE TABLE online (session_id varchar(50), brugernavn varchar(50), db varchar(25), dbuser varchar(25), rettigheder varchar(50), regnskabsaar integer, logtime varchar(15), revisor boolean)",__FILE__ . " linje " . __LINE__);
	db_modify("CREATE TABLE brugerdata (id serial NOT NULL, firmanavn varchar(50), addr1 varchar(50), addr2 varchar(50), postnr varchar(10), bynavn varchar(50), kontakt varchar(50), email varchar(50), cvrnr varchar(25), regnskab varchar(50), regnskab_id integer,brugernavn varchar(50), kodeord varchar(50), kontrol_id varchar(50), aktiv int, logtime varchar(15), PRIMARY KEY (id))",__FILE__ . " linje " . __LINE__);
	db_modify("CREATE TABLE tekster (id serial NOT NULL, sprog_id integer, tekst_id integer, tekst varchar(255), PRIMARY KEY (id))",__FILE__ . " linje " . __LINE__);
...
#	rename("../includes/connect", "../includes/connect.php");
	...
	if ($fp=fopen("../includes/connect.php","w") ) {
		...
		print "<p>Tegn en hotline-aftale for kun kr. 45,- + moms om m&aring;neden, s&aring; kan du ringe eller sende en e-mail \n";
		...
		print "<p>Se mere p&aring; <a href=\"http://www.saldi.dk/hotline\" target=\"_blank\">http://www.saldi.dk/hotline</a></p>\n\n";
		print "<p>&nbsp;</p>\n\n";
...
function skriv_connect($fp,$host,$db_bruger,$db_password,$db_navn,$db_encode,$db_type) {
	fwrite($fp," \n");
	fwrite($fp,"<?php\n");
	fwrite($fp,"// ----/includes/connect.php---------------lap 2.0.7-----2009.05.12-----\n");
	fwrite($fp,"// LICENS\n");
	fwrite($fp,"//\n");
	...
	fwrite($fp,"//\n");
	fwrite($fp,"// Copyright (c) 2004-2009 DANOSOFT ApS\n");
	fwrite($fp,"// ----------------------------------------------------------------------\n");
	fwrite($fp,"\n");
	...
	fwrite($fp,"\n");
	if ($db_type=='mysql') {
		fwrite($fp,"if (\$sqpass) \$connection = db_connect (\"\$sqhost\", \"\$squser\", \"\$sqpass\");\n");
		fwrite($fp,"else \$connection = db_connect (\"\$sqhost\", \"\$squser\", \"\$sqpass\");\n");
	} else {
