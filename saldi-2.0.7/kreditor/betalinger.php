<?php
// ---------kreditor/betalinger.php----------Patch 2.0.4---------2008-12-22----------
// LICENS
//
...
#	$liste_id=($_POST['liste_id'])? $_POST['liste_id']:Null;
	...
	for ($x=1;$x<=$antal;$x++) {
		...
		if ($slet[$x]=='on') db_modify("delete from betalinger where id='$id[$x]'",__FILE__ . " linje " . __LINE__);
		else db_modify("update betalinger set bet_type='$erh[$x]',fra_kto='$fra_kto[$x]',egen_ref='$egen_ref[$x]',til_kto='$til_kto[$x]',kort_ref='$kort_ref[$x]',modt_navn='$modt_navn[$x]',belob='$belob[$x]',valuta='$valuta[$x]',betalingsdato='$betalingsdato[$x]' where id='$id[$x]'",__FILE__ . " linje " . __LINE__);
	}
...
if ($find) {
	...
	$q=db_select("select openpost.id as id, openpost.beskrivelse as egen_ref, openpost.amount as amount, openpost.valuta as valuta, 
							 openpost.faktnr as faktnr, openpost.transdate as transdate,openpost.bilag_id as bilag_id,openpost.forfaldsdate as forfaldsdate,openpost.betal_id as betal_id, 
	...
	adresser.bank_fi as modt_fi, adresser.betalingsbet as betalingsbet, adresser.betalingsdage as betalingsdage
							from openpost, adresser where openpost.udlignet != 1 and openpost.amount < 0 and openpost.konto_id = adresser.id and adresser.art = 'K'",__FILE__ . " linje " . __LINE__);
	while ($r=db_fetch_array($q)){
		$ordre_id=0;
		$medtag=1;
		...
		if ($medtag) {
			...
			if ($r['betal_id']) {
				if (substr($r['betal_id'],0,1)=="+") {
					...
					list($kort_ref,$modt_konto)=split(";",$tmp2);
