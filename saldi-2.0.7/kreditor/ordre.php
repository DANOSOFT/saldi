<?php
// -------------kreditor/ordre.php----------lap 2.0.7-----2009-05-15----
// LICENS
//
...
$q = db_SELECT("select box4 from grupper where art = 'DIV' and kodenr = '2'",__FILE__ . " linje " . __LINE__);
$r=db_fetch_array($q); 
$hurtigfakt=$r['box4'];
...
if ($_GET['vare_id']) {
	...
	if ($_POST) {
		...
		if ($valuta && $valuta!='DKK') {
			if ($r= db_fetch_array(db_select("select valuta.kurs as kurs from valuta, grupper where grupper.art='VK' and grupper.box1='$valuta' and valuta.gruppe=".nr_cast("grupper.kodenr")." and valuta.valdate <= '$ordredate' order by valuta.valdate desc",__FILE__ . " linje " . __LINE__))) {
				$valutakurs=$r['kurs'];
			} else {
				$tmp = dkdato($ordredate);
				print "<BODY onLoad=\"javascript:alert('Der er ikke nogen valutakurs for $valuta den $ordredate')\">";
...
#			if (!$lager_id) {$lager_id='0';}
			...
			for($x=1; $x<=$linjeantal; $x++) {
				...
				elseif (($art=='KK')&&($kred_ord_id)) { ###################	 Kreditnota ####################
					...
					if (!$hurtigfakt) {
						$r = db_fetch_array(db_select("select grupper.box8 from grupper,varer where varer.id = '$vare_id[$x]' and grupper.kodenr = varer.gruppe and grupper.art='VG'",__FILE__ . " linje " . __LINE__));
						if ($r['box8'] == 'on') {
							$rest=0;
							$query = db_select("select id, rest, lager from batch_kob where vare_id = '$vare_id[$x]' and ordre_id = '$kred_ord_id'",__FILE__ . " linje " . __LINE__);
...
#					if ($posnr_ny[$x]=="-") {$antal[$x]=0;}
					...
					if (($status<2)||(($antal[$x]>0)&&($status==2)&&($antal[$x]>=$tidl_lev[$x]))||(($antal[$x]<0)&&($status==2)&&($antal[$x]<=$tidl_lev[$x]))) {
						...
						db_modify("update ordrelinjer set beskrivelse='$beskrivelse[$x]', antal='$antal[$x]', leveres='$leveres[$x]', pris='$pris[$x]', rabat='$rabat[$x]' where id='$linje_id[$x]'",__FILE__ . " linje " . __LINE__);
					}
			...
			if ($opret_ny==1)	{
				...
				for($x=1; $x<=$linjeantal; $x++) {
					if ($antal[$x]!=$tidl_lev[$x]) {
						...
						db_modify("insert into ordrelinjer (ordre_id, posnr, varenr, vare_id, beskrivelse, enhed, antal, pris, rabat, serienr, lev_varenr, momsfri) values ('$ny_id', '$posnr_ny[$x]', '$varenr[$x]', '$vare_id[$x]', '$beskrivelse[$x]', '$enhed[$x]', '$diff[$x]', '$pris[$x]', '$rabat[$x]', '$serienr[$x]', '$lev_varenr[$x]', '$momsfri[$x]')",__FILE__ . " linje " . __LINE__);
...
#				db_modify("update ordrer set kred_ord_id='$kred_ord_id' where id='$id'",__FILE__ . " linje " . __LINE__);}
			for($x=1; $x<=$linjeantal; $x++) {
				...
				if ($vare_id[$x]) {db_modify("insert into ordrelinjer (ordre_id, posnr, varenr, vare_id, beskrivelse, enhed, antal, pris, rabat, serienr, lev_varenr, kred_linje_id) values ('$id', '$posnr_ny[$x]', '$varenr[$x]', '$vare_id[$x]', '$beskrivelse[$x]', '$enhed[$x]', $antal[$x], '$pris[$x]', '$rabat[$x]', '$serienr[$x]', '$lev_varenr[$x]', $linje_id[$x])",__FILE__ . " linje " . __LINE__);}
				else {db_modify("insert into ordrelinjer (ordre_id, posnr, beskrivelse, enhed) values ('$id', '$posnr_ny[$x]', '$beskrivelse[$x]', '$enhed[$x]')",__FILE__ . " linje " . __LINE__);}
			}
		}
		$vis=1;
	transaktion("commit");
...
#				$ialt=dkdecimal($ialt);
			...
			print "<input type=hidden name=anta$x value=$antal[$x]><td align=right>$dkantal[$x]</td>";
...
#		$moms=$moms+0.0001; #Ellers runder den ned istedet for op?
		...
		if ($art!='KK') {
			print "<td align=center><input type=submit value=\"&nbsp;Kopi&eacute;r&nbsp;\" name=\"submit\" onclick=\"javascript:docChange = false;\"></td>";
			print "<td align=center><input type=submit value=\"Kredit&eacute;r\" name=\"submit\" onclick=\"javascript:docChange = false;\"></td>";
		}
	}
		...
		print "<tr><td>Postnr, by</td><td><input type=text size=4 name=postnr onfocus=\"document.forms[0].fokus.value=this.name;\" value=\"$postnr\" onchange=\"javascript:docChange = true;\"></td><td><input type=text size=19 name=bynavn onfocus=\"document.forms[0].fokus.value=this.name;\" value=\"$bynavn\" onchange=\"javascript:docChange = true;\"></td></tr>\n";
		print "<tr><td>Land</td><td colspan=2><input type=text size=25 name=land value=\"$land\" onchange=\"javascript:docChange = true;\"></td></tr>\n";
		...
		print "</tbody></table></td>";
		print "<td width=33%><table cellpadding=0 cellspacing=0 border=0 width=100>";
		print "<tr><td>CVR.nr</td><td colspan=2><input type=text size=10 name=cvrnr value=\"$cvrnr\" onchange=\"javascript:docChange = true;\"></td></tr>\n";
		print "<tr><td>Ordredato</td><td><input type=text size=10 name=ordredato value=\"$ordredato\" onchange=\"javascript:docChange = true;\"></td>";
		print "<td>Lev.&nbsp;dato</td><td><input type=text size=10 name=levdato value=\"$levdato\" onchange=\"javascript:docChange = true;\"></td></tr>\n";
		$list=array();
		$beskriv=array();
		$list[0]='DKK';
		$x=0;
		...
		if ($x>0) {
			...
			for ($x=0; $x<=$tmp; $x++) {
				if ($projekt!=$list[$x]) print "<option title=\"$beskriv[$x]\" onchange=\"javascript:docChange = true;\">$list[$x]</option>";
				else print "<option title=\"$beskriv[$x]\" selected=\"selected\" onchange=\"javascript:docChange = true;\">$list[$x]</option>";
			}
			print "</SELECT></td></tr>";
		...
		if ($status==0){print "<tr><td>Godkend</td><td><input type=checkbox name=godkend></td></tr>\n";}
		elseif ($status==1) {
			$query = db_select("select * from batch_kob where ordre_id=$id",__FILE__ . " linje " . __LINE__);
			if(db_fetch_array($query)){print "<tr><td>Dan lev. fakt.</td><td><input type=checkbox name=godkend></td></tr>\n";}
			else {
				$query = db_select("select * from batch_salg where ordre_id=$id",__FILE__ . " linje " . __LINE__);
				if(db_fetch_array($query)){print "<tr><td>Dan lev. fakt.</td><td><input type=checkbox name=godkend></td></tr>\n";}
			}
		}
#		elseif ($status==1){print "<tr><td>Modtag</td><td><input type=checkbox name=modtag></td></tr>\n";}
		else{print "<tr><td witdh=200>Fakturanr</td><td colspan=2><input type=text size=23 name=fakturanr value=\"$fakturanr\" onchange=\"javascript:docChange = true;\"></td></tr>\n";}
		print "</tbody></table></td>";
		...
		print "<tr><td>Postnr, By</td><td><input type=text size=4 name=lev_postnr value=\"$lev_postnr\" onchange=\"javascript:docChange = true;\"><input type=text size=19 name=lev_bynavn value=\"$lev_bynavn\" onchange=\"javascript:docChange = true;\"></td></tr>\n";
	...
	#		print "<tr><td><textarea style=\"font-family: helvetica,arial,sans-serif;\" name=lev_adr rows=5 cols=35>$lev_adr</textarea></td></tr>\n";
		...
		else {print "<td align=center>pos</td><td align=center>varenr</td><td align=center>Lev. vnr</td><td align=center>antal</td><td>enhed</td><td align=center>beskrivelse</td><td align=center>pris</td><td align=center>%</td><td align=center>ialt</td>";}
		print "</tr>\n";
/*		
		if ($valuta && $valuta!='DKK') {
...
#				$ialt=dkdecimal($ialt);
			...
			print "<input type=hidden name=serienr[$x] value='$serienr[$x]'>";
			print "<tr>";
			print "<td><input type=\"text\" style=\"text-align:right\" size=3 name=posn$x value='$x' onchange=\"javascript:docChange = true;\"></td>";
			print "<td><input readonly=readonly size=7 name=vare$x onfocus=\"document.forms[0].fokus.value=this.name;\" value='$varenr[$x]'></td>";
			print "<td><input type=text size=7 name=lev_varenr$x value='$lev_varenr[$x]' onchange=\"javascript:docChange = true;\"></td>";
			print "<td><input type=\"text\" style=\"text-align:right\" size=2 name=anta$x value='$dkantal[$x]' onchange=\"javascript:docChange = true;\"></td>";
			print "<td><input readonly=readonly size=3 value=\"$enhed[$x]\"></td>";
			print "<td><input type=text size=60 name=beskrivelse$x value=\"$beskrivelse[$x]\" onchange=\"javascript:docChange = true;\"></td>";
			print "<td><input type=\"text\" style=\"text-align:right\" size=10 name=pris$x value='$dkpris' onchange=\"javascript:docChange = true;\"></td>";
			...
			print "<td align=right><input readonly=\"readonly\" style=\"text-align:right\" size=10 value=\"$tmp\"></td>";
			if ($status>=1) {
				if ($vare_id[$x]) {
					...
					if (substr($dklev[$x],-1)=='0'){$dklev[$x]=substr($dklev[$x],0,-2);}
						 print "<td><input type=\"text\" style=\"text-align:right\" size=2 name=leve$x value='$dklev[$x]' onchange=\"javascript:docChange = true;\"></td>";
					if (($antal[$x]>=0)&&($art!='KK')) {
...
#print "<BODY onClick=\"JavaScript:window.open('batch.php?linje_id=$linje_id', '', 'statusbar=no,menubar=no,titlebar=no,toolbar=no,scrollbars=yes, location=1');\">";
		...
		print "<tr>";
		print "<td><input type=\"text\" style=\"text-align:right\" size=3 name=posn0 value=$x></td>";
		if ($art!='KK') {
			print "<td><input type=text size=7 name=vare0 onfocus=\"document.forms[0].fokus.value=this.name;\"></td>";
			print "<td><input type=text size=7 name=lev_v0></td>";
			print "<td><input type=\"text\" style=\"text-align:right\" size=2 name=anta0></td>";
			print "<td><input readonly=readonly size=3></td>";
		}
		else {
			print "<td><input readonly=readonly size=7></td>";
			print "<td><input readonly=readonly size=7></td>";
			print "<td><input readonly=readonly size=2></td>";
			print "<td><input readonly=readonly size=3></td>";
		}
...
#		if ($status==1) {print "<td><input type=\"text\" style=\"text-align:right\" size=2 name=modt0></td>";}
		print "</tr>\n";
		...
		if ($kreditmax=$r['kreditmax']*1) {
			...
			while($r=db_fetch_array($q)) {
				...
				elseif ($valuta!='DKK' && $r['valuta']=='DKK') {
					if ($r3=db_fetch_array(db_select("select kurs from grupper, valuta where grupper.art='VK' and grupper.box1='$valuta' and valuta.gruppe = grupper.kodenr and valuta.valdate <= '$r[transdate]' order by valuta.valdate desc",__FILE__ . " linje " . __LINE__))) {
...
#	print"<td width=2%></td>";
	...
	while ($row = db_fetch_array($query)) {
		$x++;
		if (!$vis || $row['lev_id']==$leverandor || $row['lev_id']=='0') $vis_vare_id[$x]=$row['vare_id'];
		else {
			$skjul_vare_id[$x]=$row['vare_id']; 
		}
	}
	...
	while ($row = db_fetch_array($query)) {
		$vare_id=$row['id']; 
		if (!in_array($vare_id,$skjul_vare_id)) {
		$varenr=addslashes(trim($row['varenr']));
		$vist=0;
#		if ($vis) $query2 = db_select("select * from vare_lev where vare_id = $row[id] $temp",__FILE__ . " linje " . __LINE__);
...
#			print "<td></td>";
		...
		if (!$vist && $row['samlevare']!='on' && !in_array($vare_id,$skjul_vare_id)) {
...
function sidehoved($id, $returside, $kort, $fokus, $tekst) {
	...
	else {print "<td width=\"10%\" $top_bund> $color</td>";}
...
#		if (!$lager_id) {$lager_id='0';}
	...
	elseif($firmanavn) {
		$query = db_select("select tidspkt from ordrer where id=$id and hvem='$brugernavn'",__FILE__ . " linje " . __LINE__);
		if ($row = db_fetch_array($query)) {
			db_modify("update ordrer set konto_id=$konto_id, kontonr='$kontonr', firmanavn='$firmanavn', addr1='$addr1', addr2='$addr2', postnr='$postnr', bynavn='$bynavn', land='$land', betalingsdage='$betalingsdage', betalingsbet='$betalingsbet', cvrnr='$cvrnr', notes='$notes', hvem = '$brugernavn', tidspkt='$tidspkt', valuta='$valuta' where id=$id",__FILE__ . " linje " . __LINE__);
		}
		else {			
			$query = db_select("select hvem from ordrer where id=$id",__FILE__ . " linje " . __LINE__);
