...
// -------------finans/rapport.php-------lap 2.0.7------2009-05-16-------
// LICENS
//
...
#	}
	...
	print "<option value=\"$dato_fra\">$dato_til.</option>\n";
	for ($x=1; $x <= 31; $x++) print "<option value=\"$x\">$x.</option>\n";
	print "</select>";
	print "</td></tr>\n";
...
#	print "  <a accesskey=L href=\"rapport.php?rapportart=Kontokort&regnaar=$regnaar&dato_fra=$startdato&maaned_fra=$mf&dato_til=$slutdato&maaned_til=$mt&konto_fra=$konto_fra&konto_til=$konto_til&afd=$afd\">Luk</a><br><br>";
	...
	while ($row = db_fetch_array($query)){
		...
		if (!$dim && $row['kontotype']=="S") $primo[$x]=round($row['primo']+0.0001,2);
		else $primo[$x]=0;
	}
	$kontoantal=$x;
	$ktonr=array();
	$x=0;
	$query = db_select("select kontonr from transaktioner where transdate>='$regnstart' and transdate<='$regnslut' $dim order by transdate",__FILE__ . " linje " . __LINE__);
	while ($row = db_fetch_array($query)){
		$x++;
		$ktonr[$x]=$row['kontonr'];
	}
	...
	for ($x=1; $x<=$kontoantal; $x++){
		if (in_array($kontonr[$x], $ktonr)||$primo[$x]){
			print "<tr><td colspan=6><hr></td></tr>";
			print "<tr><td></td><td></td><td>$kontonr[$x] : $kontobeskrivelse[$x] </tr>";
			print "<tr><td colspan=6><hr></td></tr>";
			$kontosum=$primo[$x];
			...
			while ($row = db_fetch_array($query)){
			 	$kontosum= $kontosum+round($row['debet']+0.0001,2)-round($row['kredit']+0.0001,2);
			}
			$tmp=dkdecimal($kontosum);
			if (!$dim) print "<tr><td></td><td></td><td>  Primosaldo </td><td></td><td></td><td align=right>$tmp </td></tr>";
			$print=1;
			...
			while ($row = db_fetch_array($query)){
				print "<tr><td>  ".dkdato($row['transdate'])." </td><td>$row[bilag] </td><td>$row[beskrivelse] </td>";
...
# echo "select * from transaktioner where transdate>='$regnaarstart' and transdate<='$regnslut' $dim order by kontonr<br>";		
	$query = db_select("select * from transaktioner where transdate>='$regnaarstart' and transdate<='$regnslut' $dim order by kontonr",__FILE__ . " linje " . __LINE__);
		...
		if (!in_array($row['kontonr'], $ktonr)) { # Her fanges konto med bevaegelser i perioden.
			...
			if ((!$afd && !$projekt_fra && !$ansat_fra) && ($r2 = db_fetch_array(db_select("select primo from kontoplan where regnskabsaar='$regnaar' and kontonr=$ktonr[$x] and kontotype='S'",__FILE__ . " linje " . __LINE__)))) {
				$kto_aar[$x]=round($r2['primo']+0.0001,2);
			}
		}
		$kto_aar[$x]=$kto_aar[$x]+round($row['debet']+0.0001,2)-round($row['kredit']+0.0001,2);
		if ($row['transdate']>=$regnstart) $kto_periode[$x]=$kto_periode[$x]+round($row['debet']+0.0001,2)-round($row['kredit']+0.0001,2);
	}
	$kto_antal=$x;	
	...
	for ($x=1; $x<=$kontoantal; $x++) {
		if (($kontonr[$x]>=$konto_fra)&&($kontonr[$x]<=$konto_til)&&(($aarsum[$x])||($periodesum[$x])||($kontotype[$x] == 'H'))) {
			if ($kontotype[$x] == 'H') {
				...
				print "<tr><td $tmp colspan=3><b>$kontobeskrivelse[$x]</b></td>";
				print "<tr><td colspan=6><hr></td></tr>";
			} elseif ($kontotype[$x] == 'Z') {
				print "<tr><td colspan=6><hr></td></tr>";
				$tmp = kontobemaerkning($kontobeskrivelse[$x]);
				print "<td $tmp colspan=4><b> $kontobeskrivelse[$x] </b></td>";
