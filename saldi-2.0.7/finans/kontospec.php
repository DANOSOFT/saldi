<?php
// ----------------------finans/kontospec.php------lap 2.0.5-----2009-02-24
// LICENS
//
...
print "<td width=\"10%\" $top_bund><font face=\"Helvetica, Arial, sans-serif\"><a href=regnskab.php accesskey=L>Luk</a></td>";
print "<td width=\"80%\" $top_bund><font face=\"Helvetica, Arial, sans-serif\" color=\"#000066\">Specifikation for ";
if($kontonr) {print "konto: $kontonr";}
if($bilag) {print "bilag: $bilag";}
print " </td><td width=\"10%\" $top_bund><font face=\"Helvetica, Arial, sans-serif\" color=\"#000066\"><br></td>";
print "</tbody></table>";
print "</td></tr>";
...
while ($row = db_fetch_array($query)) {
	...
	print "<td><font face=\"Helvetica, Arial, sans-serif\" color=\"$color\">";
	if ($row[bilag]) print "<a href=kontospec.php?bilag=$row[bilag] target=\"_blank\">$row[bilag]</a><br>";
	print "</td>";
	$dato=dkdato($row['transdate']);
	print "<td><font face=\"Helvetica, Arial, sans-serif\" color=\"$color\">$dato</a><br></td>";
	print "<td><font face=\"Helvetica, Arial, sans-serif\" color=\"$color\">$row[beskrivelse]</a><br></td>";
	$tal=dkdecimal($row['debet']);
	print "<td align=right><font face=\"Helvetica, Arial, sans-serif\" color=\"$color\">$tal</a><br></td>";
	$tal=dkdecimal($row['kredit']);
	print "<td align=right><font face=\"Helvetica, Arial, sans-serif\" color=\"$color\">$tal</a><br></td>";
	print "<td align=right><font face=\"Helvetica, Arial, sans-serif\" color=\"$color\">$row[faktura]</a><br></td>";
	print "<td align=right><font face=\"Helvetica, Arial, sans-serif\" color=\"$color\"><a href=kassekladde.php?kladde_id=$row[kladde_id]&returside=kontospec.php target=\"_blank\">$row[kladde_id]</a><br></td>";
	print "<td align=right><font face=\"Helvetica, Arial, sans-serif\" color=\"$color\">$row[afd]</a><br></td>";
	print "</tr>";
}
?>
</tbody>
