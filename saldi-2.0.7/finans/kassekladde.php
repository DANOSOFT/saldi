...
// ------------finans/kassekladde.php------lap 2.0.7---2009.05.22------
// LICENS
//
...
$afd=array(NULL);$amount=array(NULL);$ansat=array(NULL);$betal_id=array(NULL);$bilag=array(NULL);
$d_type=array(NULL);$debet=array(NULL);$faktura=array(NULL);$forfaldsdate=array(NULL);$forfaldsdato=array(NULL);
$k_type=array(NULL);$kontonr=array(NULL);$kredit=array(NULL);$momsfri=array(NULL);$projekt=array(NULL);$valuta=array(NULL);
...
#	$momsfri=if_isset($_POST['momsfri']);
	...
	for ($x=1;$x<=$antal_ny;$x++) {
		...
		if ($debet[$x]) $kontrolsum=$kontrolsum+usdecimal($belob[$x]);
		if ($kredit[$x]) $kontrolsum=$kontrolsum-usdecimal($belob[$x]);
...
#			}
		...
		if ((strpos($bilag[$x],'+'))&&($kladde_id)) indsaet_linjer($kladde_id,$bilag[$x],$dato[$x],$beskrivelse[$x],$d_type[$x],$debet[$x]*1,$k_type[$x],$kredit[$x]*1,$faktura[$x],$belob[$x],$afd[$x]*1,$ansat[$x],$projekt[$x]*1,$valuta[$x],$forfaldsdato[$x],$betal_id[$x]*1);
	...
	$fokus=$_POST['fokus'];
	if ($kladde_id) {
		$row = db_fetch_array(db_select("select tidspkt from kladdeliste where id=$kladde_id",__FILE__ . " linje " . __LINE__));
...
if (($bogfort)&&($bogfort!='-')){print "<td colspan=10 align=center><b> $kladdenote</b></td>";}
	...
	print "<td><span title= 'Angiv kontonummer til kontrol af kontobev&aelig;gelser'><input class=\"inputbox\" type=\"text\" style=text-align:right size=8 name=kontrolkonto value=\"$kontrolkonto\" onchange=\"javascript:docChange = true;\"></td>";
...
$faktura=array();$belob=array();$afd=array();$ansat=array();$ansat_id=array();$projekt=array();$valuta=array();$forfaldsdato=array();$betal_id=array();$momsfri=array();
	...
	while ($row = db_fetch_array($query)) {
		...
		$dokument[$x]=htmlentities($row['dokument'],ENT_QUOTES,$charset);
		...
		if (($d_type[$x]=='F')&&($debet[$x])&&(!$fejl)) {
			$query2 = db_select("select beskrivelse, moms from kontoplan where kontonr='$debet[$x]'",__FILE__ . " linje " . __LINE__);
		...
		if (($k_type[$x]=='F')&&($kredit[$x])&&(!$fejl)) {
			$query2 = db_select("select beskrivelse, moms from kontoplan where kontonr='$kredit[$x]'",__FILE__ . " linje " . __LINE__);
			$row2 = db_fetch_array($query2);
...
#		if (($k_type[$y]!="D")&&($k_type[$y]!="K")) $k_type[$y]="F"; #phr 20070801
		...
		print "<td title=\"Tilbagef&oslash;r postering\"><a href='kassekladde.php?kladde_id=$kladde_id&ompost=$id[$y]'><img alt=\"undo\" src=\"../ikoner/undo.gif\" style=\"border: 0px solid ; width: 18px; height: 17px;\"></a></td>";
		print "</tr>\n";
	}
	if ($bogfort=='V'){
	...
	if ($kontrolkonto) {
		...
		$query = db_select("select saldo from kontoplan where kontonr='$kontrolkonto' and regnskabsaar='$regnaar'",__FILE__ . " linje " . __LINE__);
		if ($row = db_fetch_array($query)) {
	...
	for ($y=1;$y<=$x;$y++) {
		...
		if ($vis_bilag) {
			if ($dokument[$y]) print "<td title=\"klik her for at &aring;bne bilaget: $dokument[$y]\"><a href=\"bilag.php?filnavn=$dokument[$y]&bilag_id=$id[$y]&bilag=$bilag[$y]&kladde_id=$kladde_id\"><img style=\"border: 0px solid\" alt=\"clip_m_papir\" src=\"../ikoner/paper.gif\"></a></td>";
			else print "<td title=\"klik her for at vedh&aelig;fte et bilag\"><a href=\"bilag.php?bilag_id=$id[$y]&bilag=$bilag[$y]&ny=ja&kladde_id=$kladde_id\"><img  style=\"border: 0px solid\" alt=\"clip\" src=\"../ikoner/clip.gif\"></a></td>";
		} else print "<td></td>";
		...
		print "<td><input class=\"inputbox\" type=\"text\" style=\"text-align:right\" size=10 name=debe$y onfocus=\"document.forms[0].fokus.value=this.name;\" value =\"$debet[$y]\" title=\"$debettext[$y]\" onchange=\"javascript:docChange = true;\"></td>";
		...
		print "<td><input class=\"inputbox\" type=\"text\" style=\"text-align:right\" size=10 name=kred$y onfocus=\"document.forms[0].fokus.value=this.name;\" value =\"$kredit[$y]\" title= \"$kredittext[$y]\" onchange=\"javascript:docChange = true;\"></td>";
		...
		print "<td><input class=\"inputbox\" type=\"text\" style=\"text-align:right\" size=10 name=belo$y onfocus=\"document.forms[0].fokus.value=this.name;\" value =\"$belob[$y]\" onchange=\"javascript:docChange = true;\"></td>";
		...
		if ($k_type[$y]=='K' || $d_type[$y]=='D') {
			...
			print "<td><input readonly=readonly size=10><br></td>";
			if ($vis_bet_id) print "<td><input readonly=readonly size=10><br></td>";
		}
...
# Genindsat 060408 - Tildeler bilagsnummer hvis bilag i balance
	...
	if (!isset($projekt[$x]))$projekt[$x]=NULL;if (!isset($valuta[$x]))$valuta[$x]=NULL;if (!isset($ansat[$x]))$ansat[$x]=NULL;
		...
		print "<td><input class=\"inputbox\" type=\"text\" size=1 name=\"d_ty$x\" onfocus=\"document.forms[0].fokus.value=this.name;\" onBlur=\"validerFelt(this,'Gyldig v&aelig;rdi: D, K eller Blank');\" value =\"$d_type[$x]\" onchange=\"javascript:docChange = true;\"></td>";
		...
		print "<td><input class=\"inputbox\" type=\"text\" size=1 name=k_ty$x onfocus=\"document.forms[0].fokus.value=this.name;\" onBlur=\"validerFelt(this,'Gyldig v&aelig;rdi: D, K eller Blank');\" value =\"$k_type[$x]\" onchange=\"javascript:docChange = true;\"></td>";
	...
	for ($z=$x;$z<=$y;$z++) {
		...
		print "<td><input class=\"inputbox\" type=\"text\" size=1 name=d_ty$z onfocus=\"document.forms[0].fokus.value=this.name;\" onBlur=\"validerFelt(this,'Gyldig v&aelig;rdi: D, K eller Blank');\" onchange=\"javascript:docChange = true;\"></td>";
		...
		print "<td><input class=\"inputbox\" type=\"text\" size=1 name=k_ty$z onfocus=\"document.forms[0].fokus.value=this.name;\" onBlur=\"validerFelt(this,'Gyldig v&aelig;rdi: D, K eller Blank');\" onchange=\"javascript:docChange = true;\"></td>";
...
#		print "<td><input class=\"inputbox\" type=\"text\" size=10 name=b_id$z onfocus=\"document.forms[0].fokus.value=this.name;\" onchange=\"javascript:docChange = true;\"></td>";
	...
	print "<td align=center><span title=\"Klik her for at gemme\"><input type=submit accesskey=\"g\" value=\"&nbsp;&nbsp;Gem&nbsp;&nbsp;\" name=\"submit\" onclick=\"javascript:docChange = false;\"></span></td>";
	print "<td align=center><span title=\"Opslag - din mark&oslash;rs placering angiver hvilken tabel, opslag foretages i\"><input type=submit accesskey=\"o\" value=\"Opslag\" name=\"submit\" onclick=\"javascript:docChange = false;\"></span></td>";
	if ($kladde_id && !$fejl) {
		print "<td align=center><span title=\"Simulering af bogf&oslash;ring viser bev&aelig;gelser i kontoplanen\"><input type=submit accesskey=\"s\" value=\"Simul&eacute;r\" name=\"submit\" onclick=\"javascript:docChange = false;\"></span></td>";
		print "<td align=center><span title=\"Bogf&oslash;r - der foretages f&oslash;rst en simulering, som du skal bekr&aelig;fte\"><input type=submit accesskey=\"b\" value=\"Bogf&oslash;r\" name=\"submit\" onclick=\"javascript:docChange = false;\"></span></td>";
		if (!$fejl && db_fetch_array(db_select("select id from ordrer where status=3",__FILE__ . " linje " . __LINE__))) {
			print "<td align=center><span title=\"Henter afsluttede ordrer fra ordreliste\"><input type=submit accesskey=\"h\" value=\"Hent ordrer\" name=\"submit\" onclick=\"javascript:docChange = false;\"></span></td>";
		}
		if(db_fetch_array(db_select("select * from grupper where art = 'DIV' and kodenr = '2' and box6='on'",__FILE__ . " linje " . __LINE__))) {
			 print "<td align=center><input type=submit accesskey=\"d\" value=\"DocuBizz\" name=\"submit\" onclick=\"javascript:docChange = false;\" onclick=\"return confirm('Importer data fra DocuBizz?')\"></td>";
		}	 
		print "<td align=center><span title=\"Importerer bankposteringer eller andre data fra .csv-fil (kommasepareret fil)\"><input type=submit accesskey=\"i\" value=\"Import\" name=\"submit\" onclick=\"javascript:docChange = false;\"></span></td>";
		print "<td align=center><span title=\"Finder &aring;bne poster, som modsvarer bel&oslash;b og fakturanummer\"><input type=submit accesskey=\"u\" value=\"Udlign\" name=\"submit\" onclick=\"javascript:docChange = false;\"></span></td>";
	}
	print "</form>";
	print "</tbody></table></td></tr>\n";
	print "</tbody></table>";
...
#	if ($bilag=="-"){$bilag="";} PHR 02.10.06
	...
	if (($bilag != "-")&&(($bilag)||($beskrivelse)||($kredit)||($debet)||($faktura)||($belob))) {
		...
		if (($d_type=="F")&&(strlen($debet)==1)&&($debet!='0')) {
			...
			$query = db_select("select kontonr from kontoplan where genvej='$debet'",__FILE__ . " linje " . __LINE__);
		...
		if ((!$fejl)&&($k_type=="F")&&(strlen($kredit)==1)&&($kredit!='0')) {
			...
			$query = db_select("select kontonr from kontoplan where genvej='$kredit'",__FILE__ . " linje " . __LINE__);
			if ($row = db_fetch_array($query)) $kredit=$row['kontonr'];
			else {
...
{ 
	...
	if (!$fejl) {	
		if ($antal=="=") db_modify("insert into kassekladde (bilag, kladde_id, transdate, beskrivelse, d_type, debet, k_type, kredit, faktura, amount, afd, ansat, projekt, valuta) values ('$bilag', '$kladde_id', '$date', '$beskrivelse', '$d_type', '$debet', '$k_type', '$kredit', '$faktura', '$amount', '$afd', '$ansat_id', '$projekt', '$valutakode')",__FILE__ . " linje " . __LINE__);
		else ($antal=$antal*1);
		for ($x=1;$x<=$antal;$x++) {
