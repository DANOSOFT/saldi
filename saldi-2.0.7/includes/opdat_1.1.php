<?php
// -------------------------- includes/opdat_1.1.php-------lap 2.0.7-----2009-05-20----------
// LICENS
//
...
#########################################################################################
	if ($lap_nr<4) {
		...
		if ($r['version']<'1.1.4') {
			...
			db_modify("ALTER TABLE tabelinfo ADD art varchar");
		...
		db_modify("ALTER TABLE tmpkassekl ADD valutakurs varchar");
...
#########################################################################################
	if ($lap_nr<6) {
		transaktion("begin");
		db_modify("ALTER TABLE openpost ADD valuta varchar");
		db_modify("ALTER TABLE openpost ADD valutakurs numeric");
		db_modify("UPDATE grupper set box1 = '1.1.6' where art = 'VE'");
		transaktion("commit");
		include("../includes/connect.php");
		$r=db_fetch_array(db_select("select * from regnskab where id=1"));
		if ($r['version']<'1.1.6') {
			transaktion("begin");
			db_modify("UPDATE regnskab set version='1.1.6' where id = 1");
			transaktion("commit");
			include("../includes/online.php");
		}
	}
