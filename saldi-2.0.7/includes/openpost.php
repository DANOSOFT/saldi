<?php
// //---------------------includes/openpost.php ----- lap 2.0.7 ---- 2009.05.05 ---------------------------
// LICENS
//
...
function openpost($regnaar, $maaned_fra, $maaned_til, $konto_fra, $konto_til, $rapportart, $art) 
{
	...
	{
		var agree=confirm("Bekr&aelig;ft venligst");
	...
	print "<td width=\"10%\" $top_bund><font face=\"Helvetica, Arial, sans-serif\" color=\"#000066\"><a accesskey=l href=\"rapport.php?rapportart=openpost&regnaar=$regnaar&maaned_fra=$maaned_fra&maaned_til=$maaned_til&konto_fra=$konto_fra&konto_til=$konto_til\">Luk</a></td>";
...
#	echo "$maaned_fra -$maaned_til<br>";
	...
	$x=0;
	$q = db_select("select * from adresser where kontonr>='$konto_fra' and kontonr<='$konto_til' and art = '$art' order by firmanavn",__FILE__ . " linje " . __LINE__);
	...
	for ($x=1; $x<=$kontoantal; $x++) {
		...
		$forfalden_plus90=0;
		$y=0;
		if ($regnaar) $q=db_select("select * from openpost where konto_id=$id[$x] and transdate <= '$regnslut' and udlignet!='1'",__FILE__ . " linje " . __LINE__);
		else $q=db_select("select * from openpost where konto_id=$id[$x] and udlignet!='1'",__FILE__ . " linje " . __LINE__);
		while ($r=db_fetch_array($q)) {
			if ($r['udlignet']!=1){
				$oid=$r['id'];
				...
				if (!$r['kladde_id']&&$r['refnr']) { #så er aaben posten genereret direkte fra en ordre.
					$r2=db_fetch_array(db_select("select betalingsbet, betalingsdage from ordrer where id='$r[refnr]'",__FILE__ . " linje " . __LINE__));
					$betalingsbet[$x]=trim($r2['betalingsbet']);
					$betalingsdage[$x]=trim($r2['betalingsdage']);
				} #elseif () 
				$amount=$amount*$valutakurs/100;
#				if (!$forfaldsdag) $forfaldsdag=usdate(forfaldsdag($transdate, $betalingsbet[$x], $betalingsdage[$x]));
### nedenstående er indført grundet en fejl i 2.0.3 som skrev forkert forfaldsdato i openpost				
...
################				
		...
		if (($y>0.01)||($udlignet=="0"))	{	
			...
			print "<td align=right><span style='color: rgb(255, 0, 0);'>$tmp</td>";
	...
	if ($art=='D' && db_fetch_array(db_select("select * from ordrer where art LIKE 'R%'",__FILE__ . " linje " . __LINE__))) {
		...
		print "<tr><td colspan=10 align=center> Rykkeroversigt</td></tr>\n";
		print "<tr><td>Ikke bogf&oslash;rte</td><td colspan=9><hr></td></tr>\n";
		...
		print "<tr><td colspan=10><hr></td></tr>\n";
 		...
 		while ($taeller <2) {  
			...
			print "<form name=$formnavn action=rapport.php method=post>";
 			$q1 = db_select("select * from ordrer where art LIKE 'R%' and status $status order by ordrenr desc",__FILE__ . " linje " . __LINE__);
			$x=0;
...
#					$sum[$x]=$sum[$x]+$r2['pris'];
					if ($r2['enhed']) {
						$q3 = db_select("select amount, valutakurs from openpost where id = '$r2[enhed]'",__FILE__ . " linje " . __LINE__);
...
# echo "$belob<br>";				
				print "<td>$r1[firmanavn]</td><td colspan=2 align=center>$r1[ordredate]</td><td align=center>$rykkernr</td><td colspan=3 align=right>$belob</td>";	
				$tmp = $rykkernr+1;
				$tmp = "R".$tmp;
				if (!db_fetch_array(db_select("select * from ordrer where art = '$tmp' and ordrenr = '$r1[ordrenr]'",__FILE__ . " linje " . __LINE__))) print "<td align=center><input type=checkbox name=rykkerbox[$x]>";
...
#			print "<form name=$formnavn action=rapport.php method=post>";
			...
			print "<tr><td colspan=10><hr></td></tr>\n";
			if ($x) {
			if ($taeller==1) print "<tr><td colspan=10 align=center><input type=submit value=\"  Slet  \" name=\"submit\" onClick=\"return confirmSubmit()\">&nbsp;";
			else print "<tr><td colspan=10 align=center>";
			print "<input type=submit value=\"Udskriv\" name=\"submit\">";
			if ($taeller>1) print " &nbsp;<input type=submit value=\"Ny rykker\" name=\"submit\">";
			if ($taeller==1) print " &nbsp;<input type=submit value=\"Bogf&oslash;r\" name=\"submit\" onClick=\"return confirmSubmit()\"></td></tr>\n";
			else print "</td></tr>\n";
			}
