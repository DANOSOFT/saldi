<?php
// -------------------------------------------- systemdata/tekster.php ------ patch 2.0.4 -----2008.12.11---------------
// LICENS
//
...
$tekstantal=$x;
for($x=1; $x<=$tekstantal; $x++){
	if (!$r=db_fetch_array(db_select("select id from tekster where sprog_id=$sprog_id and tekst_id=$tekst_id[$x] order by tekst",__FILE__ . " linje " . __LINE__))){
...
$q = db_select("select * from tekster where sprog_id=$sprog_id order by tekst",__FILE__ . " linje " . __LINE__);
while ($r = db_fetch_array($q)) {
	$x++;
	$id[$x]=$r['id'];
	$tekst_id[$x]=$r['tekst_id'];
...
print "<form name=\"tekster\" action=\"tekster.php?sprog_id=$sprog_id\" method=\"post\">";
print "<input type=hidden name=tekstantal value=\"$tekstantal\">";

print "<table border=1><tbody>";
print "<tr><td width=400>".findtekst(31,$sprog_id)."</td><td title=\"".findtekst(33,$sprog_id)."\">".findtekst(32,$sprog_id)."</td>";
for($x=1; $x<=$tekstantal; $x++){
	print "<input type=hidden name=id[$x] value=\"$id[$x]\">";
	print "<input type=hidden name=tekst[$x] value=\"$tekst[$x]\">";
	print "<tr><td>$tekst[$x]</td><td><input type=text size=100 name=ny_tekst[$x] value='$tekst[$x]'></td>";
}
