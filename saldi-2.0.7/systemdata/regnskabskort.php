<?php
// ----systemdata/regnskabskort.php-----lap 2.0.7----2009-05-15----------
// LICENS
//
...
Print "<table width=100% align=center border=0 cellspacing=4 cellpadding=0><tbody>"; ##############table 2b start
...
print "<td width=\"10%\" align=center><div class=\"top_bund\"><a href=\"javascript:confirmClose('regnskabskort.php','$tekst')\" accesskey=N>Ny</a></div></td>\n";
Print "</tbody></table>"; #####################################################table 2b slut.
Print "</td></tr>\n";
Print "<td align = center valign = center>";
Print "<table cellpadding=1 cellspacing=1 border=1 width=50%><tbody>"; ############################	##table 3b start
...
#		$primotal=$_POST['primotal'];
	...
	if ((($id!=0)||(!db_fetch_array(db_select("select id from grupper where kodenr = '$kodenr' and art = 'RA'",__FILE__ . " linje " . __LINE__))))&&(($startmd)&&($slutmd)&&($startdato)&&($slutdato)&&($startaar)&&($slutaar)&&($beskrivelse))) {
		transaktion("begin");
		if ($id==0){
			db_modify("insert into grupper (beskrivelse, kodenr, kode, art, box1, box2, box3, box4, box5) values ('$beskrivelse', '$kodenr', '$kode', 'RA', '$startmd','$startaar', '$slutmd', '$slutaar','$aaben')",__FILE__ . " linje " . __LINE__);
		...
		if (($id>0)&&($kodenr>0)) {
			db_modify("update grupper set beskrivelse = '$beskrivelse', kodenr = '$kodenr', kode = '$kode', box1 = '$startmd', box2 = '$startaar', box3 = '$slutmd', box4 = '$slutaar', box5 = '$aaben' where id = '$id'",__FILE__ . " linje " . __LINE__);
			if ($kodenr==1){
				for ($x=1; $x<=$kontoantal; $x++) {
					if ($saldo[$x]&&$overfor_til[$x]) db_modify ("update kontoplan set primo=primo+$saldo[$x] where kontonr='$kontonr[$x]' and regnskabsaar=$kodenr",__FILE__ . " linje " . __LINE__);
				}
			}
			else {
				...
				if($row = db_fetch_array($query)) {
					...
					for ($x=1; $x<=$kontoantal; $x++) {
						db_modify ("update kontoplan set overfor_til=$overfor_til[$x] where kontonr='$kontonr[$x]' and regnskabsaar=$kodenr-1",__FILE__ . " linje " . __LINE__);
					...
					while ($row = db_fetch_array($query)) {
						...
						db_modify("insert into kontoplan(kontonr, beskrivelse, kontotype, moms, fra_kto, til_kto, lukket, primo, regnskabsaar, overfor_til)values('$row[kontonr]',  '$beskrivelse', '$row[kontotype]', '$row[moms]', '$row[fra_kto]', '$row[til_kto]', '$row[lukket]', '$belob', '$kodenr', '$row[overfor_til]')",__FILE__ . " linje " . __LINE__);
					}
				}	
			}
		}
...
#	print "<BODY onLoad=\"javascript:velkommen=window.open('velkommen.html','velkommen','".$jsvars."';) velkommen.focus();\";"
	...
	$x=0;
	$query = db_select("select * from grupper where art = 'RA' order by kodenr desc",__FILE__ . " linje " . __LINE__);
	if ($row = db_fetch_array($query)) {
...
function aar_1($id, $kodenr, $beskrivelse, $startmd, $startaar, $slutmd, $slutaar, $aaben)
{
	...
	if ($id){print "<tr><td colspan=4 align = center><big><b>Ret 1. regnskabs&aring;r: $beskrivelse</big></td></tr>\n";}
		...
		print "<tr><td colspan=4 align = center><big><b>Velkommen til som SALDI bruger</b></big><br>
			...
			Systemet er allerede sat op, s&aring; det passer til de fleste virksomheder. Og hvis dit 1. regnskabs&aring;r<br>
			passer med perioden 1. januar $startaar til 31. december $slutaar skal du blot trykke p&aring; knappen [Gem], og<br>
			dit regnskab er klar til brug.<br><br>
	...
	print "<td align=center><table heigth=100% border=1><tbody><td>";  #################################table 6d start	print "<tr><td align=center>1. faktnr</td><td align=center>1. modt. nr.</td><tr>";
	print "<tr><td align=center>1.&nbsp;faktnummer</td><td align=center>1.&nbsp;modtagelsesnummer</td><tr>";  
	print "<tr><td align=center><input type=text style=\"text-align:right\" size=4 name=fakt value=$fakt onchange=\"javascript:docChange = true;\"></td>";
	...
	print "<td><input type=submit accesskey=\"g\" value=\"Gem\" name=\"submit\" onclick=\"javascript:docChange = false;\"></td></tr>\n";
	...
	while ($row = db_fetch_array($query)) {
		...
		print "<td width=10 align=right><input type=text style=\"text-align:right\" size=10 name=debet[$y] value=$debet[$y] onchange=\"javascript:docChange = true;\"></td>";
		print "<td width=10 align=right><input type=text style=\"text-align:right\" size=10 name=kredit[$y] value=$kredit[$y] onchange=\"javascript:docChange = true;\"></td></tr>\n";
	}
...
function aar_x($id, $kodenr, $beskrivelse, $startmd, $startaar, $slutmd, $slutaar, $aaben)
{
	...
	$query = db_select("select * from kontoplan where kontotype='S' and regnskabsaar=$tmp order by kontonr",__FILE__ . " linje " . __LINE__);
	$y=0;
	while ($row = db_fetch_array($query)) {
		$y++;
		$ny_primo[$y]=0;
    for ($x=1; $x<=$kontoantal; $x++) {
			if ($kontonr[$x]==$row['kontonr']) $ny_primo[$y]=$primo[$x]; 
		}
		$belob=0;
		$belob=$row['primo'];
		print "<tr><input type=hidden name=kontonr[$y] value=$row[kontonr]>";
