<?php
// -----------------systemdata/syssetup.php----lap 2.0.7---2009-05-15----
// LICENS
//
...
if ($_POST){ 
	...
	for($x=0; $x<=$antal; $x++) {
		...
		if (($art[$x]=='SM')||($art[$x]=='KM')||($art[$x]=='EM')||($art[$x]=='VK')) $box2[$x]=usdecimal($box2[$x]); 
		...
		if (($kode[$x])||($id[$x])) {
			...
			elseif (($id[$x]==0)&&($kode[$x])&&($kodenr[$x])&&($art[$x])) {
				...
				elseif ($art[$x]!='PV') {
					db_modify("insert into grupper (beskrivelse, kodenr, kode, art, box1, box2, box3, box4, box5, box6, box7, box8, box9, box10) values ('$beskrivelse[$x]', '$kodenr[$x]', '$kode[$x]', '$art[$x]', '$box1[$x]','$box2[$x]', '$box3[$x]', '$box4[$x]','$box5[$x]', '$box6[$x]','$box7[$x]', '$box8[$x]', '$box9[$x]', '$box10[$x]')",__FILE__ . " linje " . __LINE__);
					if ($art[$x]=='LG'){
			...
			elseif ((($id[$x]>0)&&($kodenr[$x])&&($kodenr[$x]!='-'))&&($art[$x])){ # &&(($box1[$x])||($box3[$x])||($art[$x]=='VK')))
			  if ($art[$x]=='PV') {db_modify("update grupper set kodenr='$kodenr[$x]', box1 = '$box1[$x]', box2 = '$box2[$x]', box3 = '$box3[$x]' WHERE id = '$id[$x]'",__FILE__ . " linje " . __LINE__);}
				else {
					db_modify("update grupper set kodenr='$kodenr[$x]', beskrivelse = '$beskrivelse[$x]', kode = '$kode[$x]', box1 = '$box1[$x]', box2 = '$box2[$x]', box3 = '$box3[$x]', box4 = '$box4[$x]', box5 = '$box5[$x]', box6 = '$box6[$x]', box7 = '$box7[$x]', box8 = '$box8[$x]', box9 = '$box9[$x]', box10 = '$box10[$x]' WHERE id = '$id[$x]'",__FILE__ . " linje " . __LINE__);
				}
				...
				if ($art[$x]=='LG') { #LagerGrupper
				...
				} elseif ($art[$x]=='SM'||$art[$x]=='KM'||$art[$x]=='EM') {
					$r1=db_fetch_array(db_SELECT("SELECT kodenr FROM grupper WHERE id=$id[$x]",__FILE__ . " linje " . __LINE__));
					...
					else echo db_modify("delete FROM grupper WHERE id = '$id[$x]'",__FILE__ . " linje " . __LINE__);
...
if ($nopdat!=1){
	$x=0;
	if ($db_type=='mysql') $query = db_SELECT("SELECT * FROM grupper ORDER BY CAST(kodenr AS SIGNED)",__FILE__ . " linje " . __LINE__);
	else $query = db_SELECT("SELECT * FROM grupper order by to_number(textcat('0', kodenr), text(99999999))",__FILE__ . " linje " . __LINE__);
	while ($row = db_fetch_array($query)){
		$x++;
		$id[$x]=$row['id'];
		$beskrivelse[$x]=htmlentities(stripslashes($row['beskrivelse']),ENT_COMPAT,$charset);
		$kodenr[$x]=$row['kodenr']*1;
		$kode[$x]=$row['kode'];
		$art[$x]=$row['art'];
		$box1[$x]=$row['box1'];
		$box2[$x]=$row['box2'];
...
if ($valg=='moms'){
	...
	$spantekst2='Det nummer i kontoplanen som k&oslash;momsen skal konteres p&aring;.';
	...
	print "<tr><td></td><td colspan=3><b><span title='$spantekst5'>Erhvervelsesmoms (k&oslash;b i udlandet)</td></tr>\n";
	...
	print "<tr><td><br></td></tr>\n";
	print "<tr><td></td><td colspan=3><b>Momsrapport (konti som skal indg&aring; i momsrapport)</td></tr>\n";
	print "<tr><td></td><td>Nr.</td><td align=center><span title='$spantekst1'>Beskrivelse</span></td><td align=center><span title='F&oslash;rste kontonummer som skal indg&aring; i rapporten'>Fra</span></td><td align=center><span title='Sidste kontonummer som skal indg&aring; i rapporten'>Til</span></td></tr>\n";
	$y=udskriv('MR', $x, $y, $art, $id, "R", $kodenr, $beskrivelse, $box1, '6', $box2, '6', '-', '6', '-', '6', '-', '6', '-', '6', '-', '6', '-', '6', '-', '6', '-', '6', '-', '6', '-', '6', '-', '6');
}
elseif($valg=='debitor'){
	...
	print "<td alicn=center><span title = \"$spantilte\">Provision</td></tr>\n";
	$y=udskriv('DG', $x, $y, $art, $id, 'D', $kodenr, $beskrivelse, $box1, '6', $box2, '6', $box3, '10', $box4, '10', $box5, '6', $box6, '4', $box7, '4', '-', '6', '-', '6', '-', '6', '-', '6', '-', '6', '-', '6');
	print "<tr><td><br></td></tr>\n";
...
elseif($valg=='varer'){
	$q = db_SELECT("select id from grupper where art = 'DIV' and kodenr = '2' and box4='on'");
	if (db_fetch_array($q)){
		print "<tr><td></td><td colspan=10 align=center><b>Varegrupper</td></tr><tr><td colspan=13><hr></td></tr>\n";
		print "<tr><td align=center></td><td></td><td></td><td align=center>Lager-</td><td align=center>Lager-</td><td align=center>Vare-</td><td align=center>Vare-</td><td align=center>Lager-</td><td>Max</td><td align=center>Moms-</td><td align=center>Lager-</td><td align=center>Opera-</td></tr>\n";
		print "<tr><td></td><td>Nr.</td><td align=center>Beskrivelse</td><td align=center>tilgang</td><td align=center>tr&aelig;k</td><td align=center>k&oslash;b</td><td align=center>salg</td><td align=center>regulering</td><td align=center>rabat %</td><td align=center>fri</td><td align=center>f&oslash;rt</td><td align=center>tion</td></tr>\n";
		$y=udskriv('VG', $x, $y, $art, $id, '&nbsp;', $kodenr, $beskrivelse, $box1, '6', $box2, '6', $box3, '6', $box4, '6', $box5, '6', $box6, '2', $box7, 'checkbox', $box8, 'checkbox', $box10, 'checkbox', '-', 2);
	} else {
		print "<tr><td colspan=13 align=center><b>Varegrupper</td></tr><tr><td colspan=13><hr></td></tr>\n";
		print "<tr><td align=center></td><td></td><td></td><td align=center>Lager-</td><td align=center>Lager-</td><td align=center>Vare-</td><td align=center>Vare-</td><td align=center>Lager-</td><td>Max</td><td align=center>Moms-</td><td align=center>Lager-</td><td align=center>Batch-</td><td align=center>Opera-</td></tr>\n";
		print "<tr><td></td><td>Nr.</td><td align=center>Beskrivelse</td><td align=center>tilgang</td><td align=center>tr&aelig;k</td><td align=center>k&oslash;b</td><td align=center>salg</td><td align=center>regulering</td><td align=center>rabat %</td><td align=center>fri</td><td align=center>f&oslash;rt</td><td align=center>kontrol</td><td align=center>tion</td></tr>\n";
		$y=udskriv('VG', $x, $y, $art, $id, '&nbsp;', $kodenr, $beskrivelse, $box1, '6', $box2, '6', $box3, '6', $box4, '6', $box5, '6', $box6, '2', $box7, 'checkbox', $box8, 'checkbox', $box9, 'checkbox', $box10, 'checkbox');
	}
}
...
##########################################################################################################################
function udskriv($a, $x, $y, $art, $id, $k, $kodenr, $beskrivelse, $box1, $b1, $box2, $b2, $box3, $b3, $box4, $b4, $box5, $b5, $box6, $b6, $box7, $b7, $box8, $b8, $box9, $b9, $box10, $b10){
global $valg;
global $charset;
	...
	for ($i=0; $i<=$x; $i++) {
		...
		if (($art[$i]=='SM' || $art[$i]=='KM' || $art[$i]=='EM' || $art[$i]=='VK') && $box2!='-') $box2[$i]=dkdecimal($box2[$i]);
		...
		if ($valg=='projekter'||$art[$i]=='PRJ') $size=6;
		else $size=2;
		if ($art[$i]==$a){
			print "<tr>";
			print "<td>$k</td>";
			print "<td><input type=text style=\"text-align:right\" size=$size name=kodenr[$i] value=$kodenr[$i]></td>";
			print "<td><input type=text size=40 name=beskrivelse[$i] value=\"$beskrivelse[$i]\"></td>";
			if (($box1!="-") &&($b1!="checkbox")){print "<td><input type=text style=\"text-align:right\" size=$b1 name=box1[$i] value='$box1[$i]'></td>";}
			elseif($b1=="checkbox") {
				if (strstr($box1[$i],'on')){print "<td><input type=checkbox name=box1[$i] checked></td>";}
				else {print "<td><input type=checkbox name=box1[$i]></td>";}
			}
			...
			if (($box2!="-") &&($b2!="checkbox")){print "<td><input type=text style=\"text-align:right\" size=$b2 name=box2[$i] value='$box2[$i]'></td>";}
			elseif($b2=="checkbox"){
				if (strstr($box2[$i],'on')){print "<td><input type=checkbox name=box2[$i] checked></td>";}
				else {print "<td><input type=checkbox name=box2[$i]></td>";}
			}
			print "<input type = hidden name=id[$i] value='$id[$i]'><input type = hidden name=art[$i] value='$art[$i]'><input type = hidden name=kode[$i] value='$k'>";
			if (($box3!="-") &&($b3!="checkbox")){print "<td><input type=text style=\"text-align:right\" size=$b3 name=box3[$i] value='$box3[$i]'></td>";}
			elseif($b3=="checkbox"){
				if (strstr($box3[$i],'on')){print "<td><input type=checkbox name=box3[$i] checked></td>";}
				else {print "<td><input type=checkbox name=box3[$i]></td>";}
			}
			print "<input type = hidden name=id[$i] value='$id[$i]'><input type = hidden name=art[$i] value='$art[$i]'><input type = hidden name=kode[$i] value='$k'>";
			if (($box4!="-") &&($b4!="checkbox")){print "<td><input type=text style=\"text-align:right\" size=$b4 name=box4[$i] value='$box4[$i]'></td>";}
			elseif($b4=="checkbox"){
				if (strstr($box4[$i],'on')){print "<td><input type=checkbox name=box4[$i] checked></td>";}
				else {print "<td><input type=checkbox name=box4[$i]></td>";}
			}
			print "<input type = hidden name=id[$i] value='$id[$i]'><input type = hidden name=art[$i] value='$art[$i]'><input type = hidden name=kode[$i] value='$k'>";
			if ($box5!="-") {print "<td><input type=text style=\"text-align:right\" size=$b5 name=box5[$i] value='$box5[$i]'></td>";}
			if ($box6!="-") {print "<td><input type=text style=\"text-align:right\" size=$b6 name=box6[$i] value='$box6[$i]'></td>";}
			if (($box7!="-")&&($b7!="checkbox")) {print "<td><input type=text style=\"text-align:right\" size=$b7 name=box7[$i] value='$box7[$i]'></td>";}
			elseif($b7=="checkbox"){
				if (strstr($box7[$i],'on')){print "<td align=center><input type=checkbox name=box7[$i] checked></td>";}
				else {print "<td align=center><input type=checkbox name=box7[$i]></td>";}
			}
			if (($box8!="-")&&($b8!="checkbox")) {print "<td><input type=text style=\"text-align:right\" size=$b8 name=box8[$i] value='$box8[$i]'></td>";}
			elseif($b8=="checkbox"){
				if (strstr($box8[$i],'on')){print "<td align=center><input type=checkbox name=box8[$i] checked></td>";}
				else {print "<td align=center><input type=checkbox name=box8[$i]></td>";}
			}
			if (($box9!="-")&&($b9!="checkbox")) {print "<td><input type=text style=\"text-align:right\" size=$b9 name=box9[$i] value='$box9[$i]'></td>";}
			elseif($b9=="checkbox"){
				if (strstr($box9[$i],'on')){print "<td align=center><input type=checkbox name=box9[$i] checked></td>";}
				else {print "<td align=center><input type=checkbox name=box9[$i]></td>";}
			}
			if (($box10!="-")&&($b10!="checkbox")) {print "<td><input type=text style=\"text-align:right\" size=$b10 name=box10[$i] value='$box10[$i]'></td>";}
			elseif($b10=="checkbox"){
				if (strstr($box10[$i],'on')){print "<td align=center><input type=checkbox name=box10[$i] checked></td>";}
				else {print "<td align=center><input type=checkbox name=box10[$i]></td>";}
			}
			print "</tr>\n";
			print "<input type = hidden name=id[$i] value='$id[$i]'><input type = hidden name=art[$i] value='$art[$i]'><input type = hidden name=kode[$i] value='$k'>";
		}
	...
	if (($k!='R')||(!$momsrapport)) {
		...
		print "<tr>";
		print "<td>$k</td>";			
		print "<td><input type=text size=$size name=kodenr[$y]></td>";
		print "<td><input type=text size=40 name=beskrivelse[$y]></td>";
		if (($box1!="-")&&($b1!="checkbox")) {print "<td><input type=text style=\"text-align:right\" size=$b1 name=box1[$y]></td>";}
		elseif($b1=="checkbox") {print "<td><input type=checkbox name=box1[$y]></td>";}
		print "<input type = hidden name=id[$y] value='0'><input type = hidden name=kode[$y] value='$k'><input type = hidden name=art[$y] value=$a>";
		if (($box2!="-")&&($b2!="checkbox")) {print "<td><input type=text style=\"text-align:right\" size=$b2 name=box2[$y]></td>";}
		elseif($b2=="checkbox") {print "<td><input type=checkbox name=box2[$y]></td>";}
		print "<input type = hidden name=id[$y] value='0'><input type = hidden name=kode[$y] value='$k'><input type = hidden name=art[$y] value=$a>";
		if (($box3!="-")&&($b3!="checkbox")) {print "<td><input type=text style=\"text-align:right\" size=$b3 name=box3[$y]></td>";}
		elseif($b3=="checkbox") {print "<td><input type=checkbox name=box3[$y]></td>";}
		print "<input type = hidden name=id[$y] value='0'><input type = hidden name=kode[$y] value='$k'><input type = hidden name=art[$y] value=$a>";
		if (($box4!="-")&&($b4!="checkbox")) {print "<td><input type=text style=\"text-align:right\" size=$b4 name=box4[$y]></td>";}
		elseif($b4=="checkbox") {print "<td><input type=checkbox name=box4[$y]></td>";}
		print "<input type = hidden name=id[$y] value='0'><input type = hidden name=kode[$y] value='$k'><input type = hidden name=art[$y] value=$a>";
		if ($box5!="-") {print "<td><input type=text style=\"text-align:right\" size=$b5 name=box5[$y]></td>";}
		if ($box6!="-") {print "<td><input type=text style=\"text-align:right\" size=$b6 name=box6[$y]></td>";}
		if (($box7!="-")&&($b7!="checkbox")) {print "<td><input type=text style=\"text-align:right\" size=$b7 name=box7[$y]></td>";}
		elseif($b7=="checkbox") {print "<td align=center><input type=checkbox name=box7[$y]></td>";}
		if (($box8!="-")&&($b8!="checkbox")) {print "<td align=center><input type=text style=\"text-align:right\" size=$b8 name=box8[$y]></td>";}
		elseif($b8=="checkbox") {print "<td align=center><input type=checkbox name=box8[$y]></td>";}
		if (($box9!="-")&&($b9!="checkbox")) {print "<td align=center><input type=text style=\"text-align:right\" size=$b9 name=box9[$y]></td>";}
		elseif($b9=="checkbox") {print "<td align=center><input type=checkbox name=box9[$y]></td>";}
		if (($box10!="-")&&($b10!="checkbox")) {print "<td align=center><input type=text style=\"text-align:right\" size=$b10 name=box10[$y]></td>";}
		elseif($b10=="checkbox") {print "<td align=center><input type=checkbox name=box10[$y]></td>";}
...
function nytaar($beskrivelse, $kodenr, $kode, $art, $box1, $box3, $box3, $box4, $box5, $box6)
{
	...
	{
		print "<tr><td> Overf&oslash;r &aring;bningsbalance</td><td><input type=checkbox name=aabn_bal></td></tr>\n";
	}
