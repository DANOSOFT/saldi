...
@session_start();
$s_id=session_id();
// ------debitor/mail_kontoudtog.php-------patch 2.0.7------2009-05-14--------
// LICENS
//
...
for($x=1; $x<=$kontoantal; $x++) {
	...
	print "<tr><td colspan=3>$row[firmanavn]</td></tr>";
	print "<tr><td colspan=3> $row[addr1]</td></tr>";
	print "<tr><td colspan=3> $row[addr2]</td><td colspan=3 align=right>Kontonr</td><td align=right> $row[kontonr]</td></tr>";
	print "<tr><td colspan=3> $row[postnr] $row[bynavn]</td><td colspan=3 align=right> Dato</td><td align=right> ".date('d-m-Y')."</td></tr>";
	...
	while ($row = db_fetch_array($query)) {
		 if ($row[transdate]<$fromdate[$x]) {
			...
			if ($primoprint==0) {
				...
				print "<tr bgcolor=\"$linjebg\"><td></td><td><td></td></td><td align=center> Primosaldo</td><td></td><td></td><td></td><td align=right> $tmp</td></tr>";
				$primoprint=1;
			}
			...
			if ($row[amount] > 0) {
				print "<td>  $stil".dkdato($forfaldsdag)."</td><td align=right>$stil $tmp</td><td></td>";
	...
	for($x=1; $x<=$kontoantal; $x++) {
		if (($konto_id[$x])&&($email[$x])&&($fra[$x])&&($til[$x])&&(strpos($email[$x], '@'))) {	
			...
			$row = db_fetch_array(db_select("select * from adresser where id=$konto_id[$x]"));
		 	$mailtext .= "<tr><td colspan=3>$row[firmanavn]</td></tr>\n";
			$mailtext .= "<tr><td colspan=3> $row[addr1]</td></tr>\n";
			$mailtext .= "<tr><td colspan=3> $row[addr2]</td><td colspan=3 align=right>Kontonr</td><td align=right> $row[kontonr]</td></tr>\n";
			$mailtext .= "<tr><td colspan=3> $row[postnr] $row[bynavn]</td><td colspan=3 align=right> Dato</td><td align=right> ".date('d-m-Y')."</td></tr>\n";
			...
			while ($row = db_fetch_array($query)) {
				 if ($row[transdate]<$fromdate[$x]) {
					...
					if ($primoprint==0) {
						$tmp=dkdecimal($kontosum);
						$linjebg=$bgcolor5; $color='#000000';
						$mailtext .= "<tr bgcolor=\"$linjebg\"><td></td><td><td></td></td><td align=center> Primosaldo</td><td></td><td></td><td></td><td align=right> $tmp</td></tr>\n";
						$primoprint=1;
					}
				    	if ($linjebg!=$bgcolor){$linjebg=$bgcolor; $color='#000000';}
					elseif ($linjebg!=$bgcolor5){$linjebg=$bgcolor5; $color='#000000';}
					...
					if ($row[amount] > 0) {
						$mailtext .= "<td>  $stil".dkdato($forfaldsdag)."</td><td align=right>$stil $tmp</td><td></td>\n";
						$forfaldsum=$forfaldsum+$row[amount];
					}
