<?php
// ------------- debitor/historikkort.php ----- (modul nr 6)------ lap 2.0.7 ---2009-05-14-------
// LICENS
//
...
else print "<td width=\"10%\" align=center><div class=\"top_bund\"><a href=\"javascript:confirmClose('$returside?returside=$returside&id=$ordre_id&fokus=$fokus&konto_id=$id','$tekst')\" accesskey=L>Luk</a></div></td>\n";
...
print "<tr><td><a href=debitorkort.php?id=$id&returside=historikkort.php>$firmanavn</a></td><td></td><td></td><td> Oprettet</td><td>";
...
print "</td><td></td><td> N&aelig;ste kontakt</td><td> $kontaktes</td></tr>\n";
...
print "<tr><td width=10%><table border=0><tbody>";#tabel3b start;
if ($historik_id) {
...
$q = db_select("select id, navn from ansatte where konto_id = $egen_id and lukket != 'on' and id != $ansat_id");
...
if ($historik_id) {
	...
	print "<tr><td colspan=2 align=right><input type=submit accesskey=\"g\" value=\"Gem/opdat&eacute;r\" name=\"submit\" onclick=\"javascript:docChange = false;\"></td></tr>\n";
} else {
	print "<tr><td colspan=2 align=right><input type=submit accesskey=\"g\" value=\"Gem\" name=\"submit\" onclick=\"javascript:docChange = false;\"></td></tr>\n";
}
...
while ($r = db_fetch_array($q)){
	...
	$kontakt_id=$r[kontakt_id]*1;
	$r2 = db_fetch_array(db_select("select id, navn from ansatte where id = $kontakt_id"));
	$notat=str_replace(" ","&nbsp;",htmlentities(stripslashes($r['notat'])));
	$notat=str_replace("\n","<br>",$notat);
	print "<tr><td>$r1[navn] =&gt; $r2[navn]<br>$kontaktet &nbsp; $kontaktes<br>";
#	if ($r[notedate]==date("Y-m-d")) 
	...
	print "</td><td colspan='5' style='vertical-align:top' width=90%>$notat</td><td><br></td></tr>";
	print "<tr><td colspan=6><hr></td></tr>";
}
?>
</tbody>
