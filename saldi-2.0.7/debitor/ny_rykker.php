...
// -----------------------debitor/ny_rykker.php-----lap 2.0.7--2009.04.21-------
// LICENS
//
...
if ($konto_id[0]=="alle") {
	...
	for ($x=1;$x<=$konto_antal;$x++) { # udligner alle åbne poster og sletter tilhørende åbne rykkere hvor udestaaende er 0;
		if ($udlign[$x]) {
			...
			$q=db_select("select id from ordrer where konto_id='$konto_id[$x]' and art LIKE 'R%' and status < '3'",__FILE__ . " linje " . __LINE__);
			while ($r=db_fetch_array($q)) {
	...
	$q=db_select("select id, konto_id from ordrer where art LIKE 'R%' and status < '3'",__FILE__ . " linje " . __LINE__);
	while ($r=db_fetch_array($q)) {
		if (!in_array($r['konto_id'],$konto_id)) {
			db_modify("delete from ordrelinjer where ordre_id='$r[id]'",__FILE__ . " linje " . __LINE__); 
			db_modify("delete from ordrer where id='$r[id]'",__FILE__ . " linje " . __LINE__); 
	...
	$q=db_select("select id,konto_id from ordrer where art = 'R2' and status < '3' and ordredate < '$rykkerfrist3'",__FILE__ . " linje " . __LINE__);
	while ($r=db_fetch_array($q)) {
		$x++;
		bogfor_rykker($r['id']);
		$rykker_id[$x]=$r['id'];
	...
	if (($konto_id[$i])||($rykker_id[$i])) {
		if (!$rykker_id[$i]) {
			...
			if ($valuta && $valuta!='DKK') {
				if ($r2= db_fetch_array(db_select("select valuta.kurs from valuta, grupper where grupper.art='VK' and grupper.box1='$valuta' and valuta.gruppe=grupper.kodenr and valuta.valdate <= '$rykkerdate' order by valuta.valdate",__FILE__ . " linje " . __LINE__))) {
					$valutakurs=$r2['kurs'];
				} else {
			...
			$op_id=NULL;
			$x=0;
			$q=db_select("select * from openpost where udlignet = '0' and forfaldsdate<'$rykkerfrist1' and amount>'0' and konto_id='$konto_id[$i]'",__FILE__ . " linje " . __LINE__);
			while ($r=db_fetch_array($q)) {
				
				if (!db_fetch_array(db_select("select id from ordrelinjer where enhed = '$r[id]'",__FILE__ . " linje " . __LINE__)) &&
					 !db_fetch_array(db_select("select id from ordrer where id = '$r[refnr]' and art LIKE 'R%'",__FILE__ . " linje " . __LINE__))) {
					$x++;
			...
			if ($ryk_antal) {	
				db_modify("insert into ordrer (ordrenr, konto_id, kontonr, firmanavn, addr1, addr2, postnr, bynavn, land, betalingsdage, betalingsbet, cvrnr, ean, institution, notes, art, ordredate, levdate, fakturadate, momssats, hvem, tidspkt, ref, status, valuta) 
					values ('$r_ordrenr', '$konto_id[$i]', '$kontonr', '$firmanavn', '$addr1', '$addr2', '$postnr', '$bynavn', '$land', '$betalingsdage', '$betalingsbet', '$cvrnr', '$ean', '$institution', '$notes', 'R1', '$rykkerdate', '$rykkerdate', '$rykkerdate', '0', '$brugernavn', '$tidspkt', '$ref', '2', '$valuta')",__FILE__ . " linje " . __LINE__);
...
#					$q3 = db_select("select betalingsbet, betalingsdage from ordrer where fakturanr='$r2[faktnr]'",__FILE__ . " linje " . __LINE__);
...
##########################################################################################################################		
			...
			if ($rykkernr<=3) {
				...
				db_modify("insert into ordrer (ordrenr, konto_id, kontonr, firmanavn, addr1, addr2, postnr, bynavn, land, betalingsdage, betalingsbet, cvrnr, ean, institution, notes, art, ordredate, levdate, fakturadate, momssats, hvem, tidspkt, ref, status) 
					values ('$r[ordrenr]', '$r[konto_id]', '$r1[kontonr]', '$r1[firmanavn]', '$r1[addr1]', '$r1[addr2]', '$r1[postnr]', '$r1[bynavn]', '$r1[land]', '$ffdage3', 'Netto', '$r1[cvrnr]', '$r1[ean]', '$r1[institution]', '$r1[notes]', '$art', '$rykkerdate', '$rykkerdate', '$rykkerdate', '0', '$brugernavn', '$tidspkt', '$r[ref]', '2')",__FILE__ . " linje " . __LINE__);
				$r= db_fetch_array(db_select("select id from ordrer where ordrenr ='$r[ordrenr]' and art = '$art'",__FILE__ . " linje " . __LINE__)); #Henter ordrelinjer fra basisrykker.
				$ny_rykker_id[$i]=$r['id'];
				$pos=0;
...
#				$r3=db_fetch_array(db_select("select beskrivelse from ordrelinjer where ordre_id = '$rykker_id[$i]' and vare_id > 0",__FILE__ . " linje " . __LINE__));
				...
				$pos++;
				db_modify("insert into ordrelinjer (posnr,enhed, ordre_id, serienr, beskrivelse) values ('$pos','$r4[id]', '$ny_rykker_id[$i]','$r2[fakturadate]', '$r4[beskrivelse]')",__FILE__ . " linje " . __LINE__);
				}
...
function find_rente ($rentesats,$forfaldsdate,$amount) {
	list($f_year,$f_month,$f_day)=split("-",$forfaldsdate);
	$year=date("Y");
	$month=date("m");
	$day=date("d");
