...
// -----------debitor/historik.php-----lap 2.0.6-------2009-03-04-----------
// LICENS
//
...
while ($r0 = db_fetch_array($q0)) {
	...
	if (substr($vis_liste,$x,1)=='1' || ($vis_alt==1 && !$x)) {
		...
		while ($r1 = db_fetch_array($q1)) {
			...
			if ($popup) { 
				$href="onClick=\"javascript:historikkort=window.open('historikkort.php?id=$r1[id]&returside=../includes/luk.php','historikkort','".$jsvars."');historikkort.focus();\" onMouseOver=\"this.style.cursor = 'pointer'\";";
				print "<td $href><u>$r1[kontonr]</u><br></td>";
			} else print "<td><a href=historikkort.php?id=$r1[id]&returside=historik.php>$r1[kontonr]</a><br></td>";
			print "<td> <a href=historikkort.php?id=$r1[id]>".htmlentities(stripslashes($r1['firmanavn']))."<br></a></td>
				<td> <a href=historikkort.php?id=$r1[id]>".htmlentities(stripslashes($r1['postnr']))."<br></a></td>
				<td> <a href=historikkort.php?id=$r1[id]>".htmlentities(stripslashes($r1['bynavn']))."<br></a></td>
				<td> <a href=historikkort.php?id=$r1[id]>".htmlentities(stripslashes($r1['tlf']))."<br></a></td>
				<td> <a href=historikkort.php?id=$r1[id]>".dkdato($r1['oprettet'])."<br></a></td>
				<td> <a href=historikkort.php?id=$r1[id]>".dkdato($r1['kontaktet'])."<br></a></td>
				<td> <a href=historikkort.php?id=$r1[id]>".dkdato($r1['kontaktes'])."<br></a></td>";
			...
			if ($r1['kontaktet']) {
				...
				print "<td> <a href=historikkort.php?id=$r1[id]>".htmlentities(stripslashes($r2['initialer']))."<br></a></td>";
			} else print "<td><br></td>";
