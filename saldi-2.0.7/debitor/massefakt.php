<?php	
// ------------- debitor/massefakt.php ----- (modul nr 6)------ lap 2.0.7----2009-04-24-------
// LICENS
//
...
if (trim($returside)!='varer.php') print "<td width=\"10%\" $top_bund><a href=\"../includes/luk.php\" accesskey=L>Luk</a></td>";
else print "<td width=\"10%\" onClick=\"javascript=opener.location.reload();\" $top_bund> <a href=\"../includes/luk.php?\" accesskey=L>Luk</a></td>";
...
if ($tmp<$aarstart || $tmp>$aarslut) {
	print "<BODY onLoad=\"javascript:alert('Dags dato ligger ikke i aktivt regnskabsår')\">";
...
#	print "<tr><td colspan=2>Følgende ordrer kan ikke leveres</td></tr>";
...
if (!$drop && $fakturer && $delfakturer) {
	...
	while ($r=db_fetch_array($q)) {
		...
		if ($vare_id[$x]){
			db_modify("update ordrelinjer set antal=$antal[$x] where id = '$linje_id[$x]'",__FILE__ . " linje " . __LINE__); # id fra 1. søgning.
			if ($ny_antal[$x]) {
...
# Finder næste godkendte åbne ordre hvor alt kan leveres, sætter alt til levering og returnerer ordre_id
