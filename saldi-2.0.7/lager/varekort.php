...
// ----------/lager/varekort.php---------patch 2.0.7---2009.05.17-----
// LICENS
//
...
print "<tr><td> Varenr</td><td><br></td>";

if (!$varenr) {
	$fokus="varenr";
	print "<input type=hidden name=vare_lev_id value=$vare_lev_id>";
	print "<td colspan=3><input type=text size=25 name=varenr value=\"$varenr\" onchange=\"javascript:docChange = true;\"></td></tr>";
}
else {
	if (!$beskrivelse) $fokus="beskrivelse";
print "<td colspan=3><a href=ret_varenr.php?id=$id> $varenr</a></td></tr>";
print "<input type=hidden name=varenr value=$varenr>";
print "<tr><td> Beskrivelse</td><td><br></td><td colspan=3><input type=text size=60 name=beskrivelse value=\"$beskrivelse\" onchange=\"javascript:docChange = true;\"></td></tr>";

print "<tr><td> Enhed</td><td></td>";
...
if (($enhed)&&($enhed2)) print "<tr><td> $enhed2/$enhed</td><td><br></td><td width=100><input type=text size=2 name=forhold value=\"$x\" onchange=\"javascript:docChange = true;\"></td></tr>";

$x=dkdecimal($salgspris);
print "<tr><td> Salgspris</td><td><br></td><td><input type=text style=text-align:right size=10 name=salgspris value=\"$x\" onchange=\"javascript:docChange = true;\"> /$enhed&nbsp;";
if (($enhed2)&&($forhold>0)) {
	$x=dkdecimal($salgspris/$forhold);
	print "<input type=text style=text-align:right size=10 name=salgspris2 value=\"$x\" onchange=\"javascript:docChange = true;\"> /$enhed2";
}
print "</td></tr>";
$x=dkdecimal($kostpris[0]);

if ($samlevare!='on') {
	print "<tr><td> Kostpris</td><td><br></td><td><input type=text style=text-align:right size=10 name=kostpris[0] value=\"$x\" onchange=\"javascript:docChange = true;\"> /$enhed&nbsp;";
	if ($id) {
...
}else print "<tr><td> Kostpris</td><td><br></td><td><input readonly=readonly style=text-align:right size=10 name=kostpris[0] value=\"$x\"> /$enhed&nbsp;";

print "<tr><td> Varegruppe</td><td></td>";
if (!$gruppe){$gruppe=1;}
...
if (!$beholdning || !$box9) { # box9 tilfoejet 20090210 saa gruppeskift mellem grupper med box8 er mulig.
	...
	elseif ($beholdning) $query = db_select("select * from grupper where art='VG' and kodenr!='$gruppe' and box8='on' order by kodenr");# tilfoejet 20090210 
...
print "</SELECT></td></tr>";
if ($operation) print "<tr><td> Operation nr:</td><td></td><td><input type=text size=5 style=text-align:right name=operation value=$operation>";
elseif ($box8=='on'){
	print "<tr><td> Beholdning</td><td> min:</td><td><input type=text size=5 style=text-align:right name=min_lager value=$min_lager>";
	print "&nbsp;&nbsp;max:&nbsp;<input type=text size=5 style=text-align:right name=max_lager value=$max_lager>";
	print "&nbsp;&nbsp;aktuel: $beholdning</td></tr>";
}
if ($provisionsfri == 'on') {print "<tr><td> Provisionsfri</td><td></td><td><input type=checkbox name=provisionsfri checked></td>";}
else {print "<tr><td> Provisionsfri</td><td></td><td><input type=checkbox name=provisionsfri></td>";}
print "<tr><td valign=top> Bem&aelig;rkning</td><td><br></td><td colspan=9><textarea name=\"notes\" rows=\"3\" cols=\"60\">$notes</textarea></td></tr>";
print "<tr><td colspan=2></td><td colspan=3><table width=100% border=0><tbody><tr>";
if ($serienr == 'on') {print "<td width=25%> Serienr.&nbsp;<input type=checkbox name=serienr checked></td>";}
elseif  ($box9 == 'on') {print "<td width=25%> Serienr&nbsp;<input type=checkbox name=serienr></td>";}
if (($styklister)&&($box8!='on')&&(!$lev_id[1])) { # /* Udeladt intil test af vareflow er afsluttet (2006-03-03)
	if ($samlevare == 'on') {print "<td width=25%> Samlevare&nbsp;<input type=checkbox name=samlevare checked></td>";}
	else {print "<td width=25%> Samlevare&nbsp;<input type=checkbox name=samlevare></td>";}
...
if ($lukket==0) {print "<td width=25%> Udg&aring;et&nbsp;<input type=checkbox name=lukket></td>";}
else {print "<td width=25%> Lukket&nbsp;<input type=checkbox name=lukket checked></td>";}
}
...
if ($samlevare=='on') {
	...
	print 	"<td><input type=text size=60 name=be_af_beskrivelse[0] title='Indtast varebsekrivelse p�vare som skal tilf&oslash;jes styklisten'></td></tr>";
