<?php
// ----------lager/varer.php-------------ver 2.0.7 -----2009.05.17----
// LICENS
//
...
print "<td width=\"10%\" $top_bund>$font<small><a href=$returside accesskey=L><span title='Luk varelisten og g&aring; tilbage til hovedmenuen'>Luk</span></a></small></td>\n";
if ($start<$linjeantal) {
...
if ($popup) {
	...
	print "<td width=\"5%\" $top_bund onMouseOver=\"this.style.cursor = 'pointer'\"; onClick=\"javascript:window.open('varekort.php?opener=varer.php&returside=../includes/luk.php','varekort','scrollbars=1,resizable=1');ordre.focus();\"><small>$font<span style=\"text-decoration: underline;\" title='Opret en ny vare'>Ny</a></span></small></td>";
} else {
	print "<td width=\"5%\" $top_bund><a href=varevisning.php>$font <small><span title='V&aelig;lg hvilke varegrupper og kreditorer som som vises i varelisten'><u>Visning</u></span></td>";
	print "<td width=\"5%\" $top_bund><a href=varekort.php?returside=varer.php><small>$font<span title='Opret en ny vare'>Ny</a></span></small></td>";
}
print "</td></tr>\n";
print "</tbody></table>\n";
...
if ($beskrivelse||$varenummer) $slut=999999;
...
while ($row = db_fetch_array($query)) {
	...
	if (($beskrivelse)&&($row['beskrivelse'])){
		$vis1=0; #$vis1 & $vis2 bruges til at afgøre om en vare skal med paa listen
		if ($b_startstjerne){
			if ($b_slutstjerne) {
...
// Her frasorteres varer som ikke kommer fra den valgte lev.	
	if ((isset($vis_K[1]) && $vis1==1 && isset($lev_vare_liste) && in_array($row['id'],$lev_vare_liste)) || isset($vis_K[0]));
	elseif (!$vis_K[1] && $vis1==1 && isset($lev_vare_liste) && !in_array($row['id'],$lev_vare_liste));
	elseif(!$forslag) {$vis1=0; $z--;}
	if ((isset($vis_K[1]) && $vis2==1 && isset($lev_vare_liste) && in_array($row['id'],$lev_vare_liste)) || isset($vis_K[0]));
...
// frasortering slut	
	...
	if ($udskriv) {
			...
			$kort="kort".$row['id'];
			if ($popup) print "<td onMouseOver=\"this.style.cursor = 'pointer'\"; onClick=\"javascript:$kort=window.open('varekort.php?opener=varer.php&id=$row[id]&returside=../includes/luk.php','$kort','scrollbars=1,resizable=1');$kort.focus();\"><small>$font<FONT COLOR=$color><span style=\"text-decoration: underline;\">".htmlentities(stripslashes($row['varenr']),ENT_COMPAT,$charset)."</span></small></td>";
			else print "<td> <a href=varekort.php?id=$row[id]&returside=varer.php><small>$font<FONT COLOR=$color>".htmlentities(stripslashes($row['varenr']),ENT_COMPAT,$charset)."</a></small></td>";	
			print "<td><small>$font<FONT COLOR=$color>".htmlentities(stripslashes($row['enhed']),ENT_COMPAT,$charset)."<br></small></td>";
			print "<td><small>$font<FONT COLOR=$color>".htmlentities(stripslashes($row['beskrivelse']),ENT_COMPAT,$charset)."<br></small></td>";
			if (!$vis_lev){
				if ($lagerantal>=1  && !$forslag) { 
					for ($x=0;$x<=$lagerantal; $x++) {
...
#				if (($beholdning||$forslag)&&!$udskriv) {
			...
			if ($vis_lev=='on') {
				...
				if (($row2['box8']=='on')||($row['samlevare']=='on')){
					...
					print "<td></td>";		
					print "<td><small>$font<FONT COLOR=$color> $levrow[kontonr] - ".htmlentities(stripslashes($levrow['firmanavn']),ENT_COMPAT,$charset)."</small></td>";
					print "<td><small>$font<FONT COLOR=$color> ".htmlentities(stripslashes($lev_varenr),ENT_COMPAT,$charset)."</small></td>";
				}
				else {print "<td></td>";}	 
			}
			print "</tr>\n";
...
if ($r = db_fetch_array(db_select("select * from vare_lev where vare_id = $vare_id order by posnr",__FILE__ . " linje " . __LINE__))) {
	...
	if ($r = db_fetch_array(db_select("select id, sum from ordrer where konto_id = $lev_id and status < 1 and ordredate = '$ordredate'",__FILE__ . " linje " . __LINE__))) {
		...
		if ($r['gruppe']) {
			...
			print "<BODY onLoad=\"javascript:alert('Leverandørgruppe ikke korrekt opsat for varenr $r[varenr]')\">";
		}
...
#####################################################
function find_varer_i_ordre() { #tilføjet 2008.01.28 for hastighedsoptimering af genbestilling
	$ordreliste=NULL;
