...
//
// ----------kreditor/top100.php-------lap 4.1.1------2025-11-12---
//                           LICENSE
//
...
if ($menu=='T') {
	...
	print "<td width=\"10%\"title='$tekst'><a href=../kreditor/rapport.php accesskey=L>
	...
	print "<table width=\"100%\" align=\"center\" border=\"0\" cellspacing=\"3\" cellpadding=\"0\"><tbody>"; #B
	$tekst = findtekst('2181|Klik her for at lukke Top100', $sprog_id);
	print "<td width=\"10%\" $top_bund title='$tekst'><a href=../kreditor/rapport.php accesskey=L>Luk</a></td>";
...
if ($ret) {
	...
	print "<tr><td colspan=4><hr></td></tr>\n";
	$q = db_select("select konto_id, sum(sum*valutakurs/100) as totalsum from ordrer where (art='KO' or art= 'KK') and fakturadate>='$from' and fakturadate<='$to' and status >= '3' group by konto_id order by sum(sum*valutakurs/100) desc limit 100",__FILE__ . " linje " . __LINE__);
