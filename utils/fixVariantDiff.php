...
for ($x=0;$x<count($vareId);$x++) {
	$y=0;
	$qty[$x] = $variantId[$x] = array();
	$qtxt = "SELECT variant_id, sum(antal) as bought ";
	$qtxt.= "FROM batch_kob where ";
	$qtxt.= "vare_id = '$vareId[$x]' ";
	$qtxt.= "group by variant_id order by variant_id";
#if ($vareId[$x] == 1240) echo "$qtxt<br>";
	$q = db_select($qtxt,__FILE__ . " linje " . __LINE__);
	while ($r=db_fetch_array($q)) {
		$variantId[$x][$y] = $r['variant_id'];
		$qty[$x][$y]       = $r['bought'];
		$qtxt = "SELECT sum(antal) as sold ";
		$qtxt.= "FROM batch_salg where ";
		$qtxt.= "vare_id = '$vareId[$x]' and variant_id = '".$variantId[$x][$y]."'";
		if ($r2=db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__))) $qty[$x][$y]-=$r2['sold'];
#if ($vareId[$x] == 1240) echo "$vareId[$x] ".$variantId[$x][$y]." købt $r[bought] solgt $r2[sold] ".$qty[$x][$y]."<br>";
		$y++;
	}
	for ($y=0;$y<count($variantId[$x]);$y++) {
		$qtxt = "select beholdning from lagerstatus where vare_id = $vareId[$x] and variant_id = '".$variantId[$x][$y]."'";
#cho "$qtxt<br>";
		$q = db_select($qtxt,__FILE__ . " linje " . __LINE__);
		while ($r=db_fetch_array($q)) $stock[$x][$y] = $r['beholdning'];
		if ($stock[$x][$y] != $qty[$x][$y]) {
			$qtxt =	"select varenr,beskrivelse,gruppe from varer where id = '$vareId[$x]'"; 
			$r2 = db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__));
if ($r2['gruppe'] == 2) {
echo "diff på Vare id $vareId[$x] variant id ".$variantId[$x][$y]."<br>";
echo "$r2[varenr] $r2[beskrivelse] ".$stock[$x][$y]." != ".$qty[$x][$y]."<br> ";
	}
