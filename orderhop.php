<?php
  ...
  $twoweeks = date("Y-m-d", strtotime("-6 day"));
  $now = date("Y-m-d", strtotime("+1 day"));
  //searchCriteria[filter_groups][0][filters][0][field]=created_at&searchCriteria[filter_groups][0][filters][0][condition_type]=from&searchCriteria[filter_groups][0][filters][0][value]=".$twoweeks."&searchCriteria[filter_groups][1][filters][0][field]=created_at&searchCriteria[filter_groups][1][filters][0][condition_type]=to&searchCriteria[filter_groups][1][filters][0][value]=".$now
  $ch = curl_init("https://www.havemoebelshoppen.dk/rest/V1/orders?searchCriteria[filter_groups][0][filters][0][field]=created_at&searchCriteria[filter_groups][0][filters][0][condition_type]=from&searchCriteria[filter_groups][0][filters][0][value]=".$twoweeks."&searchCriteria[filter_groups][1][filters][0][field]=created_at&searchCriteria[filter_groups][1][filters][0][condition_type]=to&searchCriteria[filter_groups][1][filters][0][value]=".$now);
...
/*   header("Content-Type: application/json");
  echo $res;
  file_put_contents("magento.json", $res); */

  $fileData = file_get_contents("magento.txt");
  $ids = explode(";", $fileData);
  $res = json_decode($res);
  ...
  foreach($res->items as $item){
    if(isset($ids) && in_array($item->increment_id, $ids)){
      continue;
    }
  ...
  if(isset($magentoData)){
    ...
    foreach($magentoData as $data){
        ...
        for($i = 0; $i < count($data["items_prices"]); $i++){
          ...
          $urltxt.="&vare_id=".urlencode($data["product_id"][$i])."&varenr=".urlencode($data["sku"][$i]);
          $urltxt.="&beskrivelse=".urlencode($data["name"][$i]);
          $urltxt.="&antal=".urlencode($data["qty_ordered"][$i]);
          $urltxt.="&pris=".urlencode($data["item_price"][$i])."&rabat=0&stregkode=&variant=&varegruppe=2";
          $urltxt .= "&momsfri=";
          $ch = curl_init();
    ...
    file_put_contents("magento.txt", $data["order_nr"].";", FILE_APPEND);
    $j++;
  }
