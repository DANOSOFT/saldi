...
// -------------finans/rapport.php-------lap 2.9.1------2010-05-16-------
// LICENS
//
...
if ($_POST){
	...
	$ansat_fra=isset($_POST['ansat_fra'])? $_POST['ansat_fra']:NULL;
	
	if ($ansat_fra) {
		list ($tmp, $tmp2) = split(":", $ansat_fra);
		$tmp=trim($tmp);
	...
	$ansat_til=isset($_POST['ansat_til'])? $_POST['ansat_til']:NULL;
	if ($ansat_til) {
		$ansatte_id=$ansat_fra;
		list ($tmp, $tmp2) = split(":", $ansat_til);
		$tmp=trim($tmp);
		for ($x=1; $x<=$antal_ansatte; $x++) {
			if ($tmp==$ansat_init[$x]) {
	...
	$projekt_fra=isset($_POST['projekt_fra'])? $_POST['projekt_fra']:NULL;
	if ($projekt_fra) {
		list ($projekt_fra, $prj_navn_fra) = split(":", $projekt_fra);
		$projekt_fra=trim($projekt_fra);
	}
	$projekt_til=isset($_POST['projekt_til'])? $_POST['projekt_til']:NULL;
	if ($projekt_til) {
		list ($projekt_til, $prj_navn_til) = split(":", $projekt_til);
	...
	$ansat_fra=isset($_GET['ansat_fra'])? $_GET['ansat_fra']:NULL;
	$ansat_til=isset($_GET['ansat_til'])? $_GET['ansat_til']:NULL;
	$projekt_fra=isset($_GET['projekt_fra'])? $_GET['projekt_fra']:NULL;
	$projekt_til=isset($_GET['projekt_til'])? $_GET['projekt_til']:NULL;
}
...
$submit($regnaar, $maaned_fra, $maaned_til, $aar_fra, $aar_til, $dato_fra, $dato_til, $konto_fra, $konto_til, $rapportart, $ansat_fra, $ansat_til, $afd, $projekt_fra, $projekt_til);
##################################################################################################
function forside($regnaar, $maaned_fra, $maaned_til, $aar_fra, $aar_til, $dato_fra, $dato_til, $konto_fra, $konto_til, $rapportart, $ansat_fra, $ansat_til, $afd, $projekt_fra, $projekt_til){
	global $connection;
	global $brugernavn;
	global $top_bund;
	global $md;
	...
	$query = db_select("select * from grupper where art='PRJ' order by kodenr",__FILE__ . " linje " . __LINE__);
	$x=0;
	while ($row = db_fetch_array($query)) {
		$x++;
		$projektnr[$x]=$row['kodenr'];
		$prj_navn[$x]=$row['beskrivelse'];
		if ($projekt_fra == $projektnr[$x]) {$projekt_fra = $projektnr[$x]." : ".$prj_navn[$x];}
		if ($projekt_til == $projektnr[$x]) {$projekt_til = $projektnr[$x]." : ".$prj_navn[$x];}
	}
	$antal_prj=$x;
...
#	print "</tbody></table>"; #B slut
	...
	elseif ($rapportart=="balance") print "<option value=\"balance\">Balance</option>\n";
	elseif ($rapportart=="resultat") print "<option value=\"resultat\">Resultat</option>\n";
	elseif ($rapportart=="budget") print "<option value=\"budget\">Resultat/budget</option>\n";
	elseif ($rapportart=="momsangivelse") print "<option value=\"momsangivelse\">Momsangivelse</option>\n";
	if ($rapportart!="kontokort") print "<option value=\"kontokort\">Kontokort</option>\n";
	if ($rapportart!="balance") print "<option value=\"balance\">Balance</option>\n";
	if ($rapportart!="resultat") print "<option value=\"resultat\">Resultat</option>\n";
	if ($rapportart!="budget") print "<option value=\"budget\">Resultat/budget</option>\n";
	if ($rapportart!="momsangivelse") print "<option value=\"momsangivelse\">Momsangivelse</option>\n";
	print "</td><tr>";
	if ($antal_afd) {
	...
	if ($antal_prj) {
		print "<tr><td>  projekt</td><td><select name=projekt_fra>\n";
		print "<option>$projekt_fra</option>\n";
		if ($projekt_fra) {print "<option></option>\n";}
		for ($x=1; $x<=$antal_prj; $x++) {
			 if ($projekt_fra != $projektnr[$x]) {print "<option>$projektnr[$x] : $prj_navn[$x]</option>\n";}
		}
		print "</select> -</td>";
		print "<td><select name=projekt_til>\n";
		print "<option>$projekt_til</option>\n";
		if ($projekt_til) {print "<option></option>\n";}
		for ($x=1; $x<=$antal_prj; $x++) {
			 if ($projekt_til != $projektnr[$x]) {print "<option>$projektnr[$x] : $prj_navn[$x]</option>\n";}
		}
		print "</select></td></tr>";
	}
	if ($antal_ansatte) {
...
#	}
	...
	if ( ($max_posteringer*1) > 0 ) {
		$loft="(nuv&aelig;rende loft ".($max_posteringer*1).")";
	}
	print "<tr><td colspan=\"3\" align=\"center\" $top_bund><font face=\"Helvetica, Arial, sans-serif\" color=\"#000000\" ";
	print "title=\"$transantal posteringer siden $tmp\"><small>$transantal posteringer det seneste &aring;r $loft</small>";
	print "</td></tr>\n";
	print "</tbody></table>\n"; #C slut
...
# endfunc forside
#################################################################################################
function kontokort($regnaar, $maaned_fra, $maaned_til, $aar_fra, $aar_til, $dato_fra, $dato_til, $konto_fra, $konto_til, $rapportart, $ansat_fra, $ansat_til, $afd, $projekt_fra, $projekt_til) {
...
#	print "  <a accesskey=L href=\"rapport.php?rapportart=Kontokort&regnaar=$regnaar&dato_fra=$startdato&maaned_fra=$mf&dato_til=$slutdato&maaned_til=$mt&konto_fra=$konto_fra&konto_til=$konto_til&afd=$afd\">Luk</a><br><br>";
	...
	print "<td width=\"10%\" $top_bund><font face=\"Helvetica, Arial, sans-serif\" color=\"#000066\"><a accesskey=L href=\"rapport.php?rapportart=kontokort&regnaar=$regnaar&dato_fra=$startdato&maaned_fra=$mf&aar_fra=$aar_fra&dato_til=$slutdato&maaned_til=$mt&aar_til=$aar_til&konto_fra=$konto_fra&konto_til=$konto_til&ansat_fra=$ansat_fra&ansat_til=$ansat_til&afd=$afd&projekt_fra=$projekt_fra&projekt_til=$projekt_til\">Luk</a></td>";
	print "<td width=\"80%\" $top_bund><font face=\"Helvetica, Arial, sans-serif\" color=\"#000066\"> Rapport - kontokort </td>";
	print "<td width=\"10%\" $top_bund><font face=\"Helvetica, Arial, sans-serif\" color=\"#000066\"><br></td>";
	print "</tbody></table>"; #B slut
	print "</td></tr>";
	print "<tr><td colspan=\"4\"><big><big><big>  Kontokort</span></big></big></big></td>";
	...
	if ($afd||$ansat_fra||$projekt_fra) {
		...
		if ($projekt_fra && $projekt_til) $dim = $dim." and projekt >= $projekt_fra and projekt <= $projekt_til ";
		elseif ($projekt_fra) $dim = $dim."and projekt = $projekt_fra ";
	}	
	...
	$ktonr=array();
	$x=0;
	$query = db_select("select kontonr from transaktioner where transdate>='$regnstart' and transdate<='$regnslut' $dim order by transdate,bilag",__FILE__ . " linje " . __LINE__);
	while ($row = db_fetch_array($query)){
		$x++;
		$ktonr[$x]=$row['kontonr'];
	}
	...
	for ($x=1; $x<=$kontoantal; $x++){
		...
		if (in_array($kontonr[$x], $ktonr)||$primo[$x]){
			print "<tr><td colspan=6><hr></td></tr>";
			print "<tr bgcolor=\"$bgcolor5\"><td></td><td></td><td colspan=4>$kontonr[$x] : $kontobeskrivelse[$x] </tr>";
			print "<tr><td colspan=6><hr></td></tr>";
			$kontosum=$primo[$x];
			$query = db_select("select debet, kredit from transaktioner where kontonr=$kontonr[$x] and transdate>='$regnaarstart' and transdate<'$regnstart' $dim order by transdate",__FILE__ . " linje " . __LINE__);
			while ($row = db_fetch_array($query)){
			...
			$print=1;
			$query = db_select("select * from transaktioner where kontonr=$kontonr[$x] and transdate>='$regnstart' and transdate<='$regnslut' $dim order by transdate",__FILE__ . " linje " . __LINE__);
			while ($row = db_fetch_array($query)){
				($linjebg!=$bgcolor5)?$linjebg=$bgcolor5:$linjebg=$bgcolor;
				print "<tr bgcolor=\"$linjebg\"><td>  ".dkdato($row['transdate'])." </td><td>$row[bilag] </td><td>$row[beskrivelse] </td>";
...
#################################################################################################
function regnskab($regnaar, $maaned_fra, $maaned_til, $aar_fra, $aar_til, $dato_fra, $dato_til, $konto_fra, $konto_til, $rapportart, $ansat_fra, $ansat_til, $afd, $projekt_fra, $projekt_til) 
{
	global $connection;
	global $top_bund;
	global $md;
...
#	if ($slutmaaned<10){$slutmaaned="0".$slutmaaned;}
 ...
 #	print "  <a accesskey=L href=\"rapport.php?rapportart=$rapportart&regnaar=$regnaar&dato_fra=$startdato&maaned_fra=$mf&dato_til=$slutdato&maaned_til=$mt&konto_fra=$konto_fra&konto_til=$konto_til&afd=$afd\">Luk</a><br><br>";
	...
	print "<table width=\"100%\" align=\"center\" border=\"0\" cellspacing=\"3\" cellpadding=\"0\"><tbody>"; #B
	print "<td width=\"10%\" $top_bund><font face=\"Helvetica, Arial, sans-serif\" color=\"#000066\"><a accesskey=L href=\"rapport.php?rapportart=$rapportart&regnaar=$regnaar&dato_fra=$startdato&maaned_fra=$mf&aar_fra=$aar_fra&dato_til=$slutdato&maaned_til=$mt&aar_til=$aar_til&konto_fra=$konto_fra&konto_til=$konto_til&ansat_fra=$ansat_fra&ansat_til=$ansat_til&afd=$afd&projekt_fra=$projekt_fra&projekt_til=$projekt_til\">Luk</a></td>";
	print "<td width=\"80%\" $top_bund><font face=\"Helvetica, Arial, sans-serif\" color=\"#000066\"> Rapport - $rapportart </td>";
	print "<td width=\"10%\" $top_bund><font face=\"Helvetica, Arial, sans-serif\" color=\"#000066\"><br></td>";
	print "</tbody></table>"; #B slut
	print "</td></tr>";
...
# echo "select * from transaktioner where transdate>='$regnaarstart' and transdate<='$regnslut' $dim order by kontonr<br>";		
...
#################################################################################################
function momsangivelse ($regnaar, $maaned_fra, $maaned_til, $aar_fra, $aar_til, $dato_fra, $dato_til, $konto_fra, $konto_til, $rapportart, $ansat_fra, $ansat_til, $afd, $projekt_fra, $projekt_til) 
{
	global $connection;
	global $top_bund;
	global $md;
	...
	$medtag_primo=isset($_GET['medtag_primo'])? $_GET['medtag_primo']:NULL;
...
#	print "  <a accesskey=L href=\"rapport.php?rapportart=$rapportart&regnaar=$regnaar&dato_fra=$startdato&maaned_fra=$mf&dato_til=$slutdato&maaned_til=$mt&konto_fra=$konto_fra&konto_til=$konto_til&afd=$afd\">Luk</a><br><br>";
	...
	print "</tbody></table>"; #B slut
	print "</td></tr>";
 	print "<tr><td colspan=\"4\"><big><big>$rapportart</span></big></big></td>";
	print "<td colspan=2 align=right><table style=\"text-align: left; width: 100%;\" border=\"0\" cellspacing=\"1\" cellpadding=\"1\"><tbody><tr>";
	if ($afd) {
		print "<td>Afdeling</span></td>";
		print "<td>$afd: $afd_navn</span></td></tr>";
	...
	if (($row[box1]) && ($row[box2])) {
		...
		for ($x=1; $x<=$kontoantal; $x++) {
			if (($kontonr[$x]>=$konto_fra)&&($kontonr[$x]<=$konto_til)) {
				...
				$tmp = kontobemaerkning($kontobeskrivelse[$x]);
				print "<td $tmp colspan=3>$kontobeskrivelse[$x] </td>";
				$row = db_fetch_array($query = db_select("select art from grupper where box1='$kontonr[$x]'",__FILE__ . " linje " . __LINE__));		
		...
		print "<meta http-equiv=\"refresh\" content=\"0;URL=rapport.php?rapportart=kontokort&regnaar=$regnaar&dato_fra=$startdato&maaned_fra=$mf&dato_til=$slutdato&maaned_til=$mt&konto_fra=$konto_fra&konto_til=$konto_til&ansat_fra=$ansat_fra&afd=$afd&projekt_fra=$projekt_fra&projekt_til=$projekt_til\">";
	}
}
