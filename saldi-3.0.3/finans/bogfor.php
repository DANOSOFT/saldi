<?php
// --------------------------------finans/bogfor.php--------lap 2.0.9c------2009.09.08----
// LICENS
//
...
print "<td width=\"10%\" $top_bund><font face=\"Helvetica, Arial, sans-serif\" color=\"#000066\"><small>$href Luk</a></small></td>";
...
#	else {
		...
		if ($funktion=='bogfor') {
			$onclick="";
			if ($sqdb=="saldi" && $max_posteringer && $max_posteringer<=8500 && $d_sum>0) {
...
#		if (abs($valutadiff[$i])>=0.01&&$v_diffkonto[$i]>0) {
			$b_sum[$i]=round($b_sum[$i]+0.0001,2);
			...
			$query = db_select("select * from kontoplan where kontonr='$b_diffkonto[$i]' and regnskabsaar='$regnaar'",__FILE__ . " linje " . __LINE__);
				...
				$d=dkdecimal($saldo+$kontodebet[$y]-$kontokredit[$y]);
				$beskrivelse=addslashes($row['beskrivelse']);
				db_modify("insert into transaktioner (kontonr, bilag, transdate, logdate, logtime, beskrivelse, debet, kredit, faktura, kladde_id,afd, ansat, projekt, valuta, valutakurs, ordre_id)values($b_diffkonto[$i], $b_bilag[$i], '$b_transdate[$i]', '$logdate', '$logtime', '$beskrivelse', '$kontodebet[$y]', '$kontokredit[$y]', '$v_faktura[$i]', $kladde_id, '$b_afd[$i]', '$b_ansat[$i]', '$b_projekt[$i]', '$b_valuta[$i]', '$b_kurs[$i]', '$b_ordre_id[$i]')",__FILE__ . " linje " . __LINE__);
			} else {
...
# og nedenst&aring;ende træder s&aring;ledes ikke i kraft naar der er tale om en finanskonto med EU moms.
...
# 2009.05.06 afrundingsdecimal rettet fra 3 til 2 grundet problem med Zen 
	...
	$nettoamount=round($nettoamount+0.0001,2); 
	$moms=round($moms+0.0001,2);
	$tmp=round ($amount-($nettoamount+$moms),2);
# Nedenstående tilfojet 20090902 jvf saldi_2_20090902-1446.sdat
	if ($tmp>0) $moms=$moms+0.01; 
	elseif ($tmp<0) $moms=$moms-0.01;
	$tmp=round ($amount-($nettoamount+$moms),2);
	if ($a!="E" && round($tmp,2)!=0) {
#	echo "$a $amount $nettoamount $moms<br>";
