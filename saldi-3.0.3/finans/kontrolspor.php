<?php
// ----------finans/kontrolspor.php-------------lap 3.0.2-----2010-05-31-----
// LICENS
//
...
print "<td align=center><span title='Angiv maksimale antal linjer, som skal vises pr. side'><input type=text size=4 style=\"text-align:right\" name=linjeantal value=$linjeantal> linjer.</td>\n";
$tmp=$start+$linjeantal;
if ($next>0) {
...
function udskriv($idnumre, $bilagsnumre, $kladdenumre, $fakturanumre, $kontonumre, $transdatoer, $logdatoer, $debetbelob, $kreditbelob, $logtid, $beskrivelse, $sort, $start, $skriv) {
	...
	if ($fakturanumre)	$udvaelg=$udvaelg.udvaelg($fakturanumre, 'transaktioner.faktura', 'NR');
	...
	while ($row =db_fetch_array($query)) {
		...
		if (($x>=$start)&&($x<$start+$linjeantal) && ($udskriv)){
			...
			if ($skriv) {
				...
				print "<td align=right> $dato<br></td>";
				$dato=dkdato($row[logdate]);
				$debetsum=round($debetsum+$row['debet']+0.0001,2);
				$kreditsum=round($kreditsum+$row['kredit']+0.0001,2);
				print "<td align=right> $dato<br></td>";
