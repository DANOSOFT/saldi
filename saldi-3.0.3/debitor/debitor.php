<?php
// ---------------debitor/debitor.php---lap 3.0.0------2010-05-17----
// LICENS
//
...
if (count($cat_liste)) {
	$x=0;
	$q=db_select("select * from grupper where art = 'DGCAT' order by box1",__FILE__ . " linje " . __LINE__);
	while ($r=db_fetch_array($q)) {
		$x++;
		$cat_id[$x]=$r['id'];
		$cat_navn[$x]=$r['box1'];
		...
		if ($cat_liste[$i3]) {
			for($i4=0;$i4<=$cat_antal;$i4++	) {
				if($cat_liste[$i3]==$cat_id[$i4]) {
					if (!$start && !$lnr) {
#						print "<td colspan=\"$colspan\"><b>$cat_navn[$i4]</b></td></tr>";
#						print "<tr><td colspan=\"$colspan\"><hr></td>";
						$tmp=$start+$linjeantal;
#						if ($antal>$slut) print "<td align=center><a href=debitor.php?start=$tmp&valg=$valg><img src=../ikoner/right.png style=\"border: 0px solid; width: 15px; height: 15px;\"></a></td><tr>";
					...
					print "<tr><td></td>$tmp<td colspan=\"2\"><b>$cat_navn[$i4]</b></td></tr>";
					print "<tr><td colspan=\"$colspan\"><hr></td>";
					$udv2=$udv1." and kategori like '%".$cat_id[$i4]."%'";	
					break;
				} 
			}	
		}
