...
@session_start();
$s_id=session_id();
// ------debitor/mail_kontoudtog.php-------lap 3.0.2------2010-05-21--------
// LICENS
//
...
for($x=1; $x<=$kontoantal; $x++) {
	...
	if ( $regnaar ) {
		...
		$query = db_select("select * from grupper where art='RA' order by box2, box1");
		$row = db_fetch_array($query);
		$startmaaned=$row['box1']*1;
		$startaar=$row['box2']*1;
...
function send_mails($kontoantal, $konto_id, $email, $fra, $til) {
	...
	for($x=1; $x<=$kontoantal; $x++) {
		if (($konto_id[$x])&&($email[$x])&&($fra[$x])&&($til[$x])&&(strpos($email[$x], '@'))) {	
			...
			$mailtext .= "<html><head><meta content=\"text/html; charset=ISO-8859-1\" http-equiv=\"content-type\">\n";
			$row = db_fetch_array(db_select("select firmanavn from adresser where art = 'S'"));
...
#	$mail->AddReplyTo("info@site.com","Information");
			...
			$mailaltbody .= "ved at dobbeltklikke.\n";
			$mailaltbody .= "-- \n";
