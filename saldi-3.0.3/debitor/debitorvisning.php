...
// --debitor/debitorvisning.php--------------------lap 3.0.2--2010-05-25--
// LICENS
//
...
$vis_felt=split(chr(9),$r['box3']);
$feltbredde=split(chr(9),$r['box4']);
$justering=split(chr(9),$r['box5']);
$feltnavn=split(chr(9),$r['box6']);
$vis_linjeantal=$r['box7'];
$vis_feltantal=count($vis_felt)-1;
$select=split(chr(9),$r['box8']);
...
#	print "<tr><td colspan=3><hr></td></tr>";
	...
	$dg_liste=split(chr(9),$r['box1']);
	$cat_liste=split(chr(9),$r['box2']);
	...
	$q = db_select("select * from grupper where art = 'DGCAT' order by beskrivelse",__FILE__ . " linje " . __LINE__);
	$x=-1;
	while ($r = db_fetch_array($q)) {
		$x++;
		if (in_array($r['id'],$cat_liste)) $tmp='checked';
		else $tmp='';
		print "<tr><td><input name=\"cat_liste[$x]\" type=\"checkbox\" $tmp>$r[box1]</td></tr>";
		print "<input type=hidden name=cat_id[$x] value=$r[id]>";
	}
