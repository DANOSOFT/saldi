...
// -----------------------debitor/ny_rykker.php-----lap 2.1.1--2009.11.17-------
// LICENS
//
...
for ($i=0; $i<=$konto_antal; $i++) {
	...
	if (($konto_id[$i])||($rykker_id[$i])) {
		if (!$rykker_id[$i]) {
			...
			while ($r=db_fetch_array($q)) {
#echo "select ordrer.id from ordrer,ordrelinjer where ordrelinjer.enhed = '$r[id]' and ordrer.id = ordrelinjer.ordre_id and ordrer.betalt != 'on'<br>";			
#				Finder tidligere rykkere på samme ordre.
			...
			$ryk_antal=$x;
#echo "ryk $ryk_antal<br>";
#exit;			
			if ($ryk_antal) {	
#echo "insert into ordrer (ordrenr, konto_id, kontonr, firmanavn, addr1, addr2, postnr, bynavn, land, betalingsdage, betalingsbet, cvrnr, ean, institution, notes, art, ordredate, levdate, fakturadate, momssats, hvem, tidspkt, ref, status, valuta,fakturanr,email,betalt) values ('$r_ordrenr', '$konto_id[$i]', '$kontonr', '$firmanavn', '$addr1', '$addr2', '$postnr', '$bynavn', '$land', '$betalingsdage', '$betalingsbet', '$cvrnr', '$ean', '$institution', '$notes', 'R1', '$rykkerdate', '$rykkerdate', '$rykkerdate', '0', '$brugernavn', '$tidspkt', '$ref', '2', '$valuta','$r_fakturanr','$email','on')<br>";				
#exit;				
				db_modify("insert into ordrer (ordrenr, konto_id, kontonr, firmanavn, addr1, addr2, postnr, bynavn, land, betalingsdage, betalingsbet, cvrnr, ean, institution, notes, art, ordredate, levdate, fakturadate, momssats, hvem, tidspkt, ref, status, valuta,fakturanr,email,betalt) 
					values ('$r_ordrenr', '$konto_id[$i]', '$kontonr', '$firmanavn', '$addr1', '$addr2', '$postnr', '$bynavn', '$land', '$betalingsdage', '$betalingsbet', '$cvrnr', '$ean', '$institution', '$notes', 'R1', '$rykkerdate', '$rykkerdate', '$rykkerdate', '0', '$brugernavn', '$tidspkt', '$ref', '2', '$valuta','$r_fakturanr','$email','-')",__FILE__ . " linje " . __LINE__);
