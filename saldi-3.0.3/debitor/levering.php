...
// --------------debitor/levering.php--------lap 2.1.2-------2010.02.23--------------
// LICENS
//
...
if ($fakturadate && $hurtigfakt=='on') $levdate=$fakturadate;
elseif ($hurtigfakt=='on') $levdate=$ordredate;
else $levdate=$row['levdate'];
$art=$row['art'];
...
if ($hurtigfakt=='on') {
	db_modify("update ordrer set levdate = '$levdate' where id = '$id'");
}
if (!$levdate){
	...
	if ($hurtigfakt!='on' && $levdate<$ordredate) {
		 print "<BODY onLoad=\"javascript:alert('Leveringsdato er f&oslash;r ordredato')\">";
		 print "<meta http-equiv=\"refresh\" content=\"0;URL=ordre.php?id=$id\">";
		 exit;
	}
	list ($year, $month, $day) = split ('-', $levdate);
	$year=trim($year);
	$ym=$year.$month;
	if ($hurtigfakt!='on' && ($ym<$aarstart || $ym>$aarslut)){
