<?php
// ----------debitor/ordre.php----------lap 3.0.2-----2010-05-31-------
// LICENS
//
...
#	}
	...
	for ($x=0; $x<=$linjeantal;$x++) {
		...
		if ($posnr_ny[$x]!="-" && $posnr_ny[$x]!="->" && $posnr_ny[$x]!="<-") $posnr_ny[$x]=$posnr_ny[$x]*1;
		$y="vare".$x;
		...
		for($x=1; $x<=$linjeantal; $x++) {
			...
			if ((($antal[$x]>0)&&($leveres[$x]<0))||(($antal[$x]<0)&&($leveres[$x]>0))) {
				...
				if (!$negativt_lager && $leveres[$x]>$beholdning[$x] && (!$hurtigfakt || $submit=="Fakturer") && $leveres[$x]>$beholdning[$x] && $leveres[$x]>0 && 
					...
					if ($hurtigfakt) {
						$tekst="Lagerbeholdning: ".dkdecimal($beholdning[$x]).". Der kan h&oslash;jest leveres $leveres[$x] fra linjen med pos.nr. $posnr_ny[$x])";
					} else{
						$tekst="Lagerbeholdning: ".dkdecimal($beholdning[$x]).". Der kan h&oslash;jest leveres $leveres[$x]. Antal reguleret (pos.nr. $posnr_ny[$x])";
					}
...
*/		
		...
		print "<td align=\"center\"><b>Pos.</b></td><td align=\"center\"><b>Varenr.</b></td><td align=\"center\"><b>Antal</b></td><td align=\"center\"><b>Enhed</b></td><td align=\"center\"><b>Beskrivelse</b></td><td align=\"center\"><b>Pris</b></td><td align=\"center\"><b>%</b></td><td align=\"center\"><b>I alt</b></td>\n";
		...
		for ($x=1; $x<=$linjeantal; $x++) {
			...
			if (($varenr[$x])&&($vare_id[$x])) {
				...
				if ($momsfri[$x]!='on') {
					$moms+=afrund($ialt*$varemomssats[$x]/100,3); #20100525 aendret fra 2 til 3 decimaler grundet momsfejl saldi_2
#					$momssum=$momssum+$ialt;
			...
			if ( $enhed[$x] ) { 
				...
				print "<input type=\"hidden\" name=\"enhed[$x]\" value=\"$enhed[$x]\"><td align=\"right\">&nbsp;</td>\n";
			}
...
#					$ialt=$ialt*-1;
				...
				print "<td align=\"right\" title=\"Kostpris $dk_kostpris[$x] * db: $dk_db[$x] * dg: $dk_dg[$x]%\">".$tmp."</td>\n";
			}
			else print "<td>&nbsp;</td>\n";
			...
			if ($ko_ant[$x]>=1) {
				for ($y=1; $y<=$ko_ant[$x]; $y++) {
					...
					print "<td align=\"right\" onClick=\"javascript:k_ordre=window.open('../kreditor/ordre.php?id=$kobs_ordre_id[$z]','ordre' ,'left=10,top=10,width=800,height=400,scrollbars=yes,resizable=yes,menubar=no,location=no');k_ordre.focus();\"onMouseOver=\"this.style.cursor = 'pointer'\" title=\"'$spantekst'\"><img src=\"../ikoner/opslag.png\"></td>\n";
				}
			}
			else {print "<td><br></td>\n";}
...
#		$moms=afrund($tmp,3);
		...
		if ($art!='DK') print "<td align=\"center\"><input type=\"submit\" value=\"&nbsp;Kopi&eacute;r&nbsp;\" name=\"submit\" title=\"Kopi&eacute;r til ny ordre med samme ordrelinjer.\"></td>\n";
...
##### pile ########		
		...
		print "</SELECT></td>\n";
		print "<td>&nbsp;</td><td>Momssats</td><td><input class=\"inputbox\" style=\"text-align:right\" type=\"text\" size=\"3\" name=\"momssats\" value=\"".dkdecimal($momssats)."\" onchange=\"javascript:docChange = true;\">%</td></tr>\n";
...
*/		
		...
		if ($fakturadato||$status>0) {
			print "<tr><td ";
			if ($art=='DO') print "title=\"Fakturadato\">Fakt. dato"; 
		...
		if ($vis_lev_addr || !$kontonr) {
			print "<tr><td colspan=\"2\" align=\"right\">Vis leveringsadresse<input type=\"checkbox\" name=\"vis_lev_addr\" checked=\"checked\"><td></tr>\n";
			...
			print "<tr><td colspan=\"2\" align=\"right\">Vis leveringsadresse<input type=\"checkbox\" name=\"vis_lev_addr\"><td></tr>\n";
			print "<tr><td colspan=\"2\"><hr><td></tr>\n";
...
#	 print "<tr><td><textarea style=\"font-family: helvetica,arial,sans-serif;\" name=\"lev_adr\" rows=\"5\" cols=\"35\">$lev_adr</textarea></td></tr>\n";
		...
		print "<tr><td align=\"center\" colspan=\"3\"><table cellpadding=\"0\" cellspacing=\"0\"><tbody>\n";
			print "<tr><td align=\"center\" title=\"Positionsnummer. R&aelig;kkef&oslash;lgen &aelig;ndres ved at overskrive positionsnumrene. En enkelt linje slettes ved at skrive minustegn som positionsnummer.\">Pos.</td><td align=\"center\">Varenr.</td><td align=\"center\" title=\"Antal enheder. Timer og minutter kan angives med : som skilletegn. Eksempelvis 5:45 som bliver til 5,75.\">Antal</td><td align=\"center\">Enhed</td><td align=\"center\">Beskrivelse</td><td align=\"center\">Pris</td><td align=\"center\">%</td><td align=\"center\">I alt</td>";
...
#			print "<input type=\"hidden\" name=\"momssats\" value=\"$momssats\">\n";
			...
			print "<td><input class=\"inputbox\" type=\"text\" style=\"text-align:right\" size=\"3\" name=\"dkan$x\" value=\"$dkantal[$x]\"></td>\n";
			print "<td><input class=\"inputbox\" readonly=\"readonly\" size=\"3\" value=\"$enhed[$x]\" onchange=\"javascript:docChange = true;\"></td>\n";
			$title=var2str($beskrivelse[$x],$id);
			...
			print "<td title=\"db: $dk_db[$x] - dg: $dk_dg[$x]%\"><input class=\"inputbox\" type=\"text\" style=\"text-align:right\" size=\"10\" name=\"pris$x\" value=\"$dkpris\" onchange=\"javascript:docChange = true;\"></td>\n";
			$title=$dkantal[$x]."*".dkdecimal(($rabat[$x]/100)*$pris[$x])." = ".dkdecimal($antal[$x]*($rabat[$x]/100)*$pris[$x]);
			...
			if ($rabat[$x]) {$db[$x]=$db[$x]-(($rabat[$x]/100)*$pris[$x]);}
			$db[$x]=$db[$x]*$antal[$x];
			if ($ialt!=0) {
				$dg[$x]=$db[$x]*100/$ialt;
			}
...
#	if ($ialt) {
			...
			print "<td align=\"right\" title=\"db: $dk_db[$x] - dg: $dk_dg[$x]%\"><input class=\"inputbox\" readonly=\"readonly\" style=\"text-align:right\" size=\"10\" value=\"$tmp\"></td>\n";
			if ($vis_projekt && !$projekt[0]) { 
...
#					print "<td title=\"kostpris\">Projekt</span></td>\n";
					...
					if (abs($antal[$x])!=abs($tidl_lev[$x])) {
						print "<td title=\"Beholdning: $beholdning[$x]\"><input class=\"inputbox\" type=\"text\" style=\"text-align:right\" size=\"2\" name=\"leve$x\" value=\"$dklev[$x]\" onchange=\"javascript:docChange = true;\"></td>\n";
						print "<td>($dk_tidl_lev[$x])</td>\n";
						if ($batchvare[$x]) print "<td align=\"center\" onClick=\"batch($linje_id[$x])\" title=\"V&aelig;lg fra k&oslash;bsordre\"><img alt=\"Serienummer\" src=\"../ikoner/serienr.png\"></td>\n";
						$levdiff=1;
					} else {
						print "<td title=\"Beholdning: $beholdning[$x]\"><input class=\"inputbox\" readonly=\"readonly\" style=\"text-align:right\" size=\"2\" name=\"leve$x\" value=\"$dklev[$x]\" onchange=\"javascript:docChange = true;\"></td>\n";
						print "<td>($dk_tidl_lev[$x])</td>\n";
					}
					db_modify("update ordrelinjer set leveret=$tidl_lev[$x] where id=$linje_id[$x]",__FILE__ . " linje " . __LINE__);
				}
			}
			if ($samlevare[$x]=='on') print "<td align=\"center\" onClick=\"stykliste($vare_id[$x])\" title=\"Vis stykliste\"><img alt=\"Stykliste\" src=\"../ikoner/stykliste.png\"></td>\n";
			if ($m_rabat[$x] && !$rabatgruppe[$x]) {
				...
				print "<td><input class=\"inputbox\" readonly=\"readonly\" style=\"text-align:right\" size=\"3\" value=$dkantal[$x]></td>\n";
		...
		if ($kontonr) {
			...
			print "<td><input class=\"inputbox\" type=\"text\" style=\"text-align:right\" size=\"3\" name=\"dkan0\"></td>\n";
...
#			$moms=afrund($tmp,3);
		...
		if ($status < 3) {
			...
			print "<td align=\"center\" width=$width><input type=\"submit\" accesskey=\"o\" value=\"Opslag\" name=\"submit\" ";
