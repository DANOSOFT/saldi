<?php
// -- debitor/historikkort.php -------------lap 3.0.2--2010-05-31-00:06--
// LICENS
//
...
$q = db_select("select id, navn, tlf, mobil, email, notes from ansatte where konto_id = $id");
while ($r = db_fetch_array($q)){
...
print "</form>";
$q = db_select("select * from historik where konto_id = $id order by kontaktet desc, id desc");
print "<tr><td colspan=6><hr></td></tr>";
while ($r=db_fetch_array($q)){
	$ansat_id=$r['ansat_id']*1;
	$kontakt_id=$r['kontakt_id']*1;
	...
	$ansat=str_replace(" ","&nbsp;",$r1['navn']);
	$r1 = db_fetch_array(db_select("select navn, tlf, mobil, email, notes from ansatte where id = $kontakt_id"));
