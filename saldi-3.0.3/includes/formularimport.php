...
// ----------/systemdata/formularimport.php-----lap 2.1.4------2010-03.21---------------------------------
// LICENS
//
...
function formularimport($formular) 
{
	...
	if ($fp) {
		...
		while (!feof($fp)) {
			...
			list($formular, $art, $beskrivelse, $justering, $xa, $ya, $xb, $yb, $str, $color, $font, $fed, $kursiv, $side, $sprog) = split(chr(9),$linje);
			...
			if ($xa>0) {
				...
				db_modify("insert into formularer (formular,art,beskrivelse,xa,ya,xb,yb,justering,str,color,font,fed,kursiv,side,sprog)values('$formular','$art','$beskrivelse','$xa','$ya','$xb','$yb','$justering','$str','$color','$font','$fed','$kursiv','$side','$sprog')"); 
			}
		}
		fclose($fp);
