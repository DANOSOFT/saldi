<?php
// --------includes/openpost.php ----- lap 2.1.4 ---- 2010.03.22 ---------------------------
// LICENS
//
...
function openpost($regnaar, $maaned_fra, $maaned_til, $konto_fra, $konto_til, $rapportart, $art) 
{
	...
	if ($maaned_fra && strstr($maaned_fra," ")) list ($x,$tmp1) = split(" ",$maaned_fra);
	if ($maaned_til && strstr($maaned_til," ")) list ($x,$tmp2) = split(" ",$maaned_til);
	if ($tmp1 && $tmp2) {
		$maaned_fra=$tmp1;
		$maaned_til=$tmp2;
	}
	...
	print "<td width=\"10%\" $top_bund><a accesskey=l href=\"rapport.php?rapportart=openpost&regnaar=$regnaar&maaned_fra=$maaned_fra&maaned_til=$maaned_til&konto_fra=$konto_fra&konto_til=$konto_til\">Luk</a></td>";
	print "<td width=\"80%\" $top_bund>Rapport - $rapportart</td>";
	print "<td width=\"10%\" $top_bund>";
	if ($skjul_aabenpost=='on') print "<a href=rapport.php?rapportart=openpost&submit=ok&regnaar=$regnaar&maaned_fra=$maaned_fra&maaned_til=$maaned_til&konto_fra=$kotno_fra&konto_til=$konto_til&skjul_aabenpost=off>Vis</a><td></tr>";
	else print "<a href=rapport.php?rapportart=openpost&submit=ok&regnaar=$regnaar&maaned_fra=$maaned_fra&maaned_til=$maaned_til&konto_fra=$konto_fra&konto_til=$konto_til&skjul_aabenpost=on>Skjul</a><td></tr>";	
#	<a accesskey=l href=\"rapport.php?rapportart=openpost&regnaar=$regnaar&maaned_fra=$maaned_fra&maaned_til=$maaned_til&konto_fra=$konto_fra&konto_til=$konto_til\"><br></a></td>";
	print "</tbody></table></td></tr>\n"; #B slut

if ($skjul_aabenpost!='on') vis_aabne_poster($regnaar,$maaned_fra,$maaned_til,$konto_fra,$konto_til,$art);
...
#		print "<tr><td><br></td></tr>\n";
 		...
 		while ($taeller <3) {  
			...
			if ($taeller==1) {
				...
				if ($skjul_aaben_rykker=='on') print "<a href=rapport.php?rapportart=openpost&submit=ok&regnaar=$regnaar&maaned_fra=$maaned_fra&maaned_til=$maaned_til&konto_fra=$kotno_fra&konto_til=$konto_til&skjul_aaben_rykker=off>Skjul</a><td></tr>";
				else print "<a href=rapport.php?rapportart=openpost&submit=ok&regnaar=$regnaar&maaned_fra=$maaned_fra&maaned_til=$maaned_til&konto_fra=$konto_fra&konto_til=$konto_til&skjul_aaben_rykker=on>Vis</a><td></tr>";	
			} elseif ($taeller==2) {
				...
				if ($skjul_bogfort_rykker=='on') print "<a href=rapport.php?rapportart=openpost&submit=ok&regnaar=$regnaar&maaned_fra=$maaned_fra&maaned_til=$maaned_til&konto_fra=$kotno_fra&konto_til=$konto_til&skjul_bogfort_rykker=off>Skjul</a><td></tr>";
				else print "<a href=rapport.php?rapportart=openpost&submit=ok&regnaar=$regnaar&maaned_fra=$maaned_fra&maaned_til=$maaned_til&konto_fra=$konto_fra&konto_til=$konto_til&skjul_bogfort_rykker=on>Vis</a><td></tr>";	
			} else  {
				...
				if ($skjul_afsluttet_rykker=='on') print "<a href=rapport.php?rapportart=openpost&submit=ok&regnaar=$regnaar&maaned_fra=$maaned_fra&maaned_til=$maaned_til&konto_fra=$kotno_fra&konto_til=$konto_til&skjul_afsluttet_rykker=off>Skjul</a><td></tr>";
				else print "<a href=rapport.php?rapportart=openpost&submit=ok&regnaar=$regnaar&maaned_fra=$maaned_fra&maaned_til=$maaned_til&konto_fra=$konto_fra&konto_til=$konto_til&skjul_afsluttet_rykker=on>Vis</a><td></tr>";	
			}
			print "</tbody></table></td></tr>";
...
function vis_aabne_poster($regnaar,$maaned_fra,$maaned_til,$konto_fra,$konto_til,$art) {
...
if (!is_numeric($startmaaned)) {
	if (strstr($startmaaned," ")) list($tmp,$startmaaned)=split(" ",$startmaaned);
	if (!is_numeric($startmaaned)) list ($tmp,$startmaaned)=split(" ",find_maaned_nr($startmaaned));
}
if (!is_numeric($slutmaaned)) {
	if (strstr($slutmaaned," ")) list($tmp,$slutmaaned)=split(" ",$slutmaaned);
	if (!is_numeric($slutmaaned)) list ($tmp,$slutmaaned)=split(" ",find_maaned_nr($slutmaaned));
}
...
#$regnslut = "2005-05-04"; 
	...
	for ($x=1; $x<=$kontoantal; $x++) {
		...
		while ($r=db_fetch_array($q)) {
#echo "$r[amount] - $r[udlignet]!=1 || ($r[transdate] <= $regnslut && $r[udlign_date] && $r[udlign_date] > $regnslut)<br>";			
			if ($r['udlignet']!=1 || ($r['transdate'] <= $regnslut && $r['udlign_date'] && $r['udlign_date'] > $regnslut)) {
#echo "$r[amount]<br>";				
				...
				if ($r['faktnr'] && !$r['kladde_id'] && $amount > 0 && $r['refnr']>1) { #rettet 20090715
...
################				
		...
		if (($y>0.01)||($udlignet=="0"))	{	
			...
			if ($popup) print "<td onClick=\"window.open('rapport.php?rapportart=kontokort&regnaar=$regnaar&maaned_fra=$maaned_fra&maaned_til=$maaned_til&konto_fra=$kontonr[$x]&konto_til=$kontonr[$x]&submit=ok','kreditorrapport','$jsvars')\" onMouseOver=\"this.style.cursor = 'pointer'\"><a>";
			else print "<td><a href=rapport.php?rapportart=kontokort&kilde=openpost&kilde_kto_fra=$konto_fra&kilde_kto_til=$konto_til&regnaar=$regnaar&maaned_fra=$maaned_fra&maaned_til=$maaned_til&konto_fra=$kontonr[$x]&konto_til=$kontonr[$x]&submit=ok>";
...
*/
			...
			$tmp=dkdecimal($y);
			if (abs($y)<0.01) {
				print "<td align=right title=\"Klik her for at udligne &aring;bne poster\"><a href=\"rapport.php?submit=ok&rapportart=openpost&regnaar=$regnaar&maaned_fra=$maaned_fra&maaned_til=$maaned_til&konto_fra=$konto_fra&konto_til=$konto_til&udlign=$id[$x]\">$tmp</a></td>";
			}
