<?php
// -------includes/oioxmlfunk.php-----patch 2.1.0-----2009-12-15--------
// LICENS
//
...
global $db_encode;
	...
	if ($db_encode!="UTF8") {
		...
		$cvr_nr=utf8_encode($r_faktura['kundeordnr']);
	} else {
...
#	$l_retur.="\t\t</com:PaymentAdvice>\n";
	...
	while ($r_linje = db_fetch_array($query)) {
		...
		if (($l_ptype=="PCP")||($l_ptype=="PIP")) { # Hvis papirbaseret
			...
			$l_retur.=sprintf("%01.2f", $l_fortegn*(($r_linje['antal']*$pris)-($r_linje['rabat']*($r_linje['antal']*$pris)/100))); # ordrelinjepris fratrukket rabat
