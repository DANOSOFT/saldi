...
// --------------index/login.php----------lap 3.0.2------2010-05-31------
// LICENS
//
...
if ($row = db_fetch_array($query)) {
	...
	if ($post_max && $db!=$sqdb) {
		...
		$post_antal=$r['box6']*1;
		if (($sqdb=="saldi" || $sqdb=="gratis") && $post_max<=9000 && $post_max < $post_antal ) {
			$diff=$post_antal-$post_max;
			if ($sqdb=="saldi") $txt="Dit maksikale posteringsantal ($post_max) er overskredet.\\n
						Der er i alt foretaget $post_antal posteringer inden for de sidste 12 m&aring;neder.\\n
						N&aelig;ste gang du bogf&oslash;rer en kassekladde vil loftet dit abonnement automatisk blive forh&oslash;jet med \\n
						1000 posteringer til i alt ".($post_max+1000)."\\n
						og der vil blive fremsendt en faktura p&aring; kr. 25 pr. m&aring; for resten af abonnementsperioden.";
			else $txt="Dit maksikale posteringsantal ($post_max) er overskredet.\\n
						Der er i alt foretaget $post_antal posteringer inden for de sidste 12 m&aring;neder.\\n
