<?php
// ----systemdata/regnskabskort.php-----lap 3.0.0----2010-05-17----------
// LICENS
//
...
function aar_x($id, $kodenr, $beskrivelse, $startmd, $startaar, $slutmd, $slutaar, $aaben)
{
	...
	$ny_sum=0;
	$resultat=0;
	$q = db_select("select * from kontoplan where kontotype='D' and regnskabsaar=$tmp order by kontonr",__FILE__ . " linje " . __LINE__);
	$y=0;
	...
	$r=db_fetch_array(db_select("select * from kontoplan where kontotype='X' and regnskabsaar=$tmp",__FILE__ . " linje " . __LINE__));
	$sideskift=$r['kontonr']*1;
	if ($sideskift) {
		$r2=db_fetch_array(db_select("select * from transaktioner where transdate>='$pre_regnstart' and transdate<='$pre_regnslut' and kontonr='$sideskift'",__FILE__ . " linje " . __LINE__));
		$resultat+=afrund($r2['debet']-$r2['kredit'],2);
		$saldosum=$resultat;
		print "<td><br></td>";
