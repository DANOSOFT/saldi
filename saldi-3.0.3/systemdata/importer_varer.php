<?php
// ------systemdata/importer_varer.php---lap 2.1.9--2010-04-29--------
// LICENS
//
...
$felt_navn=array("varenr","beskrivelse","salgspris","kostpris","notes","enhed","gruppe");
$felt_antal=30;
for ($y=0; $y<=$feltantal; $y++) {
	...
	while (!feof($fp)) {
		...
		if ($linje=trim(fgets($fp))) {
			...
			for ($y=0; $y<=$feltantal; $y++) {
				...
				if ((substr($felt[$y],0,1) == '"')&&(substr($felt[$y],-1) == '"')) $felt[$y]=substr($felt[$y],1,strlen($felt[$y])-2);
				if ($feltnavn[$y]=='salgspris')	$felt[$y]=usdecimal($felt[$y]);
				if ($feltnavn[$y]=='kostpris')	$felt[$y]=usdecimal($felt[$y]);
			}
 		}		
		if ($skriv_linje==1){
			$vare_a='';
			...
			if ($r=db_fetch_array(db_select("select id from varer where varenr='$varenr'",__FILE__ . " linje " . __LINE__))) {
				$imp_antal++;
				db_modify("delete from varer where id = '$r[id]'",__FILE__ . " linje " . __LINE__);
echo "insert into varer(id,$vare_a) values ($r[id],$vare_b)<br>";				
				db_modify("insert into varer(id,$vare_a) values ($r[id],$vare_b)",__FILE__ . " linje " . __LINE__);
			} else {
				$imp_antal++;
