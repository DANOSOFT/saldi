<?php
// -------systemdata/formularkort-------lap 3.0.2-----2010.05.30-------
// LICENS
//
...
if ($_POST) {
	$nyt_sprog=addslashes(if_isset($_POST['nyt_sprog']));
	...
	#tjekker om sprog_is er sat og hvis ikke, oprettes sprog_id
...
if ($art_nr==1) {
	...
	print "<td colspan=2></td><td align=center><input type=text style=text-align:right size=5 name=xa[$x] value=".round($row[xa],0).">";
	print "<td align=center><input type=text style=text-align:right size=5 name=ya[$x] value=".round($row[ya],0).">";
	...
	while ($row=db_fetch_array($query)){
		...
		print "<td align=center><input type=text style=text-align:right size=5 name=xa[$x] value=".round($row[xa],0).">";
		print "<td align=center><input type=text style=text-align:right size=5 name=ya[$x] value=".round($row[ya],0).">"; 
		print "<td align=center><input type=text style=text-align:right size=5 name=xb[$x] value=".round($row[xb],0).">";
		print "<td align=center><input type=text style=text-align:right size=5 name=yb[$x] value=".round($row[yb],0).">";
		print "<td align=center><input type=text style=text-align:right size=5 name=str[$x] value=".round($row[str],0).">";
		print "<td align=center><input type=text style=text-align:right size=5 name=color[$x] value=".round($row[color],0).">";
		print "</tr>";
	}	 
	$linjeantal=$x;
} elseif ($art_nr==2) {
	if ($form_nr>=6 && $form_nr<=9) {
		...
		$gebyr=$r['varenr'];
		print "<tr><td colspan=11 align=center title='Skriv det varenummer der skal bruges til rykkergebyr.'>Varenummer for rykkergebyr <input type=text size=15 name=gebyr value=$gebyr></td></tr>";
		$r=db_fetch_array(db_select("select varenr from varer where id ='$rentevnr'",__FILE__ . " linje " . __LINE__)); 
		$rentevnr=$r['varenr'];
		print "<tr><td colspan=11 align=center title='Skriv det varenummer og rentesatsen som bruges ved renteberegning. Rentesatsen g&aelig;lder pr p&aring;begyndt m&aring;ned'>Varenummer/sats for rente <input type=text size=15 name=rentevnr value=$rentevnr><input type=text size=1 name=rentesats value=$rentesats></td></tr>";
...
if ($tmp!=$nyt_sprog) {
...
} elseif ($nyt_sprog && $nyt_sprog!="yes") {
	...
	if (db_fetch_array($q=db_select("select kodenr from grupper where lower(box1) = '$tmp' and art = 'VSPR' ",__FILE__ . " linje " . __LINE__))) {
		...
		$r=db_fetch_array($q=db_select("select max(kodenr) as kodenr from grupper where art = 'VSPR' ",__FILE__ . " linje " . __LINE__));
		$kodenr=$r['kodenr']+1;
		db_modify("insert into grupper (beskrivelse,kodenr,art,box1) values ('sprog','$kodenr','VSPR','$nyt_sprog')");
	...
	print "<option></option>";
	print "</SELECT></td><tr>";
	print "<tr><td colspan=2 align=center><input type=submit accesskey=\"g\" value=\"gem\" name=\"gem\">&nbsp;<input type=submit accesskey=\"a\" value=\"fortryd\"></td></tr>";
	print "</tbody></table></td></tr>";
	exit;
}	
...
function drop_down($x,$form_nr,$art_nr,$formularsprog,$id,$beskrivelse,$xa,$xb,$ya,$yb,$str,$color,$justering,$font,$fed,$kursiv,$side){
	...
	print "</SELECT></td>";
	print "<td align=center><input type=text size=25 name=beskrivelse[$x] value=\"$beskrivelse\"></td>";
	print "<td align=center><input type=text style=text-align:right size=5 name=xa[$x] value=".round($xa,0)."></td>";
	if ($yb != "-") print "<td align=center><input type=text style=text-align:right size=5 name=ya[$x] value=".round($ya,0)."></td>";
	print "<td align=center><input type=text style=text-align:right size=5 name=str[$x] value=".round($str,0)."></td>";
	print "<td align=center><input type=text style=text-align:right size=5 name=color[$x] value=".round($color,0)."></td>";
	...
	print "<td align=center><input type=checkbox name=fed[$x] $fed></td>";
	if ($kursiv=='on') {$kursiv='checked';}
	print "<td align=center><input type=checkbox name=kursiv[$x] $kursiv></td>";
	print "</tr>";
} #endfunc drop_down		
##############################################################################################
function ordrelinjer($form_nr,$art_nr,$formularsprog){
	...
	print "<td align=center><input type=text style=text-align:right size=5 name=xa[$x] value=".round($row['xa'],0)."></td>";
	print "<td align=center><input type=text style=text-align:right size=5 name=ya[$x] value=".round($row['ya'],0)."></td>";
	print "<td align=center><input type=text style=text-align:right size=3 name=xb[$x] value=".round($row['xb'],0)."></td></tr>";
	...
	while ($row=db_fetch_array($query)){
		...
		print "</SELECT></td>";
		print "<td align=center><input type=text style=text-align:right size=5 name=xa[$x] value=".round($row['xa'],0)."></td>";
		print "<td align=center><input type=text style=text-align:right size=5 name=str[$x] value=".round($row['str'],0)."></td>";
		print "<td align=center><input type=text style=text-align:right size=5 name=color[$x] value=".round($row['color'],0)."></td>";
		print "<td><SELECT NAME=justering[$x]>";
		...
		if ($row['beskrivelse']=='beskrivelse'){print "<td align=center><input type=text style=text-align:right size=5 name=xb[$x] value=".round($row['xb'],0)."></td>";}
		print "</tr>";
	}	 
	return($x);
} #endfunc ordrelinjer		
...
if (!$r=db_fetch_array(db_select("select * from formularer where formular = '$form_nr' and art = '3' and beskrivelse = 'generelt' and sprog='$formularsprog'",__FILE__ . " linje " . __LINE__))) {
	...
	print "<td align=center><input type=text style=text-align:right size=5 name=xa[$x] value=\"$header\"></td>\n";
	print "<td align=center><input type=text style=text-align:right size=5 name=ya[$x] value=\"$footer\"></td>\n";
	print "<td align=center><input type=text style=text-align:right size=5 name=xb[$x] value=\"$linespace\"></td></tr>\n";
...
#	$x++;
	...
	while ($r=db_fetch_array($q)){
		...
		print "</SELECT></td>";
		print "<td align=center><input type=text style=text-align:right size=5 name=xa[$x] value=".round($r['xa'],0)."></td>";
		print "<td align=center><input type=text style=text-align:right size=5 name=str[$x] value=".round($r['str'],0)."></td>";
		print "<td align=center><input type=text style=text-align:right size=5 name=color[$x] value=".round($r['color'],0)."></td>";
		print "<td><SELECT NAME=justering[$x]>";
		...
		print "<td align=center><input type=checkbox name=fed[$x] $r[fed]></td>";
		if ($r['kursiv']=='on') {$r['kursiv']='checked';}
		print "<td align=center><input type=checkbox name=kursiv[$x] $r[kursiv]></td>";
		if ($r['beskrivelse']=='beskrivelse'){print "<td align=center><input type=text style=text-align:right size=5 name=xb[$x] value=".round($r['xb'],0)."></td>";}
		print "</tr>";
	}
	if ($footer) {
	$x++;
...
<tr><td	width="100%" height="2.5%" align = "center" valign = "bottom">		
		<table width="100%" align="center" border="0" cellspacing="2" cellpadding="0"><tbody>
			...
			<td width="20%" <?php echo $top_bund ?>" onClick="javascript:window.open('formular_indlaes_std.php', '','left=10,top=10,width=400,height=200,scrollbars=yes,resizable=yes,menubar=no,location=no')" onMouseOver="this.style.cursor = 'pointer'" ><font face="Helvetica, Arial, sans-serif" color="#000000">
