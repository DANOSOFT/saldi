...
// -----------------/kreditor/rapport.php---lap 2.1.4----2010.03.26-----------------
// LICENS
//
...
function forside($regnaar, $maaned_fra, $maaned_til, $konto_fra, $konto_til, $rapportart)
{
	...
	$query = db_select("select * from adresser where art = 'K' order by ".nr_cast('kontonr')."",__FILE__ . " linje " . __LINE__);
	$x=0;
	while ($row = db_fetch_array($query)) {
		$x++;
		$konto_id[$x]=$row['id'];
	...
	print "<td width=\"80%\" $top_bund>Kreditorrapport</td>";
...
function kontokort($regnaar, $maaned_fra, $maaned_til, $konto_fra, $konto_til)
{
	...
	if ($popup && $kilde != 'openpost') $returside="../includes/luk.php";
	...
	if (strstr("kreditorkort.php",$returside)) $regnaar=0;
	...
	if (!is_numeric($maaned_fra)) $maaned_fra=find_md_nr($maaned_fra);
	if (!is_numeric($maaned_til)) $maaned_til=find_md_nr($maaned_til);
	...
	print "<td width=\"10%\" $top_bund><a href=$returside>Luk</a></td>";
	print "<td width=\"80%\" $top_bund>Kreditorrapport - kontokort</td>";
	print "<td width=\"10%\" $top_bund><br></td>";
	...
	for ($x=1; $x<=12; $x++) {
		if ($startmaaned==$md[$x]){$startmaaned=$x;}
		if ($slutmaaned==$md[$x]){$slutmaaned=$x;}
		if (strlen($startmaaned)==1){$startmaaned="0".$startmaaned;}
		if (strlen($slutmaaned)==1){$slutmaaned="0".$slutmaaned;}
	}
	
	if (!is_numeric($startmaaned)) $startmaaned=find_md_nr($startmaaned);
	if (!is_numeric($slutmaaned)) $slutmaaned=find_md_nr($slutmaaned);
 	
	if ($maaned_fra) {$startmaaned=$maaned_fra;}
	if ($maaned_til) {$slutmaaned=$maaned_til;}
	if ($aar_fra && is_numeric($aar_fra)) {$startaar=$aar_fra;}
	if ($aar_til && is_numeric($aar_til)) {$slutaar=$aar_til;}
...
#	print "<tr><td colspan=5>Firmanavn</td></tr>";
	$kontonr=array();
	$x=0;
	$query = db_select("select id from adresser where ".nr_cast('kontonr').">='$konto_fra' and ".nr_cast('kontonr')."<='$konto_til' and art = 'K' order by ".nr_cast('kontonr')."",__FILE__ . " linje " . __LINE__);
	while ($row = db_fetch_array($query)) {
		$x++;
		$konto_id[$x]=$row['id'];
	}
	...
	for ($x=1; $x<=$kontoantal; $x++) {
		...
		while ($r2 = db_fetch_array($q2)) {
			...
			if ($valuta!="DKK") { # kreditors valuta er fremmed.
				# s�ger f�rst efter en valutakurs p� transaktionsdatoen - hvis den ikke findes tages �ldste kurs. 
...
# <- 2009.05.05				
		...
		print "<tr bgcolor=\"$linjebg\">";
				...
				$tmp=dkdecimal($tmp);
					if (!$forfaldsdag && $r2['faktnr']) $forfaldsdag=usdate(forfaldsdag($transdate, $betalingsbet, $betalingsdage));
				if ($r2['udlignet']!='1' && $forfaldsdag<$currentdate){$stil="<span style='color: rgb(255, 0, 0);'>";}
				else {$stil="<span style='color: rgb(0, 0, 0);'>";}
				if ($amount < 0) {
					if ($r2['udlignet']!='1') print "<td>$stil".dkdato($forfaldsdag)."</td><td></td><td align=right><span style='color: rgb(255, 0, 0);'> <a href=\"../includes/udlign_openpost.php?post_id=$r2[id]&regnaar=$regnaar&maaned_fra=$maaned_fra&maaned_til=$maaned_til&konto_fra=$konto_fra&konto_til=$konto_til&retur=../kreditor/rapport.php\">$tmp</a></td>";
					else print "<td>$stil".dkdato($forfaldsdag)."</td><td></td><td align=right><span style='color: rgb(0, 0, 0);'> $tmp</td>";
					$forfaldsum=$forfaldsum+$amount;
				} else {
					if ($r2['udlignet']!='1') print "<td></td><td align=right><span style='color: rgb(255, 0, 0);'> <a href=\"../includes/udlign_openpost.php?post_id=$r2[id]&regnaar=$regnaar&maaned_fra=$maaned_fra&maaned_til=$maaned_til&konto_fra=$konto_fra&konto_til=$konto_til&retur=../kreditor/rapport.php\">$tmp</a><td></td></td>";
...
#	global $connection;
 ...
 # $maaned_til=$maaned_til*1;
	...
	print "<td width=\"80%\" $top_bund>Kreditorrapport - Kontosaldo</td>";
	print "<td width=\"10%\" $top_bund><br></td>";
	print "</tbody></table>"; #B slut
	print "</td></tr>";
...
#	print "<tr><td colspan=5>Firmanavn</td></tr>";
	$kontonr=array();
	$x=0;
	$query = db_select("select id from adresser where ".nr_cast('kontonr').">='$konto_fra' and ".nr_cast('kontonr')."<='$konto_til' and art = 'K' order by firmanavn",__FILE__ . " linje " . __LINE__);
	while ($row = db_fetch_array($query)) {
		$x++;
		$konto_id[$x]=$row[id];
	}
