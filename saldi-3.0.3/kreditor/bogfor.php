...
@session_start();
$s_id=session_id();
// -------------------kreditor/bogfor.php-------lap 3.0.3------2010.06.01-----------------------
/// LICENS
//
	...
	if ($fejl==0){
		...
		while ($row = db_fetch_array($query)){
			if (($row[posnr]>0)&&(strlen(trim(($row[varenr])))>0)){
				...
				if ($valutakurs) $dkpris[$x]=afrund(($pris[$x]*$valutakurs/100),3); # Omregning til DKR.		
			...
			if (($vare_id[$x])&&($antal[$x]!=0)) {
				...
				if ($box8!='on'){
					db_modify("update ordrelinjer set bogf_konto='$box3' where id='$linje_id[$x]'",__FILE__ . " linje " . __LINE__);
						if ($r=db_fetch_array(db_select("select id from batch_kob where linje_id=$linje_id[$x]",__FILE__ . " linje " . __LINE__))) {
...
# følgende 4 linjer indsat 20100513 i forbindelse med aendring af bogforing af debitorordrer
# echo "select kostpris from ordrelinjer where id='$r2[linje_id]'<br>"; 							
								$r3=db_fetch_array(db_select("select kostpris from ordrelinjer where id='$r2[linje_id]'",__FILE__ . " linje " . __LINE__));
								$kostpris=$r3['kostpris'];
								if ($box1) {
# echo "$diff=$dkpris[$x]-$kostpris*1<br>";									
									$diff=$dkpris[$x]-$kostpris*1;
#									$diff=$dkpris[$x]-$row['pris']*1; #rem'et 20100513
...
#		else {$moms=afrund($r['sum']*$r['momssats']/100,2);}
		...
		if ($box1 && $box1!='E') $sum=$sum+$moms;	#moms tillaegges summen der ikke er eu moms.
########### OPENPOST	-> 	
		...
		db_modify("insert into openpost (konto_id, konto_nr, faktnr, amount, beskrivelse, udlignet, transdate, kladde_id, refnr, valuta, valutakurs) values ('$konto_id', '$lev_kontonr', '$fakturanr', $sum*-1, '$beskrivelse', '0', '$transdate', '0', '$id', '$valuta', '$valutakurs')",__FILE__ . " linje " . __LINE__);
########### <- OPENPOST	
		$tekst=findtekst(157,$sprog_id);
		if ($kontonr) {
#echo "select id from kontoplan where kontonr='$kontonr' and regnskabsaar = '$regnaar' and lukket!='on'<br>";			
...
#exit;		
		if ($sum) {
			db_modify("insert into transaktioner (bilag, transdate, beskrivelse, kontonr, faktura, debet, kredit, kladde_id, afd, logdate, logtime, projekt) values ('$bilag', '$transdate', '$beskrivelse', '$kontonr', '$fakturanr', '$debet', '$kredit', '0', $afd, '$logdate', '$logtime', '$projekt[0]')",__FILE__ . " linje " . __LINE__);
		}
			...
			for ($p=1;$p<=$projektantal;$p++) {	
				...
				while ($r = db_fetch_array($q)) {
					...
					$bogf_konto[$y]=$r['bogf_konto'];
						$pris[$y]=$r['pris']*$r['antal']-afrund($r['pris']*$r['antal']*$r['rabat']/100,3);
						$pris[$y]=afrund($pris[$y],2); 
					}
					else {
						for ($a=1; $a<=$y; $a++) {
							if ($bogf_konto[$a]==$r['bogf_konto']) {
								$pris[$a]=$pris[$a]+($r['pris']*$r['antal']-afrund($r['pris']*$r['antal']*$r['rabat']/100,3));
								$pris[$a]=afrund($pris[$a],2);
							}
						}		 
					}
				}
				...
				for ($y=1;$y<=$ordrelinjer;$y++) {
					if ($bogf_konto[$y]) {
						...
						if ($t==1 && $valutakurs) {$kredit=afrund($kredit*$valutakurs/100,3);$debet=afrund($debet*$valutakurs/100,3);} # Omregning til DKR.		
						$d_kontrol=$d_kontrol+$debet; $k_kontrol=$k_kontrol+$kredit;
						$debet=afrund($debet,2);
						$kredit=afrund($kredit,2);
						if ($pris[$y]) db_modify("insert into transaktioner (bilag, transdate, beskrivelse, kontonr, faktura, debet, kredit, kladde_id, afd, logdate, logtime, projekt) values ('$bilag', '$transdate', '$beskrivelse', '$bogf_konto[$y]', '$fakturanr', '$debet', '$kredit', '0','$afd', '$logdate', '$logtime', '$projekt[$p]')",__FILE__ . " linje " . __LINE__);
					}
				}
			}
		}
...
#echo "select box1, box2, box3 from grupper where art='EM' and kodenr='$box1'<br>";			
...
#echo "moms $moms<br>";			
			if ($moms) {
#echo "insert into transaktioner (bilag, transdate, beskrivelse, kontonr, faktura, debet, kredit, kladde_id, afd, logdate, logtime, projekt) values ('$bilag', '$transdate', '$beskrivelse', '$emomskto', '$fakturanr', '$debet', '$kredit', '0', '$afd', '$logdate', '$logtime', '$projekt[0]')<br>";				
				db_modify("insert into transaktioner (bilag, transdate, beskrivelse, kontonr, faktura, debet, kredit, kladde_id, afd, logdate, logtime, projekt) values ('$bilag', '$transdate', '$beskrivelse', '$emomskto', '$fakturanr', '$debet', '$kredit', '0', '$afd', '$logdate', '$logtime', '$projekt[0]')",__FILE__ . " linje " . __LINE__);
			}
####################
		} else {
		...
		if ($moms) {
			db_modify("insert into transaktioner (bilag, transdate, beskrivelse, kontonr, faktura, debet, kredit, kladde_id, afd, logdate, logtime, projekt) values ('$bilag', '$transdate', '$beskrivelse', '$kmomskto', '$fakturanr', '$debet', '$kredit', '0', '$afd', '$logdate', '$logtime', '$projekt[0]')",__FILE__ . " linje " . __LINE__);
		}
