<?php
// --------------------------kreditor/ordreliste.php---lap 2.0.9------2009-09-24------
// LICENS
//
...
if ($valg) {
	...
	list ($ordrenumre, $kontonumre, $fakturanumre, $ordredatoer, $lev_datoer, $fakturadatoer, $genfaktdatoer, $summer, $firma, $kontoid, $ref[0], $sort, $valg, $nysort, $modtagelsesnumre) = split(";", $_COOKIE['kred_ord_lst']);#
}
...
if ($valg=='faktura') {
	print " <td align=right width=50><b><a href='ordreliste.php?nysort=modtagelse&sort=$sort&valg=$valg$hreftext'>Modtnr.</b></td>";
	print " <td align=right width=50><b><a href='ordreliste.php?nysort=fakturanr&sort=$sort&valg=$valg$hreftext'>Faktnr.</b></td>";
}
...
if ($valg=='forslag') {print "<td><b><a href='ordreliste.php?nysort=ordredate&sort=$sort&valg=$valg$hreftext'>Forslagsdato</b></td>";}
	...
	print "<td><b><a href='ordreliste.php?nysort=levdate&sort=$sort&valg=$valg$hreftext'>Modt.dato</b></td>";
}
if ($valg=='faktura') {
...
if ($valg=='faktura') {
print "<td align=right><span title= 'Angiv et modtagelsesnummer eller angiv to adskilt af kolon (f.eks 345:350)'><input type=text size=5 name=modtagelsesnumre value=$modtagelsesnumre></td>";
...
if ($valg!='forslag') {print "<td><span title= 'Angiv en dato eller angiv to adskilt af kolon (f.eks 010605:300605)'><input type=text size=10 name=lev_datoer value=$lev_datoer></td>";}
if ($valg=='faktura') {
...
print "<td><span title= 'Navn på modtager. Der kan søges med * før og efter tekstes'><input type=text size=10 name=lev_navne value=$lev_navne></td>";
...
if ($valg=="forslag") { 
	...
	while ($row =db_fetch_array($query)) {
		...
		if ($row['art']=='KK'){print "<td align=right $javascript> (KN)&nbsp;$linjetext $understreg $row[ordrenr]$hrefslut</span><br></td>";}
...
elseif ($valg=='ordrer') {
	...
	while ($row =db_fetch_array($query)){
		...
		print "<tr bgcolor=\"$linjebg\">";
		if ($row['art']=='KK'){print "<td align=right $javascript>(KN)&nbsp;$understreg $linjetext $row[ordrenr]</span><br>$hrefslut</td>";}
	...
	while ($row =db_fetch_array($query)) {
	  ...
	  $ordre="ordre".$row['id'];
		...
		print "<tr bgcolor=\"$linjebg\">";
		if ($popup) {
			if ($row['art']=='KK'){print "<td align=right $javascript ><span style='color: rgb(255, 0, 0); text-decoration: underline;'>$row[ordrenr]<br></span></td>";}
