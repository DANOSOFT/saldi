<?php
	@session_start();
	$s_id=session_id();
// -------kreditor/modtag.php-------patch 2.0.7-------2009.05.25-----
// LICENS
//
...
#			$pris[$x]=$row['pris']-($row['pris']*$row['rabat']/100);
	...
	for ($x=1; $x<=$linjeantal; $x++) {
		...
		if (($leveres[$x]<0)&&($serienr[$x])){
			$sn_antal[$x]=0; 
			$query = db_select("select * from serienr where kobslinje_id = -$kred_linje_id[$x] and batch_salg_id<=0",__FILE__ . " linje " . __LINE__);
			while ($row = db_fetch_array($query)) {
				$sn_antal[$x]=$sn_antal[$x]+1000;
				$y=$sn_antal[$x]+$x;
				$sn_id[$y]=$row[id];
