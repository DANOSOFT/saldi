<?php
// -----------------lager/lagerstatus.php----lap 3.0.3-----2010-06-01--
	// LICENS
	//
	...
	//
	// Copyright (c) 2006-2010 DANOSOFT ApS
// ----------------------------------------------------------------------
...
print "<tr><td colspan=8 align=center>Dato:&nbsp;<input type=text name=dato value=$dato size=10></td>";
...
#			$batch_pris[$x]=$batch_pris[$x]-($r1['pris']*$r2['antal']);
	...
	if (($batch_k_antal[$x] != 0)||($batch_s_antal[$x] != 0)) {
		...
		if ($date==$dd && $batch_t_antal[$x]!=$beholdning[$x]) print "<td align=right title=\"Beholdning ($beholdning[$x]) stemmer ikke med det antal som er k&oslash;bt og solgt. Klik her for at opdatere beholdning\"><a href=".$_SERVER['PHP_SELF']."?vare_id=$vare_id[$x]&beholdning=$batch_t_antal[$x]&opdater=on onclick=\"return confirm('Opdater lagerbeholdning fra $beholdning[$x] til $batch_t_antal[$x] for denne vare?')\"><span style=\"color: rgb(255, 0, 0);\">$batch_t_antal[$x]</span></a>&nbsp;</td>";
		else print "<td align=right>$batch_t_antal[$x]&nbsp;</td>";
		print "<td align=right>".dkdecimal($batch_pris[$x])."</td>
		<td align=right title='stkpris:".dkdecimal($kostpris[$x])."'>&nbsp;".dkdecimal($kostpris[$x]*$batch_t_antal[$x])."</td>
		<td align=right>&nbsp;".dkdecimal($salgspris[$x]*$batch_t_antal[$x])."</td></tr>";
		$lagervalue=$lagervalue+$batch_pris[$x];$kostvalue=$kostvalue+$kostpris[$x]; $salgsvalue=$salgsvalue+($salgspris[$x]*$batch_t_antal[$x]);
	}
}
print "<tr><td colspan=9><hr></td></tr>";
print "<tr><td colspan=2></td><td>Samlet lagerv&aelig;rdi pr. $dato</td><td align=right></td><td align=right></td>
	...
	<td align=right>".dkdecimal($kostvalue)."</td>
