<?php
// -----------------------admin/restore.php------------lap 2.0.9-------2009-10-07-----------
// LICENS
//
...
if($_POST) {
	...
	if (basename($_FILES['uploadedfile']['name'])) {
		...
		if(move_uploaded_file($tmp, $filnavn)) {
			...
			if ($fp) {
				$linje=trim(fgets($fp));
				list($backupdate,$backupdb,$backupver,$backupnavn,$backup_encode,$backup_dbtype)=split(chr(9),$linje);
...
function upload(){
	...
	print "<tr><td width=100% align=center>V&aelig;lg datafil: <input name=\"uploadedfile\" type=\"file\"></td></tr>";
...
if (!$backup_encode) $backup_encode="LATIN9";
