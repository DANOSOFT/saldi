...
// 20260914 CDX/MJ SD-186 wrote these assertions against a select named 'hvem'. On this branch
//                  ordrer.hvem is the order-lock owner and the performer lives in
//                  ordrer.performed_by, which is the column the save path reads, so the field
//                  names here follow the rename. The behaviour under test is unchanged.
final class OrderEmployeeFieldsTest extends TestCase
{
    ...
    {
        $html = renderOrderEmployeeFields($employees, $reference, $performedBy, 'Vor ref.', 'Udført af', $disabled);
    ...
    public function testBothFieldsShareEmployeesWithIndependentSelectionsInOneRow(): void
    {
        ...
        self::assertSame($this->options($fields, 'ref'), $this->options($fields, 'performed_by'));
        self::assertSame('Anna', $fields->evaluate('string(//select[@name="ref"]/option[@selected]/@value)'));
        self::assertSame('Søren', $fields->evaluate('string(//select[@name="performed_by"]/option[@selected]/@value)'));
        self::assertSame(1, $fields->query('//tr[td/select[@name="ref"] and td/select[@name="performed_by"]]')->length);
        self::assertSame('Udført af', $fields->evaluate('string(//label[@for="order-performed_by"])'));
        self::assertSame('ref', $fields->query('//select')->item(0)->getAttribute('name'));
        self::assertSame('performed_by', $fields->query('//select')->item(1)->getAttribute('name'));
    }

    public function testBlankPerformedByStaysBlankAndCanBeRenderedWithoutEmployees(): void
    {
        $fields = $this->fields(['Anna'], 'Anna', null);
        self::assertSame('', $fields->evaluate('string(//select[@name="performed_by"]/option[@selected]/@value)'));
        self::assertSame('Anna', $fields->evaluate('string(//select[@name="ref"]/option[@selected]/@value)'));
        $empty = $this->fields([], null, null);
        self::assertSame([''], $this->options($empty, 'ref'));
        self::assertSame([''], $this->options($empty, 'performed_by'));
    ...
    public function testHistoricalSelectionIsPreservedWithoutAddingItToTheOtherField(): void
    {
        ...
        self::assertSame(['', 'Anna', 'Former employee'], $this->options($fields, 'performed_by'));
        self::assertSame('Former employee', $fields->evaluate('string(//select[@name="performed_by"]/option[@selected]/@value)'));
        self::assertSame('Former employee', $fields->evaluate('string(//input[@name="oldperformed_by"]/@value)'));
    }
    ...
    public function testNamesWithQuotesAndMarkupRoundTripAsText(): void
    {
        ...
        foreach (['ref', 'performed_by'] as $name) {
