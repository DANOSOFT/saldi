	...
	if ($procenttillag) {
		...
		if ($procentvare) {
			...
			if ($r['id']) {
				...
				opret_ordrelinje($id,$r['id'],$r['varenr'],1,$tmp,$tillag,0,100, $art,'',$posnr,0,'','on','percent','','','','','',$lager,__line__); #20140426
...
#				if (!in_array($tmp,$bogf_vat) && $r['bogf_konto']) {
				...
				if (!in_array($r['bogf_konto'],$bogf_konto)) {
					...
					if ($r['rabatart']=='amount') {
						...
						if ($r['procent'] || $r['procent']!='') $linjesum*=$r['procent']/100;
						$pris[$y]=$linjesum; #20140424b
						} else {
...
if ($brugernavn == 'saldi') echo __line__." Vat $linjesum linjemoms[$y] $linjemoms[$y]<br>";
				}	else {
					for ($a=1; $a<=$y; $a++) {
...
if ($brugernavn == 'saldi') echo __line__." $linjesum linjemoms[$a] $linjemoms[$a]<br>";
						}
					}
				}
			}
...
#						$linjemoms[$y]=afrund($linjemoms[$y]*1,2);
						...
						db_modify($qtxt,__FILE__ . " linje " . __LINE__);
						$tmp=$debet-$kredit;
if ($brugernavn == 'saldi') echo __line__." $qtxt<br>";
...
if ($brugernavn == 'saldi') echo __line__." $debet ($d_kontrol) != $kredit ($k_kontrol)<br>";
							if ($t==1 && $valutakurs) {$kredit=$kredit*$valutakurs/100;$debet=$debet*$valutakurs/100;} # Omregning til DKR.
							$kredit=afrund($kredit,3);$debet=afrund($debet,3);
							$d_kontrol=$d_kontrol+$debet; $k_kontrol=$k_kontrol+$kredit;
if ($brugernavn == 'saldi') echo __line__." $d_kontrol != $k_kontrol<br>";
							$debet=afrund($debet,2);
							$kredit=afrund($kredit,2);
							if (!$vat_account[$y]) {
