	...
	if ($r = db_fetch_array($q)) {
		...
		if ($betaling == 'Kontant') {
			...
			if ($div_kort_kto) { #20140129
				if (strpos($betaling, "|"))
					list($betaling, $kortnavn) = explode("|", $betaling);
				if (strpos($betaling2, "|"))
					list($betaling2, $kortnavn) = explode("|", $betaling2);
		...
		for ($p = 1; $p <= $projektantal; $p++) {
			...
			while ($r = db_fetch_array($q)) {
				...
				if (!in_array($r['bogf_konto'], $bogf_konto)) {
					...
					if ($brugernavn == 'saldi')
						echo __LINE__ . " Vat $linjesum linjemoms[$y] $linjemoms[$y]<br>";
				} else {
					for ($a = 1; $a <= $y; $a++) {
						if ($bogf_konto[$a] == $r['bogf_konto'] && $vat_account[$a] == $r['vat_account']) {
							...
							if ($brugernavn == 'saldi')
								echo __LINE__ . " $linjesum linjemoms[$a] $linjemoms[$a]<br>";
						}
					}
				}
			}
			...
			for ($y = 1; $y <= $ordrelinjer; $y++) {
				if ($bogf_konto[$y] && $pris[$y]) {
					...
					if ($linjemoms[$y]) {
						...
						if ($brugernavn == 'saldi')
							echo __LINE__ . " $debet ($d_kontrol) != $kredit ($k_kontrol)<br>";
						if ($t == 1 && $valutakurs) {
							$kredit = $kredit * $valutakurs / 100;
							$debet = $debet * $valutakurs / 100;
						} # Omregning til DKR.
