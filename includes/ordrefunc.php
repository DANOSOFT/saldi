...
// Denne funktion kontrollerer levering og kalder funktioner som registrerer salget i tabellerne varer,batch_salg og
// batch_kob
global $afd_lager;
global $regnaar;
global $levdate;
...
#fwrite ($log, __line__." Regnaar $regnaar\n");
	...
	$qtxt=NULL;
	if (isset($vare_id) && $vare_id) $qtxt="select * from varer where id='$vare_id'";
	elseif ($varenr) {
		...
		if (strlen($varenr)==12 && is_numeric($varenr)) $qtxt.=" or stregkode='0$varenr'";
	...
	if ($qtxt && $r=db_fetch_array(db_select("$qtxt",__FILE__ . " linje " . __LINE__))) {
		...
		if ($bogfkto && !$momsfri) {
			...
			if ($r = db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__))) {
				...
				if ($r = db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__))) {
					if ($r['box1']) $vatAccount   = (int)$r['box1'];
					if ($r['box2']) $varemomssats = (float)$r['box2'];
				}
			} else {
		...
		if (!$pris) {
			...
			if ($r2=db_fetch_array(db_select($qtxt,__FILE__ . " linje " . __LINE__))) {
				...
				if ($pris!='0') $pris=$productPrice; #20140124
