...
// ---------------------includes/formfunk.php ------patch 3.2.4----2011-11-03--------------
// LICENS
//
...
#$logoart='PS';
#echo $logo;
#echo "Y retur fra bundtekst: $y $logoart=='PS'<br>";
	...
	if ($x && $tekst && $y2/2.86>$Opkt) fwrite($fp,"/$form_font\n$str scalefont\nsetfont\nnewpath\n$x $y2 moveto (".$tekst.") $format show\n");
	return $y;
}
...
#echo "text $tekst<br>";
	...
	for ($z=1; $z<=strlen($tekst); $z++){
		 ...
		 if (strstr($tegn,"\n")){
				...
				if (strlen($nytekst)>=1){
					$tmp=$y;
# if ($y>=$Opkt) echo "$Opkt ombryd $x $y $nytekst<br>";
					if ($y>=$Opkt) $y=skriv($str,$fed,$italic,$color,$nytekst,$tekstinfo,$x,$y,$format,$form_font,$formular);
					if  ($y!=$tmp) { #aendret 2011.01.27 grundet manglemde linje efter sideskift - saldi_2 ordre id 3310
#					if ($y<$Opkt) {
...
# echo "nytext $nytekst<br>";
	$nytekst=trim($nytekst);
	if (strlen($nytekst)>=1){
		$y=skriv($str,$fed,$italic,$color,$nytekst,$tekstinfo,$x,$y,$format,$form_font,$formular);
	}
# echo "returnerer $y<br>";
	return $y;
}
...
#echo "select * from varer where id IN (select yb from formularer where beskrivelse='GEBYR' and formular='$formular')<br>";
...
system("rm -r $mappe");
$mappe="../temp/$db/".$bruger_id."_".date("his");
mkdir("$mappe", 0775);
if ($ordre_antal>1) {
...
#	$ordremomssum=$row['moms'];
	...
	if (file_exists("../logolib/$db_id/$formular.ps")) {
	...
	if ($ordre_id[$q]){
		...
		while($row = db_fetch_array($query)){
			if ($row['posnr']>0 && (!$row['samlevare'] || !is_numeric($row['samlevare']))){
				$x++;
...
#					if ($b<1) $b="0".$b; # remmet 20100729 - 0,5 og 0,05 bliver begge til 0,05
					...
					if ($momsfri[$x]!='on') {
						...
						if ($incl_moms && !$b2b) {
#echo "$varemomssats[$x] pris $row[pris]<br>";
...
#echo "Sum $sum<br>";;
			...
			if ($x>0 && $formular<4 && !$tmp && $m_rabat[$x]) {
				$y=$x;
				$x++;
				$posnr[$x]=$posnr[$y];
				$vare_id[$x]=$vare_id[$y];
...
if ($logoart=='PS') fwrite($fp,"showpage\n");
else fwrite($fp, $logo);
# echo "Bundtekst returnerer: $y<br>";
