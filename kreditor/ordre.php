...
if ((is_numeric($status) && $status < 3) || (isset($_POST['credit'])) || isset($_POST['copy'])) { #20120816
	...
	if (!$id && $konto_id && $firmanavn) {
		...
		for ($x = 1; $x <= $linjeantal; $x++) {
			...
			if (!$vare_id[$x]) {
				$vare_id[$x] = find_vare_id($varenr[$x]);
			}
...
if ($submit == 'lookup') {
	...
	if ((strstr($fokus, 'kontonr')) && ($id)) {
		kontoopslag($sort, $fokus, $id, $kontonr);
	}
	if ((strstr($fokus, 'firmanavn')) && (!$id)) {
		kontoopslag($sort, $fokus, $id, $firmanavn);
	}
	if ((strstr($fokus, 'addr1')) && (!$id)) {
...
if (($submit == 'receive' || $submit == 'return') && $bogfor != 0) {
	...
	if (!$row = db_fetch_array($query)) {
		print "Du kan ikke modtage uden ordrelinjer";
	} else {
		print "<meta http-equiv=\"refresh\" content=\"0;URL=modtag.php?id=$id\">";
	}
}
...
function sidehoved($id, $returside, $kort, $fokus, $tekst)
{
	...
	if ($menu == 'T') {
		...
		if (($kort != "../lager/varekort.php" && $returside != "ordre.php") && ($id)) {
			print "<div class=\"headerbtnRght headLink\"><a accesskey=N href=\"javascript:confirmClose('ordre.php?returside=ordreliste.php','$alerttekst')\" title='Klik her for at lave ny ordre'><i class='fa fa-plus-square fa-lg'></i></a></div>";
		} else if (($kort == "../lager/varekort.php" && $returside == "ordre.php") && ($id)) {
			print "<div class=\"headerbtnRghtheadLink\"><a accesskey=N href=\"$kort?returside=$returside&ordre_id=$id\"  title='Klik her for at lave ny ordre'><i class='fa fa-plus-square fa-lg'></i></a></div>";
			...
			print "<div class=\"headerbtnRght headLink\"><a accesskey=N href=\"javascript:confirmClose('$kort?returside=../kreditor/ordre.php&ordre_id=$id&fokus=$fokus','$alerttekst')\" title='Klik her for at lave ny ordre'><i class='fa fa-plus-square fa-lg'></i></a></div>";
		} else {
		...
		if (($kort != "../lager/varekort.php" && $returside != "ordre.php") && ($id)) {
			print "<td width=\"10%\" $top_bund> $color<a href=\"javascript:confirmClose('ordre.php?returside=ordreliste.php','$alerttekst')\" accesskey=N>" . findtekst(39, $sprog_id) . "</a></td>";
		} else if (($kort == "../lager/varekort.php" && $returside == "ordre.php") && ($id)) {
			print "<td width=\"10%\" $top_bund> $color<a href=\"$kort?returside=$returside&ordre_id=$id\" accesskey=N>" . findtekst(39, $sprog_id) . "</a></td>";
		} elseif ($kort == "../kreditor/kreditorkort.php") {
