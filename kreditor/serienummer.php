<?php
// -------------kreditor/serienummer.php----------lap 3.2.8-----2012-02-21----
// LICENS
//
      ...
      for ($x=1; $x<=$sn_antal; $x++) {
        if (trim($valg[$x])=="on") {
          ...
          if ($y>=$leveres+$leveret) {
						db_modify("update serienr set kobslinje_id=-$kred_linje_id where id=$sn_id[$x]",__FILE__ . " linje " . __LINE__);
					}
        } elseif ($sn_id[$x]) {
				db_modify("update serienr set kobslinje_id=$kred_linje_id where id=$sn_id[$x]",__FILE__ . " linje " . __LINE__);}
      }
...
if ($antal>0) {
  ...
  $query = db_select("select * from serienr where salgslinje_id<= 0 and (kobslinje_id =$kred_linje_id  or kobslinje_id =-$kred_linje_id) order by serienr",__FILE__ . " linje " . __LINE__);
  while ($row = db_fetch_array($query)) {
    if ($row['batch_kob_id']>=0) { #Hvis batch_kob_id er negativ er varen returneret.
      ...
      if ($row['kobslinje_id']<0) {
				if ($sn_antal+$leveret<=0) print " disabled";
				print " checked></td></tr>";
