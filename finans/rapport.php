...
// -------------finans/rapport.php-------lap 3.2.8------2012-02-19-------
// LICENS
//
...
if ($_POST){
	...
	if ($delprojekt) {
		...
		for ($a=0;$a<$b;$a++) {
			...
			for($d=$c;$d<$prcfg[$a];$d++) $delprojekt[$a]="?".$delprojekt[$a];  
	...
	if ($antal_prj) {
		...
		if (strpos($prj_cfg,'|')) {
			...
			for($y=0;$y<$cols;$y++) {
				$width=$prcfg[$y]*10;
				$width=$width."px";
				print "<input class=\"inputbox\" type=\"text\" name=\"delprojekt[$y]\" style=\"width:$width\" value=\"".mb_substr($projekt_fra,$pos,$prcfg[$y],$db_encode)."\">";
				$pos+=$prcfg[$y];
			}
...
#	print "  <a accesskey=L href=\"rapport.php?rapportart=Kontokort&regnaar=$regnaar&dato_fra=$startdato&maaned_fra=$mf&dato_til=$slutdato&maaned_til=$mt&konto_fra=$konto_fra&konto_til=$konto_til&afd=$afd\">Luk</a><br><br>";
	...
	if ($afd||$ansat_fra||$projekt_fra) {
		...
		if ($projekt_fra && $projekt_til && $projekt_fra!=$projekt_til) $dim = $dim." and projekt >= '$projekt_fra' and projekt <= '$projekt_til' ";
		elseif ($projekt_fra) {
			$tmp=str_replace("?","_",$projekt_fra);
			$dim = $dim."and projekt LIKE '$tmp' ";
		}
	}	
	$x=0;
	...
	for ($x=1; $x<=$kontoantal; $x++){
		...
		if (in_array($kontonr[$x], $ktonr)||$primo[$x]){
			...
			while ($row = db_fetch_array($query)){
				...
				print "<tr bgcolor=\"$linjebg\"><td>  ".dkdato($row['transdate'])." </td><td>$row[bilag] </td><td>$kontonr[$x] : $row[beskrivelse] </td>";
...
#################################################################################################
function kontokort_moms($regnaar, $maaned_fra, $maaned_til, $aar_fra, $aar_til, $dato_fra, $dato_til, $konto_fra, $konto_til, $rapportart, $ansat_fra, $ansat_til, $afd, $projekt_fra, $projekt_til) {
...
#	print "  <a accesskey=L href=\"rapport.php?rapportart=Kontokort&regnaar=$regnaar&dato_fra=$startdato&maaned_fra=$mf&dato_til=$slutdato&maaned_til=$mt&konto_fra=$konto_fra&konto_til=$konto_til&afd=$afd\">Luk</a><br><br>";
	...
	print "<tr><td colspan=\"6\" height=\"8\">";
	print "<table width=\"100%\" align=\"center\" border=\"0\" cellspacing=\"3\" cellpadding=\"0\"><tbody>"; #B
	print "<td width=\"10%\" $top_bund><font face=\"Helvetica, Arial, sans-serif\" color=\"#000066\"><a accesskey=L href=\"rapport.php?rapportart=kontokort_moms&regnaar=$regnaar&dato_fra=$startdato&maaned_fra=$mf&aar_fra=$aar_fra&dato_til=$slutdato&maaned_til=$mt&aar_til=$aar_til&konto_fra=$konto_fra&konto_til=$konto_til&ansat_fra=$ansat_fra&ansat_til=$ansat_til&afd=$afd&projekt_fra=$projekt_fra&projekt_til=$projekt_til\">Luk</a></td>";
	print "<td width=\"80%\" $top_bund><font face=\"Helvetica, Arial, sans-serif\" color=\"#000066\"> Rapport - kontokort men moms</td>";
	print "<td width=\"10%\" $top_bund><font face=\"Helvetica, Arial, sans-serif\" color=\"#000066\"><br></td>";
	print "</tbody></table>"; #B slut
	print "</td></tr>";
...
#			if (!$dim) print "<tr bgcolor=\"$linjebg\"><td></td><td></td><td>  Primosaldo </td><td></td><td></td><td align=right>$tmp </td></tr>";
			...
			while ($r = db_fetch_array($q)){
				($linjebg!=$bgcolor5)?$linjebg=$bgcolor5:$linjebg=$bgcolor;
				print "<tr bgcolor=\"$linjebg\"><td>  ".dkdato($r['transdate'])." $r[kladde_id]</td><td onMouseOver=\"this.style.cursor = 'pointer'\"; onClick=\"javascript:kassekladde=window.open('kassekladde.php?id=$r[kladde_id]&returside=../includes/luk.php','kassekladde','$jsvars')\">$r[bilag]</td><td>$kontonr[$x] : $r[beskrivelse]</td>";
				$xmoms=$r['debet']-$r['kredit'];
				print "<td align=right>".dkdecimal($xmoms)."</td>";
				$moms=$r['moms'];
				if (!$moms && $moms!='0.000' && $r['bilag']&& $r['kladde_id']) {
					$q2=db_select("select * from transaktioner where transdate='$r[transdate]' and bilag='$r[bilag]' and logdate='$r[logdate]' and logtime='$r[logtime]'and beskrivelse='$r[beskrivelse]' $momsq",__FILE__ . " linje " . __LINE__);
...
#################################################################################################
function regnskab0($regnaar, $maaned_fra, $maaned_til, $aar_fra, $aar_til, $dato_fra, $dato_til, $konto_fra, $konto_til, $rapportart, $ansat_fra, $ansat_til, $afd, $projekt_fra, $projekt_til) 
{
	global $connection;
	global $top_bund;
	global $md;
...
#	print "  <a accesskey=L href=\"rapport.php?rapportart=$rapportart&regnaar=$regnaar&dato_fra=$startdato&maaned_fra=$mf&dato_til=$slutdato&maaned_til=$mt&konto_fra=$konto_fra&konto_til=$konto_til&afd=$afd\">Luk</a><br><br>";
	...
	print "<table width=\"100%\" align=\"center\" border=\"0\" cellspacing=\"3\" cellpadding=\"0\"><tbody>"; #B
	print "<td width=\"10%\" $top_bund><font face=\"Helvetica, Arial, sans-serif\" color=\"#000066\"><a accesskey=L href=\"rapport.php?rapportart=$rapportart&regnaar=$regnaar&dato_fra=$startdato&maaned_fra=$mf&aar_fra=$aar_fra&dato_til=$slutdato&maaned_til=$mt&aar_til=$aar_til&konto_fra=$konto_fra&konto_til=$konto_til&ansat_fra=$ansat_fra&ansat_til=$ansat_til&afd=$afd&projekt_fra=$projekt_fra&projekt_til=$projekt_til\">Luk</a></td>";
	print "<td width=\"80%\" $top_bund><font face=\"Helvetica, Arial, sans-serif\" color=\"#000066\"> Rapport - ".ucfirst($rapportart)."</td>";
	print "<td width=\"10%\" $top_bund><font face=\"Helvetica, Arial, sans-serif\" color=\"#000066\"><br></td>";
	print "</tbody></table>"; #B slut
	print "</td></tr>";
