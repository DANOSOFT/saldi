...
//
// --- finans/rapport_includes/moms_oss.php --- patch 5.0.0 --- 2026-07-16 ---
// LICENSE
//
...
function moms_oss($regnaar, $maaned_fra, $maaned_til, $aar_fra, $aar_til,
    ...
    // Aggregate OSS B2C sales:
    // OSS codes = SM codes where box6 (Land) is non-empty.
    // Revenue amounts are credits → SUM(kredit - debet) gives positive taxable amount.
    // t.moms on credit lines is negative → -SUM(t.moms) gives positive VAT amount.
    $qtxt = "SELECT"
          . " g.box6 AS land,"
          ...
          . " SUM(t.kredit - t.debet) AS momspligtig_beloeb,"
          ...
          . " $dim"
          . " GROUP BY g.box6, g.beskrivelse, g.box2, g.box7"
          . " ORDER BY g.box6, g.beskrivelse";
    ...
    print "<th align='left'>Momskode</th>";
    print "<th align='right' style='width:60px'>Sats %</th>";
    print "<th align='left'  style='width:90px'>Type</th>";
    print "<th align='right' style='width:150px'>Momspligtig beloeb</th>";
    print "<th align='right' style='width:150px'>Momsbeloeb</th>";
    print "</tr></thead><tbody>";

    fwrite($csv, mb_convert_encoding(
        "Land;Momskode;Sats %;Type;Momspligtig beloeb;Momsbeloeb\n",
        'ISO-8859-1', 'UTF-8'));
    ...
    while ($row = db_fetch_array($q)) {
        ...
        $base        = (float)$row['momspligtig_beloeb'];
        $moms        = (float)$row['momsbeloeb'];
        $land        = htmlspecialchars($row['land'] ?? '');
        $kode_navn   = htmlspecialchars($row['kode_navn'] ?? '');
        $type_txt    = $row['type'] !== '' ? htmlspecialchars($row['type']) : '<span style="color:#aaa">–</span>';
        $type_csv    = $row['type'];
        $grand_base += $base;
        $grand_moms += $moms;
        ...
        print "<td style='font-size:0.85em;'>$kode_navn</td>";
        print "<td align='right'>" . ($sats > 0 ? dkdecimal($sats, 2) : '') . "</td>";
        print "<td>$type_txt</td>";
