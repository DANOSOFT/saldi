...
// // ------------finans/kassekladde.php------lap 3.2.7---2012-02-02------
// LICENS
//
...
if (!$simuler) {
	...
	print "<table width=\"100%\" height=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\"><tbody>"; # Tabel 1 -> Hovedramme
	...
	print "<td width=\"10%\" $top_bund align=\"right\"><a href=\"javascript:confirmClose('kassekladde.php','$tekst')\" accesskey=\"N\">Ny</a></td></tr>";
...
print "<tr><td valign=\"top\" height=\"1%\ align=\"center\"><table cellpadding=\"0\" cellspacing=\"0\" border=\"0\" align = \"center\" valign = \"top\"><tbody>";
#if(($bogfort)&&($bogfort!='-')) {print "<table height=\"100%\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\" align = \"center\" valign = \"top\">";}
#elseif ($browser=="opera" || $browser=="firefox") print "<table  height=\"100%\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\" align = \"center\" valign = \"top\">";
#else print "<table  height=\"100%\" cellpadding=\"0\" cellspacing=\"0\" border=\"1\" align = \"center\" valign = \"top\" id=\"inputTable\" onkeydown=\"doKeyDown()\">";
print "</tr>\n<tbody>"; # Tabel 1.2 -> bemærkningstekst
print "<tr>";
	print "<td><b> <span title= 'Her kan skrives en bem&aelig;rkning til kladden'>Bem&aelig;rkning:$nbsp</b>";
...
if ($bogfort=="-") {
	...
	print "<td align=\"center\"><span title=\"Klik her for at opdatere\"><input type=submit style=\"width: 5em\" accesskey=\"o\" value=\"Opdater\" name=\"submit\" onclick=\"javascript:docChange = false;\"></span></td>";
}
#print "</tr><tr><td><br color=\"$bgcolor5\" \"align=center\"></td></tr>\n";
print "</tbody></table>";# Tabel 1.2 <- bemærkningstekst
#}
# ####################################################################################################
print "<tr><td height=\"98%\" width=\"100%\"><div class=\"vindue\">"; 
if(($bogfort)&&($bogfort!='-')) print "<table cellpadding=\"0\" cellspacing=\"2\" border=\"0\" align = \"center\" valign = \"top\">";
...
print "<td align = center><b> <span title= 'Angiv belob - klik p&aring; opslag for at sl&aring; op i &aring;bne poster'><a href=kassekladde.php?kladde_id=$kladde_id&kksort=amount&tjek=$kladde_id>Bel&oslash;b</a></b></td>";
...
#		if ($vis_projekt) print "<td align=right> <a href=ret_projektnr.php?linje_id=$id[$y]>$projekt[$y]</a></td>";
...
#	$faktura[$x]=htmlentities($faktura[$x],ENT_QUOTES,$charset);
	...
	$y=0;
	if ($find) $tmp="select kontotype, kontonr, beskrivelse, moms, genvej, lukket from kontoplan where (kontotype ='H' or ((kontotype ='D' or kontotype ='S') $find)) and regnskabsaar='$regnaar' order by kontonr";
	else $tmp="select kontotype, kontonr, beskrivelse, moms, genvej, lukket from kontoplan where (kontotype ='D' or kontotype ='S'or kontotype ='H') and regnskabsaar='$regnaar' order by kontonr";
...
#			$beskrivelse[$x]=urlencode(trim($beskrivelse[$x]));
			...
			if ($kontotype[$y]=='H') print "<td colspan=4><b>$kontobeskrivelse[$y]</font></b></td>";
			else {
				...
				print "<td><span title='$spantekst'>$momskode[$y]</font></span></td>";
				print "<td><span title='Klik for at overf&oslash;re kontonr til kassekladde'>$href $genvej[$y]</font></a><span></td>";
			}
			print "</tr>\n";
		}
	}
...
function sidste_5($kontonr,$art,$dk) {
	...
	if (is_numeric($kontonr)) {
		...
		while ($x<5 && ($r = db_fetch_array($q))) {
			if ($r['kontonr']) {
				$x++;
				$retur.="<tr><td align=right>".$r['bilag']."</td><td>".dkdato($r['transdate'])."</td><td>".$r['beskrivelse'].".</td><td>".$r['kontonr']."</td></tr>";
			}
		}
		$retur.="</tbody></table>";
	}
