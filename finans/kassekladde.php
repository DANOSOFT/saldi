...
//
// --- finans/kassekladde.php --- ver 5.0.0 --- 2026-06-19 ---
...
// 20260612 MJ Preload cash journal grid lookup data for large databases.
// 20260620 MJ Clear stale cash journal grid searches on normal journal open so saved filters do not hide vouchers.
...
function kassekladde_sql_in($values, $numeric = false) {
	$items = array();
	foreach ($values as $value) {
		$value = trim((string)$value);
		if ($value === '') continue;
		if ($numeric) {
			if (is_numeric($value)) $items[] = (int)$value;
		} else {
			$items[] = "'" . db_escape_string($value) . "'";
		}
	}
...
function preload_kassekladde_grid_data($rows, $regnaar) {
	global $kassekladdeDocLookup, $kassekladdeAccountLookup, $kassekladdeKontoplanLookup;

	$kassekladdeDocLookup = array();
	$kassekladdeAccountLookup = array();
	$kassekladdeKontoplanLookup = array();
	$sourceIds = $financeAccounts = $accountByType = array();

	foreach ($rows as $row) {
		if (!empty($row['id'])) $sourceIds[] = $row['id'];
		$dType = trim(if_isset($row, '', 'd_type'));
		$kType = trim(if_isset($row, '', 'k_type'));
		$debet = trim(if_isset($row, '', 'debet'));
		$kredit = trim(if_isset($row, '', 'kredit'));
		if ($debet && ($dType == 'F' || !$dType)) $financeAccounts[] = $debet;
		if ($kredit && ($kType == 'F' || !$kType)) $financeAccounts[] = $kredit;
		if ($debet && ($dType == 'D' || $dType == 'K')) $accountByType[$dType][] = $debet;
		if ($kredit && ($kType == 'D' || $kType == 'K')) $accountByType[$kType][] = $kredit;
	}

	$sourceIdSql = kassekladde_sql_in($sourceIds, true);
	if ($sourceIdSql) {
		$qtxt = "select id, filename, filepath, source_id from documents where source = 'kassekladde' and source_id in ($sourceIdSql) order by source_id, id";
		$q = db_select($qtxt, __FILE__ . " linje " . __LINE__);
		while ($r = db_fetch_array($q)) {
			if (!isset($kassekladdeDocLookup[$r['source_id']])) $kassekladdeDocLookup[$r['source_id']] = $r;
		}
	}

	$financeSql = kassekladde_sql_in($financeAccounts, true);
	if ($financeSql) {
		$qtxt = "select kontonr, beskrivelse, moms from kontoplan where regnskabsaar='$regnaar' and kontonr in ($financeSql)";
		$q = db_select($qtxt, __FILE__ . " linje " . __LINE__);
		while ($r = db_fetch_array($q)) {
			$kassekladdeKontoplanLookup[trim($r['kontonr'])] = $r;
		}
	}

	$where = array();
	foreach ($accountByType as $type => $accounts) {
		$accountSql = kassekladde_sql_in($accounts);
		if ($accountSql) $where[] = "(art = '" . db_escape_string($type) . "' and kontonr in ($accountSql))";
	}
	if (count($where)) {
		$qtxt = "select kontonr, art, firmanavn, betalingsbet, betalingsdage from adresser where " . implode(" or ", $where);
		$q = db_select($qtxt, __FILE__ . " linje " . __LINE__);
		while ($r = db_fetch_array($q)) {
			$kassekladdeAccountLookup[$r['art'] . "|" . trim($r['kontonr'])] = $r;
		}
	}
}
...
$columns = array(
    ...
    'render' => function($value, $row, $column) use ($kladde_id, $sprog_id) {
        global $kassekladdeDocLookup;
        $id = $row['id'];
        $bilag = $row['bilag'];
        $dokument = $row['dokument'];
        
        if (is_array($kassekladdeDocLookup) && array_key_exists($id, $kassekladdeDocLookup)) {
            $docRow = $kassekladdeDocLookup[$id];
        } else {
...
array(
    ...
    "valueGetter" => function ($value, $row, $column) {
	    ...
	    'render' => function($value, $row, $column) use ($regnaar) {
	        global $regnaar, $kassekladdeAccountLookup, $kassekladdeKontoplanLookup;
	        $value = strip_tags($value);
	        $debettext = '';
	        if ($row['d_type'] == 'F' && $value) {
	            if (is_array($kassekladdeKontoplanLookup) && isset($kassekladdeKontoplanLookup[$value])) {
	                $row2 = $kassekladdeKontoplanLookup[$value];
	            } else {
	            ...
	            $lookupKey = $row['d_type'] . "|" . $value;
	            if (is_array($kassekladdeAccountLookup) && isset($kassekladdeAccountLookup[$lookupKey])) {
	                $row2 = $kassekladdeAccountLookup[$lookupKey];
	            } else {
...
array(
    ...
    'searchable' => false,
	    'render' => function($value, $row, $column) use ($regnaar) {
	        global $regnaar, $kassekladdeKontoplanLookup;
	        ...
	        if ($saved_vat === null && $debet && $dtype != 'D' && $dtype != 'K') {
	            if (is_array($kassekladdeKontoplanLookup) && isset($kassekladdeKontoplanLookup[$debet])) {
	                $row2 = $kassekladdeKontoplanLookup[$debet];
	            } else {
		...
		'sortable' => false,
			...
			'render' => function($value, $row, $column) use ($regnaar) {
				...
				if ($saved_vat === null && $kredit && $ktype != 'D' && $ktype != 'K') {
					if (is_array($kassekladdeKontoplanLookup) && isset($kassekladdeKontoplanLookup[$kredit])) {
						$row2 = $kassekladdeKontoplanLookup[$kredit];
					} else {
...
$grid_data = array(
    ...
    'columns' => $columns,
    'filters' => $filters,
    'preload' => function($rows) use ($regnaar) {
        preload_kassekladde_grid_data($rows, $regnaar);
    },
...
if (($bogfort && $bogfort != '-') || $udskriv) {
   # echo "<center>";
	...
	print "<div style='width: 100%; height: calc(98vh - 34px - 18px);'>";
		...
		$cashGridId = "kass_$bruger_id";
		if (!$udskriv && !isset($_GET['search'][$cashGridId])) {
			$qtxt = "UPDATE datatables SET search_setup='[]', \"offset\"=0 WHERE user_id = $bruger_id AND tabel_id='$cashGridId'";
			db_modify($qtxt, __FILE__ . " linje " . __LINE__);
		}
