...
<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0"><tbody>
  <tr><td align="center" valign="top">
    <table width="100%" align="center" border="0" cellspacing="0" cellpadding="0"><tbody>
      <td width="25%" bgcolor="<? echo $bgcolor2 ?>"><font face="Helvetica, Arial, sans-serif" color="#000066"><small><a href=kontoplan.php  accesskey=T>Tilbage</a></small></td>
      <td width="50%" bgcolor="<? echo $bgcolor2 ?>" align="center"><font face="Helvetica, Arial, sans-serif" color="#000066"><small>Kontokort</small></td>
      <td width="25%" bgcolor="<? echo $bgcolor2 ?>" align = "right"><font face="Helvetica, Arial, sans-serif" color="#000066"><small><a href=kontokort.php  accesskey=N>Ny</a><br></small></td>
    </tbody></table>
  </td></tr>
...
<?
  ...
  if ($HTTP_POST_VARS['slet'])
  {
    $id=$HTTP_POST_VARS['id'];
    $query = db_modify("delete from kontoplan where id = $id");
    $id=0;
  }
  elseif ($HTTP_POST_VARS['gem'])
  {
    $id=$HTTP_POST_VARS['id'];
    $kontonr=$HTTP_POST_VARS['kontonr'];
    $beskrivelse=addslashes($HTTP_POST_VARS['beskrivelse']);
    $kontotype=$HTTP_POST_VARS['kontotype'];
    $katagori=$HTTP_POST_VARS['katagori'];
    $moms=$HTTP_POST_VARS['moms'];
    $fra_kto=$HTTP_POST_VARS['fra_kto'];
    $til_kto=$HTTP_POST_VARS['til_kto'];
    ...
    elseif ($id==0)
    {
      ...
      else
      {
        ...
        for ($y=$regnaar; $y<=$x; $y++)
        {
          ...
          if(!$row = db_fetch_array($query))
          {
            db_modify("insert into kontoplan (kontonr, beskrivelse, kontotype, primo, regnskabsaar) values ($kontonr, '$beskrivelse', '$kontotype', '0', '$y')");
          }
        }
        $query = db_select("select id from kontoplan where kontonr = $kontonr and regnskabsaar = '$regnaar'");
    ...
    elseif ($id > 0)
    {
      ...
      $query = db_modify("update kontoplan set kontonr = $kontonr,  beskrivelse = '$beskrivelse', kontotype = '$kontotype', moms = '$moms', fra_kto = '$fra_kto', til_kto = '$til_kto', lukket = '$lukket' where id = '$id'");
    }
  ...
  if ($id)
  {
    print "<tr><td><font face=\"Helvetica, Arial, sans-serif\">Kontonr</td><td><br></td><td>$kontonr</td></tr>";
  ...
  if ($kontotype=='Drift')
  {
    ...
    print "<option></option>";
    $query = db_select("select kode, kodenr from grupper where art = 'KM' or art = 'SM'");
    while ($row = db_fetch_array($query))
    {
      print "<option>";
