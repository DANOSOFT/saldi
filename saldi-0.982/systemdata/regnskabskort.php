...
// ------------------------------------------------------------systemdata/regnskabskort.php  patch 0.972 -----------------------
// LICENS
//
...
if ($HTTP_POST_VARS)
{
  ...
  $startmd=$HTTP_POST_VARS['startmd'];
  $startaar=$HTTP_POST_VARS['startaar'];
  $slutmd=$HTTP_POST_VARS['slutmd'];
  $slutaar=$HTTP_POST_VARS['slutaar'];
  $aaben=trim($HTTP_POST_VARS['aaben']);
  $fakt=$HTTP_POST_VARS['fakt']*1;
  $modt=$HTTP_POST_VARS['modt']*1;
  $faktbill=trim($HTTP_POST_VARS['faktbill']);
  $modtbill=trim($HTTP_POST_VARS['modtbill']);
  $kontoantal=$HTTP_POST_VARS['kontoantal'];
  $kontonr=$HTTP_POST_VARS['kontonr'];
  $debet=$HTTP_POST_VARS['debet'];
  $kredit=$HTTP_POST_VARS['kredit'];
#    $primotal=$HTTP_POST_VARS['primotal'];
  $aar=date(Y);
  $topaar=$aar+10;
  $bundaar=$aar-10;
  $fejl=0;
  ...
  if (($startmd<0)||($startmd>12))
  {
    Print "<BODY onLoad=\"javascript:alert('Startm&aring;ned skal v&aelig;e mellem 1 og 12!')\">";
    $startmd="";
  }
  elseif ($startmd<10){$startmd="0".$startmd;};
  if (($slutmd<0)||($slutmd>12))
  {
    Print "<BODY onLoad=\"javascript:alert('Slutm&aring;ned skal v&aelig;e mellem 1 og 12!')\">";
    $slutmd="";
  }
  elseif ($slutmd<10){$slutmd="0".$slutmd;};
  if (($startaar<$bundaar)||($startaar>$topaar))
  {
    print "<BODY onLoad=\"javascript:alert('Start&aring;r skal v&aelig;e mellem $bundaar og $topaar!')\">";
    $startaar="";
  }
  if (($slutaar<$bundaar)||($slutaar>$topaar))
  {
    print "<BODY onLoad=\"javascript:alert('Slut&aring;r skal v&aelig;e mellem $bundaar og $topaar!')\">";
    $slutaar="";
  }
...
else
{
  ...
  {
    aar_x($id, $x, $beskrivelse, $startmd, $startaar, $slutmd, $slutaar, $aaben);
  }
}
...
<tr><td align = "center" valign = "bottom">
    <table width="100%" align="center" border="1" cellspacing="0" cellpadding="0"><tbody>
      <td width="100%" bgcolor="#ffcc00"><font face="Helvetica, Arial, sans-serif" color="#000066"><small>Copyright (C) 2004 ITz ApS</small></td>
    </tbody></table>
</td></tr>
</tbody></table>
</body></html>
