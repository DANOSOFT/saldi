<?
  @session_start();
  $s_id=session_id();
// ----------------------------------kreditor/modtag.php-------patch 0.958------------
// LICENS
//
...
echo  "insert into batch_kob(vare_id, linje_id, kobsdate, ordre_id, antal, pris, lager) values ($vare_id[$x], $linje_id[$x], '$levdate', $id, $leveres[$x], '$pris[$x]', $lager)<br>";           
...
/*
  if ($serienr) {
    for ($y=-1; $y>=$batch_rest[$x]; $y--) {
      ...
      $z=$sn_start+$x;
      db_modify("update serienr set batch_kob_id=$batch_kob_id where id=$sn_id[$z]");
    }
  $sn_start=$y;
  }
*/
