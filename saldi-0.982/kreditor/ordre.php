<?
// -------------------------------------------------------kreditor/ordre.php----------Patch 0.971------
// LICENS
//
...
<?
  ...
  if ($konto_id=$_GET['konto_id']) {$id = indset_konto($id, $konto_id);}
  ...
  if ($HTTP_POST_VARS) {
    ...
    $ordrenr = $HTTP_POST_VARS['ordrenr'];
    ...
    $lev_postnr = $HTTP_POST_VARS['lev_postnr'];
    ...
    $levdate = usdate(trim($HTTP_POST_VARS['levdato']));
...
#      if (!$lager_id) {$lager_id='0';}
      db_modify("insert into ordrer (ordrenr, konto_id, kontonr, firmanavn, addr1, addr2, postnr, bynavn, land, kontakt, lev_navn,  lev_addr1,  lev_addr2,  lev_postnr,  lev_bynavn, lev_kontakt, betalingsdage, betalingsbet, cvrnr, notes, art, ordredate, momssats, status, ref, sum, lev_adr, hvem, tidspkt) values ($ordrenr, $konto_id, '$kontonr', '$firmanavn', '$addr1', '$addr2', '$postnr', '$bynavn', '$land', '$kontakt', '$lev_navn',  '$lev_addr1',  '$lev_addr2',  '$lev_postnr',  '$lev_bynavn', '$lev_kontakt', '$betalingsdage', '$betalingsbet', '$cvrnr', '$notes', '$art', '$ordredate', '$momssats', $status, '$ref', '$sum', '$lev_adr', '$brugernavn', '$tidspkt')");
    ...
    elseif($firmanavn) {
      ...
      for($x=1; $x<=$linjeantal; $x++) {
        ...
        elseif (($art=='KK')&&($kred_ord_id)) { ###################   Kreditnota ####################
          ...
          $rest=0;
          $query = db_select("select id, rest from batch_kob where vare_id = '$vare_id[$x]' and ordre_id = '$kred_ord_id'");
...
#              $query = db_select("select * from batch_salg where linje_id = '$linje_id[$x]' and ordre_id=$id and vare_id = $vare_id[$x]");}
              ...
              if (($posnr_ny[$x]=="-")&&($tidl_lev[$x]==0)&&($solgt[$x]==0)&&($antal[$x]==0)) {
                db_modify("delete from reservation where linje_id='$linje_id[$x]'");
                db_modify("delete from ordrelinjer where id='$linje_id[$x]'");
              }
              elseif ($antal[$x]>0) {
      ...
      if ($posnr_ny[0]>0) {
        if ($varenr[0])  {
          ...
          if ($row = db_fetch_array($query)) {
            ...
            db_modify("insert into ordrelinjer (ordre_id, posnr, varenr, vare_id, beskrivelse, enhed, antal, pris, rabat, serienr, lev_varenr, momsfri) values ('$id', '$posnr_ny[0]', '$varenr[0]', '$vare_id[0]', '$beskrivelse[0]', '$enhed[0]', '$antal_ny[0]', '$pris[0]', '$rabat[0]', '$serienr[0]', '$lev_varenr[0]', '$momsfri[0]')");
          }
...
#        else {print "<BODY onLoad=\"javascript:alert('Du er blevet smidt af')\">";}
  ...
  if (strstr($submit,'Opslag')) {
    if (strstr($fokus,'kontonr')) {kontoopslag($sort, $fokus, $id);}
    if (strstr($fokus,'vare')) {vareopslag($sort, $fokus, $id, $vis, $ref);}
    if (strstr($fokus,'kontakt')){ansatopslag($sort, $fokus, $id, $vis);}
  }
  if ((strstr($submit,'Bogf'))&&($bogfor!=0)&&($status==2)) {
    if(!fakturanummer) {print "fakturanummer skal anf&oslash;res";}
    else  {
...
#        $ialt=dkdecimal($ialt);
    ...
    if ($art!='KK') {print "<td align=center><input type=submit value=\"&nbsp;Kopier&nbsp;\" name=\"submit\"></td>";}
    if ($art!='KK') {print "<td align=center><input type=submit value=\"Krediter\" name=\"submit\"></td>";}
  }
...
#      if (substr($dkantal[$x],-1,1)==","){$dkantal[$x]=(substr($dkantal[$x],0,strlen($antal[$x])-1));}
      ...
      if ($status>=1) {
        if ($vare_id[$x]) {
          ...
          $row = db_fetch_array(db_select("select box8 from grupper where kodenr = $row[gruppe] and art = 'VG'"));
          $box8[$x] = trim($row['box8']);
          $tidl_lev[$x]=0;
          ...
          if (($antal[$x]>=0)&&($art!='KK')) {
           ...
           if ($tidl_lev[$x]<$antal[$x]){$status=1;}
            ...
            elseif ($antal[$x]-$tidl_lev[$x]!=$row[antal])  {db_modify("insert into reservation (linje_id, vare_id, batch_salg_id, antal) values  ($linje_id[$x], $vare_id[$x], 0, $antal[$x]-$tidl_lev[$x])");}
          }
          if ($antal[$x]<0){
            $tidl_lev[$x]=0;
      ...
      if (($antal[$x]<0)&&($art!='KK')&&($box8[$x]=='on')) {print "<td align=center onClick=\"batch($linje_id[$x])\">$font<span title= 'V&aelig;lg fra k&oslash;bsordre'><small><img alt=\"K&oslash;bsordre\" src=../ikoner/serienr.png></td></td>";}
...
#####################################################################################################################################
function vareopslag($sort, $fokus, $id, $vis, $ref) {
  global $kontonr;
  global $font;
  global $bgcolor;
  global $bgcolor5;
...
function sidehoved($id, $returside, $kort, $fokus, $tekst) {
  ...
  if (($returside != "ordre.php")&&($id)) {print "<td width=\"25%\" bgcolor=\"$bgcolor2\" align=\"right\">$font $color<small><a href=ordre.php?returside=ordreliste.php accesskey=N>Ny</a></small></td>";}
  elseif (($id)||($kort!="../lager/varekort.php")) {print "<td width=\"25%\" bgcolor=\"$bgcolor2\" align=\"right\">$font $color<small><a href=$kort?returside=../kreditor/ordre.php&ordre_id=$id&fokus=$fokus accesskey=N>Ny</a></small></td>";}
  else {print "<td width=\"25%\" bgcolor=\"$bgcolor2\" align=\"right\">$font $color</td>";}
...
function indset_konto($id, $konto_id) {
  ...
  if ($gruppe) {
    $query = db_select("select box1 from grupper where art='KG' and kodenr='$gruppe'");
    $row = db_fetch_array($query);
    $query = db_select("select box2 from grupper where art='KM' and kodenr='$row[box1]'");
    $row = db_fetch_array($query);
...
#    }
#    if (!$lager_id) {$lager_id='0';}
    db_modify("insert into ordrer (ordrenr, konto_id, kontonr, firmanavn, addr1, addr2, postnr, bynavn, land, betalingsdage, betalingsbet, cvrnr, notes, art, ordredate, momssats, status, hvem, tidspkt) values ($ordrenr, '$konto_id', '$kontonr', '$firmanavn', '$addr1', '$addr2', '$postnr', '$bynavn', '$land', '$betalingsdage', '$betalingsbet', '$cvrnr', '$notes', 'KO', '$ordredate', '$momssats', '0', '$brugernavn', '$tidspkt')");
    $query = db_select("select id from ordrer where kontonr='$kontonr' and ordredate='$ordredate' order by id desc");
    if ($row = db_fetch_array($query)) {$id=$row[id];}
  }
  elseif($firmanavn) {
