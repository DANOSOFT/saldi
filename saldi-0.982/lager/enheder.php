<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN"><html><head><title>SALDI - Enheder / materialer</title><meta http-equiv="content-type" content="text/html; charset=ISO-8859-1"></head>
<?
// ----------------------------------------------------------------------050306----------
// LICENS
//
...
#  include("../includes/db_query.php");
  ...
  if ($HTTP_POST_VARS['enheder'])
  {
    $enh_id=$HTTP_POST_VARS[enh_id];
    $enh_betegnelse=$HTTP_POST_VARS['enh_betegnelse'];
    $enh_beskrivelse=$HTTP_POST_VARS['enh_beskrivelse'];
  ...
  if ($HTTP_POST_VARS['materialer'])
  {
    $mat_id=$HTTP_POST_VARS[mat_id];
    $mat_beskrivelse=$HTTP_POST_VARS['mat_beskrivelse'];
    $mat_densitet=$HTTP_POST_VARS['mat_densitet'];
    
    $mat_beskrivelse[0]=trim($mat_beskrivelse[0]);
    $mat_beskrivelse[$mat_id]=trim($mat_beskrivelse[$mat_id]);
 
    $mat_densitet[0]=+$mat_densitet[0];
  ...
  print "<td width=\"50%\" bgcolor=\"$bgcolor2\" align=\"center\">$font<small>Enheder & materialer</small></td>";
