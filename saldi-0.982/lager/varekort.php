...
<?
// ---------------------------------/lager/varekort.php (modul nr. 9)-----------patch 0.958--------
// LICENS
//
...
if ($HTTP_POST_VARS){
  ...
  $varenr=addslashes(trim($HTTP_POST_VARS['varenr']));
  $beskrivelse=addslashes(trim($HTTP_POST_VARS['beskrivelse']));
  $enhed=addslashes(trim($HTTP_POST_VARS['enhed']));
  $enhed2=addslashes(trim($HTTP_POST_VARS['enhed2']));
  $forhold=usdecimal($HTTP_POST_VARS['forhold']);
  $salgspris=usdecimal($HTTP_POST_VARS['salgspris']);
  $salgspris2=usdecimal($HTTP_POST_VARS['salgspris2']);
  $kostpris=$HTTP_POST_VARS['kostpris'];
  $kostpris[0]=usdecimal($kostpris[0]);
  $kostpris2=$HTTP_POST_VARS['kostpris2'];
  $provisionsfri=trim($HTTP_POST_VARS['provisionsfri']);
  list ($leverandor) = split(':', $HTTP_POST_VARS['leverandor']);
  $vare_lev_id=$HTTP_POST_VARS['vare_lev_id'];
  $lev_varenr=$HTTP_POST_VARS['lev_varenr'];
  $lev_antal=$HTTP_POST_VARS['lev_antal'];
  $lev_pos=$HTTP_POST_VARS['lev_pos'];
  $gruppe=$HTTP_POST_VARS['gruppe'];
  $min_lager= $HTTP_POST_VARS['min_lager']; 
  $max_lager= $HTTP_POST_VARS['max_lager']; 
  $lukket=$HTTP_POST_VARS['lukket'];
  $serienr=addslashes(trim($HTTP_POST_VARS['serienr']));
  ...
  $delvare=$HTTP_POST_VARS['delvare'];
  $samlevare=$HTTP_POST_VARS['samlevare'];
  $fokus=$HTTP_POST_VARS['fokus'];
  $be_af_ant=$HTTP_POST_VARS['be_af_ant'];
  $be_af_id=$HTTP_POST_VARS['be_af_id'];
  $ant_be_af=$HTTP_POST_VARS['ant_be_af'];
  $indg_i_id=$HTTP_POST_VARS['indg_i_id'];
  $indg_i_ant=$HTTP_POST_VARS['indg_i_ant'];
  $ant_indg_i=$HTTP_POST_VARS['ant_indg_i'];
  $indg_i_pos=$HTTP_POST_VARS['indg_i_pos'];
  $be_af_pos=$HTTP_POST_VARS['be_af_pos'];
  ...
  # if ($samlevare=='on'){$kostpris=0;}
    ...
    if ($id==0) {
      ...
      if ($row[id]) {
        print "<big><b>Der findes allerede en vare med varenr: $varenr</b></big><br><br>";
        $id=0;
      }
      else {
...
print "      <td width=\"25%\" bgcolor=\"$bgcolor2\">$font <small><a href=$returside?id=$ordre_id&fokus=$fokus&varenr=$varenr accesskey=T>Tilbage</a></small></td>";
...
print "<td colspan=3>$font $varenr</td></tr>";
print "<input type=hidden name=varenr value=$varenr>";
print "<tr><td>$font Beskrivelse</td><td><br></td><td colspan=3><input type=text size=50 name=beskrivelse value=\"$beskrivelse\"></td></tr>";
  ...
  if ($id) {
   ...
   $query = db_select("select * from vare_lev where vare_id='$id' order by posnr");
    ...
    for ($x=1; $x<=$lev_ant; $x++) {
      ...
      print "<td><input type=text size=1 name=lev_pos[$x] value=$x></td><td>$font $row[kontonr]:".htmlentities(stripslashes($row[firmanavn]))."</td><td><input type=text align=right size=9 name=lev_varenr[$x] value=$lev_varenr[$x]></td><td align=right><input type=text align=right size=9 name=kostpris[$x] value=\"$y\"></td>";
      if (($enhed2)&&($forhold>0)) {
...
#    print "<tr><td><input type=text size=1 name=lev_pos[0] value=$x></td><td><SELECT NAME=leverandor>";
#    $query = db_select("select id, kontonr, firmanavn from adresser where art='K' order by kontonr");
#    print "<option> </option>";
#    while ($row = db_fetch_array($query))
#    {
#     if (!in_array($row[id], $lev_id)) {print "<option>$row[kontonr]:$row[firmanavn]</option>";}
#    }
#    print "</SELECT></td></tr>";
    print "</tbody></table>";
  }
  else {
#    print "<tr><td>$font Leverandr</td><td></td><td><SELECT NAME=leverandor>";
 #   $query = db_select("select id, kontonr, firmanavn from adresser where art='K' order by kontonr");
#    print "<option> </option>";
#    while ($row = db_fetch_array($query)){print "<option>$row[kontonr]:$row[firmanavn]</option>";}
#    print "</SELECT></td></tr>";
  }
}
# print "<tr><td>$font Leverand&oslash;r</td><td></td>";
# print "<td><input type=text align=right size=9 name=leverandor ></td></tr>";

# print "<tr><td>$font Kostpris</td><td></td><td>$font $kpris</td></tr>";
...
print "</SELECT></td></tr>";
print "<tr><td>$font Min.beholdning</td><td></td><td><input type=text size=5 align=right name=min_lager value=$min_lager></td></tr>";
print "<tr><td>$font Max.beholdning</td><td></td><td><input type=text size=5 align=right name=max_lager value=$max_lager></td></tr>";
...
print "<tr><td valign=top>$font Bem&aelig;rkning</td><td><br></td><td colspan=9><textarea name=\"notes\" rows=\"3\" cols=\"85\">$notes</textarea></td></tr>";
...
/*  Udeladt intil test at vareflow er afsluttet (2006-03-03)
...
function cirkeltjek($id, $retning) {
  $s_id[0]=trim($id); 
  $query = db_select("select id from varer where varenr='$id'"); #Varen ikke er tilføjet i "styklister endnu"   
  if ($row = db_fetch_array($query)) {  
    $id=$row[id];
    $x=0;
...
print "    </tbody></table></td></tr>";
  ...
  print"<td><small><b><font face=\"Helvetica, Arial, sans-serif\"><a href=varekort.php?sort=kontonr&funktion=kontoOpslag&id=$id>Kundenr</b></small></td>";
  print"<td><small><b><font face=\"Helvetica, Arial, sans-serif\"><a href=varekort.php?sort=firmanavn&funktion=kontoOpslag&id=$id>Navn</b></small></td>";
  print"<td><small><b><font face=\"Helvetica, Arial, sans-serif\"><a href=varekort.php?sort=addr1&funktion=kontoOpslag&id=$id>Adresse</b></small></td>";
  print"<td><small><b><font face=\"Helvetica, Arial, sans-serif\"><a href=varekort.php?sort=addr2&funktion=kontoOpslag&id=$id>Adresse2</b></small></td>";
  print"<td><small><b><font face=\"Helvetica, Arial, sans-serif\"><a href=varekort.php?sort=postnr&funktion=kontoOpslag&id=$id>Postnr</b></small></td>";
  print"<td><small><b><font face=\"Helvetica, Arial, sans-serif\"><a href=varekort.php?sort=bynavn&funktion=kontoOpslag&id=$id>bynavn</b></small></td>";
  print"<td><small><b><font face=\"Helvetica, Arial, sans-serif\"><a href=varekort.php?sort=land&funktion=kontoOpslag&id=$id>land</b></small></td>";
#  print"<td><small><b><font face=\"Helvetica, Arial, sans-serif\"><a href=varekort.php?sort=kontakt&funktion=kontoOpslag&id=$id>Kontaktperson</b></small></td>";
  print"<td><small><b><font face=\"Helvetica, Arial, sans-serif\"><a href=varekort.php?sort=tlf&funktion=kontoOpslag&id=$id>Telefon</b></small></td>";
  print" </tr>";
  ...
  while ($row = db_fetch_array($query))
  {
    ...
    print "<tr>";
    print "<td><small><font face=\"Helvetica, Arial, sans-serif\"><a href=varekort.php?fokus=$fokus&id=$id&konto_id=$row[id]>$row[kontonr]</a></small></td>";
