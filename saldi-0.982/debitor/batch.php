...
/*      
        ...
        if ($valg[$x] < $max_antal[$x])  {print "<BODY onLoad=\"javascript:alert('Ordrenr: $ordrenr[$x] - Der kan ikke v&aelig;lges færre end $max_antal[$x]!')\">";}
        $valgt_antal=$valgt_antal-$valg[$x];
        $rest_antal=$rest_antal-$rest[$x];
      }
*/    
...
#  print "<tr><td colspan=3><hr></td></tr>\n";
  $x=0;
  $rest=array();
  $query = db_select("select * from batch_kob where vare_id=$vare_id and rest > 0 and lager = $lager");
  while ($row = db_fetch_array($query))
  {
    $x++;
    ...
    print "<tr>";
    print "<td>$font<span title= 'kostpris: $pris[$x]'>$ordrenr[$x]</td>";
    print "<td align = right>$font $rest[$x]</td>";
  ...
  while ($row = db_fetch_array($query))
  {
    ...
    if ($rest[$x]>0) {
      print "<tr>";
      print "<td>$font $ordrenr[$x]</td><td align = right>$font $rest[$x]</td>";
      print "<td align = right><input type=text style=\"text-align:right\" size=3 name=valg_$x value=$valg[$x]></td>";
      print "</tr>\n";
    }
