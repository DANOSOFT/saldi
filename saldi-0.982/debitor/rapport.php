...
#    print "<tr><td colspan=8><hr></td></tr>";
 ...
 #       <td><small><small>".forfaldsdag($row[transdate], $betalingsbet, $betalingsdage)."</small></small></td>";
        ...
        if ($row[amount] > 0) 
        {
          print "<td>$stil<small><small>".dkdato($forfaldsdag)."</small></small></td><td align=right>$stil<small><small>$tmp</small></small></td><td></td>";
...
#  echo "$maaned_fra -$maaned_til<br>";
  ...
  $x=0;
  $query = db_select("select * from adresser where kontonr>='$konto_fra' and kontonr<='$konto_til' and art = 'D' order by firmanavn");
  while ($row = db_fetch_array($query))
  {
    $x++;
    $id[$x]=$row[id];
