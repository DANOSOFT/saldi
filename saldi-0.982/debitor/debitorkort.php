...
// ----------------------------------------------------debitor/debitorkort.php-----------patch0.971----------
// LICENS
//
...
if ($HTTP_POST_VARS)
{
  ...
  if (($firmanavn)&&(($ny_kontonr < 1)||(!$ny_kontonr))) {
    if (!$id) {$id="0";}
    $x=0;
    $query = db_select("select kontonr from adresser where art = 'D' and id != $id order by kontonr");
...
############################
  ...
  if ($submit=="Slet") {db_modify("delete from adresser where id = $id");}
    ...
    if ($id==0) {
      $query = db_select("select id from adresser where kontonr = '$ny_kontonr' and art = 'D'");
    ...
    elseif ($id > 0) {
      if ($ny_kontonr!=$kontonr) {
        $query = db_select("select kontonr from adresser where art = 'D' order by kontonr");
        while ($row = db_fetch_array($query)) {
          $x++;
          $ktoliste[$x]=$row[kontonr];
        }
...
print "<tr><td>$font Kundenr</td><td><br></td><td><input type=text size=25 name=ny_kontonr value=\"$kontonr\"></td>";
