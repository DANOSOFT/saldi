...
// -------------------------------------------------------debitor/formularprint-----patch 0937----------
// LICENS
//
...
# Angiv antal ordrelinjer pr side
# $font="<font face=helvetica, ariel, sans-serif>";
...
if (($formular ==4)||($formular ==5)) { 
  $query = db_select("select art, ref from ordrer where id = $ordre_id[$q]");
...
if ($ordre_id[$q]){
    ...
    if ($row[posnr]>0)
    {
      ...
      if ($varenr[$x])
      {
        ...
        if ($formular==3){
          ...
          if ($r2 = db_fetch_array($q2)){$lev_antal[$x]=$r2[antal];}
          else {$lev_antal[$x]=0;}
          $tidl_lev[$x]=0;
          $q2 = db_select("select antal from batch_salg where linje_id = $linje_id[$x] and lev_nr < $lev_nr");
  ...
  for ($x=1;$x<=$linjeantal; $x++) { 
    if (($formular!=3)||($varenr[$x])) {  #Fordi tekst uden varenr ikke skal med p�flgesedlen
...
# skriv(10, '', '', '0',"Nettosum", 'slut', 132, 70,'v');

#skriv(10, '', '', '0',$sum, 'slut', 183, 70,'h');
#skriv(10, '', '', '0',"$momssats% moms", 'slut', 132, 63,'v');
#skriv(10, '', '', '0', $moms, 'slut', 183, 63,'h');
#skriv(10, '', '', '0',"I alt", 'slut', 132, 56,'v');
#skriv(10, '', '', '0', $ialt, 'slut', 183, 56,'h');
