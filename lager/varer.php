...
#	else $udvalg=$udvalg." and (varenr ~ '$varenummer' or stregkode ~ '$varenummer')"; 
	...
	if (!strstr($searchTerm, '%')) {
		$udvalg .= " or lower(beskrivelse) like '%$lowTerm%' or lower(beskrivelse_alias) like '%$lowTerm%'";
		$udvalg .= " or lower(trademark) like '%$lowTerm%'";
	}
...
#				$salgspris[$v]=dkdecimal($salgspris[$v]*(100+$incl_moms)/100,2);
				if ($csv) fwrite($csvfil,"\"".dkdecimal($kostpris[$v],2)."\";\"".dkdecimal($vatPrice[$v],2)."\"\n");
					...
					if ($show_dg) {
						// Check if the necessary values are set and valid
						$dg_percentage = $dg[$v];
