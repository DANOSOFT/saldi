...
if ($bogfor) {
	...
	if (isset($_POST['nulstil_ej_optalt']) && $_POST['nulstil_ej_optalt'])
		$nulstil_ej_optalt = $_POST['nulstil_ej_optalt'];
	$dato = if_isset($_GET['dato']);
	if (isset($_POST['dato']))
		$dato = $_POST['dato'];
...
if (!$dato) { # 20140625
	...
	if ($r = db_fetch_array($q)) {
		...
		if (date("Y-m-d") > $regnslut)
			$dato = dkdato($regnslut);
		else
			$dato = date("d-m-Y");
	}
}
...
if ($varenr)
	print "<a style=\"text-decoration:none\" href=optalling.php?lager=$lager><input type=\"button\" value=\"" . findtekst('159|Fortryd', $sprog_id) . "\"></a>";
print "</td></tr>\n";
...
if ($gentael)
	gentael($lager);
if ($bogfor)
	bogfor($lager, $nulstil_ej_optalt, $dato, $bogfor, $godkend_regdif);
elseif ($vis_ej_optalt) {
	$optalt = vis_ej_optalt($lager);
} else
	$optalt = vis_optalling($lager, $vnr, 0);

if ($optalt >= 1) {
	if (!$dato)
		$dato = date('d-m-Y'); # 20140625
...
#			}
			...
			$lagervalue += $kostsum;
			if ($vare_id[$x] != $vare_id[$x + 1])
				print "<tr bgcolor=\"$baggrund\"><td><b>Optalt ialt</b></td><td><br></td><td><br></td><td><br></td><td align=\"right\"><b>" . dkdecimal($behold, 2) . "</b></td><td align=\"right\"><b>" . dkdecimal($opt, 2) . "</b></td><td align=\"right\"><b>" . dkdecimal($kpris, 2) . "</b></td><td align=\"right\"><b>" . dkdecimal($kostsum, 2) . "</b></td><td align=\"right\"><b>" . dkdecimal($lagervalue, 2) . "</b></td><tr>\n";
			#			$x++;
		} else {
			$aar = substr($tidspkt[$x], 0, 4);
			$md = substr($tidspkt[$x], 4, 2);
	...
	if (!$vnr) {
		...
		$vareantal = 0;
		if ($gruppe)
			$r = db_fetch_array(db_select("select count(id) as antal from varer where ($gruppe) and lukket != 'on'", __FILE__ . " linje " . __LINE__));
		$vareantal += $r['antal'];
		#		if ($antal) {	
		print "<tr><td colspan=\"8\">";
		if ($antal)
			print findtekst('2232|Optalt i alt', $sprog_id) . " $antal " . findtekst('2233|varer ud fra en samlet vareliste på', $sprog_id) . " $vareantal " . findtekst('2234|lagerførte varer.', $sprog_id);
		...
		if ($antal)
			print "Klik <a href=\"optalling.php?nulstil=1&lager=$lager&dato=$dato\" onclick=\"return confirm('Vi du nulstille hele optællingen?')\">her</a> for at \"0-stille\" listen";
		print "<td></tr>";
		#		}
	}
	print "<tr><td colspan=\"8\"><hr></td></tr>";
...
function vis_ej_optalt($lager)
{
	...
	while ($r = db_fetch_array($q)) {
		if ($gruppe)
			$gruppe .= " or gruppe = '" . $r['kodenr'] . "'";
		else
			$gruppe = "gruppe = '" . $r['kodenr'] . "'";
	}
	if ($gruppe) { #20140625
				...
				if (isset($reguleres[$id]) && $reguleres[$id]) {
					...
					if ($bogfor > 1) { #20150109			
						...
						*/
						...
						$r = db_fetch_array(db_select($qtxt, __FILE__ . " linje " . __LINE__));
						if ($r['id'])
							$qtxt = "update lagerstatus set beholdning = '$ny_beholdning[$id]' where id = '$r[id]'";
						elseif ($ny_beholdning[$id])
							$qtxt = "insert into lagerstatus (vare_id,variant_id,lager,beholdning) values ('$vare_id[$v]','$variant_id[$v]','$lager','$ny_beholdning[$id]')";
						db_modify($qtxt, __FILE__ . " linje " . __LINE__);
						if ($reguleres[0] < 0) {
							$tmp = $reguleres[0] * -1;
...
#									$r2=db_fetch_array(db_select("select max(id) as id from batch_kob where vare_id='$id' and linje_id=0",__FILE__ . " linje " . __LINE__));
	...
	#exit;	
	if ($bogfor > 1)
		opdat_behold($reg_vare_id); # $reg_vare_id ADDED 20200629
	transaktion('commit');
	if ($bogfor == 1)
		print "<tr><td colspan=\"6\">Klik <a href=optalling.php?bogfor=2&nulstil_ej_optalt=$nulstil_ej_optalt&dato=$dato&lager=$lager&godkend_regdif=$godkend_regdif>her</a> for endelig lagerregulering pr. $dato</td></tr>";
		...
		for ($x = 0; $x < count($reg_vare_id); $x++) {
			...
			if ($reg_diff[$id]) {
				...
				if ($y > 1)
					...
					vis_optalling($lager, $r['varenr'], 0);
			}
		}
		print "<script language=\"javascript\">\n";
		print "document.optalling.optalt.focus();\n";
