...
// -----------lager/rapport.php------------patch 3.2.9-------2013.03.18----
// LICENS
//
...
// 2013.03.18 $modulnd ændret fra 12  til 15
...
if (isset($_POST['submit']) && $_POST['submit']) {
	...
	$md=$_POST['md'];
	list ($varenr_fra, $firmanavn) = explode(":", $_POST['varenr_fra']);
	list ($varenr_til, $firmanavn) = explode(":", $_POST['varenr_til']);
...
#	print "</tbody></table>"; #B slut
	...
	print "<td> Varegruppe </td><td><select class=\"inputbox\" name=varegruppe>";
	...
	print "<tr><td> Periode</td><td colspan=2><select class=\"inputbox\" name=maaned_fra>";
	...
	print "</td>";
	print "<td colspan=2><select class=\"inputbox\" name=maaned_til>";
	...
	print "</td></tr>";
	print "<tr><td> Varenr (fra)</td><td colspan=4><select class=\"inputbox\" name=varenr_fra>";
	print "<option>$varenr_fra</option>";
	for ($x=1; $x<=$antal_konti; $x++) {
		print "<option>$varenr[$x] : $varenavn[$x]</option>";
	}
	print "</td></tr>";
	print "<tr><td> Varenr (til)</td><td colspan=4><select class=\"inputbox\" name=varenr_til>";
	...
	print "</form>";
	print "</tbody></table>";
	print "<tr><td ALIGN=center><table cellpadding=\"1\" cellspacing=\"1\" border=\"0\"><tbody>\n";
	if ($popup) {
...
#	else print "<tr><td colspan=3 ALIGN=center onClick=\"javascript:lagerstatus=window.open('lagerstatus.php','lagerstatus','$jsvars');lagerstatus.focus();\"><span title='Se lagerstatus p&aring; vilk&aring;rlig dato'><input type=submit value=\"Lagerstatus\" name=\"submit\"></span></td></tr>";
	} else {
		print "";
		print "<tr><td ALIGN=center><form action=lagerstatus.php><span title='Se lagerstatus p&aring; vilk&aring;rlig dato'><input style=\"width:120px;\" type=submit value=\"Lagerstatus\" name=\"submit\"></span></form></td>";
		print "<td ALIGN=center><form action=optalling.php><span title='Funktion til opt&aelig;lling og regulering af varelager'><input style=\"width:120px;\" type=submit value=\"Lageropt&aelig;lling\" name=\"submit\"></span></form></td></tr>";
	}
	print "</tbody></table>\n";
	print "</td></tr>";
...
#	echo "select box9 from grupper where kodenr ='$gruppenr' and art='VG'<br>";
...
#	if ($slutmaaned<10){$slutmaaned="0".$slutmaaned;}
	...
	if ($regnstart == '0-0-01') $regnstart = '1970-01-01';
	if ($regnslut == '0-0-27') $regnslut = '2999-12-31';
	#$regnslut = "2005-05-15";
#	print "<tr><td colspan=5>Firmanavn</td></tr>";
	$kontonr=array();
	$x=0;
	$tmp="";
	if ($gruppenr) $tmp = nr_cast(gruppe)."=$gruppenr and "; 
...
# echo "select vare_id, pris from batch_kob where fakturadate>='$regnstart' and fakturadate<='$regnslut' order by vare_id<br>";	
