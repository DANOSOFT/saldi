<?php
// -------------------- systemdata/diverse.php ------ patch 3.4.3-----2014.07.01--------
//
...
if ($_POST) {
	...
	#######################################################################################
		...
		$box6=if_isset($_POST['box6']);#Automatisk lagerværdi
		...
		if ($box6) {
			if ($r=db_fetch_array(db_select("select id from grupper where art = 'VG' and box1 != box2",__FILE__ . " linje " . __LINE__))) {
				...
				print "<BODY onLoad=\"JavaScript:alert('$txt')\">";
				print "<meta http-equiv=\"refresh\" content=\"0;URL=../includes/konv_lager.php\">\n";
				exit;
			}
		}
