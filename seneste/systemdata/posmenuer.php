<?php
// -------------------- systemdata/posmenuer.php ------ patch 3.4.9--2015-01-07--------
// LICENS..
//
...
if ($master) {
	...
	if ($menuvalg=='ny') {
		...
		if (!$id) $id=$x+1;
		if ($id && $beskrivelse) {
			db_modify("insert into grupper(beskrivelse,art,kodenr,box1,box2,box3,box4,box5,box6,box7,box8,box9,box10,box11) values ('POS menu knapper','POSBUT','$id','$beskrivelse','$cols','$rows','$height','$width','$master','$begin','$end','$projekt','$fontsize','$radius')",__FILE__ . " linje " . __LINE__);
		}
  } else {
		list($id,$tmp)=explode(":",$menuvalg);
		$id*=1;
		if ($menu_id==$id && $cols && $rows && $beskrivelse !='-') {
			db_modify("update grupper set box1='$beskrivelse',box2='$cols',box3='$rows',box4='$height',box5='$width',box6='$master',box7='$begin',box8='$end',box9='$projekt',box10='$fontsize',box11='$radius' where art='POSBUT' and kodenr='$id'",__FILE__ . " linje " . __LINE__);
			for ($x=1;$x<=$rows;$x++) {
				for ($y=1;$y<=$cols;$y++) {
		...
		} elseif ($menu_id==$id && $beskrivelse=="-") { #20150107
			db_modify("delete from grupper where art='POSBUT' and kodenr='$id'",__FILE__ . " linje " . __LINE__);
			db_modify("delete from pos_buttons where menu_id='$id'",__FILE__ . " linje " . __LINE__);
		}
 	}
...
if (!$id && $id !='0') {
	...
	if (!$beskrivelse) {
		...
		db_modify("insert into grupper(beskrivelse,art,kodenr,box1,box2,box3,box4,box5,box6,box7,box8,box9,box10,box11) values ('POS menu knapper','POSBUT','0','$beskrivelse','4','4','$height','$width','$master','$begin','$end','$projekt','$fontsize','$radius')",__FILE__ . " linje " . __LINE__);
	}
}
...
#		style=\"width:".$width."px;height:".$height."px;text-align:center;font-size:".$fontsize."px; background-color:#$b;\"
		print "<td><input type=\"button\" style=\"$style\" value= \"$a\"></td>";
