<?php
// -----------------systemdata/projekter.php----lap 3.2.5---2011-11-08----
// LICENS
//
...
if ($gem) {
	if ($tilpas) {
		...
		$beskrivelse=addslashes(trim(if_isset($_POST['beskrivelse'])));
		...
		$projektnr=addslashes(trim($projektnr));
#		echo "cfg $cfg $projekt_nr<br";
		if(db_fetch_array(db_select("SELECT id FROM grupper where beskrivelse='$beskrivelse' and art = 'PRJ' and kodenr = '$projektnr' and id != '$id'",__FILE__ . " linje " . __LINE__))) {
			...
			elseif ($beskrivelse && $projektnr) db_modify("insert into grupper (beskrivelse,art,kodenr) values ('$beskrivelse','PRJ','$projektnr')",__FILE__ . " linje " . __LINE__);
