<?php
// ----------finans/bankimport.php------------patch 3.4.2------2014.07.08-----------
// LICENS
//
			...
			if ($skriv_linje==1){
				...
				if ($amount>0) {
					if (strlen($beskrivelse)==22 && substr($beskrivelse,0,3)=='IK ' && is_numeric(substr($beskrivelse,3,19))) { # ?
						...
						$r=db_fetch_array(db_select("select fakturanr,kontonr from ordrer where felt_1 LIKE '$betalings_id' and sum = '$amount'",__FILE__ . " linje " . __LINE__));
						$faktura=$r['fakturanr'];
						$kredit=$r['kontonr']*1;
						$k_type='D';
