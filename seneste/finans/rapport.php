...
// ------------ finans/rapport.php --------------- lap 3.4.3 --- 2014-08-25 ---
// LICENS
//
...
#cho "$qtxt<br>";
	...
	for ($x=0;$x<count($kontonr);$x++){
		...
		if (in_array($kontonr[$x],$ktonr)||$primo[$x]){
			...
			if ($aut_lager && (in_array($kontonr[$x],$varekob) || in_array($kontonr[$x],$varelager_i) || in_array($kontonr[$x],$varelager_u))) {
				$z=0;
				$lager=array();
				$gruppe=array();
...
#						$lager_u[$z]=$r['box2'];
				...
				$kobsdebet=array();
				$kobskredit=array();
				$q=db_select("select vare_id,antal,kobsdate from batch_kob where kobsdate >= '$regnstart' and kobsdate <= '$regnslut' order by kobsdate,vare_id",__FILE__ . " linje " . __LINE__);
				while ($r=db_fetch_array($q)) {
				...
				$salgsdebet=array();
				$salgkredit=array();
				$q=db_select("select vare_id,antal,salgsdate from batch_salg where salgsdate >= '$regnstart' and salgsdate <= '$regnslut' order by salgsdate,vare_id",__FILE__ . " linje " . __LINE__);
				while ($r=db_fetch_array($q)) {
...
#cho "$kto_periode[$x]<br>";
		...
		if ($aut_lager) {
			if (in_array($ktonr[$x],$varekob)) {
				$l_a_primo[$x]=find_lagervaerdi($ktonr[$x],$regnstart);
				$l_a_sum[$x]=find_lagervaerdi($ktonr[$x],$regnslut);
				$l_p_primo[$x]=find_lagervaerdi($ktonr[$x],$regnaarstart);
	...
	for ($x=1; $x<=$kontoantal; $x++) {
		if ($kontonr[$x]>=$konto_fra && $kontonr[$x]<=$konto_til && ($aarsum[$x] || $periodesum[$x] || $kontotype[$x] == 'H' || $kontotype[$x] == 'R')) {
			if ($kontotype[$x] == 'H') {
				...
				$tmp=dkdecimal($periodesum[$x]);
				print "<td align=\"right\" title=\"$title\">$tmp</td>";
