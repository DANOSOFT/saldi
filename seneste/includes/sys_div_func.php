<?php
// // ------------includes/sys_div_func.php------lap 3.4.8---2015.01.05------
// LICENS
//
...
#	$hurtigfakt=NULL; $incl_moms=NULL; $folge_s_tekst=NULL; $negativt_lager=NULL; $straks_bogf=NULL; $vis_nul_lev=NULL;
	...
	$r = db_fetch_array($q);
	$id=$r['id'];
	$beskrivelse=$r['beskrivelse'];$kodenr=$r['kodenr'];$box1=trim($r['box1']);$box2=trim($r['box2']);$box3=trim($r['box3']);$box4=trim($r['box4']);$box5=trim($r['box5']);$box6=trim($r['box6']);
	($box6)?$box6='checked':$box6=NULL; #Automatisk lagerværdi
...
#			include("../includes/online.php");
	...
	print "<tr><td colspan=6><hr></td></tr>";
	print "<tr bgcolor=\"$bgcolor5\"><td colspan=6><b><u>Bilagshåndtering</u></b></td></tr>";
	print "<tr><td colspan=6><br></td></tr>";
	...
	print "<tr><td colspan=6>Du kan vælge at lade os opbevare dine scannede bilag. Det koster kr. 25,- pr md pr Gb og giver samtidig mulighed for</td></tr>";
	print "<tr><td colspan=6>at sende scannede bilag pr e-mail til serveren og derefter importere dem i kassekladden</td></tr>";
	print "<tr><td colspan=6>Billag sendes til bilag_$db@$_SERVER[SERVER_NAME].</td></tr>";
	print "<tr><td colspan=6>Du kan også vælge selv at sætte en ftp server op til formålet. Det koster ikke noget.</td></tr>";
	print "<tr><td colspan=6><br></td></tr>";
	...
	else $onclick="onclick=\"return confirm('Intern bilagsopbevaring koster kr. 25,- pr. md. pr. Gb.')\"";
	print "<tr><td title=\"".findtekst(212,$sprog_id)."\">".findtekst(211,$sprog_id)."</td><td colspan=\"2\" title=\"".findtekst(212,$sprog_id)."\"><INPUT $onclick CLASS=\"inputbox\" TYPE=\"checkbox\" name=\"box6\" $intern_ftp></td></tr>";
	print "<tr><td title=\"".findtekst(720,$sprog_id)."\">".findtekst(719,$sprog_id)."</td><td colspan=\"2\" title=\"".findtekst(720,$sprog_id)."\"><INPUT $onclick CLASS=\"inputbox\" TYPE=\"checkbox\" name=\"box7\" $google_docs></td></tr>";
	if (!$intern_ftp) {
		print "<tr><td>Navn eller IP p&aring; ftpserver</td><td colspan=2><INPUT CLASS=\"inputbox\" TYPE=text name=box1 size=20 value=\"$ftpsted\"></td></tr>";
		print "<tr><td>Brugernavn p&aring; ftpserver</td><td colspan=2><INPUT CLASS=\"inputbox\" TYPE=text name=box2 size=20 value=\"$ftplogin\"></td></tr>";
		print "<tr><td>Adgangskode til ftpserver</td><td colspan=2><INPUT CLASS=\"inputbox\" TYPE=password name=box3 size=20 value=\"$ftpkode\"></td></tr>";
