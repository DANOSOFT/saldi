<?php
// ----------includes/bilag.php------------patch 3.4.8-----2015.01.05-----------
// LICENS
//
...
function upload_bilag($kilde_id,$kilde,$bilag_id,$bilag,$fokus,$filnavn,$funktion){
	...
	if ($puljefil || file_exists("../temp/$db/$filnavn")) {
		if (!file_exists("../owncloud/".$db)) system ("mkdir ../owncloud/".$db); 
		...
		if($box6=$r['box6']) {
			...
			if ($kilde=="kassekladde" || $kilde=="ordrer") {
				...
				if (!file_exists("../owncloud/".$db."/".$mappe)) {
					system ("mkdir ../owncloud/".$db."/".$mappe);
					if (!file_exists("../owncloud/".$db."/".$mappe)) {
				...
				if ($kilde=="kassekladde") {
					...
					if (!file_exists("../owncloud/".$db."/".$mappe."/".$undermappe)) {
						system ("mkdir ../owncloud/".$db."/".$mappe."/".$undermappe);
						if (!file_exists("../owncloud/".$db."/".$mappe."/".$undermappe)) {
							print "<BODY onLoad=\"javascript:alert('A indl&aelig;sning af $filnavn fejlet')\">";
							exit;
						}
				...
				if (!file_exists("../owncloud/".$db."/".$mappe."/".$undermappe)) system ("mkdir ../owncloud/".$db."/".$mappe."/".$undermappe);
...
#echo "mv '$fra' '$til'\n<br>";
			...
			if ($tjek != 'OK') {
				print "<BODY onLoad=\"javascript:alert('Upload af $filnavn fejlet')\">";
#				return();
				exit;
			}
