...
for ($o=0; $o<$ordre_antal; $o++) {
	...
	$sum='';$transportsum=0;
 	if ($preview) {
		...
		$query = db_select("select status,email,ordrenr,fakturanr,mail_fakt,pbs,art,ref,sprog,udskriv_til,mail_subj,mail_text,dokument,procenttillag from ordrer where id = '$ordre_id[$o]'",__FILE__ . " linje " . __LINE__);
		$row = db_fetch_array($query);
		$status=$row['status'];
		$art=$row['art'];
		$ref=$row['ref'];
...
#	$ordremomssum=$row['moms'];
	...
	#cho "formular: $formular bg: $bgr<br>"; exit();
...
#xit;
		...
		if ($vis_saet) {
			$r=db_fetch_array(db_select("select id from ordrelinjer where ordre_id = $ordre_id[$o] and varenr='R'",__FILE__ . " linje " . __LINE__));
			($r['id'])?$rvnr=1:$rvnr=0;
		}
#		if ($rvnr) $qtxt="select * from ordrelinjer where ordre_id = '$ordre_id[$o]' order by saet,posnr";
#		else 
		#20150302 
		if ($vis_saet && $formular==3) $qtxt="select * from ordrelinjer where ordre_id = '$ordre_id[$o]' and samlevare != 'on' and varenr != 'R' order by posnr";
