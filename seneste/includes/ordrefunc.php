<?php
//----------------- debitor/ordrefunc.php -----ver 3.4.9---- 2015.01.12 ----------
// LICENS
//
...
#		} else db_modify("insert into batch_salg(batch_kob_id, vare_id, linje_id, salgsdate, ordre_id, antal, pris, lev_nr) values (0, '$vare_id', '$linje_id', '$levdate', '$id', '$antal', '$pris', '$lev_nr')",__FILE__ . " linje " . __LINE__);
		if ($antal) db_modify("insert into batch_salg(batch_kob_id, vare_id, linje_id, salgsdate, ordre_id, antal, pris, lev_nr) values (0, '$vare_id', '$linje_id', '$levdate', '$id', '$antal', '$pris', '$lev_nr')",__FILE__ . " linje " . __LINE__);
	} else {
...
#cho "insert into batch_kob(vare_id, linje_id, ordre_id, antal,rest,pris) values ('$vare_id', '0', '0','0','$tmp',$kostpris)<br>";
					db_modify("insert into batch_kob(vare_id, linje_id, ordre_id, antal,rest,pris) values ('$vare_id', '0', '0','0','$tmp',$kostpris)",__FILE__ . " linje " . __LINE__);
...
#cho "insert into batch_salg(batch_kob_id, vare_id, linje_id, salgsdate, ordre_id, antal, pris, lev_nr) values ('$r[id]', '$vare_id', '$linje_id', '$levdate', '$id', '$tmp2', '$pris', '$lev_nr')<br>";
					db_modify("insert into batch_salg(batch_kob_id, vare_id, linje_id, salgsdate, ordre_id, antal, pris, lev_nr) values ('0', '$vare_id', '$linje_id', '$levdate', '$id', '$tmp2', '$pris', '$lev_nr')",__FILE__ . " linje " . __LINE__);
				}
			} else {
				$tmp=$antal;
#cho "select * from batch_kob where vare_id = '$vare_id' and rest > 0 order by id<br>";
				$q=db_select("select * from batch_kob where vare_id = '$vare_id' and rest > 0 order by id",__FILE__ . " linje " . __LINE__);
				while($r=db_fetch_array($q)) {
					$ny_rest=$r['rest'];
					if ($tmp && $ny_rest) { 
...
#cho __LINE__." insert into batch_salg(batch_kob_id, vare_id, linje_id, salgsdate, ordre_id, antal, pris, lev_nr) values ('$r[id]', '$vare_id', '$linje_id', '$levdate', '$id', '$tmp', '$pris', '$lev_nr')<br>";
							db_modify("insert into batch_salg(batch_kob_id, vare_id, linje_id, salgsdate, ordre_id, antal, pris, lev_nr) values ('$r[id]', '$vare_id', '$linje_id', '$levdate', '$id', '$tmp', '$pris', '$lev_nr')",__FILE__ . " linje " . __LINE__);
							$tmp=0;
#cho __LINE__." tmp $tmp ny_rest $ny_rest<br>";
...
#cho __LINE__." insert into batch_salg(batch_kob_id, vare_id, linje_id, salgsdate, ordre_id, antal, pris, lev_nr) values ('$r[id]', '$vare_id', '$linje_id', '$levdate', '$id', '$ny_rest', '$pris', '$lev_nr')<br>";
							db_modify("insert into batch_salg(batch_kob_id, vare_id, linje_id, salgsdate, ordre_id, antal, pris, lev_nr) values ('$r[id]', '$vare_id', '$linje_id', '$levdate', '$id', '$ny_rest', '$pris', '$lev_nr')",__FILE__ . " linje " . __LINE__);
						}
#cho __LINE__." tmp $tmp ny_rest $ny_rest<br>";
							$tmp-=$ny_rest;
							$ny_rest=0; 
...
#cho "insert into batch_kob(vare_id, linje_id, ordre_id, antal,rest,pris) values ('$vare_id', '0', '0','0','$tmp2',$kostpris)<br>";
					db_modify("insert into batch_kob(vare_id, linje_id, ordre_id, antal,rest,pris) values ('$vare_id', '0', '0','0','$tmp2',$kostpris)",__FILE__ . " linje " . __LINE__);
					$r=db_fetch_array(db_select("select max(id) as id from batch_kob where vare_id='$vare_id' and linje_id=0",__FILE__ . " linje " . __LINE__));
...
#	if (!is_numeric($pris)) $pris=0; #20130903 - fjernet 20140124
	...
	elseif ($id && $beskrivelse && $posnr) {
		$qtxt="insert into ordrelinjer (ordre_id,vare_id,varenr,enhed,beskrivelse,antal,rabat,rabatart,procent,m_rabat,pris,kostpris,momsfri,momssats,posnr,projekt,folgevare,rabatgruppe,bogf_konto,kred_linje_id,kdo,serienr,variant_id,leveres,samlevare,omvbet) values ('$id','0','','','$beskrivelse','0','0','','100','0','0','0','','0','$posnr','0','0','0','0','0','','','0','0','','$omvbet')";
#cho "A $qtxt<br>";
		db_modify($qtxt,__FILE__ . " linje " . __LINE__);
...
# exit;
		...
		if ($id && is_numeric($posnr)) {
			...
			$antal*=1;
			$qtxt="insert into ordrelinjer (ordre_id,vare_id,varenr,enhed,beskrivelse,antal,rabat,rabatart,procent,m_rabat,pris,kostpris,momsfri,momssats,posnr,projekt,folgevare,rabatgruppe,bogf_konto,kred_linje_id,kdo,serienr,variant_id,leveres,samlevare,omvbet) values ('$id','$vare_id','$varenr','$enhed','$beskrivelse','$antal','$rabat','$rabatart','$procent','$m_rabat','$pris','$kostpris','$momsfri','$varemomssats','$posnr','','$folgevare','$rabatgruppe','$bogfkto','$kred_linje_id','$kdo','$serienr','$variant_id','$leveres','$samlevare','$omvbet')";
#cho "$qtxt<br>";			
			db_modify($qtxt,__FILE__ . " linje " . __LINE__);
...
function find_kostpris($vare_id,$linje_id) { #Returnerer kostpris i DKK
	...
	$r=db_fetch_array(db_select("select antal,kred_linje_id from ordrelinjer where id = '$linje_id'",__FILE__ . " linje " . __LINE__));
...
#	if ($art!='PO') print"<td align=right><b><a href=$href?sort=beholdning&funktion=vareOpslag&fokus=$fokus&id=$id&vis_kost=$vis_kost>Beh.</a></b></td>";
	...
	if (!$sort) $sort = 'varenr';
	if ($find) $query = db_select("select * from varer where lukket != '1' and lower($fokus) like '$find' order by $sort",__FILE__ . " linje " . __LINE__);
...
// onmouseover=\"ChangeBackgroundColor(this)\" onmouseout=\"RestoreBackgroundColor(this)\" 
		...
		print "<td $rowheight ><a onfocus=\"this.style.fontSize = '20px';\" onblur=\"this.style.fontSize = '12px';\" id=\"opslag_$x\" href=\"$href?vare_id=$row[id]&fokus=$fokus&id=$id&bordnr=$bordnr\">$row[varenr]</a></td>";
