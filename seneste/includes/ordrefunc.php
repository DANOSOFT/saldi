<?php
//----------------- debitor/ordrefunc.php -----ver 3.5.0---- 2015.01.23 ----------
// LICENS
//
...
#echo "A1 $afrunding $tmp2-$tmp1<br>";
#					$diff=1;
					$diff=$afrunding;
...
#						 		 echo "zz $id,$vare_id,$varenr,$antal,$beskrivelse,$pris,$rabat_ny,$procent,$art,$momsfri,$posnr,$linje_id,$incl_moms,$kdo,$rabatart,$kopi,$saet,$fast_db<br>";
#exit;
...
#	if (!is_numeric($pris)) $pris=0; #20130903 - fjernet 20140124
	...
	elseif ($id && $beskrivelse && $posnr) {
		$qtxt="insert into ordrelinjer (ordre_id,vare_id,varenr,enhed,beskrivelse,antal,rabat,rabatart,procent,m_rabat,pris,kostpris,momsfri,momssats,posnr,projekt,folgevare,rabatgruppe,bogf_konto,kred_linje_id,kdo,serienr,variant_id,leveres,samlevare,omvbet,saet,fast_db) values ('$id','0','','','$beskrivelse','0','0','','100','0','0','0','','0','$posnr','0','0','0','0','0','','','0','0','','$omvbet','$saet','$fast_db')";
#cho "A $qtxt<br>";
		db_modify($qtxt,__FILE__ . " linje " . __LINE__);
...
# exit;
		...
		if ($id && is_numeric($posnr)) {
			...
			$antal*=1;
			$qtxt="insert into ordrelinjer (ordre_id,vare_id,varenr,enhed,beskrivelse,antal,rabat,rabatart,procent,m_rabat,pris,kostpris,momsfri,momssats,posnr,projekt,folgevare,rabatgruppe,bogf_konto,kred_linje_id,kdo,serienr,variant_id,leveres,samlevare,omvbet,saet,fast_db,lev_varenr) values ('$id','$vare_id','$varenr','$enhed','$beskrivelse','$antal','$rabat','$rabatart','$procent','$m_rabat','$pris','$kostpris','$momsfri','$varemomssats','$posnr','','$folgevare','$rabatgruppe','$bogfkto','$kred_linje_id','$kdo','$serienr','$variant_id','$leveres','$samlevare','$omvbet','$saet','$fast_db','$lev_varenr')";
			db_modify($qtxt,__FILE__ . " linje " . __LINE__);
...
#				$kostpris[$x]=dkdecimal($r2['pris']*$valutakurs/100);
		...
		if (!$koid[$x]) {
			...
			$lagerfort=$r2['box8'];
			#20150119 ->
			$r2 = db_fetch_array(db_select("select ordredate from ordrer where id = '$ordre_id'",__FILE__ . " linje " . __LINE__));
...
#		($art=='PO')?$hreftxt="$href?vare_id=$row[id]&fokus=$fokus&id=$id&bordnr=$bordnr":$hreftxt="";
		...
		if ($lager>=1){
			...
			$r2= db_fetch_array(db_select("select beholdning from lagerstatus where vare_id='$vare_id' and lager=$lager",__FILE__ . " linje " . __LINE__));
...
echo "A $antal_ny<br>";	
	...
	for ($x=0;$x<count($vare_id);$x++) {
		opret_ordrelinje($id,$vare_id[$x],$varenr[$x],$antal[$x],'',$pris[$x],$rabat,100,'PO','','','0','on','','','',$saetnr);
...
#		if ($momssats && !$momsfri[$x]) $linjesum+=afrund($linjesum*$momssats/100,2);
	...
	opret_ordrelinje($id,$r['id'],$r['varenr'],1,'',$diff,20,100,'PO','','','0','','','','',$saetnr);
...
#cho "gendanner<br>";
	if ($r=db_fetch_array(db_select("select id from ordrelinjer where varenr = 'R' and ordre_id = $id",__FILE__ . " linje " . __LINE__))) {
	db_modify("update ordrelinjer set rabat = '0' where ordre_id = '$id'",__FILE__ . " linje " . __LINE__);
#cho "delete from ordrelinjer where id='$r[id]'<br>";
...
#cho "$saetsum,$rabat<br>";
			if ($rabat*=1) {
#cho "update ordrelinjer set rabat = '$rabat' where id = '$r[id]'<br>";
