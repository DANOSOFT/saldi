...
function find_lagervaerdi($kontonr,$slut) {
	global $regnaar;
	$x=0;
	$lagervaerdi=0;
	$lager=array();
	...
	for ($x=0;$x<count($vare_id);$x++) {
		if ($kob) {
#cho "$kontonr :: select sum(antal) as antal from batch_kob where vare_id = $vare_id[$x] and salgsdate <= '$slut'<br>";
			$r=db_fetch_array(db_select("select sum(antal) as antal from batch_kob where vare_id = $vare_id[$x] and kobsdate <= '$slut'",__FILE__ . " linje " . __LINE__));
			$antal[$x]+=$r['antal'];
		}
		if ($salg) {
#cho "$kontonr :: select sum(antal) as antal from batch_salg where vare_id = $vare_id[$x] and salgsdate <= '$slut'<br>";
			$r=db_fetch_array(db_select("select sum(antal) as antal from batch_salg where vare_id = $vare_id[$x] and salgsdate <= '$slut'",__FILE__ . " linje " . __LINE__));
			$antal[$x]-=$r['antal'];
		}
#cho "Antal $antal[$x]<br>";		
		$vaerdi[$x]=$antal[$x]*$kostpris[$x];
		$lagervaerdi+=$vaerdi[$x];
# if ($antal[$x]) echo "$vare_id[$x] $antal[$x] $vaerdi[$x]<br>";
	}
#cho "(LV) $lagervaerdi <br>";
	return($lagervaerdi);
}}
