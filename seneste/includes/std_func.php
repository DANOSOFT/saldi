...
#cho "$kontonr :: select sum(antal) as antal from batch_kob where vare_id = $vare_id[$x] and salgsdate <= '$slut'<br>";
			$r=db_fetch_array(db_select("select sum(antal) as antal from batch_kob where vare_id = $vare_id[$x] and (fakturadate <= '$slut' or kobsdate <= '$slut')",__FILE__ . " linje " . __LINE__));
			$antal[$x]+=$r['antal'];
		}
		if ($salg) {
