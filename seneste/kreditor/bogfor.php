...
@session_start();
$s_id=session_id();
// -------------------kreditor/bogfor.php-------lap 3.4.4----2014-10-05--
/// LICENS
//
...
// 20131108 Fejl v. fifo og vareafgang v. negativt varekøb.Søg #20141117
...
#cho "$posnr[$x] Vareid $vare_id[$x]<br>";
echo "select * from grupper where art='VG' and kodenr='$gruppe[$x]'<br>";
...
#cho "($bogf_beh*$gl_kostpris[$x]+$antal[$x]*$dkpris[$x])/($bogf_beh+$antal[$x])<br>";
							if ($bogf_beh>0) $ny_kostpris=($bogf_beh*$gl_kostpris[$x]+$antal[$x]*$dkpris[$x])/($bogf_beh+$antal[$x]);
							else $ny_kostpris=$dkpris[$x];
...
#cho __LINE__." A diff $diff<br>";
										if ($diff) {
#cho __LINE__." insert into ordrelinjer (posnr, antal, pris, rabat, ordre_id, bogf_konto,projekt) values ('-1', '1', '$diff', 0, $id, $box3,'$projekt[$x]')<br>";
											...
											$diff=$diff*-1;
#cho __LINE__." insert into ordrelinjer (posnr, antal, pris, rabat, ordre_id, bogf_konto,projekt) values ('-1', '1', '$diff', 0, $id, $box1,'$projekt[$x]')<br>";
...
#						/*
							if ($diff && !$aut_lager) {
#cho __LINE__." insert into ordrelinjer (posnr, antal, pris, rabat, ordre_id, bogf_konto,projekt) values ('-1', $antal[$x], '$diff', 0, $id, $box1,'$projekt[$x]')<br>";
#Ombyttet 2011.11.29 grundet fejl v. returnering af vare saldi_252 ordre nr 874 
								...
								$diff=$diff*-1;
#cho __LINE__." insert into ordrelinjer (posnr, antal, pris, rabat, ordre_id, bogf_konto,projekt) values ('-1', $antal[$x], '$diff', 0, $id, $box3,'$projekt[$x]')<br>";
