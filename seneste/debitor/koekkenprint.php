<?php
// -------------debitor/pos_print/koekkenprint.php----------lap 3.4.4-----2014-12-02----
// LICENS
//
...
#cho "den findes<br>";
	...
	} else $url="$url/debitor/pos_ordre.php?id=$id";
...
#		else $bestil[$x]=$antal[$x]; 
	...
	$x=0;
	$q=db_select("select ordrelinjer.id,ordrelinjer.antal,ordrelinjer.leveres,ordrelinjer.leveret,ordrelinjer.beskrivelse,varer.notes,varer.kategori from ordrelinjer,varer where ordrelinjer.ordre_id='$id' and ordrelinjer.vare_id=varer.id  order by ordrelinjer.posnr desc",__FILE__ . " linje " . __LINE__);
	while ($r=db_fetch_array ($q)) {
		$linje_id[$x]=$r['id'];
		$antal[$x]=$r['antal']*1;
