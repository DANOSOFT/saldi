...
#	$momssats = trim($_POST['momssats']);
	...
	if (($godkend == "on") && ($status==0) && ($art=='DO' || $art == 'DK') && $sag_id) { # Kopi af original Tilbud. 20140716
		...
		if (!in_array("OT", $art_tjk)) {
			...
			db_modify("insert into ordrer (konto_id,firmanavn,addr1,addr2,postnr,bynavn,land,kontakt,email,mail_fakt,udskriv_til,kundeordnr,lev_navn,lev_addr1,lev_addr2,lev_postnr,lev_bynavn,lev_kontakt,ean,institution,betalingsbet,betalingsdage,kontonr,cvrnr,art,valuta,$valutakurs sprog,projekt,ordredate,$levdate $fakturadate notes,ordrenr,sum,momssats,status,ref,fakturanr,$modtagelse $kred_ord_id lev_adr,kostpris,moms,hvem,tidspkt,betalt,$nextfakt pbs,mail,mail_cc,mail_bcc,mail_subj,mail_text,felt_1,felt_2,felt_3,felt_4,felt_5,vis_lev_addr,restordre,sag_id,tilbudnr,datotid,nr,returside,sagsnr,betalings_id,mail_bilag,dokument,procenttillag) values ('$r[konto_id]','$r[firmanavn]','$r[addr1]','$r[addr2]','$r[postnr]','$r[bynavn]','$r[land]','$r[kontakt]','$r[email]','$r[mail_fakt]','$r[udskriv_til]','$r[kundeordnr]','$r[lev_navn]','$r[lev_addr1]','$r[lev_addr2]','$r[lev_postnr]','$r[lev_bynavn]','$r[lev_kontakt]','$r[ean]','$r[institution]','$r[betalingsbet]','$r[betalingsdage]','$r[kontonr]','$r[cvrnr]','OT','$r[valuta]',$valutakurs_value '$r[sprog]','$r[projekt]','$r[ordredate]',$levdate_value $fakturadate_value '$r[notes]','$r[ordrenr]','$r[sum]','$r[momssats]','0','$r[ref]','$r[fakturanr]',$modtagelse_value $kred_ord_id_value '$r[lev_adr]','$r[kostpris]','$r[moms]','$r[hvem]','$r[tidspkt]','$r[betalt]',$nextfakt_value '$r[pbs]','$r[mail]','$r[mail_cc]','$r[mail_bcc]','$r[mail_subj]','$r[mail_text]','$r[felt_1]','$r[felt_2]','$r[felt_3]','$r[felt_4]','$r[felt_5]','$r[vis_lev_addr]','$r[restordre]','$r[sag_id]','$r[tilbudnr]','$r[datotid]','$r[nr]','$r[returside]','$r[sagsnr]','$r[betalings_id]','$r[mail_bilag]','$r[dokument]','$r[procenttillag]')",__FILE__ . " linje " . __LINE__);
...
#				}
				...
				$qtxt="update ordrelinjer set varenr='$varenr[$x]',beskrivelse='$beskrivelse[$x]',leveres='$leveres[$x]',pris='$pris[$x]',rabat='$rabat[$x]',procent='$procent[$x]',projekt='$projekt[$x]',kdo='$kdo[$x]',omvbet='$omvbet[$x]' where id='$linje_id[$x]'";
...
#cho "tilbudnr: $tilbudnr<br>"; exit();
	...
	if ((!$id)&&($konto_id)&&($firmanavn)){
		...
		$qtext="insert into ordrer (ordrenr,konto_id,kontonr,kundeordnr,firmanavn,addr1,addr2,postnr,bynavn,land,kontakt,lev_navn,lev_addr1,lev_addr2,lev_postnr,lev_bynavn,lev_kontakt,betalingsdage,betalingsbet,cvrnr,ean,institution,email,mail_fakt,notes,art,ordredate,momssats,status,ref,lev_adr,valuta,projekt,sprog,pbs,restordre,procenttillag,sag_id,sagsnr,tilbudnr,datotid,nr,returside,omvbet) values ($ordrenr,'$konto_id','$kontonr','$kundeordnr','$firmanavn','$addr1','$addr2','$postnr','$bynavn','$land','$kontakt','$lev_navn','$lev_addr1','$lev_addr2','$lev_postnr','$lev_bynavn','$lev_kontakt','$betalingsdage','$betalingsbet','$cvrnr','$ean','$institution','$email','$mail_fakt','$notes','$art','$ordredate','$momssats',$status,'$ref','$lev_adr','$valuta','$projekt[0]','$formularsprog','$pbs','0','$procenttillag','$sag_id','$sagsnr','$tilbudnr','$datotid','$nr','$returside','$omkunde')";
		db_modify($qtext,__FILE__ . " linje " . __LINE__);
		$query = db_select("select id from ordrer where kontonr='$kontonr' and ordredate='$ordredate' order by id desc",__FILE__ . " linje " . __LINE__);
		if ($row = db_fetch_array($query)) {
			$id=$row['id'];
...
########################## del_ordre  - SKAL VAERE PLACERET FOER "FAKTURER" ################################
	if ($submit=='del_ordre') {
		...
		transaktion("begin");
		$r=db_fetch_array(db_select("select * from ordrer where id = '$id'",__FILE__ . " linje " . __LINE__));
		db_modify("insert into ordrer (ordrenr,konto_id,kontonr,firmanavn,addr1,addr2,postnr,bynavn,land,kontakt,kundeordnr,betalingsdage,betalingsbet,cvrnr,ean,institution,notes,art,ordredate,momssats,tidspkt,ref,status,lev_navn,lev_addr1,lev_addr2,lev_postnr,lev_bynavn,lev_kontakt,valuta,projekt,sprog,email,mail_fakt,pbs,restordre,omvbet) values ('$r[ordrenr]','$r[konto_id]','$r[kontonr]','".db_escape_string($r['firmanavn'])."','".db_escape_string($r['addr1'])."','".db_escape_string($r['addr2'])."','".db_escape_string($r['postnr'])."','".db_escape_string($r['bynavn'])."','".db_escape_string($r['land'])."','".db_escape_string($r['kontakt'])."','".db_escape_string($r['kundeordnr'])."','$r[betalingsdage]','$r[betalingsbet]','".db_escape_string($r['cvrnr'])."','".db_escape_string($r['ean'])."','".db_escape_string($r['institution'])."','".db_escape_string($r['notes'])."','$r[art]','$r[ordredate]','$r[momssats]','$r[tidspkt]','".db_escape_string($r['ref'])."','$r[status]','".db_escape_string($r['lev_navn'])."','".db_escape_string($r['lev_addr1'])."','".db_escape_string($r['lev_addr2'])."','".db_escape_string($r['lev_postnr'])."','".db_escape_string($r['lev_bynavn'])."','".db_escape_string($r['lev_kontakt'])."','$r[valuta]','$r[projekt]','".db_escape_string($r['sprog'])."','".db_escape_string($r['email'])."','$r[mail_fakt]','$r[pbs]','1','$r[omvbet]')",__FILE__ . " linje " . __LINE__);
...
#					$momssum=$momssum+$ialt;
			...
			print "<input type=\"hidden\" name=\"pris$x\" value=\"$dkpris\"><td align=\"right\" title=\"Kostpris $dk_kostpris[$x]\">$dkpris<br></td>\n";
		...
		if ($r = db_fetch_array($q)) {
			...
			while ($r2 = db_fetch_array($q2)) {
				...
				if ($x==1) {
					...
					print "<td colspan=\"3\"><select class=\"inputbox\" name=\"ref\" $disabled>\n";
...
#			print "<td align=\"center\">".dkdecimal($procenttillag)."% tillæg:&nbsp;".dkdecimal($tillag)."</td>\n";
		...
		if ($status < 3) {
			...
			if (($linjeantal>0)&&($art=='DO' || $art=='OT')) { # skal også med ved 'original tilbud' (OT) #20140716
...
function ordrelinjer($x,$sum,$dbsum,$blandet_moms,$moms,$antal_ialt,$leveres_ialt,$tidl_lev_ialt,$levdiff,$masterprojekt,$linje_id,$kred_linje_id,$posnr,$varenr,$beskrivelse,$enhed,$pris,$rabat,$rabatart,$procent,$antal,$leveres,$vare_id,$momsfri,$rabatgruppe,$m_rabat,$varemomssats,$serienr,$samlevare,$folgevare,$projekt,$kdo,$kobs_ordre_pris,$ko_ant,$kostpris,$db,$dg,$dk_db,$dk_dg,$readonly,$omvbet) {
	...
	($art=='OT')?$disabled='disabled':$disabled=NULL; // Her disables inputfield hvis art er OT. #20140716
	...
	if ($art=='OT') { // Når input fields er 'disabled' bliver de ikke opdateret, derfor tilføjes hidden fields
	...
	print "<td valign=\"top\" title=\"$title\"><textarea class=\"autosize inputbox ordreText comment\" $readonly rows=\"1\" cols=\"58\" name=\"beskrivelse$x\" onchange=\"javascript:docChange = true;\" $disabled>$beskrivelse</textarea></td>\n"; #2013.11.27 Ændret til textarea, så hele texten vises
