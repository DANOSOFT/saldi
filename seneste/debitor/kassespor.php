<?php
// ----------debitor/kassespor.php-------------lap 3.4.4-----2014-11-19-----
// LICENS
//
...
// ----------------------------------------------------------------------
// 20141119 PHR Tilføjer summer og bord
...
#	$nysort = if_isset($_POST['nysort']);
	...
	$cookievalue="$fakturadatoer;$logtime;$afdelinger;$sort;$nysort;$idnumre;$fakturanumre;$sum;$betalinger;$betalinger2;$modtagelser;$modtagelser2;$kasser;$ref;$linjeantal;$borde";
	setcookie("saldi_kassespor", $cookievalue);
	
} else {

	list ($fakturadatoer,$logtime,$afdelinger,$sort,$nysort,$idnumre,$fakturanumre,$sum,$betalinger,$betalinger2,$modtagelser,$modtagelser2,$kasser,$ref,$linjeantal,$borde) = explode(";", $_COOKIE['saldi_kassespor']);
...
if (!$fakturadatoer&&!$logtime&&!$afdelinger&&!$sort&&!$nysort&&!$idnumre&&!$fakturanumre&&!$sum&&!$betalinger&&!$betalinger2&&!$modtagelser&&!$modtagelser2&&!$kasser&&!$ref&&!$borde) {
...
while ($r=db_fetch_array($q)) {
	if (!in_array($r['betalingstype'],$bet_type)) {
...
($r['box7'])?$bord=explode(chr(9),$r['box7']):$bord=NULL; #20141119
...
$next=udskriv($fakturadatoer,$logtime,$afdelinger,$sort,$nysort,$idnumre,$fakturanumre,$sum,$betalinger,$betalinger2,$modtagelser,$modtagelser2,$kasser,$ref,$linjeantal,$sort,$start,'',$borde);
if ($start>=$linjeantal) {
...
print "<td align=center><span title= 'Vælg et bord'><select class=\"inputbox\" type=\"text\" style=\"text-align:right;width:100px;height:20px;font-size:12px\" name=\"borde\">\n";
if ($borde) print "<option>$borde</option>\n";
print "<option></option>\n";
for ($x=0;$x<count($bord);$x++) {
	if ($bord[$x] != $borde) print "<option>$bord[$x]</option>\n";
}
print "</select></td>\n";
...
print "</form></tr>\n";
udskriv($fakturadatoer,$logtime,$afdelinger,$sort,$nysort,$idnumre,$fakturanumre,$sum,$betalinger,$betalinger2,$modtagelser,$modtagelser2,$kasser,$ref,$linjeantal,$sort,$start,'skriv',$borde);

print "<tr><td colspan=\"8\" align=\"right\"><b>".dkdecimal($omsaet)."</b></td><td colspan=\"2\" align=\"right\"><b>".dkdecimal($modtaget)."</b></td><td colspan=\"1\" align=\"right\"><b>".dkdecimal($retursum)."</b></td></tr>"; 

for ($z=0;$z<count($bet_type);$z++) {
	print "<tr><td colspan=\"9\" align=\"right\"><b>$bet_type[$z]</b></td><td align=\"right\"><b>".dkdecimal($bet_sum[$z])."</b></td></tr>"; 
}
for ($z=0;$z<count($bet_type);$z++) {
	if (strtolower($bet_type[$z])=='kontant'){
		print "<tr><td colspan=\"9\" align=\"right\"><b>Tilgang kasse</b></td><td align=\"right\"><b>".dkdecimal($bet_sum[$z]-$retursum)."</b></td></tr>"; 
	}
}
...
function udskriv($fakturadatoer,$logtime,$afdelinger,$sort,$nysort,$idnumre,$fakturanumre,$sum,$betalinger,$betalinger2,$modtagelser,$modtagelser2,$kasser,$ref,$linjeantal,$sort,$start,$skriv,$borde) {
...
#	if ($modtagelser2) $udvaelg=$udvaelg.udvaelg($modtagelser2, 'ordrer.felt_4', 'TEXT');
	if ($sum) $udvaelg=$udvaelg.udvaelg($sum, 'ordrer.sum+ordrer.moms', 'BELOB');
#	if ($modtagelser) $udvaelg=$udvaelg.udvaelg($modtagelser, 'ordrer.felt_5', 'TEXT');
	...
	if ($borde || $borde=='0') $udvaelg=$udvaelg.udvaelg($borde, 'ordrer.nr','NR');
	...
	$x=0;
	$qtxt="select * from ordrer $udvaelg status >= 3 and art = 'PO' order by $sort";
		...
		if (($x>=$start)&&($x<$start+$linjeantal) && ($udskriv)){
			...
			if ($skriv) {
				...
				for ($y=0;$y<count($ordre_id);$y++) {
					...
					print "<td align=right>$bord[$x]<br></td>\n";
