...
if ($submit=$_POST['submit']){
	...
	$cookievalue="$sort;$nysort;$fakturadatoer;$logtime;$afdelinger;$sort;$nysort;$idnumre;$fakturanumre;$sum;$betalinger;$betalinger2;$modtagelser;$modtagelser2;$kasser;$ref;$linjeantal;$borde;$status";
	setcookie("saldi_kassespor", $cookievalue);
} else {
	list ($sort,$nysort,$fakturadatoer,$logtime,$afdelinger,$sort,$nysort,$idnumre,$fakturanumre,$sum,$betalinger,$betalinger2,$modtagelser,$modtagelser2,$kasser,$ref,$linjeantal,$borde,$status) = explode(";", $_COOKIE['saldi_kassespor']);
...
$next=udskriv($fakturadatoer,$logtime,$afdelinger,$sort,$nysort,$idnumre,$fakturanumre,$sum,$betalinger,$betalinger2,$modtagelser,$modtagelser2,$kasser,$ref,$linjeantal,$sort,$start,'',$borde,$status);
if ($start>=$linjeantal) {
...
if ($status==3) print "<option value=\"3\">3</option>"; 
...
print "</select></td>\n";
print "<td align=center><span title= 'Angiv brugernavn p&aring; ekspedient'><input class=\"inputbox\" type=\"text\" style=\"text-align:right;width:150px;height:20px;font-size:12px\" name=\"ref\" value=\"$ref\"></td>\n";
...
print "</form></tr>\n";
udskriv($fakturadatoer,$logtime,$afdelinger,$sort,$nysort,$idnumre,$fakturanumre,$sum,$betalinger,$betalinger2,$modtagelser,$modtagelser2,$kasser,$ref,$linjeantal,$sort,$start,'skriv',$borde,$status);
...
function udskriv($fakturadatoer,$logtime,$afdelinger,$sort,$nysort,$idnumre,$fakturanumre,$sum,$betalinger,$betalinger2,$modtagelser,$modtagelser2,$kasser,$ref,$linjeantal,$sort,$start,$skriv,$borde,$status) {
		...
		if (($x>=$start)&&($x<$start+$linjeantal) && ($udskriv)){
			...
			if ($skriv) {
				...
				for ($y=0;$y<count($ordre_id);$y++) {
					...
					print "<td></td><td align=right>$id[$x]</span><br></td>\n";
