...
function vis_optalling($vnr,$gentael) {
	...
	if ($vnr) $q=db_select("select varer.id,varer.varenr,varer.beskrivelse,varer.kostpris,regulering.beholdning,regulering.tidspkt,regulering.optalt,regulering.lager from regulering,varer where varer.varenr='$vnr' and varer.id=regulering.vare_id and regulering.bogfort = '0'order by regulering.tidspkt",__FILE__ . " linje " . __LINE__);
	else $q=db_select("select varer.id,varer.varenr,varer.beskrivelse,varer.kostpris,regulering.beholdning,regulering.tidspkt,regulering.optalt,regulering.variant_id,regulering.lager from regulering,varer where varer.id=regulering.vare_id and regulering.bogfort = '0' order by varer.varenr,regulering.tidspkt",__FILE__ . " linje " . __LINE__);
	while($r=db_fetch_array($q)) {
			...
			while($optalt[$x][$y]) {
				...
				if ($y==0) {
					if ($variant_stregkode[$x][$y]) {
						print "<tr bgcolor=\"$baggrund\"><td><b>$varenr[$x]</b></td><td><b>$beskrivelse[$x]</b></td><td><b>".$lager[$x][$y]."</b></td><td align=\"center\"><br></td><td align=\"right\"><br></td><td align=\"right\"><br></td><td colspan=\"4\"><br></td><tr>\n";
						print "<tr bgcolor=\"$baggrund\"><td>".$variant_stregkode[$x][$y]."</td><td><b>".$lager[$x][$y]."</b></td><td><br></td><td align=\"center\">$dag.$md.$aar&nbsp;$time:$minut</td><td align=\"right\">".dkdecimal($beholdning[$x][$y])."</td><td align=\"right\">".dkdecimal($optalt[$x][$y])."</td><tr>\n";
					} else print "<tr bgcolor=\"$baggrund\"><td><b>$varenr[$x]</b></td><td><b>$beskrivelse[$x]</b></td><td><b>".$lager[$x][$y]."</b></td><td align=\"center\">$dag.$md.$aar&nbsp;$time:$minut</td><td align=\"right\">".dkdecimal($beholdning[$x][$y])."</td><td align=\"right\">".dkdecimal($optalt[$x][$y])."</td><td colspan=\"4\" align=\"right\" title=\"Klik her for at slette denne vare fra opt&aelig;llingen.\"><a style=\"text-decoration:none\" href=\"optalling.php?vare_id=$vare_id[$x]&varenr=$varenr[$x]&slet=y&gentael=$gentael\" onclick=\"return confirm('Vil du slette denne vare fra liste og opt&aelig;lle den igen?')\"><font color=\"#ff0000\"><b>X</b></font></a></td><tr>\n";
				} else {
