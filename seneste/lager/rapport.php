...
#	print "<tr><td width=10%> Dato</td><td width=10%> Bilag</td><td width=50%> Tekst</td><td width=10% align=right> Debet</td><td width=10% align=right> Kredit</td><td width=10% align=right> Saldo</td></tr>";
	...
	if (!$detaljer) {
		print "<tr><td><b>Varenr.</b></td>
				...
				<td align=\"right\"><b>Beholdning</b></td></tr>";
	}
	$tt_kobt=0;
	$tt_solgt=0;
	$tt_regul=0;
	...
	for ($x=1; $x<=$vareantal; $x++) {
		...
		while ($r = db_fetch_array($q)) {
			...
			if ($r['fakturadate'] && $r['fakturadate'] <= $date_to && $r['fakturadate'] >= $date_from) {
				...
				if ($ok) {
					$t_kobt+=$antal[$y];
					$t_k_pris+=$pris[$y]*$antal[$y];
					$tt_kobt+=$antal[$y];
					$tt_k_pris+=$pris[$y]*$antal[$y];
					$t_moms+=$moms[$y];
					$y++;
				}
			} else {
...
#				else $tt_regul+=$r['antal'];
		...
		if ($detaljer) {
			...
			print "<tr><td colspan=\"$cols\"><hr></td></tr>";
			for ($y=0;$y<count($antal);$y++) {
				if ($detaljer) print "<tr><td>".dkdato($fakturadate[$y])."</td><td align=\"right\">".dkdecimal($antal[$y])."</td>";
				$linjepris=$pris[$y]*$antal[$y];
				$kobssum+=$t_kobt;
				print "<td align=\"right\">".dkdecimal($pris[$y])."</td><td align=\"right\">".dkdecimal($moms[$y])."</td><td align=\"right\">".dkdecimal($pris[$y]+$moms[$y])."</td>";
				print "<td align=right onClick=\"javascript:k_ordre=window.open('../kreditor/ordre.php?id=$ordre_id[$y]&returside=../includes/luk.php','k_ordre','width=800,height=400,$jsvars')\"> <u>Se</u></td></tr>";
			}
			print "<tr><td colspan=\"$cols\"><hr></td></tr>";
			print "<tr><td></td><td align=\"right\"><b>".dkdecimal($t_kobt)."</b></td><td align=\"right\"><b>".dkdecimal($t_k_pris)."</b></td><td align=\"right\"><b>".dkdecimal($t_moms)."</b></td><td align=\"right\"><b>".dkdecimal($t_k_pris+$t_moms)."</b></td></tr>";
			print "<tr><td colspan=\"$cols\"><hr></td></tr>";
		}
		$fakturadate=array();
		$linje_id=array();
		...
		while ($r = db_fetch_array($q)) {
			...
			if ($r['fakturadate'] && $r['fakturadate'] <= $date_to && $r['fakturadate'] >= $date_from) {
				...
				$linje_id[$y]=$r['linje_id'];
				if ($linje_id[$y]) {
					if ($r1 = db_fetch_array(db_select("select id,kostpris,momssats,momsfri,omvbet from ordrelinjer where id='$linje_id[$y]'",__FILE__ . " linje " . __LINE__))){
...
#				if ($ok) {
					if ($ordre_id[$y]) {
						$t_solgt+=$antal[$y];
						$t_s_pris+=$pris[$y]*$antal[$y];
						$tt_solgt+=$antal[$y];
						$tt_s_pris+=$pris[$y]*$antal[$y];
						$t_moms+=$moms[$y]*$antal[$y];
						$t_kost+=$kostpris[$y]*$antal[$y];
						$tt_kost+=$kostpris[$y]*$antal[$y];
						$db[$y]=$pris[$y]-$kostpris[$y];
						if ($antal[$y]<0)$db[$y]*=-1;
						$t_db+=$db[$y]*$antal[$y];
						$tt_db+=$db[$y]*$antal[$y];
						if ($pris[$y]) $dg[$y]=$db[$y]*100/$pris[$y];
						else $dg[$y]=0;
					} else {
						$t_regul+=$antal[$y];
						$tt_regul+=$antal[$y];
...
#				else $tt_regul+=$r['antal'];
		...
		if ($detaljer) {
			...
			for ($y=0;$y<count($linje_id);$y++) {
				if ($ordre_id[$y]) {
					print "<tr><td>".dkdato($fakturadate[$y])."</td><td align=right>".dkdecimal($antal[$y])."</td>";
					print "<td align=right>".dkdecimal($pris[$y])."</td>";
					print "<td align=right>".dkdecimal($moms[$y])."</td>";
					print "<td align=right>".dkdecimal($pris[$y]+($moms[$y]))."</td>";
					print "<td align=right>".dkdecimal($kostpris[$y])."</td>";
					print "<td align=right>".dkdecimal($db[$y])."</td>";
					print "<td align=right> ".dkdecimal($dg[$y])."%</td>";
					print "<td align=right onClick=\"javascript:s_ordre=window.open('../debitor/ordre.php?id=$ordre_id[$y]&returside=../includes/luk.php','s_ordre','width=800,height=400,$jsvars')\"> <u>Se</u></td></tr>";
				}
			}
			print "<tr><td colspan=\"$cols\"><hr></td></tr>";
			print "<tr><td></td>";
			print "<td align=right> <b>".dkdecimal($t_solgt)."</b></td>";
			...
			print "<td align=right> <b>".dkdecimal($t_kost)."</b></td>";
			print "<td align=right> <b>".dkdecimal($t_db)."</b></td>";
			print "<td align=right> <b>".dkdecimal($t_dg)."%</b></td></tr>";
			print "<tr><td colspan=\"$cols\"><hr></td></tr>";
			print "<tr><td>Lagerreguleret</td><td align=\"right\">Antal</td></tr>";
			for ($y=0;$y<count($linje_id);$y++) {
				if (!$ordre_id[$y]) {
					print "<tr><td>".dkdato($fakturadate[$y])."</td><td align=right>".dkdecimal($antal[$y])."</td></tr>";
				}
			}
			print "<tr><td colspan=\"$cols\"><hr></td></tr>";
			print "<tr><td>Samlet til-/afgang i perioden</td><td align=right> <b>".dkdecimal($t_kobt-$t_solgt-$t_regul)."</b></td><tr>";
		} else {
			print "<tr><td>$varenr[$x]</td>";
			print "<td>$enhed[$x]</td>";
			print "<td>$beskrivelse[$x]</td>";
			print "<td align=right>".dkdecimal($t_kobt)."</td>";
			print "<td align=right>".dkdecimal($t_k_pris)."</td>";
			print "<td align=right>".dkdecimal($t_solgt)."</td>";
			print "<td align=right>".dkdecimal($t_s_pris)."</td>";
			print "<td align=right>".dkdecimal($t_regul)."</td>";
			print "<td align=right>".dkdecimal($t_db)."</td>";
			print "<td align=right>".dkdecimal($t_dg)."%</td>";
			print "<td align=right>".dkdecimal($t_kobt-$t_solgt-$t_regul)."</td><tr>";
		}
		if ($detaljer) print "<tr><td colspan=\"$cols\"><hr></td></tr>";
	}
	if (!$detaljer) {
		if ($tt_s_pris && $t_db) $tt_dg=$t_db*100/$tt_s_pris;
		else $tt_dg=100;
			print "<tr><td colspan=\"$cols\"><hr></td></tr>";
		...
		print "<td>$enhed[$x]</td>";
		print "<td>$beskrivelse[$x]</td>";
		print "<td align=right> <b>".dkdecimal($tt_kobt)."</b></td>";
		print "<td align=right> <b>".dkdecimal($tt_k_pris)."</b></td>";
		print "<td align=right> <b>".dkdecimal($tt_solgt)."</b></td>";
		print "<td align=right> <b>".dkdecimal($tt_s_pris)."</b></td>";
		print "<td align=right> <b>".dkdecimal($tt_regul)."</b></td>";
		print "<td align=right> <b>".dkdecimal($tt_db)."</b></td>";
		print "<td align=right> <b>".dkdecimal($tt_dg)."%</b></td>";
		print "<td align=right><b>".dkdecimal($tt_kobt-$tt_solgt-$tt_regul)."</b></td><tr>";
	}
/*
